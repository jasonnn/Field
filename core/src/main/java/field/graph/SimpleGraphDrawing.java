...
class SimpleGraphDrawing {
    ...
    class SimpleAxis extends Cobj implements iAxis, iLayout {
        ...
        protected
        boolean isTrue(Prop x, boolean b) {
            Object o = getProperty(x);
            if (o == null) return b;
            if (o instanceof Boolean) return ((Boolean) o);
            if (o instanceof Number) return ((Number) o).floatValue() != 0;
            throw new ClassCastException("cannot convert " + (o.getClass()) + " to boolean");
        }

        protected
        float getFloat(Prop<? extends Number> x, float b) {
            Object o = getProperty(x);
            if (o == null) return b;
