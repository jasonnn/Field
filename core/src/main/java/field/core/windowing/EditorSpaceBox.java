...
public
class EditorSpaceBox {

    public static final VisualElementProperty<Boolean> isEditorSpace =
            new VisualElementProperty<Boolean>("isEditorSpace");
    public static final VisualElementProperty<VisualElementReference> isEditorSpace_belongsTo =
            new VisualElementProperty<VisualElementReference>("isEditorSpace_belongsTo");
    ...
    private
    ArrayList<Vector2> buildLineMap() {
        ...
        try {
            ...
            for (int i = 0; i < t.getLineCount() + 1; i++) {
                ...
                ll = Launcher.display.map(t, window.getFrame(), ll);
        ...
        try {
            ...
            for (Map.Entry<iVisualElement, Pair<Rect, StyledTextPositionSystem.Position>> e : frozen.entrySet()) {
                Rect r = transform(e.getValue(),
                                   frozenAt,
                                   thawAt,
                                   frozenAtLineMap.get(e.getValue().right.was),
                                   thrawAtLineMap.get(t.getLineAtOffset(e.getValue().right.at)));

                if (r == null) continue;
                if (e.getKey().getFrame(null) == null) continue;
    ...
    private
    int getOffsetForFrame(Rect frame) {
        PythonPluginEditor p = (PythonPluginEditor) PythonPluginEditor.python_plugin.get(root);
        StyledText t = p.getEditor().getInputEditor();
        ...
        for (int i = 0; i < frozenAtLineMap.size(); i++) {
            if (frozenAtLineMap.get(i).y > ll.y) return t.getOffsetAtLine(Math.max(0, i - 1));
        }
