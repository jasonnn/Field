...
class OverlayAnimationManager {
            ...
            void run() {
                ...
                while (true) {
                    ...
                    for (CachedLine c : draw) {
                        ...
                        iLinearGraphicsContext context = GLComponentWindow.fastContext == null
    ...
    public static
    void fadeUpAndDownOnWindow(final GLComponentWindow window,
                               final List<CachedLine> draw,
                               final int fadeInOver,
                               final int remainFor,
    ...
    public
    TaskQueue getAnimationQueue() {
        return animationQueue;
    }


    public
    void notifyText(GLComponentWindow window, String text, Rect mark) {
        notifyText(window, text, mark, 1, new Vector4(1, 1, 1, 0.15f));
    }

    public static
    void notifyTextOnWindow(GLComponentWindow window, String text, Rect mark, float speed, Vector4 back) {
        ...
        textLine.getInput()
                .setPointAttribute(iLinearGraphicsContext.font_v, new Font(Constants.defaultFont, Font.PLAIN, size));

        Rectangle2D bounds = new Font(Constants.defaultFont, Font.PLAIN, size).getStringBounds(text,
                                                                                               new FontRenderContext(null,
                                                                                                                     true,
                                                                                                                     true));

        bounds.setRect(bounds.getX() + ox + ow / 2 - bounds.getWidth() / 2,
                       bounds.getY() + oy + oh / 2 - 6 + bounds.getHeight() / 2,
                       bounds.getWidth(),
                       bounds.getHeight());
        ...
        java.awt.geom.RoundRectangle2D.Double rr = new RoundRectangle2D.Double(bounds.getX() - 20,
                                                                               bounds.getY() - 23,
                                                                               bounds.getWidth() + 20 + 20,
                                                                               bounds.getHeight() + 23 + 23,
                                                                               30,
                                                                               30);
    ...
    public
    void notifyText(GLComponentWindow window, String text, Rect mark, float speed, Vector4 back) {
