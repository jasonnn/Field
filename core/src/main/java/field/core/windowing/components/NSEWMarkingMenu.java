...
public
class NSEWMarkingMenu {

    public
    enum Direction {
        north(new Vector2(0, 0), -0.25f, 0), south(new Vector2(1, 1), 0.25f, 0), east(new Vector2(1, 0),
                                                                                      0,
                                                                                      1), west(new Vector2(0, 1),
                                                                                               0.5f,
                                                                                               -1);
    ...
    public
    List<CachedLine> paintNow(Rect center, HashSet<Direction> armed) {
        List<CachedLine> c = new ArrayList<CachedLine>();

        if (menus.isEmpty()) return c;
        ...
        for (Map.Entry<Direction, String> e : menus.entrySet()) {
            ...
            Vector2[] shape = {new Vector2(0, 0),
                               new Vector2(size, -size / 3),
                               new Vector2(size, size + size / 3),
                               new Vector2(0, size),
                               new Vector2(0, 0)};
            Vector4[] color = {armed.contains(d) ? new Vector4(0.5, 0, 0, 0.25f) : new Vector4(1, 1, 1, 0.1f),
                               armed.contains(d) ? new Vector4(0.5, 0, 0, 0.25f) : new Vector4(1, 1, 1, 0.1f),
                               new Vector4(),
                               new Vector4(),
            ...
            for (Vector2 r : shape) {
                ...
                if (i == 0) frame.getInput().moveTo(r.x, r.y);
                else frame.getInput().lineTo(r.x, r.y);
