 ...
 */
public abstract
class SwingBridgeComponent extends PlainDraggableComponent {
    ...
    class NamedComponent extends SwingBridgeComponent {
        ...
        public
        void setClass(Class<? extends JComponent> className) {
            if (componentToRender != null && className.isInstance(componentToRender)) return;
        ...
        public
        void setInstance(JComponent className) {
    ...
    void setSelected(boolean selected) {
        if (this.selected != selected) setDirty();
    ...
    float isHit(Event event) {
        if (componentToRender == null) return Float.NEGATIVE_INFINITY;
    ...
    void mouseClicked(ComponentContainer inside, Event arg0) {
        ...
        else super.mouseClicked(inside, arg0);
    ...
    void mouseDragged(ComponentContainer inside, Event arg0) {
        ...
        else super.mouseDragged(inside, arg0);
    }
    ...
    void mouseEntered(ComponentContainer inside, Event arg0) {
        ...
        else super.mouseEntered(inside, arg0);
    }
    ...
    void mouseExited(ComponentContainer inside, Event arg0) {
        ...
        else super.mouseExited(inside, arg0);
    }
    ...
    void mousePressed(ComponentContainer inside, Event arg0) {
        ...
        else super.mousePressed(inside, arg0);
    ...
    void mouseReleased(ComponentContainer inside, Event arg0) {
        ...
        else super.mouseReleased(inside, arg0);
    }
...
//		;//System.out.println(" component to render is <" + componentToRender + ">");
        ...
        try {
            ...
            ReflectionTools.illegalSetObject(componentToRender,
                                             "next",
                                             GLComponentWindow.getCurrentWindow(this).getFrame());
...
//		dirty = true;
        ...
        if (componentToRender != null) {
            if (dirty) {
                componentToRender.setBounds((int) this.getBounds().x,
                                            (int) this.getBounds().y,
                                            (int) this.getBounds().w,
                                            (int) this.getBounds().h);
                // componentToRender.setBounds(0,0,
                // (int)
                // this.getBounds().w,
                // (int)
                // this.getBounds().h);
                getAribitraryComponent().resetImage(componentToRender,
                                                    backgroundR,
                                                    backgroundG,
                                                    backgroundB,
                                                    backgroundA);
        ...
        if (doName) {
            ...
            c.getInput()
             .moveTo((float) (this.getBounds().x + this.getBounds().w + 4),
                     (float) (this.getBounds().y + this.getBounds().h - 6));
        ...
        }
        {
            ...
            c.getInput()
             .lineTo((float) (this.getBounds().x + this.getBounds().w),
                     (float) (this.getBounds().y + this.getBounds().h));
            ...
            c.getProperties()
             .put(iLinearGraphicsContext.color, selected ? new Vector4(0, 0, 0, 1f) : new Vector4(1, 1, 1, 0.1f));
    ...
    void setBounds(Rect r) {
        if (!lastPaintedRect.equals(bounds)) {
            if (bounds.w != lastPaintedRect.w || bounds.h != lastPaintedRect.h) dirty = true;
            setDirty();
        }
        super.setBounds(r);
    }
    ...
    protected
    MouseEvent rewriteCoordinates(Event arg0, boolean isDrag) {
        ...
        switch (arg0.type) {
            case (SWT.MouseDown): {
                MouseEvent m = new MouseEvent(componentToRender,
    ...
    protected
    void setAribitraryComponent(AribitraryComponent aribitraryComponent) {
        this.aribitraryComponent = aribitraryComponent;
    }

    protected
    AribitraryComponent getAribitraryComponent() {
