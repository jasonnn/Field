...
public
class PlainDraggableComponent implements iComponent, iDraggableComponent {

    public static final VisualElementProperty<Boolean> hasMouseFocus =
            new VisualElementProperty<Boolean>("hasMouseFocus");
    ...
    public
    PlainDraggableComponent(Rect bounds) {
        this.bounds = bounds;
...
//						d.addToSelection(PlainDraggableComponent.this);
    ...
    void mouseReleased(ComponentContainer inside, Event arg0) {
        if (arg0.button == 2) return;
        arg0.doit = false;
        if (decoration.up(arg0)) inside.requestRedisplay();
    ...
    void mouseDragged(ComponentContainer inside, Event arg0) {
        this.setInside(inside);
        if (arg0.button != 1) return;

        if (decoration.drag(arg0)) inside.requestRedisplay();
        if ((arg0.stateMask & SWT.ALT) != 0) return;
        if ((arg0.stateMask & Platform.getCommandModifier()) != 0) return;
    ...
    class MouseDragger {
        ...
        void mousePressed(Event arg0) {
            ...
            if (arg0.x - getX() > PlainDraggableComponent.this.getWidth() - zone) currentResize.add(Resize.right);
            if (arg0.y - getY() < zone) currentResize.add(Resize.up);
            if (arg0.y - getY() > PlainDraggableComponent.this.getHeight() - zone) currentResize.add(Resize.down);

            if (bounds.w < 15 && bounds.h < 15) currentResize.clear();

            if (currentResize.size() == 0) currentResize.add(Resize.translate);

            if (!canDrag) currentResize.clear();
...
//			GLComponentWindow.getCurrentWindow(PlainDraggableComponent.this).getCanvas().setCursor(Cursor.getDefaultCursor());
            GLComponentWindow.getCurrentWindow(PlainDraggableComponent.this)
                             .getCanvas()
        ...
        void mouseDragged(Event arg0) {
            ...
            for (SelectionGroup<iComponent> s : getSelectionGroups()) {
                for (iComponent d : s.getSelection()) {
                    if (d != PlainDraggableComponent.this) d.handleResize(currentResize, dx, dy);
                }
            }
    ...
    void handleResize(Set<Resize> currentResize, float dx, float dy) {
        if (currentResize.contains(Resize.translate))
            PlainDraggableComponent.this.shouldSetSize((float) PlainDraggableComponent.this.bounds.x + dx,
                                                       (float) PlainDraggableComponent.this.bounds.y + dy,
                                                       PlainDraggableComponent.this.getWidth(),
                                                       PlainDraggableComponent.this.getHeight());
        if (currentResize.contains(Resize.left)) {
            PlainDraggableComponent.this.shouldSetSize((float) PlainDraggableComponent.this.bounds.x + dx,
                                                       (float) PlainDraggableComponent.this.bounds.y,
                                                       (float) PlainDraggableComponent.this.bounds.w - dx,
                                                       (float) PlainDraggableComponent.this.bounds.h);
        }
        if (currentResize.contains(Resize.right)) {
            PlainDraggableComponent.this.shouldSetSize((float) PlainDraggableComponent.this.bounds.x,
                                                       (float) PlainDraggableComponent.this.bounds.y,
                                                       (float) PlainDraggableComponent.this.bounds.w + dx,
                                                       (float) PlainDraggableComponent.this.bounds.h);
        }
        if (currentResize.contains(Resize.up)) {
            PlainDraggableComponent.this.shouldSetSize((float) PlainDraggableComponent.this.bounds.x,
                                                       (float) PlainDraggableComponent.this.bounds.y + dy,
                                                       (float) PlainDraggableComponent.this.bounds.w,
                                                       (float) PlainDraggableComponent.this.bounds.h - dy);
        }
