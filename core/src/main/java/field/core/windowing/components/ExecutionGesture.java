...
class ExecutionGesture {
    ...
    void paint() {
        ...
        for (Vector2 v : gesture) {
            if (c.events.isEmpty()) ii.moveTo(v.x, v.y);
            else ii.lineTo(v.x, v.y);
        }
...

        {
            ...
            Vector2 n = new Vector2((0.5f + gesture.get(0).y) - gesture.get(gesture.size() - 1).y,
                                    (0.25f - gesture.get(0).x) + gesture.get(gesture.size() - 1).x).normalize()
                                                                                                   .scale(15);
//			;//System.out.println(" label should be here <" + n + ">");
            ...
            d.getProperties()
             .put(iLinearGraphicsContext.color,
                  new Vector4(Constants.execution_color.x,
                              Constants.execution_color.y,
                              Constants.execution_color.z,
                              0.5));
            d.getProperties()
             .put(iLinearGraphicsContext.fillColor,
                  new Vector4(Constants.execution_color.x,
                              Constants.execution_color.y,
                              Constants.execution_color.z,
                              0.5));
            ...
            d.getInput()
             .setPointAttribute(iLinearGraphicsContext.font_v, new Font(Constants.defaultFont, Font.PLAIN, 10));
        ...
        for (Triple<iVisualElement, Vector2, Vector2> gt : ordering) {
            {
                ...
                d.getProperties()
                 .put(iLinearGraphicsContext.pointColor,
                      new Vector4(Constants.execution_color.x,
                                  Constants.execution_color.y,
                                  Constants.execution_color.z,
                                  0.5));
...

            {
                ...
                Vector2 n =
                        new Vector2((0.5f + gt.middle.y) - gt.right.y, (0.25f + gt.right.x) - gt.middle.x).normalize()
                                                                                                          .scale(15);

                d.getInput()
                 .moveTo(n.x + (((gt.middle.x * 3) + gt.right.x) / 4), n.y + (((gt.middle.y * 3) + gt.right.y) / 4));
                d.getProperties().put(iLinearGraphicsContext.containsText, true);
                d.getProperties()
                 .put(iLinearGraphicsContext.color,
                      new Vector4(Constants.execution_color.x,
                ...
                d.getInput()
                 .setPointAttribute(iLinearGraphicsContext.font_v, new Font(Constants.defaultFont, Font.BOLD, 15));
