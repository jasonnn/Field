...
class GLComponentWindow implements Listener, IUpdateable, iProvidesQueue, iThreedDrawingSurface {
    ...
    class ComponentContainer implements iComponent {
        ...
        public
        void addAsAllEventHandler(iComponent component) {
            if (!allEvents.contains(component)) allEvents.add(component);
        }
        ...
        public
        void display(ComponentContainer inside, iCoordinateFrame frameSoFar) {
        ...
        public
        iComponent hit(Event arg0) {
            if (focusLocked) return focusLockedTo;
            iComponent hit = null;
        ...
        public
        iComponent hit(GLComponentWindow window, Vector2 arg0) {
        ...
        public
        void keyPressed(ComponentContainer from, Event arg0) {

            // TODO swt keyboard dispatch
            if (arg0.keyCode == SWT.CAPS_LOCK) setNeedsRedisplay(true);

            if (keysDown.add(arg0.keyCode)) setNeedsRedisplay(true);

            if (arg0.keyCode == SWT.F1) {
                getCurrentWindow(this).resetViewParameters();
                setNeedsRedisplay(true);
            }
            else if (arg0.keyCode == '='
        ...
        public
        void keyReleased(ComponentContainer from, Event arg0) {
            ...
            if (keysDown.remove(arg0.keyCode)) setNeedsRedisplay(true);

        }

        public
        void keyTyped(ComponentContainer from, Event arg0) {
            for (iComponent c : allEvents)
                if (c != mouseFocus) {
        ...
        public
        void mouseClicked(ComponentContainer from, Event arg0) {
            beginMouseFocus(this, arg0);

            for (iComponent c : new ArrayList<iComponent>(allEvents))
                if (c != mouseFocus) c.mouseClicked(this, arg0);
        ...
        public
        void mouseDragged(ComponentContainer from, Event arg0) {
            GLComponentWindow.currentWindow.alwaysVisible = false;

            for (iComponent c : allEvents)
                if (c != mouseFocus) c.mouseDragged(this, arg0);
        ...
        public
        void mouseEntered(ComponentContainer from, Event arg0) {
            beginMouseFocus(this, arg0);
            if (mouseFocus != null) {
        ...
        public
        void mouseExited(ComponentContainer from, Event arg0) {
            if (mouseFocus != null) {
        ...
        public
        void mouseMoved(ComponentContainer from, Event arg0) {
            ...
            if (h != null) h.mouseMoved(this, arg0);

            for (iComponent c : allEvents)
                if (c != h) c.mouseMoved(this, arg0);

        }

        public
        void mousePressed(ComponentContainer from, Event arg0) {
            beginMouseFocus(this, arg0);
        ...
        public
        void mouseReleased(ComponentContainer from, Event arg0) {
        ...
        void paint(ComponentContainer inside, iCoordinateFrame frameSoFar, boolean visible) {
            ...
            for (iComponent c : new ArrayList<iComponent>(components)) {
                ...
                if (c instanceof DraggableComponent) {
                    ...
                    if (bounds.x > w * sx + tx
                        || bounds.y > h * sy + ty
                        || (bounds.x + bounds.w) < tx
                        || (bounds.y + bounds.h) < ty) {
                        skip = true;
                    }
                }
        ...
        public
        void removeAsAllEventHandler(iComponent component) {
        ...
        void requestRedisplay() {
            setNeedsRedisplay(true);
            if (parent != null) parent.requestRedisplay();
        ...
        private
        void beginMouseFocus(ComponentContainer from, Event arg0) {
        ...
        private
        void endMouseFocus(ComponentContainer from, Event arg0) {

            if (mouseFocus != null) mouseFocus.endMouseFocus(this);
            if (mouseFocusStack.size() > 0) mouseFocusStack.pop();
            if (mouseFocusStack.size() > 0) mouseFocus = mouseFocusStack.peek();
    ...
    @Woven
    public
    class NUpPNGSaver extends TaskQueue.Task {
        ...
        public
        NUpPNGSaver(String filename, TaskQueue queue, int width, int height) {
            this(filename, queue, width, height, width, height);
        }

        public
        NUpPNGSaver(String filename, TaskQueue queue, int width, int height, int maxW, int maxH) {
            queue.super();
    ...
    Vector4 background =
            new Vector4(SystemProperties.getDoubleProperty("canvasBackground.r", (Platform.isMac() ? 200 : 220) / 255f),
                        SystemProperties.getDoubleProperty("canvasBackground.g", (Platform.isMac() ? 200 : 220) / 255f),
                        SystemProperties.getDoubleProperty("canvasBackground.b", (Platform.isMac() ? 200 : 220) / 255f),
                        0);
    ...
    public
    GLComponentWindow(String name, TaskQueue q) {
        ...
        if (Platform.isMac()) {
            ...
            try {
                canvas = (Canvas) this.getClass()
                                      .getClassLoader()
                                      .loadClass("field.core.windowing.GLCanvas_field")
                                      .getConstructor(Composite.class, Integer.TYPE, GLData.class)
                                      .newInstance(hsplit, SWT.NO_BACKGROUND, data);
                canvasInterface = (iCanvasInterface) this.getClass()
                                                         .getClassLoader()
                                                         .loadClass("field.core.windowing.MacOSXCanvasInterface")
                                                         .getConstructor(Canvas.class)
                                                         .newInstance(canvas);
            } catch (Throwable t) {
                t.printStackTrace();
        ...
        if (Platform.isLinux()) canvas.addListener(SWT.Paint, new Listener() {
        ...
        new Thread(new Runnable() {
            ...
            void run() {
                while (true) {
                    ...
                    if (fps > 0) System.err.println("canvas at fps:" + fps);
                    canvasFps = fps;
                }
            }
        ...
        componentWindowStatusbar = new ComponentWindowStatusbar(this, this.textSystem) {
            ...
            String getText() {
                ...
                boolean caps = Platform.getOS() == OS.mac
                ...
                if (present) {
                    t += " <b>Presentation</b> mode: " + (draft
                                                          ? "<b>drafting</b> <i>(toggle with F2)</i>"
                                                          : "<b>preview</b> <i>(toggle with F2)</i>");
                }
                else {
                    t += " Rendering mode: " + (draft
                                                ? "<b>drafting</b> <i>(toggle with F2)</i>"
                                                : "<b>preview</b> <i>(toggle with F2)</i>");
                }
                ...
                t += " Focus: <b>" + (root.focusLocked
                                      ? "locked</b> <i>(press ` to unlock)</i>"
                                      : "free</b> <i>(press ` to select)</i>");
                ...
                if (disableRepaint || disableRepaintNext) {
                    t += " <font face='"
                         ...
                         + "' size=-2 color='#660000'>Repainting is disabled</font>";
                    // ((JFrame)
                ...
                if (continuousRepaint) {
                    t += " <font face='"
                         + Constants.defaultFont
                         + "' size=-2 color='#660000'><b>Canvas is animating</b></font>";
                }
    ...
    public
    GLComponentWindow(TaskQueue queue) {
    ...
    public
    Vector2 getCurrentMouseInWindowCoordinates() {

        Vector2 currentMouse = getCurrentMousePosition();

        return new Vector2((currentMouse.x) * sx + tx,
                           (Platform.isLinux() ? -53 : 0) + yoffset + (currentMouse.y) * sy + ty);
    }
    ...
    public
    TaskQueue getRunQueue() {
        return runQueue;
    }

    public
    BasicSceneList getSceneList() {
        return sceneList;
    }
    ...
    public
    float getDYTranslation() {
        return dty;
    }

    @InQueue
    public
    void keyPressed(Event arg0) {
        currentWindow = this;
    ...
    public
    void toggleContinuousRepaintNow() {
    ...
    public
    void saveNUpWindowAsPNG(final String filename, int width, int height) {
    ...
    public
    void setEventProcessingTaskQueue(TaskQueue eventProcessingTaskQueue) {
    ...
    public
    void transformMouseEvent(Event arg0) {
    ...
    public
    void transformDrawingToWindow(Vector2 v) {
        v.x = (v.x - tx) / sx;
        v.y = (v.y - ty) / sy;
    }

    public
    void transformWindowToDrawing(Vector2 v) {
        v.x = (v.x * sx + tx);
        v.y = (v.y * sy + ty);
    }
    ...
    public
    Vector2 transformWindowToCanvas(Vector2 v) {
    ...
    public
    void untransformMouseEvent(Event arg0) {
    ...
    void update() {
        ...
        if (!disableRepaint && (root.isNeedsRedisplay() || (continuousRepaint && ((continuousRepaintCount++
                                                                                   % continuousRepaintStrobe) == 0)))) {
            frameNumber++;
    ...
    void display() {
        ...
        dsx = sx;
        dsy = sy;
        if (editorSpaceHelper != null) editorSpaceHelper.thaw();

        // ThreadedLauncher.lock2.lock();
        try {

            if (disableRepaint) return;
    ...
    void doMagnificationGesture() {
        ...
        if (Math.abs(1 - magnificationMomentum) > 1e-2) requestRepaint();
    ...
    void init() {
        ...
        // ,
        // false));
        masterProgram = new BasicGLSLangProgram("content/shaders/glComponentVertex.glslang",
    ...
    void handleEvent(Event event) {
        ...
        switch (event.type) {
            ...
            case SWT.MouseDown:
                ...
                mousePressed(event);
                if ((event.stateMask & SWT.BUTTON1) != 0) handleEvent_mouseDown.add(1);
                if ((event.stateMask & SWT.BUTTON2) != 0) handleEvent_mouseDown.add(2);
                if ((event.stateMask & SWT.BUTTON3) != 0) handleEvent_mouseDown.add(3);
            ...
            case SWT.MouseMove:
                ...
                if (handleEvent_mouseDown.size() == 0) {
                    ...
                    event.button = handleEvent_mouseDown.contains(1)
                                   ? 1
                                   : (handleEvent_mouseDown.contains(2)
                                      ? 2
                                      : (handleEvent_mouseDown.contains(3) ? 3 : 0));
                    mouseDragged(event);
                }
    ...
    public
    Set<Integer> getMouseButtonsDown() {
    ...
    void setEditorSpaceHelper(IVisualElement root) {
        ...
        Launcher.getLauncher().addPostUpdateable(new IUpdateable() {
            ...
            public
            void update() {
                if (canvas.isDisposed()) Launcher.getLauncher().deregisterUpdateable(this);
                else editorSpaceHelper.freeze();
            }
        });
    }
