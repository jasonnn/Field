 ...
 */
public
class QuaquaDecoratedFrameResizeFix implements MouseListener, MouseMotionListener {
    ...
    public
    QuaquaDecoratedFrameResizeFix(JComponent container, JFrame frame) {
    ...
    private
    boolean redispatchMouse(MouseEvent e, boolean repaint) {
        ...
        if ((componentPoint.x > (b.width - 15)) && (componentPoint.y > (b.height - 15))) {
            ...
            try {
                processMouseevent.invoke(frame,
                                         new MouseEvent(frame,
                                                        e.getID(),
                                                        e.getWhen(),
                                                        e.getModifiers(),
                                                        rootPanePoint.x,
    ...
    private
    void redispatchMouseMotion(MouseEvent e, boolean repaint) {
        Point componentPoint = e.getPoint();
        Container container = component;
        Rectangle b = container.getBounds();
