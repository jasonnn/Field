...
class CocoaUIEnhancer {
     ...
     */
    public
    CocoaUIEnhancer(String appName) {
    ...
    private
    void initialize(Object callbackObject) throws Exception {
    ...
    private static
    long registerName(Class<?> osCls, String name) throws
    ...
    private static
    Object invoke(Class<?> clazz, Object target, String methodName, Object[] args) {
        try {
            Class<?>[] signature = new Class<?>[args.length];
            for (int i = 0; i < args.length; i++) {
                Class<?> thisClass = args[i].getClass();
                if (thisClass == Integer.class) signature[i] = int.class;
                else if (thisClass == Long.class) signature[i] = long.class;
                else if (thisClass == Byte.class) signature[i] = byte.class;
                else if (thisClass == Boolean.class) signature[i] = boolean.class;
