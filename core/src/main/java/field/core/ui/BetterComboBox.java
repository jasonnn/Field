...
class BetterComboBox {
    ...
    public
    BetterComboBox(Composite inside, String[] labels) {
        ...
        combo.setFont(new Font(combo.getFont().getDevice(),
                               combo.getFont().getFontData()[0].name,
                               GraphNodeToTreeFancy.baseFontHeight(combo),
                               SWT.NORMAL));
                        ...
                        map.put(((n == currentlySelected) ? "!" : "") + BetterComboBox.this.labels[n],
                                new iUpdateable() {
                                    ...
                                    void update() {
                                        ...
                                        if (!fixedText) combo.setText(BetterComboBox.this.labels[currentlySelected]);
    ...
    void forceSelection(String label) {
        for (int n = 0; n < labels.length; n++) {
            if (labels[n].equals(label)) {
                currentlySelected = n;
                if (!fixedText) combo.setText(label);
                combo.redraw();
            }
        }
    ...
    void selectNext() {
        ...
        if (!fixedText) combo.setText(labels[currentlySelected]);
        combo.redraw();
    ...
    void setLabels(String[] labels) {
        ...
        for (int i = 0; i < this.labels.length; i++)
            if (labels[i].equals(c)) currentlySelected = i;
        if (currentlySelected >= labels.length) currentlySelected = labels.length - 1;

        if ((currentlySelected != -1) && !fixedText) combo.setText(labels[currentlySelected]);
    }

    public
    void setLabelsWithHTML(String[] labels) {
    ...
    public
    void setHoverUpdate(iHoverUpdate iHoverUpdate) {
        this.hu = iHoverUpdate;

    }

    public
    BetterComboBox setRightJustified(boolean rightJustified) {
        this.rightJustified = rightJustified;
        return this;
    }

    public
    BetterComboBox setNoDrag(boolean noDrag) {
        this.noDrag = noDrag;
        return this;
    }

    public
    BetterComboBox setFixedText(boolean fixedText) {
