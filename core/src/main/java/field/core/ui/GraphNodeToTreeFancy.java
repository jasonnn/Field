...
public
class GraphNodeToTreeFancy {

    public static
    int baseFontHeight(Control install) {
        if (Platform.isMac()) return install.getFont().getFontData()[0].getHeight() - 1;
        else return 8;
    }
    ...
    public
    GraphNodeToTreeFancy(Tree target) {
        this.target = target;
    }
    ...
    class Pretty {
        ...
        Pattern SMALLER_PATTERN = Pattern.compile("(<font size=-3 color='#"
                                                  + Constants.defaultTreeColorDim
                                                  + "'>)(?=\\S)(.+?)(?<=\\S)(</font>)");
        ...
        public
        Pretty(Tree install, final int fixedWidth) {
