...
public
class NewInspectorFromProperties_swing {

    public
    NewInspectorFromProperties_swing() {
    }

    public static LinkedHashMap<String, Class<? extends JComponent>> knownProperties =
            new LinkedHashMap<String, Class<? extends JComponent>>();
    public static LinkedHashMap<String, String> knownAliases = new LinkedHashMap<String, String>();

    public static List<Triple<String, LinkedHashMap<String, Class<? extends JComponent>>, Boolean>> activeSets =
            new ArrayList<Triple<String, LinkedHashMap<String, Class<? extends JComponent>>, Boolean>>();
    ...
    public
    void add(Inspected i) {
        Inspected b = build.get(i.name);
        if (b == null) build.put(i.name, i);
        else build.put(i.name, coallesce(b, i));
    }

    public
    List<Inspected> complete() {
    ...
    <A, B extends JComponent> Inspected<A, B> coallesce(final Inspected<A, B> a, final Inspected<A, B> b) {
        ...
        Inspected<A, B> ii = new Inspected<A, B>() {
            ...
            public
            Status getStatus() {
                if (a.getStatus() == Status.unset) return Status.unset;
                if (b.getStatus() == Status.unset) return Status.unset;
                if (a.getStatus() == Status.mixed) return Status.mixed;
                if (b.getStatus() == Status.mixed) return Status.mixed;
        ...
        ii.name = a.name;
        ii.editor = a.editor;
        if (ii.editor instanceof SetValueForControl) ((SetValueForControl) ii.editor).setValueForControl(ii);
        return ii;
    }
...
//			add(color2);
            ...
            for (final Triple<String, LinkedHashMap<String, Class<? extends JComponent>>, Boolean> t : activeSets) {
                ...
                for (Map.Entry<String, Class<? extends JComponent>> ee : t.middle.entrySet()) {
                    final VisualElementProperty p = new VisualElementProperty(ee.getKey());
...
//		editor.setComponentPopupMenu(new SmallMenu().createMenu(pop));
    ...
    Class<? extends JComponent> componentForValue(Object value) {
        if (value instanceof String) return TextControl.class;
        if (value instanceof Integer) return IntControl.class;
        if (value instanceof Boolean) return BooleanControl.class;
        if (value instanceof Vector4) return ColorControl.class;
        if (value instanceof Action) return ActionControl.class;
