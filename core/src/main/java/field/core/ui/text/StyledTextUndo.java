...
class StyledTextUndo {
    ...
    class Change {
        ...
        public
        Change(int start, int length, String replacedBy, int selectionStart, int selectionLength) {
            super();
            this.start = start;
            this.length = length;
    ...
    public
    StyledTextUndo(final StyledText target) {
        ...
        target.addExtendedModifyListener(new ExtendedModifyListener() {
            ...
            void modifyText(ExtendedModifyEvent event) {
                ...
                if (ignore) {
                    Change c =
                            new Change(event.start, event.length, event.replacedText, selectionStart, selectionLength);

                    redo.add(c);
                    if (redo.size() > limit) redo.remove(0);
                }
                else {
                    Change c =
                            new Change(event.start, event.length, event.replacedText, selectionStart, selectionLength);
                    ...
                    changes.add(c);
                    if (changes.size() > limit) changes.remove(0);

                    if (!isRedoing) redo.clear();
    ...
    public
    void fromMemo(Memo m) {
        changes.clear();
        if (m != null) changes.addAll(m.changes);
        redo.clear();
        if (m != null) redo.addAll(m.redo);
    }
}
