...
public
class ExecutableAreaFinder {

    public int lastSubstring_start;
    public int lastSubstring_end;

    public
    String findExecutableSubstring(int caretPosition, String string, int key) {
        ...
        if (m.find()) {
            ...
            Matcher m2 = Pattern.compile("#--\\{[^\\{]*\\\\" + key + "\\.")
                                .matcher(string.substring(caretPosition, string.length()));
            if (m2.find()) {
    ...
    public
    String findExecutableSubstringAnyKey(int caretPosition, String string) {
