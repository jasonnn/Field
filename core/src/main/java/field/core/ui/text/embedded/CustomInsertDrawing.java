...
@Woven
public
class CustomInsertDrawing {

    float margin = 0;

    public
    interface iInsertRenderingContext {
        public
        StyledText getText();
    ...
    public
    interface iAcceptsInsertRenderingContext {
        public
    ...
    public
    class Nub implements iInsertRenderingContext {
        ...
        public
        Nub(final JComponent component, StyledText target, int width, int height) {
            this.component = component;
            this.target = target;
            this.width = width;
            this.height = height;

            input = GraphicsEnvironment.getLocalGraphicsEnvironment()
            ...
            ((Graphics2D) inputGraphics).setRenderingHint(RenderingHints.KEY_ANTIALIASING,
                                                          RenderingHints.VALUE_ANTIALIAS_ON);
            ((Graphics2D) inputGraphics).setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
            ...
            canvas.addPaintListener(new PaintListener() {
                ...
                void paintControl(PaintEvent e) {
                    ...
                    output = updageAndMakeImage(e.gc.getDevice(),
                                                Math.max(Nub.this.width, e.width),
                                                Math.max(Nub.this.height, e.height));

                    if (output != null) {
                        Rectangle mapped = Launcher.display.map(canvas,
                                                                Nub.this.target,
                                                                new Rectangle(0,
                                                                              0,
                                                                              Math.max(Nub.this.width, e.width),
...
//						System.out.println("CA : "+Nub.this.target.getTopPixel());
                        ...
                        if (ly == Nub.this.target.getTopPixel()) e.gc.drawImage(output, 0, 0);
                        else System.out.println(" hidden control <" + ly + ' ' + Nub.this.target.getTopPixel());
                    }
                }
            });
...
//					;//System.out.println(" event in component <" + event + ">");
                    ...
                    switch (event.type) {
                        case (SWT.MouseMove):
                            if (down) {
                                java.awt.event.MouseEvent m = new java.awt.event.MouseEvent(Nub.this.component,
                ...
                int modifiers(Event event) {
                    int m = 0;
                    if ((event.stateMask & SWT.SHIFT) != 0) m |= java.awt.Event.SHIFT_MASK;
                    if ((event.stateMask & SWT.ALT) != 0) m |= java.awt.Event.ALT_MASK;
                    if ((event.stateMask & SWT.COMMAND) != 0) m |= java.awt.Event.META_MASK;
                    return m;
        ...
        protected
        Image updageAndMakeImage(Device d, int width, int height) {
    ...
    public
    CustomInsertDrawing(final StyledText target)
