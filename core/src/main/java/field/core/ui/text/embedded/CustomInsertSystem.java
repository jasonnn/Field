...
public
class CustomInsertSystem implements iCustomInsertSystem {
    ...
    class iPossibleComponent {
        ...
        public
        iPossibleComponent(String name, Class<? extends ProvidedComponent> clazz) {
            this.name = name;
            this.clazz = clazz;
        }
    ...
    class ProvidedComponent {
        ...
        public
        ProvidedComponent(JComponent js) {
            component = js;
        }
        ...
        public
        void executeThisLine(iInsertRenderingContext irc, ExecutesWhat enclosing) {
            ...
            if (indexOf != -1) {
                ...
                for (int n = 0; n < lines.length; n++) {
                    ...
                    if (start <= indexOf && end > indexOf) {
                        ...
                        EditorExecutionInterface eei =
                                ((PythonPluginEditor) PythonPluginEditor.python_plugin.get(inside)).getEditor()
                                                                                                   .getInterface();

                        if (enclosing == ExecutesWhat.enclosingBlock) {
                            String substring =
                                    new ExecutableAreaFinder().findExecutableSubstringAnyKey(editor.getCustomInsertSystem()
                                                                                                   .convertUserTextPositionToExecutableTextPosition(text,
                                                                                                                                                    indexOf),
                                                                                             editor.getCustomInsertSystem()
                                                                                                   .convertUserTextToExecutableText(text));
        ...
        public
        int getPositionFor(CustomInsertDrawing.iInsertRenderingContext context, boolean startPosition) {
        ...
        public
        Rect boundsOfRange(CustomInsertDrawing.iInsertRenderingContext context, int start, int end) {
    ...
    public static List<Triple<String, iPossibleComponent, iPossibleComponent>> possibleWrappers =
            new ArrayList<Triple<String, iPossibleComponent, iPossibleComponent>>();
    ...
    void defaultPossibleInserts() {
        possibleComponents.add(new iPossibleComponent(" \u25a5\t<b>Label</b> (commented section with keyboard shortcut)",
                                                      MinimalTextField.Component.class));
        ...
        possibleComponents.add(new iPossibleComponent(" \u25a7\tBlock \u2014 <b>Text Transform begin</b>",
                                                      MinimalTextField_blockMenu.Component_transformBlockStart.class));
        possibleComponents.add(new iPossibleComponent(" \u25a7\tBlock \u2014 <b>Text Transform end</b>",
                                                      MinimalTextField_blockMenu.Component_transformBlockEnd.class));
        possibleComponents.add(new iPossibleComponent(" \u21a6\t<b>Slider</b> (returns <i>float</i>)",
                                                      MinimalSlider.Component.class));
        possibleComponents.add(new iPossibleComponent(" \u21a3\t<b>Slider</b> (returns <i>Lazy / iFloatProvider</i>)",
                                                      MinimalSlider.Component_iFloatProvider.class));
        possibleComponents.add(new iPossibleComponent(" \u2698\t<b>Color</b> (returns <i>Vector4</i> between 0 and 1)",
                                                      MinimalColorWell.Component.class));
        possibleComponents.add(new iPossibleComponent(" \u2698\t<b>Color * 255</b> (returns <i>Vector4</i> between 0 and 255)",
                                                      MinimalColorWell.Component255.class));
        // possibleComponents.add(new
        // iPossibleComponent(" \u22a1\t<b>File</b> (returns <i>java.io.File</i>)",
        // MinimalFileBox.Component.class));
        possibleComponents.add(new iPossibleComponent(" \u22a1\t<b>x,y</b> (returns <i>2-tuple</i>)",
                                                      MinimalXYSlider.Component_tuple.class));
        possibleComponents.add(new iPossibleComponent(" \u22a1\t<b>x,y</b> (returns <i>Vector2</i>)",
                                                      MinimalXYSlider.Component_vector2.class));
        possibleComponents.add(new iPossibleComponent(" \u22a1\t<b>x,y</b> (returns <i>Vector3</i>)",
                                                      MinimalXYSlider.Component_vector3.class));
        possibleComponents.add(new iPossibleComponent(" \u22a1\t<b>x,y</b> (returns <i>Lazy / iProvider<Vector3></i>",
                                                      MinimalXYSlider.Component_vector3_provider.class));
        // possibleComponents.add(new
        // iPossibleComponent(" \u223f\t<b>graph</b>",
        // MinimalGraphWidget.Component_tuple.class));
        possibleComponents.add(new iPossibleComponent(" \u223f\t<b>graph editor</b>",
                                                      MinimalGraphWidget2.Component_tuple.class));
        possibleComponents.add(new iPossibleComponent(" \u223f\t<b>image histogram (experiemental)</b>",
                                                      MinimalImageHistogram.Component.class));
        possibleComponents.add(new iPossibleComponent(" \u223f\t<b>stats box (experiemental)</b>",
                                                      MinimalStatsBox.Component.class));
        // possibleComponents.add(new
        ...
        possibleComponents.add(new iPossibleComponent(" \u21b7\t<b>stepper</b> for generators",
                                                      MinimalStepper.Component_Stepper.class));
        ...
        possibleWrappers.add(new Triple<String, iPossibleComponent, iPossibleComponent>(" \u25a8 wrap selected text in a <b>new labeled block</b> ",
                                                                                        new iPossibleComponent("",
                                                                                                               MinimalTextField.Component.class) {
                                                                                            @Override
                                                                                            public
        ...
        possibleWrappers.add(new Triple<String, iPossibleComponent, iPossibleComponent>(" \u25a7 wrap selected text in a <b>new transform block</b>",
                                                                                        new iPossibleComponent("",
                                                                                                               MinimalTextField_blockMenu.Component_transformBlockStart.class) {
                                                                                            @Override
                                                                                            public
    ...
    public
    ProvidedComponent addComponent(ProvidedComponent comp) {
        while (comps.containsKey(currentMaxTagNumber)) currentMaxTagNumber++;
    ...
    public
    int convertUserTextPositionToExecutableTextPosition(String text, int caretPosition) {
        String input = text;
        ...
        for (int i = replacements.size() - 1; i >= 0; i--) {
            ...
            if (replace.from < caretPosition) caretPosition -= (replace.to - replace.from) - replace.with.length();
        }
    ...
    public
    String mergeInText(Pair<String, Object> text) {

        if (text.right == null) return text.left;
        ...
        try {
            for (int i = 0; i < replacements.size(); i++) {
                ...
                if (replace.comp == null || replace.comp.tagNumber != -1) {
                    input = input.substring(0, replace.from)
                            + (replace.comp == null
                               ? replace.was
                               : stringForTag(replace.comp.tagNumber))
                            + input.substring(replace.from + replace.with.length(), input.length());
                    if (replace.comp != null) comps.put(replace.comp.tagNumber, replace.comp);
                }
                else {
                    // need
                    // to
                    ...
                    input = input.substring(0, replace.from) + replace.was + input.substring(replace.from + replace.with
                                                                                                                    .length(),
                                                                                             input.length());
                    // input
                    // =
    ...
    public
    Pair<String, Object> mergeOutText(String text) {
        String input = text;
        ...
        while (matcher.find()) {
            ...
            replace.with =
                    (replace.comp == null ? "((missing component))" : replace.comp.getCurrentRepresentedString());
    ...
    private
    ProvidedComponent componentForTag(int i) {
        ProvidedComponent c = comps.get(i);
        if (c == null) System.err.println(ANSIColorUtils.red("no component for <" + i + "> <" + comps + ">"));
        return c;
    }

    private
    ProvidedComponent duplicateComponent(ProvidedComponent component) {
    ...
    boolean foundIn(List<Replace> replacements, int tagnumber) {
        for (Replace r : replacements) {
            if (r.comp != null && r.comp.tagNumber == tagnumber) return true;
        }
        return false;
    }
