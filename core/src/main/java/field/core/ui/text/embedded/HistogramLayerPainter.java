...
public
class HistogramLayerPainter implements iMinimalLayerPainter {
    ...
    void paintNow(Graphics2D g, Dimension2D size) {
        if (drawStyle == DrawStyle.histogram) paintNowHistogram(g, size);
        else if (drawStyle == DrawStyle.stripchart) paintNowStripchart(g, size);
    }

    public
    void paintNowHistogram(Graphics2D g, Dimension2D size) {
    ...
    public
    void paintNowStripchart(Graphics2D g, Dimension2D size) {
        minX = Float.POSITIVE_INFINITY;
        maxX = Float.NEGATIVE_INFINITY;
        minY = Float.POSITIVE_INFINITY;
        maxY = Float.NEGATIVE_INFINITY;
    ...
    public
    iMinimalLayerPainter setHistogram(Histogram<Number> r) {
        forDisplayHistogram = r;
        return this;
    }

    private
    void draw(float prevx,
              float prevy,
              float nextx,
              float nexty,
              float nx,
        ...
        g.setFont(font);
        g.setColor(baseColor);
        g.drawString(BaseMath.toDP(vx, 2) + '/' + BaseMath.toDP(vy, 2),
                     (prevx + nextx) / 2,
                     (float) ((size.getHeight() / 2) - (ny / 2)));

    }

    protected
    void draw(float prevx, float nextx, float nx, float ny, Graphics2D g, Dimension2D size, float vx, float vy) {
