...
public
class MinimalXYSlider extends MinimalExpandable {
    public static
    class Component_tuple extends ProvidedComponent {
        ...
        void deserialize(IVisualElement inside) {
            ...
            component = new MinimalXYSlider() {
                ...
                @Override
                public
                void setValue(float lvalue, float lvalueY) {
        ...
        public
        void preserialize() {
            if (component != null) when = ((MinimalXYSlider) component).when;
            if (component != null) what = ((MinimalXYSlider) component).what;
        }

        protected
        void updateValue() {
            if (notify != null) notify.update();
        }
    }
    ...
    class Component_vector3_provider extends Component_tuple {
        ...
        public
        String getCurrentRepresentedString() {
            if (localKey == null) localKey = new OKey<Vector3>(name).rootSet(new Vector3(value, valueY, 0));
    ...
    void setValue(float lvalue, float lvalueY) {
        ...
        if (Float.isNaN(lvalue)) return;
        if (Float.isNaN(lvalueY)) return;
    ...
    void paintComponent(Graphics g) {
        ...
        if (on) g2.fillRect(leftX - 2, topY - 2, 5 + rightX - leftX, 5 + bottomY - topY);
        ...
        g2.drawString(label,
                      (maxCollapse - collapseness) * 5 + Math.max(2,
                                                                  Math.min(bounds.width - w - 12,
                                                                           (leftX + rightX - w) / 2)),
                      Math.min(bounds.height - sliderHeight / 3,
                               (topY + bottomY) / 2 + 3 + (maxCollapse - collapseness) * 4));
    ...
    void processMouseEvent(MouseEvent e) {
        ...
        if (e.getID() == MouseEvent.MOUSE_PRESSED && (e.getModifiers() & InputEvent.SHIFT_MASK) == 0) {
            int leftX = leftX();
            int rightX = rightX();
            int bottomY = bottomY();
            int topY = topY();
        ...
        else if (e.getID() == MouseEvent.MOUSE_PRESSED && (e.getModifiers() & InputEvent.SHIFT_MASK) != 0) {
    ...
    void processMouseMotionEvent(MouseEvent e) {
        ...
        int x = e.getX() - 0 * this.bounds().x;
        int y = e.getY() - 0 * this.bounds().y;
        if (e.getID() == MouseEvent.MOUSE_DRAGGED && on) {
