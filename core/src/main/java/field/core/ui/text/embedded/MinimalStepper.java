...
public
class MinimalStepper extends JComponent {
...
//						return null;
        ...
        String getCurrentRepresentedString() {
            if (localKey == null) localKey = new OKey(name).rootSet(value);
    ...
    public
    MinimalStepper(String name, int width) {
        this(name);
        this.width = width;
    }
    ...
    void processMouseEvent(MouseEvent e) {
        ...
        if ((e.getModifiers() & Event.ALT_MASK) == 0 && e.getID() == MouseEvent.MOUSE_RELEASED) {
            ...
            arm = false;
            repaint();
            if (auto != null) Launcher.getLauncher().deregisterUpdateable(auto);
            auto = null;
            executed = false;
            repaint();
        }
