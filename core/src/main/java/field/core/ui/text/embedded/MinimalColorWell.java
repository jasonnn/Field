...
@Woven
public
class MinimalColorWell extends JComponent implements iAcceptsInsertRenderingContext {
    ...
    class Component extends ProvidedComponent {
        ...
        public
        void preserialize() {
            if (component != null) when = ((MinimalColorWell) component).when;
            if (component != null) what = ((MinimalColorWell) component).what;
        }
    ...
    void changeColor() {
        if (Platform.isMac()) {
            ...
            org.eclipse.swt.widgets.ColorDialog d =
                    new org.eclipse.swt.widgets.ColorDialog(CustomInsertDrawing.currentNub.canvas.getShell(),
                                                            SWT.PRIMARY_MODAL);
...
//		color.w = d.getAlpha() / 255f;
    ...
    void setValue(Vector4 value) {
        ...
        if (color.distanceFrom(value) > 1e-5f) changed = true;
        color.set(value);
        if (changed) {
            repaint();
            if (when != ExecutesWhen.never) needsExecuting = true;
        }
    }
