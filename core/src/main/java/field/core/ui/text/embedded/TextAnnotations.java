...
class TextAnnotations {
    ...
    public static final VisualElementProperty<Package> textEditorAnnotationPackage =
            new VisualElementProperty<Package>("__textEditorAnnotationPackage");
    ...
    class Annotation implements iExtensible {
        ...
        public
        boolean draw(JEditorPane ed, Graphics2D g) {
        ...
        public
        void set(int oldStart, int oldEnd, Document d) {
    ...
    class Package {
        ...
        private HashMapOfLists<VisualElementProperty, Annotation> annotations =
                new HashMapOfLists<VisualElementProperty, Annotation>();
...
//		;//System.out.println(" setting annotation package to <" + pack.getAnnotations(property) + ">");
    ...
    public
    void draw(JEditorPane d, Graphics2D g) {
        LinkedHashSet<Annotation> dead = new LinkedHashSet<Annotation>();
        for (Annotation a : pack.getAnnotations(property)) {
            if (!a.draw(d, g)) {
                dead.add(a);
    ...
    public
    List<Annotation> isInside(int position) {
