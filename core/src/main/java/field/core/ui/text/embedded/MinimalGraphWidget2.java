...
public
class MinimalGraphWidget2 extends MinimalExpandable implements iRegistersMinimalLayerPainter {
    ...
    class Component_tuple extends ProvidedComponent {
        ...
        void deserialize(IVisualElement inside) {
            ...
            component = new MinimalGraphWidget2() {
                ...
                @Override
                protected
                void setValue(int on, float x, float y) {
        ...
        public
        void preserialize() {
            if (component != null) when = ((MinimalGraphWidget2) component).when;
            if (component != null) what = ((MinimalGraphWidget2) component).what;
        }
    ...
    class Component_tuple2 extends ProvidedComponent {
        ...
        public
        String getCurrentRepresentedString() {
            if (localKey == null) localKey = new OKey<InterpolatorWithWidget>(name).rootSet(interpolator);

            interpolator.widget = (iRegistersMinimalLayerPainter) component;
            return "transFilter(OKeyByName(\""
                   + name
                   + "\", u.fromXML(r\"\"\""
                   + new PythonUtils().toXML(interpolator,
                                             true)
                   + "\"\"\")).get())";
        }
    ...
    public
    CubicInterpolatorDynamic<BreakpointFloat> getInterpolator() {
        return interpolator;
    }
    ...
    public
    void setInterpolator(CubicInterpolatorDynamic<BreakpointFloat> interpolator) {
    ...
    void drawBox(Graphics2D g2,
                 CubicInterpolatorDynamic.Sample<BreakpointFloat> breakpointFilter,
                 Vector2 p,
                 CubicInterpolatorDynamic.Sample<BreakpointFloat> before,
                 CubicInterpolatorDynamic.Sample<BreakpointFloat> after) {
        int width = 5;
        ...
        if (after != null && breakpointFilter.data.next == Next.cubic) {
            Vector2 controlIs = transformRelativeToFrame(this.getBounds(),
                                                         new Vector2(breakpointFilter.time
                                                                     + (after.time - breakpointFilter.time) / 3,
                                                                     breakpointFilter.data.controlNext));

            Ellipse2D.Float circle =
                    new Ellipse2D.Float(controlIs.x - width / 2, controlIs.y - width / 2, width, width);
        ...
        if (before != null && before.data.next == Next.cubic) {
            Vector2 controlIs = transformRelativeToFrame(this.getBounds(),
                                                         new Vector2(breakpointFilter.time
                                                                     + (before.time - breakpointFilter.time) / 3,
                                                                     breakpointFilter.data.controlPrevious));
    ...
    void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < interpolator.getNumSamples(); i++) {
            ...
            drawBox(g2,
                    sample,
                    p,
                    i == 0 ? null : interpolator.getSample(i - 1),
                    i == interpolator.getNumSamples() - 1 ? null : interpolator.getSample(i + 1));
        }
    ...
    private
    void addPoint(Rectangle bounds, GeneralPath theCurve, Vector2 p, float alpha) {
        BreakpointFloat v = new BreakpointFloat(0);
        interpolator.getValue(alpha, v);
        transformRelativeToFrame(bounds, p.set(alpha, v.value));

        if (theCurve.getCurrentPoint() == null) theCurve.moveTo(p.x, p.y);
        else theCurve.lineTo(p.x, p.y);
    }
...
//					pop(menu, e);
                ...
                if (initialDown == -1) {
                    for (int i = 0; i < interpolator.getNumSamples() - 1; i++) {
                        if (interpolator.getSample(i).data.next != Next.cubic) continue;

                        Vector2 controlIs = transformRelativeToFrame(this.getBounds(),
                                                                     new Vector2(interpolator.getSample(i).time
                                                                                 + (interpolator.getSample(i + 1).time
                                                                                    - interpolator.getSample(i).time)
                                                                                   / 3,
                                                                                 interpolator.getSample(i).data.controlNext));
                ...
                if (initialDown == -1) {
                    for (int i = 1; i < interpolator.getNumSamples(); i++) {
                        if (interpolator.getSample(i - 1).data.next != Next.cubic) continue;
                        ...
                        Vector2 controlIs = transformRelativeToFrame(this.getBounds(),
                                                                     new Vector2(interpolator.getSample(i).time
                                                                                 + (interpolator.getSample(i - 1).time
                                                                                    - interpolator.getSample(i).time)
                                                                                   / 3,
                                                                                 interpolator.getSample(i).data.controlPrevious));
                        Vector2 m = new Vector2(x, this.getHeight() - y);
        ...
        if (e.getID() == MouseEvent.MOUSE_RELEASED) {
            ...
            repaint();
            if (when == ExecutesWhen.always || when == ExecutesWhen.onMouseUp) execute();
...
//			menu.show(window.getCanvas(), e.getX()+this.getBounds().x,e.getY()+this.getBounds().y-30);
    ...
    void processMouseMotionEvent(MouseEvent e) {
        ...
        int x = e.getX() - 0 * this.bounds().x;
        int y = this.getHeight() - (e.getY() - 0 * this.bounds().y);
        if (e.getID() == MouseEvent.MOUSE_DRAGGED && on == Dragging.on) {
            ...
            if (lvalueY > 1) lvalueY = 1;
            if (lvalueY < 0) lvalueY = 0;
            setValue(initialDownOn, lvalue, lvalueY);
            repaint();

            if (when == ExecutesWhen.always) execute();
        }
        if (e.getID() == MouseEvent.MOUSE_DRAGGED && on == Dragging.controlAfter) {
            int dx = x - initialDown;
            initialDown = x;
            int dy = y - initialDownY;
            initialDownY = y;
            ...
            float lvalue =
                    interpolator.getSample(initialDownOn).time + dx / (float) (this.getBounds().width - sliderWidth);
            if (lvalue > 1) lvalue = 1;
            if (lvalue < 0) lvalue = 0;
            float lvalueY =
                    interpolator.getSample(initialDownOn).data.controlNext + dy / (float) (this.getBounds().height
                                                                                           - sliderHeight);
            if (lvalueY > 1) lvalueY = 1;
            if (lvalueY < 0) lvalueY = 0;
        ...
        if (e.getID() == MouseEvent.MOUSE_DRAGGED && on == Dragging.controlBefore) {
            int dx = x - initialDown;
            initialDown = x;
            int dy = y - initialDownY;
            initialDownY = y;
            ...
            if (lvalue < 0) lvalue = 0;
            float lvalueY =
                    interpolator.getSample(initialDownOn).data.controlPrevious + dy / (float) (this.getBounds().height
                                                                                               - sliderHeight);
            if (lvalueY > 1) lvalueY = 1;
            if (lvalueY < 0) lvalueY = 0;
    ...
    protected
    void setControlNext(int on, float x, float y) {
        Sample<BreakpointFloat> sample = interpolator.getSample(on);
        interpolator.removeSample(sample);
        boolean changed = sample.time != x || sample.data.value != y;
    ...
    protected
    void setControlPrevious(int on, float x, float y) {
        Sample<BreakpointFloat> sample = interpolator.getSample(on);
        interpolator.removeSample(sample);
        boolean changed = sample.time != x || sample.data.value != y;
        BreakpointFloat bf = new BreakpointFloat(sample.data.value);
