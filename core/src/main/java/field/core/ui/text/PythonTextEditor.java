...
@Woven
public
class PythonTextEditor extends BaseTextEditor2 {
    ...
    class PickledCompletionInformation {
        ...
        public
        PickledCompletionInformation(List info) {
            super();
            this.info = info;
        }
    }
    ...
    class ClassRecord {
        ...
        public
        ClassRecord(String firstName, String fullName) {
    ...
    public
    interface EditorExecutionInterface {
    ...
    public
    iOutputCapture setOutputCapture(iOutputCapture outputCapture) {
    ...
    class OutputFlusher implements IUpdateable {
...

        {
            ...
            inputStyle.font = new Font(Launcher.display,
                                       ed.getFont().getFontData()[0].name,
                                       Constants.defaultFont == ed.getFont().getFontData()[0].name ? 10 : 12,
                                       SWT.ITALIC);
        ...
        Pattern[] errorLineNoPatterns = {Pattern.compile("'<string>', (\\d+)|on line (\\d+)|mfm.*?(\\d+)"),
                                         Pattern.compile("', (\\d+), (\\d+), "),
                                         Pattern.compile("null\\:(\\d+)")};
        ...
        void update() {
            ...
            try {
                ...
                boolean go = input.getBuffer().length() > 0
                             || output.getBuffer().length() > 0
                             || error.getBuffer().length() > 0;
                ...
                if (inputText.length() > 0) append(edOut, inputText + "\n", inputStyle);
                if (outputText.length() > 0) append(edOut, limit(outputText, 12500) + "\n", outputStyle);
                if (errorText.length() > 0) append(edOut, errorText + "\n", errorStyle);
                ...
                try {
                    ...
                    for (Pattern errorLineNoPattern : errorLineNoPatterns) {
                        ...
                        if (m.find()) {
                            String gg = m.group(1);
                            if (gg == null && m.groupCount() > 1) gg = m.group(2);
                            if (gg == null && m.groupCount() > 2) gg = m.group(3);
                            ...
                            if (cp >= getInputEditor().getText().length()) return;
                            ...
                            int lineStartAts =
                                    subLineStarts.get(Math.min(subLineStarts.size() - 1, Math.max(0, i - 1)));
                            int lineEndsAts =
                                    subLineStarts.get(Math.min(subLineStarts.size() - 1, Math.max(0, i - 1) + 1));

                            addPositionAnnotation(new ErrorAnnotation(lineStartAts,
                                                                      lineEndsAts).setToolTip(error.toString()
                                                                                              + "\n"
        ...
        private
        void append(StyledText target, String string, TextStyle style) {
    ...
    private
    class ErrorAnnotation implements iPositionAnnotation {
        private String tt;
        ...
        // offset
        public
        ErrorAnnotation(int start, int end) {
        ...
        public
        boolean drawRect(Rectangle r, GC g) {
        ...
        public
        StyledTextPositionSystem.Position getEndPosition() {
            return end;
        }

        public
        StyledTextPositionSystem.Position getStartPosition() {
            return start;
        }

        public
        iPositionAnnotation setToolTip(String error) {
            tt = error.toString();
            return this;
        }
    ...
    Comparator<ClassRecord> sortClassRecord = new Comparator<ClassRecord>() {
        public
        int compare(ClassRecord o1, ClassRecord o2) {
    ...
    public
    List<Completion> buildCompletionsForClassName(final String cc) {
        if (allClasses == null) {
                    ...
                    void update() {
                        if (PythonInterface.getPythonInterface().getVariable(rr.firstName) == null) {
                            if (rr.firstName.indexOf('$') == -1) {
                                String aa = "from "
                                            + rr.fullName.substring(0, rr.fullName.lastIndexOf('.'))
                                            + " import "
                                            + rr.firstName
                                            + "\n";
                                ...
                                ed.getContent()
                                  .replaceTextRange(ed.getCaretOffset(), 0, rr.firstName.substring(cc.length()));
        ...
        Collections.sort(r, new Comparator<Completion>() {
            public
            int compare(Completion o1, Completion o2) {
        ...
        for (int x = 0; x < r.size(); x++) {
            ...
            if (!c.text.substring(0, c.text.lastIndexOf('.')).equals(old2)) {
                List<Comp> customcomp =
                        ClassDocumentation.getPackageCustomCompletion(c.text.substring(0, c.text.lastIndexOf(".")),
                                                                      c.text.substring(c.text.lastIndexOf(".")));
                ...
                if (customcomp != null) {
                    for (Comp co : customcomp) {
                        if (co.longDocumentation != null) {
                            ...
                            completionsFromReflectingUpon(((CompProxy) co).proxyTo,
                                                          cc,
                                                          (ArrayList<Completion>) r,
                                                          true);
                        }
                    }
                }
    ...
    void executeHandle() {
        ...
        else executionRuler.getExecutedAreas()
    ...
    public
    EditorExecutionInterface getInterface() {
        return inter;
    }

    public
    List<Integer> getLineStarts() {
    ...
    public
    OutputFlusher getOutputFlusher() {
    ...
    public
    PythonTextEditor setInterface(EditorExecutionInterface inter) {
        this.inter = inter;
        return this;
    }
    ...
    private
    void addClassToStaticallyImportedClasses(String name, Map<String, ClassRecord> allClassesMap) {
        try {
    ...
    private static
    void addSourceDirToAllClasses(String root, String s, Map<String, ClassRecord> allClassesMap) {
        try {
            File d = new File(s);
            ...
            if (d.isDirectory()) {
                ...
                for (File f : classes) {
                    ...
                    if (upTo.endsWith(n2 + ".java")) upTo = upTo.substring(0, upTo.length() - (n2 + ".java").length());

                    if (!upTo.endsWith(".")) upTo = upTo + '.';

                    if (upTo.startsWith(".")) upTo = upTo.substring(1);
    ...
    public
    void completionsFromLocalNamespace(final String cc, Map<String, Object> variables, ArrayList<Completion> comp) {
                    ...
                    if (e.getValue() instanceof PyMethod) {
                        ...
                        Completion c = new Completion() {
                            ...
                            void update() {
                                int old = ed.getCaretOffset();
                                ed.getContent()
                                  .replaceTextRange(ed.getCaretOffset() - cc.length(), cc.length(), e.getKey());

                                if (cc.length() == 0) ed.setCaretOffset(old + e.getKey().length());
                        ...
                        PyObject d = t.getDoc();
                        if (d != null && d != Py.None) doc = Py.tojava(d, String.class);
                        ...
                        c.text = "\u03bd  <b>"
                                 ...
                                 + "</b> = "
                                 + limit(e.getValue())
                                 + smaller(" a python class ")
                                 + ((!doc.isEmpty() ? ("- <i>" + limitDocumentation(doc) + "</i>") : ""));
                        c.enabled = true;
                        // c.optionalDocumentation =
                        // convertPythonDoc(doc);

                        PyObject constructor = t.getDict().__findattr__("__init__");
                        ...
                        c.optionalPlainText = e.getKey().substring(cc.length());
                        c.text = "\u03bd  <b>" + e.getKey() + "</b> = " + limit(e.getValue()) + smaller(" of "
                                                                                                        + (e.getValue()
                                                                                                           == null
                                                                                                           ? null
                                                                                                           : e.getValue()
    ...
    private
    Completion completionForPyFunction(final String leftText, final PyFunction f) {
        return completionForPyFunction(leftText, f, false);
    }

    private
    Completion completionForPyFunction(final String leftText, final PyFunction f, boolean ignoreFirst) {
        // System.out.println(" enter ");

        if ((((PyTableCode) f.__code__).co_flags.isFlagSet(CodeFlag.CO_VARARGS))
            || (((PyTableCode) f.__code__).co_flags.isFlagSet(CodeFlag.CO_VARKEYWORDS))
            || (f.func_defaults != null)) {
            ...
            // System.out.println(" p1 ");
            String s = PythonInterface.getPythonInterface()
                                      .eval("inspect.formatargspec(*inspect.getargspec(__f))")
                                      .toString();
            ...
            Completion c = new Completion() {
                ...
                void update() {
                    int old = ed.getCaretOffset();
                    ed.getContent()
                      .replaceTextRange(ed.getCaretOffset() - leftText.length(),
                                        leftText.length(),
                                        rewriteOperator(f.__name__));

                    if (leftText.length() == 0) ed.setCaretOffset(old + rewriteOperator(f.__name__).length());
            ...
            if (doc != null && !"None".equals(doc.toString())) ddoc = "- " + doc.toString();
    ...
    private
    Completion completionForConstructor(final String leftText, final PyType clazz) {
        // different technique
        PythonInterface.getPythonInterface().setVariable("__f", clazz);
        String s = PythonInterface.getPythonInterface()
                                  .eval("inspect.formatargspec(*inspect.getargspec(__f.__init__))")
                                  .toString();
        ...
        Completion c = new Completion() {
            ...
            void update() {
                int old = ed.getCaretOffset();
                ed.getContent()
                  .replaceTextRange(ed.getCaretOffset() - leftText.length(), leftText.length(), clazz.getName());

                if (leftText.length() == 0) ed.setCaretOffset(old + clazz.getName().length());
    ...
    private
    Completion completionForPyMethod(final String leftText, final PyMethod f, boolean pythonOnly) {
    ...
    private
    void completionsFromPythonList(Object ret,
                                   PyList list,
                                   final String right,
                                   ArrayList<Completion> comp,
                                   PyObject po,
        ...
        for (Object p : alist) {
            ...
            if (p instanceof PyJavaType) {
                ...
                completionsFromReflectingUponClass(right,
                                                   comp,
                                                   (Class) ((PyJavaType) p).__tojava__(Class.class),
                                                   publicOnly);
            ...
            if (p instanceof Comp) {
                if (p instanceof CompProxy) {
                    ...
                    else completionsFromReflectingUpon(((CompProxy) p).proxyTo, right, comp, true);
                }
                else makeCompletion(right, comp, ((Comp) p));
            }
            else {
                ...
                if (name.startsWith(right) && (!name.startsWith("_")
                                               || !publicOnly
                                               || knownOperators.containsKey(name))) {
                    String ddoc = "";
                    Completion c = new Completion() {
                        @Override
                        public
                        void update() {
                            int old = ed.getCaretOffset();
                            ed.getContent()
                              .replaceTextRange(ed.getCaretOffset() - right.length(),
                                                right.length(),
                                                rewriteOperator(name));

                            if (right.length() == 0) ed.setCaretOffset(old + rewriteOperator(name).length());
                    ...
                    try {
                        ...
                        if (a instanceof PyFunction) {
                            ...
                            c.text += "- " + limit(o) + " " + (o == null
                                                               ? ""
                                                               : smaller(strip(String.valueOf(o.getClass()))));
                        }
                        else {
    ...
    protected static
    String rewriteOperator(String name) {
        return knownOperators.containsKey(name) ? knownOperators.get(name) : name;
    }

    public
    void completionsFromReflectingUpon(Object ret, final String right, ArrayList<Completion> comp, boolean publicOnly) {
        if (ret == null) return;
        ...
        if (ret instanceof PyObjectDerived) {
            ...
            if (a == null || a == Py.None) {
                ...
                if (type instanceof PyJavaType) ret = PythonUtils.maybeToJava(ret);
            }
            else {
                pythonOnly = true;
            }
        ...
        if (ret instanceof PyJavaType) {
            ...
            String doc = ClassDocumentation.getClassDocumentation(right, null, c);
            if (doc != null) {
            ...
            List<Comp> customcomp = ClassDocumentation.getClassCustomCompletion(right, null, c);
            if (customcomp != null) {
            ...
            String doc = ClassDocumentation.getClassDocumentation(right, ret, poClass);
            if (doc != null) {
                Completion cc = new Completion() {
            ...
            List<Comp> customcomp = ClassDocumentation.getClassCustomCompletion(right, ret, po.getClass());
            if (customcomp != null) {
    ...
    private
    String convertPythonDoc(String ascii) {
        return convertPythonDoc(ascii, "");
    }

    private static
    String convertPythonDoc(String ascii, String defaultTitle) {
        ascii = ascii.trim();
        ...
        String formatted =
                ascii.substring(firstLine).replace("\n\n", "\n").replace("-->", "\u2192").replace("->", "\u2192");
        ...
        String a = ((firstLine > 0
                     ? "<b>" + (ascii.substring(0, firstLine)).trim() + "</b>\n" + (formatted).trim()
                     : formatted)).trim();
    ...
    protected
    void completionsFromReflectingUponClass(String right,
                                            ArrayList<Completion> comp,
                                            Class class1,
                                            boolean publicOnly) {
        Object ret = null;
        ...
        if (c != null) completionsFromReflectingUponClass(right, comp, c, publicOnly);
    }

    private static
    boolean notHidden(Field f) {
        return f.getAnnotation(HiddenInAutocomplete.class) == null && Modifier.isPublic(f.getModifiers());
    }

    private static
    boolean notHidden(Method m) {
    ...
    protected
    void completionsFromReflectingUponConstructors(String right,
                                                   ArrayList<Completion> comp,
                                                   Class class1,
                                                   boolean publicOnly) {
        Object ret = null;
    ...
    private
    void completionsFromReflectingUponInstance(Object ret,
                                               final String right,
                                               ArrayList<Completion> comp,
        ...
        if ((!publicOnly || notHidden(class1))) {
            ...
            String doc = ClassDocumentation.getClassDocumentation(right, ret, class1);
            if (doc != null) {
                Completion cc = new Completion() {
            ...
            List<Comp> customcomp = ClassDocumentation.getClassCustomCompletion(right, ret, class1);
            if (customcomp != null) {
                for (Comp c : customcomp) {
                    makeCompletion(right, comp, c);
                }
        ...
        Class c = class1.getSuperclass();
        if (c != null && c != Object.class) completionsFromReflectingUponInstance(ret, right, comp, c, publicOnly);
    }

    private static
    boolean notHidden(Class<? extends Object> class1) {
    ...
    private
    void makeCompletion(final String right, ArrayList<Completion> comp, Comp c) {
        if (c.isTitle) {
            Completion cc = new Completion() {
            ...
            final Completion co = new Completion() {
                ...
                void update() {
                    ...
                    if (right.length() == 0) ed.setCaretOffset(old + f.length());
    ...
    private
    Completion getCompletionFor(Object ret, Class<? extends Object> class1, final Field f, final String right) {
        ...
        final Completion c = new Completion() {
            ...
            void update() {
                ...
                if (right.length() == 0) ed.setCaretOffset(old + f.getName().length());
        ...
        c.text = (Modifier.isPublic(f.getModifiers()) ? "\u22f6 " : "\u22f7 <grey>")
                 + " <b>"
                 + strip(f.getName()
                          .toString())
                 + "</b>"
                 + " ["
                 + strip(f.getType().toString())
                 + "] "
                 + value;
        ...
        if (name != null) {
            ...
            if (jc != null) {
                ...
                if (method != null) {
                    ...
                    c.text = (Modifier.isPublic(f.getModifiers()) ? "\u1d39  " : "\u1d50 <grey>")
                             + ctext
                             + (comment.length() > 0 ? " - " + smaller(limitDocumentation(comment)) : "");
    ...
    private
    JavaClass resolveJavaClass(Class<?> declaringClass, String name) {
    ...
    private
    Completion getCompletionFor(Object ret, Class<? extends Object> class1, final Method m, final String right) {
        ...
        c.text = "\u1d39 " + staticprefix + strip(m.getName()) + "(" + (m.getParameterTypes().length == 0
                                                                        ? ""
                                                                        : parameterNames) + ")";
            ...
            if (jc != null) {
                ...
                if (method != null) {
                    if (method.getParameters() != null) {
                        ...
                        c.text = (Modifier.isPublic(m.getModifiers()) ? "\u1d39  " : "\u1d50 <grey>")
                                 + ctext
                                 + (comment.length() > 0 ? " - " + smaller(limitDocumentation(comment)) : "");
    ...
    private static
    ZipEntry findEntry(JarFile jar, String simpleName) {
        Enumeration<JarEntry> e = jar.entries();
        while (e.hasMoreElements()) {
            JarEntry j = e.nextElement();
            String name = j.getName();
            if (name.endsWith(simpleName)) return j;
        }
        return null;
    }
    ...
    private
    Completion getCompletionFor(Class<? extends Object> class1, final Constructor m, final String right) {
        Completion c = new Completion() {
            @Override
            public
            void update() {
                ...
                int old = ed.getCaretOffset();
                ed.getContent()
                  .replaceTextRange(old - right.length(), right.length(), m.getDeclaringClass().getSimpleName());
                if (right.length() == 0) {
        ...
        c.text = "\u1d39 "
                 + strip(m.getName())
                 + "("
                 + (parametertypes.length == 0 ? "" : Arrays.asList(parametertypes))
                 + ")";
        ...
        if (name != null) {
            ...
            if (jc != null) {
                ...
                for (int i = 0; i < parametertypes.length; i++) {
                    types[i] = new DefaultJavaType(Platform.getCanonicalName(parametertypes[i])
                                                           .replace("[", "")
                                                           .replace("]", ""), parametertypes[i].isArray() ? 1 : 0);
                }

                JavaMethod method =
                        jc.getMethodBySignature(m.getDeclaringClass().getSimpleName(), Arrays.asList(types));
                ...
                if (method != null) {
                    if (method.getParameters() != null) {
                        ...
                        for (int i = 0; i < method.getParameters().size(); i++) {
                            ctext += strip(Platform.getCanonicalName(parametertypes[i]))
                                     + " <i>"
                                     + method.getParameters().get(i).getName()
                                     + "</i>"
                                     + (i != method.getParameters().size() - 1 ? ", " : "");
                        ...
                        c.text = (Modifier.isPublic(m.getModifiers()) ? "\u24d2  " : "\u24d2 <grey>")
                                 + ctext
                                 + (!comment.isEmpty() ? " - " + smaller(limitDocumentation(comment)) : "");
                        // c.optionalDocumentation =
    ...
    private
    Completion getSeparatorFor(Object ret, Class<? extends Object> class1, String right) {
        Completion c = new Completion() {
            @Override
            public
            void update() {
    ...
    static public
    boolean isSafeToPrint(Field f, Object ret) {
        if (f.getType().isPrimitive()) return true;
        if (f.getType().equals(String.class)) return true;
        if (Number.class.isAssignableFrom(f.getType())) return true;
        if (f.getType().isAssignableFrom(Number.class)) return true;
        if (f.getType().equals(Vector2.class)) return true;
        if (f.getType().equals(Vector3.class)) return true;
        if (f.getType().equals(Vector4.class)) return true;
        if (f.getType().equals(VectorN.class)) return true;
        if (f.getType().equals(Quaternion.class)) return true;
        if (f.getType().equals(File.class)) return true;
        if (f.getType().equals(Method.class)) return true;
        if (f.getType().equals(Field.class)) return true;
        if (f.getType().equals(Class.class)) return true;

        return false;
    }

    static public
    String limit(Object value) {
        return limit(value, 250);
    }

    static public
    String limit(Object value, int i) {
    ...
    static public
    String limitDocumentation(Object value) {
        String s = value.toString();
        int max = 100;
        if (s.length() > max) {
            String[] q = s.split("\n\n");
    ...
    private static
    void print(PyJavaPackage topLevelPackage, String prefix, Map<String, ClassRecord> allClassesMap) {
        ...
        while (true) {
            ...
            if (a != null) {
                // " +
                // a);
                allClassesMap.put(topLevelPackage.__name__ + "." + a.toString(),
                                  new ClassRecord(a.toString(), topLevelPackage.__name__ + "." + a.toString()));
            }
            else {
                break;
            }
    ...
    private static
    String printValue(Field f, Object ret) {
        // System.out.println(" print value <" + f + "> <" + ret +
        // ">");
        String v = "";
        try {
            f.setAccessible(true);
            v = String.valueOf(f.get(ret));
    ...
    protected
    List<ClassRecord> buildAllClasses() {
        ...
        for (Class c : allLoadedClasses) {
            ...
            if (!allClassesMap.containsKey(c.getName())) {
                if (c.getName().indexOf(".") != -1) allClassesMap.put(c.getSimpleName(),
                                                                      new ClassRecord(c.getName()
                                                                                       .substring(c.getName()
                                                                                                   .lastIndexOf(".")
                                                                                                  + 1,
                                                                                                  c.getName().length()),
    ...
    @Override
    public
    List<Completion> getCompletions(String leftText, boolean publicOnly, final iKeystrokeUpdate u) {
        ArrayList<Completion> comp = new ArrayList<Completion>();
        if (inter == null) return comp;
        ...
        if (cc.lastIndexOf('.') > 0) {
            ...
            else completionsFromReflectingUpon(ret, right, comp, publicOnly);
        }
        else {
        ...
        if (comp.size() > 100) return comp.subList(0, 100);

        return comp;
    }

    protected
    boolean globalCompletionHook(String leftText, boolean publicOnly, ArrayList<Completion> comp) {
    ...
    protected static
    String balanceBack(String leftText) {
        int round = 0;
        int square = 0;
        int curly = 0;
        ...
        for (int i = 0; i < leftText.length(); i++) {
            ...
            if (round >= 0 && square >= 0 && curly >= 0) cc = c + cc;
            else break;
        }
        return cc;
    }
    ...
    private
    void completionsFromPickledCompletionInfo(PickledCompletionInformation pickledCompletionInformation,
                                              final String right,
                                              ArrayList<Completion> comp,
                                              boolean publicOnly) {
        ...
        for (Object o : pickledCompletionInformation.info) {
            ...
            if ("field".equals(type)) {
                ...
                if (name.startsWith(right) && (!publicOnly || !name.startsWith("__"))) {
                    ...
                    cc.text = "\u22f6  <b>" + name + "</b>" + " [" + (String.valueOf(i.get(1))) + "] " + i.get(3);
                ...
                if (name.startsWith(right) && (!publicOnly || !name.startsWith("__"))) {
                    ...
                    cc.optionalPlainText = "";
                    cc.text = "\u1d3e  <b>"
                              + strip(rewriteOperator(name))
                              + " (</b>"
                              + (String.valueOf(i.get(2)))
                              + "<b>) </b> ";
    ...
    @Override
    protected
    List<Completion> getImportations(String leftText) {
        String cc = "";
        for (int i = 0; i < leftText.length(); i++) {
            char c = leftText.charAt(leftText.length() - i - 1);
    ...
    String[] getSourceDirs() {
        ...
        for (int i = 0; i < s.length; i++) {
            try {
                if (s[i].endsWith(".jar/")) s[i] = s[i].substring(0, s[i].length() - 1);

                File ff = new File(s[i]);
                if (ff.exists()) rr.add(ff.getCanonicalPath());
            } catch (IOException e) {
            }
        }
    ...
    public
    void setTopicString(String string) {
    ...
    public
    StyledText getOutputEditor() {
        return edOut;
    }

    public
    ExecutionRuler getExecutionRuler() {
        return executionRuler;
    }

    public
    void setAreas(State areaz) {
        executionRuler.getExecutedAreas().setState(areaz);
        ed.redraw();
    }
    ...
    public
    int doSearchAndReplace(String a, String b) {
