...
class ExecutedAreas {
    ...
    class Area {
        ...
        public
        void freeze() {
            if (exec == null) exec = new ArrayList<ExecutionRecord>();
            trackingStart = null;
            trackingEnd = null;
        }
        ...
        public
        void update(StyledText text) {
            if (textend == null) textend = new Dict();
            if (exec == null) exec = new ArrayList<ExecutionRecord>();

            if (trackingStart == null) {
                StyledTextPositionSystem tracker = StyledTextPositionSystem.get(text);
                trackingStart = tracker.createPosition(positionForLineStart(lineStart, text.getText()));
                trackingEnd = tracker.createPosition(positionForLineEnd(lineEnd, text.getText()));

            }
            else {
                int x1 = lineForOffset(trackingStart.at, text.getText());
                int x2 = lineForOffset(trackingEnd.at, text.getText());
                if (x1 == -1 || x2 == -1) invalid = true;
                else {
                    lineStart = x1;
                    lineEnd = x2;
                }
            }
            if (extend == null) extend = new Dict();
        }
    ...
    class ExecutionRecord {
        ...
        public
        ExecutionRecord(String stringAtExecution) {
    ...
    class State {
        ...
        boolean equals(Object obj) {
            ...
            if (areas == null) {
                if (other.areas != null) return false;
            }
            else if (!areas.equals(other.areas)) return false;
            if (rootHistogram == null) {
                if (other.rootHistogram != null) return false;
            }
            else if (!rootHistogram.equals(other.rootHistogram)) return false;
            return true;
        }
        ...
        int hashCode() {
            ...
            result = prime * result + ((rootHistogram == null) ? 0 : rootHistogram.hashCode());
            return result;
        }

    }

    public static final Prop<Histogram<Area>> forwardTransitions = new Prop<Histogram<Area>>("forwardTransitions");
    public static final Prop<Character> keyboardShortcut = new Prop<Character>("keyboardShortcut");

    public static final Prop<Boolean> isUnitTest = new Prop<Boolean>("isUnitTest");
    public static final Prop<Boolean> passedUnitTest = new Prop<Boolean>("passedUnitTest");
    ...
    int lineForOffset(int offset, String text) {
        ...
        if (at >= offset) return lines.length;
        return -1;
    }
    ...
    public
    void deleteArea(Area target) {
        areas.remove(target);
        for (Area a : areas) {
            Histogram<Area> t = a.extend.get(forwardTransitions);
            if (t != null) t.removeKey(target);
        }
        rootHistogram.removeKey(target);
    }

    public
    Area execute(int offsetStart, int offsetEnd) {
        return execute(offsetStart, offsetEnd, null);
    }

    public
    Area execute(int offsetStart, int offsetEnd, String textAtExecution) {
    ...
    public
    Histogram<Area> getRootHistogram() {
    ...
    public
    void markTransition(Area from, Area to) {
        Histogram<Area> t;
        if (from != null) {
            if (from.extend == null) from.extend = new Dict();
            t = from.extend.get(forwardTransitions);
            if (t == null) from.extend.put(forwardTransitions, t = new Histogram<Area>());
        }
        else {
            t = rootHistogram;
        }
    ...
    public
    void setAreas(Collection<Area> a) {
        areas.clear();
        areas.addAll(a);
        for (Area aa : a) {
            if (aa.lastSeenAt > clock) clock = aa.lastSeenAt + 1;
            aa.update(text);
        }
    ...
    boolean overlaps(Area newArea, Area a) {
        if (newArea.allocation == a.allocation && newArea.lineStart <= a.lineEnd && newArea.lineEnd >= a.lineStart)
            return true;
        return false;
    }
