...
public
class ExecutionRuler implements iRuler {
    ...
    public
    ExecutionRuler(StyledText p, Canvas ruler) {
        this.p = p;
        this.ruler = ruler;
        areas = new ExecutedAreas(p) {
            @Override
            protected
            void merge(Area a, Area b) {
    ...
    public
    void drawConnection(GC g, Vector2 from, Area to, Vector4 color, float scale) {
    ...
    public
    ExecutedAreas getExecutedAreas() {
        return areas;
    }

    public
    void highlight(Area a) {
        highlighted.add(a);
    }

    public
    void keyEvent(Event e, int caretPosition, int selectionStart, int selectionEnd) {
        ...
        if (((e.stateMask & SWT.CTRL) != 0) && ((e.stateMask & SWT.COMMAND) != 0)) {
            ...
            if (m != null) browseExecutionHistory(m).update();
        }
    }

    public
    Area getMin(int caretPosition) {
    ...
    public
    boolean mouseOverAt(float x, float y) {
    ...
    public
    void paintNow(final GC g, StyledText p, Canvas ruler) {
        g.setAdvanced(true);
        ...
        for (int i = 0; i < splat.length; i++) {
            if (splat[i].trim().isEmpty()) continue;
        ...
        for (Area a : all) {
            ...
            if (!a.invalid) {
                Path drawBracket = drawBracket(g,
                                               ...
                                               a.lineStart,
                                               a.lineEnd,
                                               (a == currentBest) ? 2 : 0.5f,
                                               highlighted.contains(a) ? 0.5f : 0);
        ...
        if (histogram != null) {
            ...
            h.average(new iAverage<Area, Object>() {
                ...
                public
                <X extends Area> void accept(X accept, double weight) {
                    drawOrigin(g, accept, new Vector4(0, 0, 0, weight / 2));
                }

                public
                void begin(int num, double totalWeight) {
                }
    ...
    private
    void drawOpportunisticSlider(OpportinisticSlider slider, GC g, int i, int lineStart, int lineEnd) {
    ...
    private
    void drawMonitor(GC g, int x, int q, int lineStart, int lineEnd) {
        boolean over = false;
        Rectangle start = rectangleForLine(lineStart);
        Rectangle end = rectangleForLine(lineEnd);
    ...
    private
    DragType computeDrag(MouseEvent arg0) {
        mouseAt.x = arg0.x;
        mouseAt.y = arg0.y;
        ...
        if (dragTarget != null) {
            ...
            // end.height);
            if (Math.abs(arg0.y - start.y) < 5) return DragType.dragTop;
            if (Math.abs(arg0.y - end.y - end.height) < 5) return DragType.dragBottom;
    ...
    private
    float distance(Vector2 mouseAt2, Area a) {
    ...
    private
    void drawForwardTransitions(final GC g, Area a, Histogram<Area> histogram) {
        ...
        histogram.average(new iAverage<Area, Object>() {
            ...
            <X extends Area> void accept(X accept, double weight) {
                if (accept != null) {
                    ...
                    try {
                        drawBracket(g,
                                    (accept.allocation * 10) + 2,
                                    10,
                                    accept.lineStart,
                                    accept.lineEnd,
                                    new Vector4(0, 0, 0, amount),
    ...
    private
    void drawKeyboardShortcut(GC g, char c, int x, int lineStart, int lineEnd, boolean best) {
        try {
            Rectangle start = rectangleForLine(lineStart);
            Rectangle end = rectangleForLine(lineEnd);
    ...
    private
    void drawOrigin(GC g, Area a, Vector4 fillColor) {
        Rectangle start = rectangleForLine(a.lineStart);
        Rectangle end = rectangleForLine(a.lineEnd);
...
//		g.drawOval((int) x, (int) (midY - r / 2), (int) r, (int) r);
    ...
    private
    void finalizeDrag(Event e) {
    }

    private
    Vector2 hitPointFor(Area a, float scale) {
        Rectangle start = rectangleForLine(a.lineStart);
        Rectangle end = rectangleForLine(a.lineEnd);
    ...
    public
    Path drawBracket(GC g, int x, int width, int lineStart, int lineEnd, float weight, float shade) {
        return drawBracket(g,
                           x,
                           width,
                           lineStart,
                           lineEnd,
                           new Vector4((1 - shade) * shade, shade, 0, weight * 0.3f),
                           new Vector4((1 - shade) * shade, shade, 0, weight * 0.3f),
                           1);
    }

    protected
    Path drawBracket(GC g,
                     int x,
                     int width,
                     int lineStart,
                     int lineEnd,
...
//		g.drawPath(bracket);
        ...
        fillBracket.cubicTo(x,
                            start.y,
                            x + width,
                            (start.y + end.y + end.height) / 2,
                            x,
                            (start.y + end.y + end.height) / 2);
        fillBracket.cubicTo(x + width,
                            (start.y + end.y + end.height) / 2,
                            x,
                            (end.y + end.height),
                            x + width,
...
//			g.drawOval(x, (start.y + end.y + end.height) / 2 - r / 2, r, r);
    ...
    void popup(MouseEvent e, DragType dt, final Area target) {
        ...
        if (!menu.isEmpty()) new SmallMenu().createMenu(menu, ruler.getShell(), null)
                                            .show(Launcher.display.map(ruler, ruler.getShell(), new Point(e.x, e.y)));
    }
    ...
    public
    Rectangle rectangleForLine(int line) {
    ...
    public
    Rectangle rectangleForPosition(int pos) {
