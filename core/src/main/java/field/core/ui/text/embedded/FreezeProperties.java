...
class FreezeProperties {
    ...
    class Freeze {
        ...
        public
        Freeze(Set<String> include, Set<String> exclude, boolean optimistic) {
            this.optimistic = optimistic;
        }

        public
        Freeze freeze(VisualElement ele) {
            frozen.clear();
                ...
                if (((e.getKey() instanceof VisualElementProperty)
                     && (((VisualElementProperty) e.getKey()).isFreezable())) || ((include != null)
                                                                                  && include.contains(((VisualElementProperty) e.getKey())
                                                                                                              .getName())))
                    if ((include == null) || include.contains(((VisualElementProperty) e.getKey()).getName()))
                        if ((exclude == null) || !exclude.contains(((VisualElementProperty) e.getValue()).getName())) {
        ...
        public
        Map<VisualElementProperty, Object> getMap() {
            return frozen;
        }

        public
        void thaw(VisualElement ele) {
            ...
            for (Entry<VisualElementProperty, Object> e : es) {
                ...
                if (v != null) e.getKey().set(ele, ele, v);
            }
        }
    ...
    public
    interface iCloneHelper {
        public
        boolean canClone(Object name, Class o);
    ...
    public
    <T> void cloneInto(Map<T, Object> cc, T key, Object o, boolean nothingOnFailure) {
        if (o == null) cc.put(key, null);
        else {
    ...
    public
    List<Object> cloneList(List<Object> q, boolean nothingOnFailure) {
        ArrayList<Object> cc = new ArrayList<Object>();
        for (Object e : q) {
            if (e == null) cc.add(null);
            else {
    ...
    public
    <T> Map<T, Object> cloneMap(Map<T, Object> q, boolean nothingOnFailure) {
        HashMap<T, Object> cc = new HashMap<T, Object>();
        Set<Entry<T, Object>> name = q.entrySet();
        for (Entry<T, Object> e : name) {
            if (e.getValue() == null) cc.put(e.getKey(), null);
            else {
    ...
    public
    iCloneHelper getCloneFor(Object name, Class c) {
    ...
    public
    FreezeProperties setExclude(Set<String> exclude) {
        if (exclude != null) {
    ...
    public
    FreezeProperties setInclude(Set<String> include) {
        if (include != null) {
