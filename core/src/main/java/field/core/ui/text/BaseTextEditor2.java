...
class BaseTextEditor2 {
    ...
    class LineRec {
        ...
        public
        LineRec(int offset, String string) {
            super();
            this.offset = offset;
            this.string = string;
        }
    ...
    BaseTextEditor2() {
        ...
        rulerCanvas.addPaintListener(new PaintListener() {
            ...
            void paintControl(PaintEvent e) {
                try {
                    if (currentRuler != null) currentRuler.paintNow(e.gc, ed, rulerCanvas);
                } catch (Exception ee) {
                    ee.printStackTrace();
                }
            }
        ...
        ed = new StyledText(vsplit, SWT.WRAP | SWT.H_SCROLL | SWT.V_SCROLL) {
            ...
            protected
            String getClipContents() {
                TextTransfer plainTextTransfer = TextTransfer.getInstance();
                String clipContents =
                        (String) new Clipboard(Launcher.display).getContents(plainTextTransfer, DND.CLIPBOARD);
                return clipContents;
            }
        ...
        fandr = new FindAndReplace(ed, 0) {
            ...
            @Override
            protected
            void replaceInCurrent(int start, int end, String with) {
        ...
        ed.addListener(SWT.Paint, new Listener() {
            ...
            void handleEvent(Event event) {
                ...
                if (!ed.getText().equals(prevText) && ed.getVerticalBar().getSelection() != lastS) lcw.requestRepaint();
                ...
                for (Control cc : c) {
                    ...
                    if (data instanceof Nub) {
                        ...
                        if (comp instanceof iOutOfBandDrawing) ((iOutOfBandDrawing) comp).expandDamage(r);
        ...
        int size = SystemProperties.getIntProperty("editorFontSize",
                                                   (int) (Platform.isMac()
                                                          ? (Launcher.display.getSystemFont()
        ...
        ed.addVerifyKeyListener(new VerifyKeyListener() {
            ...
            void verifyKey(VerifyEvent event) {
                ...
                if (event.keyCode == '\r' && (event.stateMask & Platform.getCommandModifier2()) != 0) {
                    ...
                    else if ((event.stateMask & SWT.SHIFT) == 0) executeHandle();
                    else executeHandleSpecial();
                    event.doit = false;
                }
                ...
                else if ((event.keyCode == 'z') && ((event.stateMask & SWT.SHIFT) != 0
                ...
                if (currentRuler != null) rulerCanvas.redraw();
            }
        });
        ...
        ed.addPaintListener(new PaintListener() {
            ...
            void paintControl(PaintEvent e) {
                ...
                if (ed.isEnabled() && searchString != null) paintSearchString(e.gc, searchString);
        ...
        ed.addListener(SWT.KeyDown, new Listener() {
            ...
            void handleEvent(Event arg0) {
                currentRuler.keyEvent(arg0,
                                      ed.getCaretOffset(),
                                      ed.getSelectionRanges()[0],
                                      ed.getSelectionRanges()[1] + ed.getSelectionRanges()[0]);
            }
        });
        ...
        ed.addLineStyleListener(new LineStyleListener() {

            {
                BaseTextEditor2.this.cache = new LinkedHashMap<LineRec, StyleRange[]>() {
                    protected
                    boolean removeEldestEntry(java.util.Map.Entry<LineRec, StyleRange[]> eldest) {
                        return size() > 100;
                    }
                };
            }
            ...
            void lineGetStyle(LineStyleEvent event) {
                ...
                if (!doNotCache) cache.put(lr, event.styles);
            }
        ...
        executionRuler = new ExecutionRuler(ed, rulerCanvas) {
            public
            void executeArea(Area area) {
                BaseTextEditor2.this.executeArea(area);
            }

            protected
            void executeAreaSpecial(Area area) {
            ...
            protected
            field.core.util.LocalFuture<Boolean> runAndCheckArea(Area area) {
    ...
    protected
    void bracketMatchClose(char op, char cl) {
        ...
        if (here != -1) {
            ...
            addPositionAnnotation(new iPositionAnnotation() {
                ...
                boolean drawRect(Rectangle r, GC g) {
                    ...
                    r.width += 8;
                    g.setAlpha(128);
                    g.setBackground(Launcher.display.getSystemColor(SWT.COLOR_DARK_BLUE));
                    g.fillRectangle(r);
                    ...
                    Path pp = new Path(Launcher.display);
                    pp.moveTo(loc.x + 4, loc.y - 2);
                    pp.cubicTo(loc.x + 4,
                               Math.min(loc.y - 8, r.y - 8),
                               r.x + r.width / 2,
                               Math.min(loc.y - 8, r.y - 8),
                               r.x + r.width / 2,
                               r.y);
            ...
            final Position qq =
                    StyledTextPositionSystem.get(ed).createPosition(backTo == 0 ? ed.getCaretOffset() - 1 : backTo);
            ...
            addPositionAnnotation(new iPositionAnnotation() {
                ...
                boolean drawRect(Rectangle r, GC g) {
                    ...
                    r.width += 8;
                    g.setAlpha(128);
                    g.setBackground(Launcher.display.getSystemColor(SWT.COLOR_DARK_RED));
                    g.fillRectangle(r);
    ...
    protected
    void bracketMatchOpen() {
    }

    protected
    void globalEditorPaintHook(PaintEvent e, Control ed) {
    ...
    protected
    void paintSearchString(GC gc, String ss) {
        String m = ed.getText();
    ...
    protected
    String localCopy(String clipAtCopy) {
        return null;

    }

    protected
    String localPaste(String clipAtCopy, String localCopyRewritten) {
        return null;
    }

    protected
    boolean globalShortcutHook(VerifyEvent event) {
        return false;
    }

    protected
    void handleMouseEventOnArea(MouseEvent arg0, Area currentArea) {
    }

    protected
    void getStyleForEmbeddedString(StyleRange s, String substring) {
    ...
    public
    void addPositionAnnotation(iPositionAnnotation annotation) {
    ...
    public
    void completionHandle(final boolean publicOnly) {
    ...
    public
    void removePositionAnnotation(iPositionAnnotation annotation) {
    ...
    // @NextUpdate(delay=4)
    public
    BaseTextEditor2 setVisible(boolean b) {
        ...
        if (b) {
            ...
            try {
                if (Platform.getOS() == OS.mac)
                    ReflectionTools.findFirstMethodCalled(ReflectionTools.illegalGetObject(frame, "window").getClass(),
                                                          "setHidesOnDeactivate")
                                   .invoke(ReflectionTools.illegalGetObject(frame, "window"), true);
    ...
    void completionsForFilename(final String string, LinkedHashMap<String, iUpdateable> completions) {
        ...
        if (left.startsWith("file://")) left = left.substring("file://".length());
            ...
            completions.put((f.isDirectory() ? "\u2208 <b>" : "\u2802 <i>") + f.getName() + (f.isDirectory()
                            ...
                            new iUpdateable() {
                                ...
                                void update() {
                                    int pos = ed.getCaretOffset();
                                    ed.getContent()
                                      .replaceTextRange(pos - string.length(),
                                                        string.length(),
                                                        f.getAbsolutePath() + (f.isDirectory() ? "/" : ""));
                                    if (f.isDirectory()) completionQuotedHandle();
                                }
                            });
        }

        if (allFiles.length == 1) {
            completions.put((allFiles[0].isDirectory()
                             ? "\u2208 reveal in Finder - <b>"
                             : "\u2802 <i> reveal in Finder - ")
                            + allFiles[0].getName()
                            + ' '
    ...
    protected
    void executeBrowseHandle(String s) {
    }
    ...
    protected
    List<Completion> getCompletions(String leftText, boolean publicOnly, iKeystrokeUpdate ks) {
    ...
    protected
    void indentSelectionOrCurrentLine() {
    ...
    protected
    void outdentSelectionOrCurrentLine() {
        int start = ed.getSelectionRanges()[0];
        int end = ed.getSelectionRanges()[1] + start;
    ...
    protected
    void paintPositionAnnotations(GC g2, int width) {
        ...
        while (ii.hasNext()) {
            ...
            try {
                ...
                if (left.y == right.y) {
                    if (!pa.drawRect(left.union(right), g2)) ii.remove();
                }
                else {
                    ...
                    left.width = 0;
                    left.add(right);
                    if (!pa.drawRect(left, g2)) ii.remove();
                }
    ...
    public
    PythonCallableMap getTextDecoration() {
        return textDecoration;
    }

    public
    StyledTextUndo getUndoHelper() {
        return undoHelper;
    }

    public
    int highlightSearch(String text) {
        searchString = text;
        if (ed.isEnabled()) {
            String m = ed.getText();
            int s = 0;
