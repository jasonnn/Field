...
@Woven
public
class FindAndReplace extends Composite {
     ...
     */
    public
    FindAndReplace(Composite parent, int style) {
        ...
        btnNewButton.addSelectionListener(new SelectionListener() {
            ...
            void widgetSelected(SelectionEvent e) {
                Point pp = Launcher.display.map(btnNewButton, null, new Point(e.x, e.y));
        ...
        btnReplaceFind.addSelectionListener(new SelectionAdapter() {
            ...
            void widgetSelected(SelectionEvent e) {
                replace(text.getText(),
                        text_1.getText(),
                        mntmThisBoxOnly.getSelection(),
                        mntmThisPropertyOnly.getSelection(),
                        mntmUseRegularExpressions.getSelection());
            }
        });
        ...
        if (Platform.isLinux()) setBackgroundRecursively(this);
    ...
    void rebuild() {
        ...
        found = new FindAll(text.getText(),
                            mntmThisBoxOnly.getSelection(),
                            mntmThisPropertyOnly.getSelection(),
                            mntmUseRegularExpressions.getSelection());
        found.build(getRoot());
        ...
        if (x > 0) {
            ...
            if (q.box == getThisBox() && q.prop.equals(getThisProperty())) {
                ...
                if (ff.size() == 1) navigateTo(q);
                else navigateTo(getThisBox(), getThisProperty(), ff);
            }
        }
    }
    ...
    class FindAll {
        ...
        public
        FindAll(String text, boolean boxOnly, boolean propertyOnly, boolean regex) {
            super();
        ...
        void findIn(iVisualElement e) {
            ...
            for (Entry<Object, Object> ee : me) {
                if (ee.getValue() instanceof String) {
                    if (!propertyOnly || (ee.getKey() instanceof VisualElementProperty) && ee.getKey()
                                                                                             .equals(getThisProperty())) {
    ...
    @NextUpdate(delay = 3)
    protected
    void replaceAll(String find, String with, boolean boxOnly, boolean propertyOnly, boolean regex) {
    ...
    protected
    void findNext(String find, boolean boxOnly, boolean propertyOnly, boolean regex) {
        findNext(find, boxOnly, propertyOnly, regex, true);
    }

    protected
    void findNext(String find, boolean boxOnly, boolean propertyOnly, boolean regex, boolean allowWrap) {
        current = null;
    ...
    protected
    void replace(String find, String with, boolean boxOnly, boolean propertyOnly, boolean regex) {
        if (current == null) {
...
//			rebuild();
    ...
    private
    void repair(Found c, String with) {
    ...
    private
    void replace(Found z, String with) {
        navigateTo(z);
    ...
    private
    void setBackgroundRecursively(Composite c) {
