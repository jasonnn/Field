...
class ColorDialog extends JDialog implements ChangeListener {
    ...
    public
    ColorDialog(Color cc, Frame owner, Point pointOnScreen) {
...
//		
    ...
    void updateRGB() {
        if (saturation == 0) red = green = blue = brightness;
            ...
            float[] rgb;
            if (i == 0) rgb = new float[]{v, t, p};
            else if (i == 1) rgb = new float[]{q, v, p};
            else if (i == 2) rgb = new float[]{p, v, t};
            else if (i == 3) rgb = new float[]{p, q, v};
            else if (i == 4) rgb = new float[]{t, p, v};
            else rgb = new float[]{v, p, q};
    ...
    void updateHSB() {
        ...
        if (s != 0) {
            if (red == v) h = 0 + (green - blue) / d;
            else if (green == v) h = 2 + (blue - red) / d;
    ...
    public
    class ColorField extends JButton {
        ...
        ColorField() {
            ...
            addMouseListener(new MouseAdapter() {
                ...
                void mouseClicked(MouseEvent e) {
                    if (e.getX() >= 2 && e.getX() <= 70 && e.getY() >= 38 && e.getY() <= 70) setColor(color);
                }
            });
        }
    ...
    class ColorPanel extends JPanel implements ChangeListener {
        ...
        public
        ColorPanel(String colorComponent) {
            this(ColorComponent.valueOf(colorComponent.toUpperCase()));
        }

        public
        ColorPanel(ColorComponent colorComponent) {
        ...
        public
        void updateDraggableNumber() {
            float range = 0.0F;
            if (colorRange == ColorRange.ABSOLUTE) range = 255.0F;
            else if (colorRange == ColorRange.PERCENTAGE) range = 100.0F;
            switch (colorComponent) {
                case RED:
    ...
    public
    class ColorSlider extends JComponent {
        ...
        void paintComponent(Graphics g) {
            ...
            g.drawLine(HALF_WIDTH_OFFSET,
                       r.height - HEIGHT_OFFSET - 1,
                       r.width - HALF_WIDTH_OFFSET - 1,
                       r.height - HEIGHT_OFFSET - 1);
            g.drawLine(r.width - HALF_WIDTH_OFFSET - 1,
                       0,
                       r.width - HALF_WIDTH_OFFSET - 1,
                       r.height - HEIGHT_OFFSET - 1);
            ...
            int i = Math.round(value * (r.width - WIDTH_OFFSET));
        ...
        void drawAlpha(Graphics g) {
            ...
            for (int i = 0; i < width; i++) {
                ...
                g.setColor(c);
                if ((i / 1) % 20 < 10) g.fillRect(i + HALF_WIDTH_OFFSET, 0, 1, (r.height - HEIGHT_OFFSET) / 2);
                else g.fillRect(i + HALF_WIDTH_OFFSET,
                                (r.height - HEIGHT_OFFSET) / 2,
                                1,
                                (r.height - HEIGHT_OFFSET) / 2);
            }
        ...
        private
        void drawSaturation(Graphics g) {
            Rectangle r = g.getClipBounds();
            int width = r.width - WIDTH_OFFSET;
            for (int i = 0; i < width; i++) {
        ...
        private
        void drawBrightness(Graphics g) {
            Rectangle r = g.getClipBounds();
            int width = r.width - WIDTH_OFFSET;
            for (int i = 0; i < width; i++) {
        ...
        void processMouseEvent(MouseEvent e) {
            if (e.getID() == MouseEvent.MOUSE_PRESSED) {
                ...
                int x = e.getX();
                if (x < HALF_WIDTH_OFFSET) panel.setValue(0);
                else if (x > width + WIDTH_OFFSET) panel.setValue(1);
                else panel.setValue((float) (x - HALF_WIDTH_OFFSET) / width);
            }
        }
