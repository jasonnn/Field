...
class NewInspector2 {
    ...
    class Inspected<T> extends Composite {
        ...
        public
        Inspected(Composite parent, int style, iIO<T> io) {
            super(parent, style);
            this.io = io;
            if (io != null) this.name = io.name;
        }
        ...
        protected
        void updateValue(T s) {
            if (io != null) io.setValue(s);
        }
    ...
    public
    NewInspector2(String icon, ToolBarFolder folder, ScrolledComposite scroller, Composite contents) {
        init(icon, folder, scroller, contents);
    }

    protected
    void init(String icon, ToolBarFolder folder, ScrolledComposite scroller, Composite contents) {
    ...
    public abstract
    class BaseControl<T> extends Inspected<T> {

        protected Control editor;

        public
        BaseControl(iIO<T> io) {
            super(contents, 0, io);
        ...
        void paint(Event event) {
            ...
            if (Platform.isMac()) event.gc.setFont(new Font(event.display, Constants.defaultFont, 11, SWT.NORMAL));
            else event.gc.setFont(new Font(event.display, Constants.defaultFont, 9, SWT.NORMAL));
    ...
    public
    class TextControl extends BaseControl<String> {

        public
        TextControl(iIO<String> io) {
            super(io);
    ...
    public
    class InfoControl extends BaseControl<Object> {

        public
        InfoControl(iIO<Object> ignored) {
            super(ignored);
    ...
    public
    class SpinnerControl extends BaseControl<Number> {

        public
        SpinnerControl(iIO<Number> io) {
            super(io);
        }
        ...
        void makeControl() {
            ...
            if (getValue() != null) ((Spinner) editor).setSelection(getValue().intValue());
    ...
    public
    class BooleanControl extends BaseControl<Boolean> {

        public
        BooleanControl(iIO<Boolean> io) {
            super(io);
        ...
        void makeControl() {
            ...
            if (getStatus() != Status.valid) ((Button) editor).setGrayed(true);
            else {
                Object g = getValue();
                if (g instanceof Number) g = ((Number) g).intValue() != 0;
    ...
    public
    class ColorControl extends BaseControl<Vector4> {

        public
        ColorControl(iIO<Vector4> io) {
            super(io);
        ...
        void makeControl() {
            ...
            editor.addPaintListener(new PaintListener() {
                ...
                void paintControl(PaintEvent e) {
                    ...
                    if (v != null) {
                        try {
                            Color cc = new Color(Launcher.display,
                                                 (int) (255 * v.x),
                                                 (int) (255 * v.y),
                                                 (int) (255 * v.z));
            ...
            editor.addListener(SWT.Selection, new Listener() {
                ...
                void handleEvent(Event event) {
                    value = getValue();
                    if (value == null) value = new Vector4();
                    ColorDialog d = new org.eclipse.swt.widgets.ColorDialog(editor.getShell());
                    RGB rgb = new RGB((int) (Math.max(0, Math.min(255, 255 * value.x))),
                                      (int) (Math.max(0, Math.min(255, 255 * value.y))),
                                      (int) (Math.max(0, Math.min(255, 255 * value.z))));
    ...
    class MoreControl extends BaseControl<LinkedHashMap<String, iUpdateable>> {
        ...
        void makeControl() {
            ...
            editor.setBackground(ToolBarFolder.background);
            ...
            editor.addListener(SWT.Selection, new Listener() {
                ...
                void handleEvent(Event event) {
                    ...
                    m.createMenu(io.getValue(), editor.getShell(), null)
                     .show(Launcher.display.map(editor, editor.getShell(), new Point(event.x, event.y)));
