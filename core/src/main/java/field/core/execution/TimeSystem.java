...
class TimeSystem implements iDoubleProvider {
    ...
    class Manipulation {
        ...
        public
        Manipulation(float deltaProcessingTime, float deltaExecutionTime) {
    ...
    TimeSystem() {
        processingTime = new FilterStack<Double>(new iProvider<Double>() {
            ...
            @Override
            public
            java.lang.Double addImpl(java.lang.Double a, double w, java.lang.Double to) {
                return to + (a * w);
            }

            @Override
            protected
            java.lang.Double blendImpl(java.lang.Double a, java.lang.Double b, double ea) {
                return (a * (1 - ea)) + (ea * b);
            }
        };
    ...
    public
    FilterStack<Double> getRateFilterStack() {
        return processingTime_rate;
    }

    public
    FilterStack<Double> getTimeFilterStack() {
    ...
    public
    boolean setSuppressNewStackAdditions(boolean supress) {
    ...
    public
    void supplyTimeManipulation(float deltaProcessingTime, float deltaExecutionTime) {
        if (supressNewStack) {
            if (manipulations.isEmpty()) manipulations.add(new Manipulation(deltaProcessingTime, deltaExecutionTime));
        }
        else {
            if (clearOnce) {
                manipulations.clear();
...
//		System.err.println(" -- updating <" + manipulations + "> at <" + processingTimeNow + "> with rate <" + processingTime_rate.get() + "> output is <"+executionTimeNow+">");
        ...
        if (manipulations.isEmpty()) return true;
        ...
        if (m.deltaProc == 0) {
            ...
            if (a1 > m.deltaProc) a1 = m.deltaProc;
            else if (a1 < 0) a1 = 0;

            if (a2 > m.deltaProc) a2 = m.deltaProc;
            else if (a2 < 0) a2 = 0;

            double change = (m.deltaEx * (a1 - a2)) / m.deltaProc;

            System.err.println(" change is <"
                               + change
                               + "> -- looking for a total change of <"
                               + m.deltaEx
                               + "> over <"
                               + m.deltaProc
                               + "> and we are at <"
                               + a1
                               + ">\n   executionTimeNow was <"
                               + executionTimeNow
                               + '>');
