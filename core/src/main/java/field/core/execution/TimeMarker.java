...
class TimeMarker extends DefaultOverride {
    ...
    public static final VisualElementProperty<Object> transitionDuration =
            new VisualElementProperty<Object>("transitionDuration_i");
    ...
    TraversalHint paintNow(IVisualElement source, Rect bounds, boolean visible) {
        ...
        if (source == forElement) {
            ...
            if ((label == null) || scaleHasChanged()) {
                ...
                label = TextSystem.textSystem.new RectangularLabel(printName,
                                                                   new Font(Constants.defaultFont,
                                                                            Font.PLAIN,
                                                                            1).deriveFont(15f),
                                                                   (float) (Math.PI / 2));
            ...
            (label).drawIntoMeshRotated(labelTriangle,
                                        1,
                                        1,
                                        1,
                                        1,
    ...
    private
    List<CachedLine> makeCachedLine(Vector2 lower0, Vector2 upper0) {
    ...
    boolean scaleHasChanged() {
        Vector4 nextViewParameters = new Vector4(0 * GLComponentWindow.getCurrentWindow(null).getXTranslation(),
                                                 0 * GLComponentWindow.getCurrentWindow(null).getYTranslation(),
                                                 1 / GLComponentWindow.getCurrentWindow(null).getXScale(),
                                                 1 / GLComponentWindow.getCurrentWindow(null).getYScale());
