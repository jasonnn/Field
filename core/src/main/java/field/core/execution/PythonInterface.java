...
class PythonInterface implements ScriptingInterface {
    ...
    public final
    class LocalDictionary extends PyStringMap implements iLocalDictionary {
        ...
        public
        String findReverse(Object o, Class c) {
        ...
        @Override
        public
        void __setitem__(String arg0, PyObject arg1) {
            try {
                ...
                if (!globalTrap.isEmpty()) {
                    ...
                    if (!shared.isEmpty() && !arg0.startsWith("_")) {
                        ...
                        try {
                            ...
                            if (t != Py.NoConversion) for (ScriptingInterface s : shared) {
    ...
    protected
    void addExtensionsDirectory(File file) {
        ...
        for (String p : pythonFiles) {
            try {
                ...
                if (firstLine.contains("field-library")) {
                    System.err.println(" --- executing file from extensions directory <"
                                       + file.getAbsolutePath()
                                       + '>');
                    executeFile(file.getAbsolutePath() + '/' + p);
                }
                else {
                    System.err.println(" --- importing module from extensions directory <"
                                       + p.replace(".py", "")
                                       + '>');
    ...
    public
    void addSharedScriptingInterface(ScriptingInterface s) {
        shared.add(s);
    }

    public
    Object eval(String fragment) {
        ...
        try {
            ...
            try {
                ...
                if (errOut == null) t.printStackTrace();
                else {
    ...
    public
    void execString(String fragment) {
                ...
                if (errOut == null) t.printStackTrace();
                    ...
                    if (t instanceof PyException) {
                        ...
                        try {
                            ...
                            w.println("on line " + ((((PyException) t).traceback != null
                                                     ? ((PyException) t).traceback.tb_lineno
                                                     : "null")));
                            t.printStackTrace(w);
    ...
    private
    void writeException(Throwable t, PrintWriter w) {
        w.println("python exception <" + ((PyException) t).value + '>');
        try {
            w.println("on line "
                      + ((PyException) t).traceback.tb_lineno
                      + ' '
                      + moduleNameFor(((PyException) t).traceback.tb_frame.f_code.co_filename)
                      + " / "
                      + ((PyException) t).traceback.tb_frame.f_code.co_name
                      + ' '
                      + HyperlinkedErrorMessage.error(((PyException) t).traceback.tb_frame.f_code.co_filename,
                                                      ((PyException) t).traceback.tb_lineno));
            ...
            while (n != null) {
                w.println("from line "
                          + n.tb_lineno
                          + ' '
                          + moduleNameFor(n.tb_frame.f_code.co_filename)
                          + " / "
                          + n.tb_frame.f_code.co_name
                          + ' '
            ...
            if (cause != null) {
                ...
                if (last instanceof PyException) writeException(last, w);
                else {
                    ...
                    for (int i = 0; i < s.length; i++) {
                        ...
                        if (!s[i].getClassName().startsWith("java.")) nn++;
                        if (nn > 3) break;
                    }
                }
            }
    ...
    private
    void exec(final PythonInterpreter i, final String f) {
        ...
        PyCode code = Py.compile_flags(f,
                                       moduleName,
                                       CompileMode.exec,
                                       (CompilerFlags) ReflectionTools.illegalGetObject(i, "cflags"));
        i.exec(code);
    ...
    public
    PyObject executeStringReturnPyObject(String script, String tag) {
        script = clean(script);
        try {
    ...
    public
    Object executeStringReturnRawValue(String script, String tag) {
        script = clean(script);
        try {
            exec(interpreter, script);
            PyObject py = (interpreter).get(tag);
    ...
    public
    Object executeStringReturnValue(String script, String tag) {
        script = clean(script);
    ...
    public
    Stack<Writer> getOutputRedirects() {
        return outputRedirects;
    }

    public
    Stack<Writer> getErrorRedirects() {
    ...
    public
    void importJava(String pack, String clas) {
    ...
    public
    void popOutput() {
        if (insideShared) return;
        insideShared = true;
        try {
            if (outputRedirects.size() > 0) outputRedirects.pop();
            if (errorRedirects.size() > 0) errorRedirects.pop();

            if (outputRedirects.size() > 0) interpreter.setOut(wrapWriter(outputRedirects.peek()));
            // else
    ...
    public
    void printError(String p) {
        ((PyFile) state.stderr).write(p);
        ((PyFile) state.stderr).flush();
    }

    public
    void pushGlobalTrap(iGlobalTrap gt) {
        globalTrap.add(gt);
    }

    public
    void pushOutput(Writer output, Writer error) {
        if (insideShared) return;
        insideShared = true;
        try {
    ...
    void handlePythonException(final iVisualElement element, final iVisualElement parentElement, final Throwable t) {
        Logging.logging.addEvent(new iLoggingEvent() {
            ...
            String getLongDescription() {
                ...
                String m = "<html>" + PopupInfoWindow.title("Exception") + PopupInfoWindow.content(t.getMessage()
                                                                                                   + " / ("
                                                                                                   + t.getClass()
                                                                                                      .getSimpleName()
                                                                                                   + ')') + "<br>";
            ...
            String getTextDescription() {
                return "<html><font bgcolor=#ffeeee><b> Exception  "
                       + (t.getMessage() == null ? "" : t.getMessage())
                       + " ("
                       + t.getClass().getSimpleName()
                       + ") thrown in element "
                       + ElementInvocationLogging.describeElementLink(element)
                       + (parentElement == null
                          ? ""
                          : ("called from " + ElementInvocationLogging.describeElementLink(parentElement)));
            }
    ...
    void forceExit() {
        ...
        if (ts.frame != null) ts.frame.tracefunc = ts.tracefunc = new TraceFunction() {

            @Override
            public
            TraceFunction traceReturn(PyFrame frame, PyObject ret) {
            ...
            @Override
            public
            TraceFunction traceLine(PyFrame frame, int line) {
                // System.out.println(frame + " " +
                // line);
                ts.tracefunc = null;
                throw new IllegalStateException("aborted");
            ...
            @Override
            public
            TraceFunction traceException(PyFrame frame, PyException exc) {
                // System.out.println(frame + " " +
                // exc);
                ts.tracefunc = null;
                throw new IllegalStateException("aborted");
            ...
            @Override
            public
            TraceFunction traceCall(PyFrame frame) {
                // System.out.println(frame);
                ts.tracefunc = null;
                throw new IllegalStateException("aborted");
            }
