...
class PluginList {
    ...
    public
    class PluginOrExtensionNode extends field.math.graph.NodeImpl<PluginOrExtensionNode> {
        protected final String rootname;
        protected final boolean enabled;
        protected File file;

        public
        PluginOrExtensionNode(String rootname, boolean enabled) {
            this.rootname = rootname;
            this.enabled = enabled;
        }

        public
        PluginOrExtensionNode associateFile(File f) {
            this.file = f;
            return this;
        }
        ...
        String dressName() {
            ...
            return splat.length == 1
                   ...
                   : ((enabled ? "<b>" : "")
                      + splat[0]
                      + (enabled ? "</b>" : "")
                      + rootname.substring(splat[0].length()));
        }
    ...
    void invalidateAndRebuild() {
        selectionUI.setLabel("<html><font color='#aa0000' face='"
                             + Constants.defaultFont
                             + "'>the plugins list has been modified, <b>restart</b> <i>Field</i> for these changes to take effect</font>");
        reconstructRootModel();
    }

    public
    class JARNode extends PluginOrExtensionNode {

        public
        JARNode(String rootname, boolean enabled) {
    ...
    public
    class MFNode extends PluginOrExtensionNode {

        public
        MFNode(String rootname, boolean enabled) {
            super(rootname, enabled);
    ...
    public
    class ClasspathNode extends PluginOrExtensionNode {

        public
        ClasspathNode(String rootname, boolean enabled) {
            super(rootname, enabled);
        }
    ...
    public
    class InformationNode extends PluginOrExtensionNode {

        public
        InformationNode(String rootname, boolean enabled) {
            super(rootname, enabled);
        }
    ...
    public
    class PluginNode extends PluginOrExtensionNode {

        public
        PluginNode(String rootname, boolean enabled) {
            super(rootname, enabled);
        }
    ...
    public
    class ExtensionNode extends PluginOrExtensionNode {

        public
        ExtensionNode(String rootname, boolean enabled) {
            super(rootname, enabled);
        }
    ...
    public
    class DirNode extends PluginOrExtensionNode {

        public
        DirNode(String rootname, boolean enabled) {
            super(rootname, enabled);
        }
    ...
    protected
    void handleDrop(String string) throws IOException {
        ...
        if (f.isDirectory()) {
            ...
            try {
                BufferedWriter writer =
                        new BufferedWriter(new FileWriter(new File(extensionsDir + "/" + n + "_extension.mf")));
                writer.write("Field-RedirectionPath: " + f.getCanonicalPath() + (f.isDirectory() ? "/**" : "") + '\n');
                writer.close();
            } catch (IOException ee) {
                BufferedWriter writer =
                        new BufferedWriter(new FileWriter(new File(extensionsDir2 + "/" + n + "_extension.mf")));
                writer.write("Field-RedirectionPath: " + f.getCanonicalPath() + (f.isDirectory() ? "/**" : "") + '\n');
                writer.close();
                ...
                u.put(" \u271A  Add jar/directory to classpath", new iUpdateable() {
                    ...
                    void update() {
                        ...
                        if (f != null) {
                            ...
                            try {
                                BufferedWriter writer = new BufferedWriter(new FileWriter(new File(extensionsDir2
                                                                                                   + "/"
                                                                                                   + n
                                                                                                   + "_extension.mf")));
                                writer.write("Field-RedirectionPath: " + f.getCanonicalPath() + (f.isDirectory()
                ...
                u.put(" \u21e5 Copy jar into application extensions directory", new iUpdateable() {
                    ...
                    void update() {
                        ...
                        if (f != null) {
                            try {
                                new ExecuteCommand(".",
                                                   new String[]{"/bin/cp", f.getCanonicalPath(), extensionsDir + "/"},
                                                   true).waitFor();
                                invalidateAndRebuild();
                            } catch (IOException e) {
                                e.printStackTrace();
                ...
                u.put(" \u21e5 Copy jar into user extensions directory", new iUpdateable() {
                    ...
                    void update() {
                        ...
                        if (f != null) {
                            try {
                                new ExecuteCommand(".",
                                                   new String[]{"/bin/cp", f.getCanonicalPath(), extensionsDir2 + "/"},
                                                   true).waitFor();
                                invalidateAndRebuild();
                            } catch (IOException e) {
                                e.printStackTrace();
...
//			combo.selectNext();
        ...
        label.setFont(new Font(Launcher.display,
                               ...
                               GraphNodeToTreeFancy.baseFontHeight(label) + 2,
                               SWT.NORMAL));
    ...
    static public
    void addStringToPluginEntry(Class c, String html) {
    ...
    private
    void reconstructRootModel(File extensionsDir, PluginOrExtensionNode root, boolean on) {
    ...
    void flipFile(File f) {
        ...
        if (f.getName().endsWith("_")) {
            if (!f.renameTo(new File(ap.substring(0, ap.length() - 1)))) {
                ...
                new ExecuteCommand("/bin/cp",
                                   new String[]{ap,
                                                System.getProperty("user.home")
                                                + "/Library/Application Support/Field/extensions/"
                                                + ap.substring(0, ap.length() - 1)},
                                   true).waitFor();
            }
        }
        else {
    ...
    protected
    void parseManifest(Manifest m, MFNode n) {
        ...
        if (aa != null) {
            if (aa.endsWith("**")) {
                ...
                if (dir.exists()) {
                    ...
                    if (ll != null) for (String a : ll) {
                        ClasspathNode c = new ClasspathNode(new File(dir.getAbsolutePath() + "/" + a).getAbsolutePath(),
                                                            n.enabled);
                        n.addChild(c);
            ...
            else for (String a : aa.split(":")) {
                File f = new File(a);
                if (!f.isAbsolute()) f = new File(extensionsDir + "/" + a);
    ...
    protected
    String dumpNode(PluginOrExtensionNode pluginOrExtensionNode) {
