...
class TranscriptCompressors {
                ...
                if (e instanceof iSuspendedContext) {
                    ...
                    if (beginMatching != null) {
                        ...
                        for (int c = 0; c < currentContext.size(); c++) {
                            ...
                            if (beginMatching.isInstance(currentContext.get(a))
                                && ((iSuspendedContext) currentContext.get(a)).getToken().equals(s.getToken())) {
                        ...
                        if (s.isExclusivelyContextural()) {
                            ...
                            assert found : "no beginign for end in transcript"
                                           + s
                                           + ' '
                                           + currentContext
                                           + ' '
    ...
    class Join implements Logging.iTranscriptCompression {
        ...
        public
        Join(iTranscriptCompression[] compression) {
            super();
    ...
    class LastNumberOfCycles implements Logging.iTranscriptCompression {
        ...
        public
        LastNumberOfCycles(int maxCycle) {
    ...
    class MaxNumEvents implements Logging.iTranscriptCompression {
        ...
        ArrayList<iLoggingEvent> compress(ArrayList<iLoggingEvent> events) {
            if (events.size() > maxEvents)
                return new ArrayList<iLoggingEvent>(events.subList(events.size() - maxEvents, events.size()));
            return events;
        }
    }
