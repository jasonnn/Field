...
class Logging {
    ...
    public
    interface iTranscriptCompression {
        public
        ArrayList<iLoggingEvent> compress(ArrayList<iLoggingEvent> events);
    }

    public
    interface iTranscriptViewGeneration {
        public
        boolean generateView(ArrayList<iLoggingEvent> transcript, LoggingEventModel root);
    }

    public
    interface iTranscriptViewTransformation {
        public
    ...
    class LoggingEventModel extends NodeImpl<LoggingEventModel> implements IMutableContainer<Object, LoggingEventModel>,
        ...
        public
        LoggingEventModel(LoggingEventType type, iLoggingEvent node, String label) {
            this.type = type;
            this.node = node;
            this.label = label;
        }
    ...
    class UpdateCycle implements iLoggingEvent {
        ...
        public
        UpdateCycle(long num, long time) {
            this.num = num;
            this.time = time;
        }
    ...
    static public
    boolean enabled(Status moreThan) {
...
//			new Exception().printStackTrace();
    ...
    public
    void addEvent(iLoggingEvent event) {
        if (event != null) for (int i = 0; i < sinks.size(); i++) {
            sinks.get(i).addEvent(event);
        }
    }

    public
    void addSink(iLoggingSink s) {
        sinks.add(s);
    }

    public
    void removeSink(iLoggingSink sink) {
        sinks.remove(sink);
    }
