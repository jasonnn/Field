...
public
class AssemblingLogging extends InvocationLogging {

    public
    interface iBlendSupport<T> {

        public
        T blend(List<T> t, List<Number> weights);
    ...
    public
    interface iCloneSupport<T> {
        public
        boolean isCloneSupported(Object o);
    ...
    public
    interface iConstructionSupport<T> {
        public
        String construct(T t);

        public
        boolean isConstructionSupported(Object o);
    }
    ...
    public
    interface iMutationSupport<T> {
        public
        boolean isMutationSupported(Object o);
        ...
        public
        void mutate(T target, T value);

        public
        void mutate(T target, T value, float amount);

        public
        String mutationExpression(T target, T value, String valueText);
    }

    public
    interface iProvidesSimpleChange {
        public
        SimpleChange getSimpleChange();
    }
    ...
    class Link {
        ...
        String toString() {
            ...
            if (type == LinkType.call) r += "args=" + args + " <" + before + " -> " + after + '>';
            if (type == LinkType.set) r += " <" + before + " <- " + to + " -> " + after + "> " + ((before != current)
                                                                                                  ? (" (now <"
                                                                                                     + current
                                                                                                     + ">)")
                                                                                                  : "");
            ...
            if (type == LinkType.origin) r += " currently <" + current + '>';
            return r;
        }
    ...
    public
    class MoveEvent implements Logging.iLoggingEvent, iUndoable, iProvidesSimpleChange, iProvidesContextStack {
        ...
        public
        MoveEvent(String prefix, Move move) {
            super();
            this.move = move;
            this.prefix = prefix;
        }
        ...
        void executeSimpleUndo() {
            if (((Link) move.links.get(move.links.size() - 1)).type == LinkType.call) {
                ...
                ReflectionTools.illegalSetObjectPrimativeAware(((Link) move.links.get(move.links.size() - 2)).current,
                                                               ((Link) move.links.get(move.links.size() - 1)).name,
                                                               before);
            }
        }
        ...
        String getDoExpression() {
            try {
                ...
                if (((Link) move.links.get(move.links.size() - 1)).type == LinkType.call) {
                    iMutationSupport m = getMutationFor(current);
                    return expressionForLink(move, move.links.size() - 3) + m.mutationExpression(current,
                                                                                                 after,
                                                                                                 getConstructionFor(after));
                }
                if (((Link) move.links.get(move.links.size() - 1)).type == LinkType.set) {
                    return expressionForLink(move, move.links.size() - 2)
                           + "."
                           + ((Link) move.links.get(move.links.size() - 1)).name
                           + " = "
                           + getConstructionFor(after);
        ...
        public
        String getDoExpressionLHS() {
            if (((Link) move.links.get(move.links.size() - 1)).type == LinkType.call) {
                return expressionForLink(move, move.links.size() - 3);
            }
            if (((Link) move.links.get(move.links.size() - 1)).type == LinkType.set) {
                return expressionForLink(move, move.links.size() - 2) + "." + ((Link) move.links.get(move.links.size()
                                                                                                     - 1)).name;
            }
            return null;
        }
            ...
            if (((Link) move.links.get(move.links.size() - 1)).type == LinkType.call) {
                ...
                if (m != null) {
                    ...
                    return new SimpleChange(move.before, move.after, getDoExpressionLHS()) {
                        ...
                        @Override
                        public
                        iProvidesSimpleChange getSource() {
                        ...
                        @Override
                        public
                        void writeChange(Object value) {
                            m.mutate(tar, value);
                        }

                        @Override
                        public
                        void writeChange(Object value, float amount) {
                ...
                return new SimpleChange(move.before, move.after, getDoExpressionLHS()) {
                    ...
                    void writeChange(Object value, float amount) {
                        ...
                        if ((c instanceof PyInstance) || (c instanceof PyObject)) {
                            ...
                            if (bs == null) ReflectionTools.illegalSetObjectPrimativeAware(c, n, value);
                            else {
                                List in = new ArrayList();
                                in.add(value);
                                in.add(current);
    ...
    class PartiallyEvaluatedFunction {
        ...
        public
        Object call() {
            if (function != null) return function.get();
            return evalautedTo;
        }
    }
    ...
    class SimpleChange {
        ...
        public
        SimpleChange(Object previousValue, Object value, String target) {
            super();
    ...
    AssemblingLogging() {
        ...
        mutations.add(new iMutationSupport<Vector3>() {
            ...
            public
            void mutate(Vector3 target, Vector3 value) {
                target.setValue(value);
            }

            public
            void mutate(Vector3 target, Vector3 value, float amount) {
                target.lerp(target, value, amount);
            }

            public
            String mutationExpression(Vector3 target, Vector3 value, String valueText) {
        ...
        mutations.add(new iMutationSupport<Vector4>() {
            ...
            public
            void mutate(Vector4 target, Vector4 value) {
                target.setValue(value);
            }

            public
            void mutate(Vector4 target, Vector4 value, float amount) {
                target.lerp(target, value, amount);
            }

            public
            String mutationExpression(Vector4 target, Vector4 value, String valueText) {
                return ".setValue(" + valueText + ")";
            }
        });
                ...
                for (IBlendAlgebra b : t) {
                    ...
                    z = b.blendRepresentation_add(z,
                                                  b.blendRepresentation_multiply(ww.floatValue(),
                                                                                 b.blendRepresentation_duplicate(b)),
                                                  z);
                    tot += ww.floatValue();
                }
        ...
        blendings.add(new iBlendSupport<Integer>() {
            ...
            public
            Integer blend(List<Integer> t, List<Number> weights) {
                if (t.size() == 0) return null;
        ...
        blendings.add(new iBlendSupport<Float>() {
            ...
            public
            Float blend(List<Float> t, List<Number> weights) {
                if (t.size() == 0) return null;
        ...
        blendings.add(new iBlendSupport<Double>() {
            ...
            public
            Double blend(List<Double> t, List<Number> weights) {
                if (t.size() == 0) return null;
        ...
        blendings.add(new iBlendSupport<Number>() {
            ...
            public
            Number blend(List<Number> t, List<Number> weights) {
                if (t.size() == 0) return null;
        ...
        blendings.add(new iBlendSupport<Object[]>() {
            ...
            public
            Object[] blend(List<Object[]> t, List<Number> weights) {
                if (t.size() == 0) return null;
        ...
        blendings.add(new iBlendSupport<ArrayList>() {
            ...
            public
            ArrayList blend(List<ArrayList> t, List<Number> weights) {
                if (t.size() == 0) return null;
    ...
    public
    Move buildMoveFor(LinkNode leaf) {
    ...
    public
    String expressionForLink(Move m, int n) {
        String expression = "";
        assert n < m.links.size();
        for (int i = n; i >= 0; i--) {
            Link at = ((Link) m.links.get(i));

            if (at.type == LinkType.dot) expression = "." + at.name + expression;
            if (at.type == LinkType.origin) expression = at.path + expression;
            if (at.type == LinkType.set) expression = "." + at.name + expression + " = " + getConstructionFor(at.to);
            if (at.type == LinkType.call) expression = "(" + getConstructionsFor(at.args) + ")" + expression;
        }
        return expression;
    }

    public
    iBlendSupport getBlendFor(Object o) {
    ...
    public
    ArrayList<SimpleChange> getSimpleChangeSet(ArrayList<Move> m) {
            ...
            Launcher.getLauncher().registerUpdateable(new IUpdateable() {
                ...
                void update() {
                    ...
                    for (int i = 0; i < moves.size(); i++) {
                        ...
                        if (Logging.enabled()) Logging.logging.addEvent(new MoveEvent("(out of order)", moves.get(i)));
                    }
                }
            });
        }
    ...
    public
    Move linearize(LinkNode leaf) {
        Move m = new Move();
        String expression = "";

        LinkNode at = leaf;
        while (at != null) {
            if (at.link.type == LinkType.dot) expression = "." + at.link.name + expression;
            if (at.link.type == LinkType.origin) expression = at.link.path + expression;
            if (at.link.type == LinkType.set)
                expression = "." + at.link.name + expression + " = " + getConstructionFor(at.link.to);
            if (at.link.type == LinkType.call) expression = "(" + getConstructionsFor(at.link.args) + ")" + expression;
...
//			;//System.out.println(" link <" + at + "> expression so far <" + expression + ">");
            ...
            if (at.getParents() == null) at = null;
            else if (at.getParents().size() == 0) at = null;
            else at = (LinkNode) at.getParents().get(0);
        }
    ...
    @Override
    public
    void linkCall(PyObject in, PyObject args, String path, boolean isRoot, Object result, PyObject from) {
        ...
        assert invokingLink.link.type == LinkType.origin
               || invokingLink.link.type == LinkType.dot : invokingLink.link.type;
    ...
    @Override
    public
    void linkCallFinish(PyObject parent, PyObject newChild) {
        ...
        if (installed) if (Logging.enabled()) Logging.logging.addEvent(new MoveEvent("call", buildMoveFor(childNode)));
    ...
    @Override
    public
    PyObject linkGetAttr(PyObject in, String name, boolean isRoot, String path, PyObject from) {
        ...
        if (parent == null) {
            if (!isRoot) {
                System.err.println(" warning, no root found for <"
                                   + name
                                   + "> <"
                                   + path
                                   + "> <"
                                   + from
                                   + ">, yet this is not marked as root ? ");
            }
    ...
    @Override
    public
    void linkGetAttrFinish(PyObject parent, PyObject newChild) {
        LinkNode childNode = partialLinkNodes.remove(parent);
    ...
    @Override
    public
    void linkSetAttr(PyObject in, String name, PyObject value, boolean isRoot, String path, PyObject from) {
        ...
        if (installed) if (Logging.enabled()) Logging.logging.addEvent(new MoveEvent("set ", buildMoveFor(child)));

        setFinished(child);
    }

    public
    ArrayList<Move> resetReturningMoves() {
    ...
    protected
    void callFinished(LinkNode childNode) {
    }

    protected
    Object getCloneFor(Object o) {
        if (o == null) return null;

        Class<? extends Object> c = o.getClass();
        iCloneSupport cached = cloneSupportCache.get(c);
        if (cached != null) return cached.save(o);
    ...
    protected
    String getConstructionFor(Object o) {
        if (o == null) return null;

        Class<? extends Object> c = o.getClass();
        iConstructionSupport cached = constructionSupportCache.get(c);
        if (cached != null) return cached.construct(o);
    ...
    protected
    String getConstructionsFor(ArrayList<Object> args) {
        String r = "";
    ...
    protected
    iMutationSupport getMutationFor(Object o) {
        if (o == null) return null;
    ...
    protected
    void setFinished(LinkNode childNode) {
    }
