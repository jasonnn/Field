...
class OutputInserts {
    ...
    public static VisualElementProperty<OutputInserts> outputInserts =
            new VisualElementProperty<OutputInserts>("outputInserts_");
    ...
    protected static
    UndoableEdit closeFold(String name, OutputInserts oi) {
    ...
    void specialPrint(Object o, iVisualElement inside) {
        ...
        try {
            for (iHandlePrintClass c : handlers) {
                if (c.handle(o, oi, inside)) return;
            }
            println(o, oi);
        } finally {
            ((PythonPluginEditor) ed).getEditor().getOutputFlusher().update();
    ...
    private static
    void println(Object o, OutputInserts oi) {
        try {
    ...
    public
    OutputInserts(DefaultStyledDocument document) {
    ...
    public
    void printComponent(JComponent component) {
        try {
            document.insertString(document.getEndPosition().getOffset() - 1,
                                  "((printed component))",
                                  getAttributeSetForComponent(component));
    ...
    private
    void printComponent(JComponent component, String name) {
        try {
            document.insertString(document.getEndPosition().getOffset() - 1,
                                  styleNameForName(name),
                                  getAttributeSetForComponent(component));
            document.insertString(document.getEndPosition().getOffset(), "\n", null);
