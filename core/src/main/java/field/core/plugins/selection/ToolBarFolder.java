...
class ToolBarFolder {
    ...
    public static Color background =
            Platform.isMac() ? new Color(Launcher.display, 200, 200, 200) : new Color(Launcher.display, 200, 200, 200);
    public static Color firstLineBackground =
            Platform.isMac() ? new Color(Launcher.display, 170, 170, 170) : new Color(Launcher.display, 220, 220, 220);
    public static Color sashBackground =
            Platform.isMac() ? new Color(Launcher.display, 200, 200, 200) : new Color(Launcher.display, 200, 200, 200);
    ...
    public
    ToolBarFolder(final Rectangle r) {
        this(r, false);
    }

    public
    ToolBarFolder(final Rectangle r, boolean forceWindow) {
        if (useGLComponentWindowLeftSash && !forceWindow) {
            ...
            tools = new Shell(Launcher.display,
                              SWT.SHELL_TRIM | (Platform.getOS() == OS.mac ? (SWT.TOOL | SWT.ON_TOP) : 0));
        ...
        try {
            if (Platform.getOS() == OS.mac)
                ReflectionTools.findFirstMethodCalled(ReflectionTools.illegalGetObject(tools, "window").getClass(),
                                                      "setHidesOnDeactivate")
                               .invoke(ReflectionTools.illegalGetObject(tools, "window"), true);
    ...
    protected
    void deferredOpen() {
        if (tools instanceof Shell) ((Shell) tools).open();
    }

    public
    Shell getShell() {
        if (tools instanceof Shell) return ((Shell) tools);
        return null;
    }
    ...
    public
    void add(Image icon, final Control c) {
    ...
    public
    void add(final String icon, final Control c) {
