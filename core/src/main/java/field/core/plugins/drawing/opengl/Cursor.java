...
class Cursor {
    ...
    public
    Cursor(CachedLine line, float dot) {
     ...
     */
    public
    Cursor forwardD(float distance) {
     ...
     */
    public
    Pair<CachedLine, CachedLine> split() {
     ...
     */
    public
    Cursor setT(float dot) {
        // ;//System.out.println(" set T <"+dot+">");
        if (dot < 0) return setT(lengthT() - 1 - Math.abs(dot));

        if (dot > lengthT()) dot = lengthT();
     ...
     */
    public
    Cursor forwardT(float dt) {
        if (dt == 0) return this;
        if (dt < 0) return backwardT(-dt);
     ...
     */
    public
    Cursor backwardT(float dt) {
        if (dt == 0) return this;
        if (dt < 0) return forwardT(-dt);
...
//				;//System.out.println(" next linear frame for interpolation <"+va+" "+vb+">");
    ...
    public
    <T> T[] attribute(Prop<T> p, Class<? super T> t) {
     ...
     */
    public
    Object attribute(String p) {
        Object[] o = cursor.nextAttributeFrame(new Prop(p), Object.class);

        if (o == null) return null;
        if (o[0] instanceof Vector3) return new Vector3().lerp((Vector3) o[0], (Vector3) o[1], t);
        if (o[0] instanceof Vector4) return new Vector4().lerp((Vector4) o[0], (Vector4) o[1], t);
        if (o[0] instanceof Vector2) return new Vector2().lerp((Vector2) o[0], (Vector2) o[1], t);
    ...
    public
    Vector2 tangentBackward() {
        Vector2 va = new Vector2();
        Vector2 vc1 = new Vector2();
        Vector2 vc2 = new Vector2();
        Vector2 vb = new Vector2();
        if (t == 0) {
            if (!cursor.previousCubicFrame(va, vc1, vc2, vb)) return null;
            ...
            if (!cursor.nextCubicFrame(va, vc1, vc2, vb)) return null;
    ...
    public
    Vector3 tangentBackward3() {
        Vector3 va = new Vector3();
        Vector3 vc1 = new Vector3();
        Vector3 vc2 = new Vector3();
        Vector3 vb = new Vector3();
        if (t == 0) {
            if (!cursor.previousCubicFrame3(va, vc1, vc2, vb)) return null;
            ...
            if (!cursor.nextCubicFrame3(va, vc1, vc2, vb)) return null;
