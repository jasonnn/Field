...
public class HorizontalMarqueeTool implements iSimpleSelectionTool {
	...
	public void paint() {
        ...
        marqueLine.getInput().moveTo(GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2()).x, downAt.y);
        marqueLine.getInput().lineTo(GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(5000, 0)).x, downAt.y);
        marqueLine.getInput().lineTo(GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(5000, 0)).x, upAt.y);
        marqueLine.getInput().lineTo(GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(0, 0)).x, upAt.y);
        marqueLine.getInput().lineTo(GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(0, 0)).x, downAt.y);
