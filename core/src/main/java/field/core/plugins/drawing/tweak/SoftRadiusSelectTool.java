...
public
class SoftRadiusSelectTool implements iSimpleSelectionTool {
        ...
        for (Entry<Vector2, Selectable> s : es) {
            ...
            if ((v.x > x1) && (v.x < x2) && (v.y > y1) && (v.y < y2)) {
                ...
                if (!inside.selection.contains(sel.current)) {
                    ...
                    float r = (float) Math.sqrt(((v.x - ((x1 + x2) / 2)) * (v.x - ((x1 + x2) / 2))) + ((v.y - ((y1 + y2)
                                                                                                               / 2))
                                                                                                       * (v.y - ((y1
                                                                                                                  + y2)
                                                                                                                 / 2))));
                    ...
                    for (SubSelection ss : sel.current.whatSelected)
                        sel.current.setAmountSelectedFor(ss, Math.max(0.1f, 1 - (r / rMax)));
    ...
    void paint() {
        ...
        marqueLine.getProperties()
                  .put(iLinearGraphicsContext.strokeType,
                       new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 1, new float[]{5, 5}, 0));
