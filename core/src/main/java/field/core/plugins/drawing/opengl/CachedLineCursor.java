...
class CachedLineCursor {
    ...
    public
    CachedLineCursor(CachedLine on) {
        this.on = on;
        currentIndex = -1;
    }

    public
    CachedLineCursor(CachedLine on, int minAtIndex) {
        this.on = on;
    ...
    public
    CachedLine.Event getAfter() {
        return after;
    }

    public
    boolean getAt(Vector2 a) {
        ...
        if (current.args == null) return false;
        if (current.args[current.args.length - 2] == null) return false;
        if (current.args[current.args.length - 1] == null) return false;
    ...
    public
    CachedLine.Event getBefore() {
        return before;
    }

    public
    CachedLine.Event getCurrent() {
        return current;
    }
    ...
    public
    Vector2 getNextDestination(Vector2 a) {
        if (after == null) return null;
        if (after.method.equals(iLine_m.close_m)) return on.events.get(lastMoveTo).getDestination(a);
    ...
    public
    CachedLineCursor next() {
        if (after == null) {
            currentIndex++;
            if (currentIndex >= on.events.size()) currentIndex = on.events.size() - 1;
            ...
            if (after != null) if (after.method.equals(iLine_m.moveTo_m)) after = null;
        }
        else {
            before = current;
            current = after;
        ...
        if (current.method.equals(iLine_m.moveTo_m)) lastMoveTo = currentIndex;
        return this;

    }

    public
    boolean nextCubicFrame(Vector2 a, Vector2 c1, Vector2 c2, Vector2 b) {
    ...
    public
    boolean nextCubicFrame3(Vector3 a, Vector3 c1, Vector3 c2, Vector3 b) {
        if (after == null) return false;
    ...
    public
    <T> T[] nextAttributeFrame(Dict.Prop<T> p, Class<? super T> c) {
    ...
    boolean nextIsCubic() {
        if (after == null) return false;
        if (after.method.equals(iLine_m.cubicTo_m)) return true;
        return false;
    }
    ...
    public
    boolean nextLinearFrame(Vector2 a, Vector2 b) {
    ...
    public
    boolean nextLinearFrame3(Vector3 a, Vector3 b) {
...
//				;//System.out.println(" looking at <"+current.getAttributes()+">");
    ...
    public
    void getThreeFrame(Vector3 b, Vector3 c, Vector3 a) {
    ...
    public
    boolean previousCubicFrame(Vector2 a, Vector2 c1, Vector2 c2, Vector2 b) {
    ...
    public
    boolean previousCubicFrame3(Vector3 a, Vector3 c1, Vector3 c2, Vector3 b) {
        if (before == null) return false;
        if (current.method.equals(iLine_m.cubicTo_m)) {
            a.x = ((Number) (before.args)[before.args.length - 2]).floatValue();
            a.y = ((Number) (before.args)[before.args.length - 1]).floatValue();
