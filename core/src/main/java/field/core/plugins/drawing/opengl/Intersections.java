...
class Intersections {
    ...
    class Intersection_CubicCubic {
        ...
        // position
        public
        boolean intersectCubic(Vector2 la,
                               Vector2 l1,
                               Vector2 l2,
                               Vector2 lb,
                               Vector2 ra,
            ...
            if ((mlx + fuzz >= irx) && (mly + fuzz >= iry) && (mrx + fuzz >= ilx) && (mry + fuzz >= ily)) {
                ...
                LineUtils.splitCubicFrame(la, l1n, l2n, lb, 0.5f, c12l, ml, c21l, t);
                LineUtils.splitCubicFrame(ra, r1n, r2n, rb, 0.5f, c12r, mr, c21r, t);
    ...
    class Intersection_LineLine {
        ...
        public
        boolean intersectLine(Vector2 la, Vector2 lb, Vector2 ra, Vector2 rb, Vector2 out) {
    ...
    List<Vector2> intersect(CachedLine line, Vector2 a, Vector2 b) {
        ...
        for (int i = 1; i < line.events.size(); i++) {
            if (!line.events.get(i - 1).hasDestination()) continue;
            ...
            if (line.events.get(i).method.equals(iLine_m.lineTo_m)) {
                ...
                if (lc.intersectCubic(a,
                                      b,
                                      prev,
                                      line.events.get(i).getAt(0),
                                      line.events.get(i).getAt(1),
                                      line.events.get(i).getAt(2),
                                      o,
                                      0)) {
    ...
    public static
    List<Pair<CachedLine.Event, CachedLine.Event>> intersectAndSubdivide(List<CachedLine> p1,
                                                                         List<CachedLine> p2,
                                                                         int limit) {
        ...
        while (skipOut) {
            for (CachedLine c1 : p1) {
                for (int i1 = 1; i1 < c1.events.size(); i1++) {
                    ...
                    if (e1.method.equals(iLine_m.moveTo_m)) continue;

                    Event e1_last = c1.events.get(i1 - 1);

                    if (i1 > limit) return ret;

outer:
                    for (CachedLine c2 : p2) {
                        for (int i2 = 1; i2 < c2.events.size(); i2++) {

                            Event e2 = c2.events.get(i2);
                            Event e2_last = c2.events.get(i2 - 1);
                            if (e2 == e1 || e2 == e1_last || e2_last == e1) continue;
                            if (e2.method.equals(iLine_m.moveTo_m)) continue;
                            if (!e2.hasDestination()) continue;
                            if (!e1.hasDestination()) continue;
                            if (!e2_last.hasDestination()) continue;
                            if (!e1_last.hasDestination()) continue;
                            ...
                            if (e1.method.equals(iLine_m.cubicTo_m)) {
                                if (e2.method.equals(iLine_m.cubicTo_m)) {
                                    ...
                                    if (cc.intersectCubic(a1, c1_cx1, c1_cx2, b1, a2, c2_cx1, c2_cx2, b2, out, 0)) {
                                        ...
                                        if (out.x * a1.distanceFrom(b1) > tol
                                            && (1 - out.x) * a1.distanceFrom(b1) > tol) {
...
//											intersectionLeft = e1_last;
                                        if (out.y * a2.distanceFrom(b2) > tol
                                            && (1 - out.y) * a2.distanceFrom(b2) > tol) {
...
//											intersectionRight = e2_last;
                                    ...
                                    if (lc.intersectCubic(a2, b2, a1, c1_cx1, c1_cx2, b1, out, 0)) {
                                        ...
                                        if (out.y * a1.distanceFrom(b1) > tol
                                            && (1 - out.y) * a1.distanceFrom(b1) > tol) {
                                            c1.events.remove(i1);
...
//										else
//											intersectionLeft = e1_last;
                                        if (out.x * a2.distanceFrom(b2) > tol
                                            && (1 - out.x) * a2.distanceFrom(b2) > tol) {
                                            c2.events.remove(i2);
...
//							;//System.out.println(" left :"+intersectionLeft+" "+intersectionRight);
    ...
    Event resolve(final Event left) {
        if (left.container.events.contains(left)) return left;

        return ReflectionTools.argMin(left.container.events, new Object() {
            public
            float f(Event e) {
                if (e.hasDestination()) return e.getDestination().distanceFrom(left.getDestination());
