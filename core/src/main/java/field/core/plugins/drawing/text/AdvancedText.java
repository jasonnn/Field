...
class AdvancedText {
    ...
    public
    void acceptString(String string, iNativeTextLayoutVisitor visitor) {
        ...
        while (!pi.isDone()) {
            ...
            if (s == PathIterator.SEG_MOVETO) visitor.visitPathMoveTo(cc[0], cc[1]);
            else if (s == PathIterator.SEG_LINETO) visitor.visitPathLineTo(cc[0], cc[1]);
...
//				visitor.visitPathCubicCurveTo(cc[2], cc[3], (cc[0] - lx) * (2 / 3f) + lx, (cc[1] - ly) * (2 / 3f) + ly, (cc[0] - cc[2]) * (2 / 3f) + cc[2], (cc[1] - cc[3]) * (2 / 3f) + cc[3]);
                visitor.visitPathCubicCurveTo(cc[2],
                                              cc[3],
                                              ((cc[0] - lx) * (2 / 3f)) + lx,
                                              ((cc[1] - ly) * (2 / 3f)) + ly,
                                              cc[0] + ((cc[2] - cc[0]) / 3f),
    ...
    public
    void setFont(String string, int size) {
        font = new Font(string, 0, size);
    }

    public
    void setFont(String string, int flags, int size) {
