...
public
class OptimizeCachedLineSet {

    public
    OptimizeCachedLineSet() {
    }

    public
    void doOptimization(List<CachedLine> c, String[] excludeKeys) {
        ...
        for (CachedLine cc : c) {
            ...
            if ((target != null) && (hash != -1)) {
                ...
                if (displaced != null) extra.add(displaced);
            }
        }
    ...
    long getHashFor(CachedLine cc, Set<String> ex) {
        if (cc.properties == null) return 0;
    ...
    private
    void concat(CachedLine cc, CachedLine target) {
