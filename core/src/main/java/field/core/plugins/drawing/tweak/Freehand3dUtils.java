...
class Freehand3dUtils {
    ...
    CachedLine projectAtZeroPoint(RawSplineData d) {
        ...
        for (Vector2 v : d.points) {
            ...
            if ((states != null) && states.hasNext()) s = (State) states.next();
    ...
    Vector3 singleProject(Vector2 v, Projector p, State s, Rect frame) {
        ...
        p.createIntersectionRay((float) (v.x - frame.x),
                                (float) ((frame.h - v.y) + frame.y),
                                o1,
                                o2,
                                (int) frame.w,
                                (int) frame.h);
        ...
        LinePointIntersectionInfo intersection =
                IntersectionPrimatives.lineToPlane(o1, new Vector3(o2).sub(o1).normalize(), target, ray);

        return intersection.closestPoint;
    }

    public
    void paintGuidePlaneNow(List<CachedLine> contents, ThreedContext defaultContext) {
    ...
    private
    void scanAndIntersect(CachedLine c, Vector3 center, Vector3 ray, Vector3 position, ThreedContext defaultContext) {
    ...
    private
    void intersect(Vector3 ll,
                   Vector3 dd,
                   Vector3 center,
                   Vector3 ray,
                   Vector3 position,
                   ThreedContext defaultContext) {
        t1.sub(ll, center);
        float q = t1.dot(ray);
        ...
        if (a1 != a2) {
            LinePointIntersectionInfo a = IntersectionPrimatives.lineToPlane(new Vector3(ll),
                                                                             new Vector3(dd).sub(ll).normalize(),
                                                                             center,
                                                                             ray);
