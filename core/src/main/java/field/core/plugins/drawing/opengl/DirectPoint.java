...
class DirectPoint {
    ...
    public
    class LineCache extends OnePassElement {

        private final DynamicPointlist inside;

        public
        LineCache(DynamicPointlist inside) {
...
//	}
    ...
    class LineAdaptor {
        ...
        public
        void clearAndCopy(DynamicPointlist inside) {
...
//				;//System.out.println(" cache :"+cache+" "+cache.getUnderlyingGeometry());
    ...
    public
    SubMesh makeConcrete(CachedLine from, SubMesh prev) {
...
//		cache.beginSpline(null);
        ...
        defaults.put(iLinearGraphicsContext.pointSize_v,
                     from.getProperties().getFloat(iLinearGraphicsContext.pointSize, 1f));
        ...
        trackedProperties.put(iLinearGraphicsContext.strokeColor_v, GLConstants.color0_id);
...
//		cache.makeCopy();
    ...
    private
    void setAux(SubMesh cache, Integer value, Object a) {
    ...
    public
    void wire(DynamicPointlist inside, CachedLine target) {
    ...
    public
    void unwire(DynamicPointlist inside, CachedLine target) {
        LineCache c = lineCache.get(inside);
        if (c == null) {
            return;
        }
