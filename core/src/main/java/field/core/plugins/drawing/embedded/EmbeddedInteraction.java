...
class EmbeddedInteraction {
    ...
    class MousePeer implements iMousePeer {
        ...
        List<Triple<iNodeCallBack, CachedLine, CachedLine.Event>> releaseStack =
                new ArrayList<Triple<iNodeCallBack, CachedLine, CachedLine.Event>>();

        public
        MousePeer(BaseGLGraphicsContext inside) {
            this.context = inside;
        }
    ...
    public
    interface NodeTrigger {
        public
        List<iNodeCallBack> edgeAfter();

        public
        List<iNodeCallBack> edgeBefore();

        public
        List<iNodeCallBack> insideLine();

        public
        List<iNodeCallBack> node();
    }
    ...
    List<Triple<iNodeCallBack, CachedLine, CachedLine.Event>> perform(Set<CachedLine> al,
        ...
        List<Triple<iNodeCallBack, CachedLine, CachedLine.Event>> sent =
                new ArrayList<Triple<iNodeCallBack, CachedLine, CachedLine.Event>>();
                            ...
                            if (n1 != null && n1.size() > 0) {
                                ...
                                if (is.distanceFrom(vector2) < scale) {
                                    for (iNodeCallBack c : n1) {
                                        if (doNotCall.contains(c)) continue;
                                        fire(c, l, b.current, mirrorNoReturnMethod, vector2, arg0);
                                        sent.add(new Triple<iNodeCallBack, CachedLine, CachedLine.Event>(c,
                                                                                                         l,
                                                                                                         b.current));
                                        doNotCall.add(c);
                                    }
    ...
    void fire(iNodeCallBack n1,
        ...
        try {
            ...
            if (promise != null) promise.endExecute();
        }
    }
