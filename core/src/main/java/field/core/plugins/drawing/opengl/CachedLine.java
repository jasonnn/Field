...
class CachedLine {
    ...
    public
    class Event<T extends Number> implements iExtensible {
        ...
        public
        Dict getAttributes() {
            if (attributes == null) attributes = new Dict();
            return attributes;
        }
        ...
        Event<T> copy() {
            ...
            e.method = method;
            if (attributes != null) e.attributes = new Dict().putAll(attributes);
            if (args != null) {
                // 1.6
...
//				e.args = new Object[args.length];
        ...
        public
        Vector2 getAt(int i, @NotNull Vector2 v) {
//			Vector2 v;
//            if (s.length == 0) {
//                v = new Vector2();
//            }
...
//            }
        ...
        public
        Vector2 getDestination(@NotNull Vector2 v) {
            //	Vector2 v;
//            if (s.length == 0) {
//                v = new Vector2();
//            }
...
//            }
        ...
        public
        void setAt(Vector2 last) {
            setAt(-1, last);
        }

        public
        void setAt(int lastIndexToSet, Vector2 last) {
            if (lastIndexToSet == -1) lastIndexToSet = args.length / 2 - 1;
            args[2 * lastIndexToSet] = last.x;
            args[((2 * lastIndexToSet) + 1)] = last.y;
        }

        public
        void setAt3(int lastIndexToSet, Vector3 last) {
            if (lastIndexToSet == -1) lastIndexToSet = args.length / 2 - 1;
            ...
            if (zwas == null) {
                ...
                if (args.length == 6) {
                    if (zwas instanceof Vector3) ((Vector3) zwas).set(lastIndexToSet, last.z);
                    else getAttributes().put(iLinearGraphicsContext.z_v, last.z);
                }
                else {
                    getAttributes().put(iLinearGraphicsContext.z_v, last.z);
                }
        ...
        public
        CachedLine getContainer() {
            return container;
        }

        public
        void setContainer(CachedLine c) {
            container = c;
        }
    ...
    iLine getInput() {
        ...
        return cachedInput = (iLine) Proxy.newProxyInstance(this.getClass().getClassLoader(),
    ...
    protected
    void interpolateAttribute(Prop property,
                              Object propertyValue,
                              HashMap<Prop, Integer> lastSeenAt,
                              int lastMoveToAt,
                              int currentIndex,
                              List<Event> eventStack) throws NotInterpolateable {

        // strings are not interpolated
        if (propertyValue instanceof String) return;

        // fonts are not interpolated
        if (propertyValue instanceof Font) return;
    ...
    void interpolateAttribute(Prop property,
        ...
        try {
            for (int i = backAt; i < currentIndex; i++) {
                Object in = interpolateProperty(property,
                                                backValue,
                                                propertyValue,
                                                (i - backAt) / (float) (currentIndex - backAt));
                if (in != null) {
            ...
            for (int i = currentIndex + 1; i < (forwardAt + 1); i++) {
                Object in = interpolateProperty(property,
                                                propertyValue,
                                                fowardValue,
                                                (i - currentIndex) / (float) (forwardAt - currentIndex));
                if (in != null) {
                    eventStack.get(i).getAttributes().put(property, in);
                }
            }
    ...
    protected
    Object interpolateProperty(Prop property, Object a, Object b, float alpha) throws NotInterpolateable {
        if (a == null) return b;
        if (b == null) return a;

        Object oa = a;

        if (property.getName().endsWith(".noInterpolate")) return null;

        if (!(a instanceof float[])) a = convert(a);
        if (!(b instanceof float[])) b = convert(b);

        if (a == null) throw new NotInterpolateable();
        if (b == null) throw new NotInterpolateable();
    ...
    float[] convert(Object o) {
        if (o instanceof iToFloatArray) return ((iToFloatArray) o).get();
        if (o instanceof Float) return new float[]{((Number) o).floatValue()};
        if (o instanceof Double) return new float[]{((Number) o).floatValue()};
        return null;
    }

    protected static
    Object convertBack(Object o, float[] now) {
        if (o instanceof Vector4) return new Vector4(now[0], now[1], now[2], now[3]);
        if (o instanceof Float) return now[0];
        if (o instanceof Double) return now[0];
        return o;
    }
    ...
    public
    void getClassCustomCompletion(String prefix, Object o) {
