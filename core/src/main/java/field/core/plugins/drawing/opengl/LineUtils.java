...
class LineUtils {
    ...
    class AllDistanceMinima {
        ...
        public static
        double ptSegDistSqT(double x1, double y1, double x2, double y2, double px, double py) {
            x2 -= x1;
            y2 -= y1;
            px -= x1;
            py -= y1;
            ...
            if (Double.isNaN(dotprod)) dotprod = 0;
            if (Double.isInfinite(dotprod)) dotprod = 0;
        ...
        Vector2 a = new Vector2(), c1 = new Vector2(), c2 = new Vector2(), b = new Vector2(), c21 = new Vector2(), c12 =
                                                                                                                           new Vector2(),
                m = new Vector2(), t = new Vector2();
        ...
        public
        AllDistanceMinima(CachedLine line, final Vector2 point) {
            this.on = line;
            brent = new Brent();
            cubicF = new Brent.F() {
                public
                double f(double x) {
                    ...
                    x = 1 - x;
                    double ax =
                            a.x * x * x * x + 3 * c1.x * x * x * (1 - x) + 3 * c2.x * x * (1 - x) * (1 - x) + b.x
                                                                                                              * (1
                                                                                                                 - x)
                                                                                                              * (1 - x)
                                                                                                              * (1 - x);
                    double ay =
                            a.y * x * x * x + 3 * c1.y * x * x * (1 - x) + 3 * c2.y * x * (1 - x) * (1 - x) + b.y
                                                                                                              * (1
                                                                                                                 - x)
                                                                                                              * (1 - x)
                                                                                                              * (1 - x);
                ...
                if (cc.nextIsCubic()) {
                    if (cc.nextCubicFrame(oa, oc1, oc2, ob)) {
...

                        {
                            ...
                            if ((d <= d0) && (d <= d1)) newMinimum(x / 2, d, cc.getCurrentIndex());
                        }
                        b.setValue(ob);
                        a.setValue(m);
                        c2.setValue(oc2);
                        c1.setValue(c21);
                        {
                            ...
                            double d0 = cubicF.f(Math.max(0, x - 1e-2));
                            double d1 = cubicF.f(Math.min(1, x + 1e-2));
                            if ((d <= d0) && (d <= d1)) newMinimum(0.5 + x / 2, d, cc.getCurrentIndex());
                        }
                    }
                }
                else {
        ...
        private
        void newMinimum(double x, double d, int currentIndex) {
    ...
    class ClosestEvent {
        ...
        public
        ClosestEvent(CachedLine target) {
            this.target = target;
        }

        public
        CachedLine.Event closestTo(Vector2 v) {
        ...
        public
        int returnClosestIndex() {
            return closetAt;
        }

        public
        float returnLastDistance() {
            return d;
        }
    }
    ...
    class ClosestPointToSpline {
        ...
        public
        ClosestPointToSpline(CachedLine line, final Vector2 point) {
            this.on = line;
            brent = new Brent();
            cubicF = new Brent.F() {
                public
    ...
    class ClosestPointToSpline3 {
        ...
        Vector3 a = new Vector3(), c1 = new Vector3(), c2 = new Vector3(), b = new Vector3(), c21 = new Vector3(), c12 =
                                                                                                                           new Vector3(),
                m = new Vector3(), t = new Vector3();
        ...
        public
        ClosestPointToSpline3(CachedLine line, final Vector3 point) {
            this.on = line;
            brent = new Brent();
            cubicF = new Brent.F() {
                public
                double f(double x) {
                    ...
                    double ay =
                            a.y * x * x * x + 3 * c1.y * x * x * (1 - x) + 3 * c2.y * x * (1 - x) * (1 - x) + b.y
                                                                                                              ...
                                                                                                              * (1 - x);
                    double az =
                            a.z * x * x * x + 3 * c1.z * x * x * (1 - x) + 3 * c2.z * x * (1 - x) * (1 - x) + b.z
                                                                                                              * (1
                                                                                                                 - x)
                                                                                                              * (1 - x)
                                                                                                              * (1 - x);

                    double d = (point.x - ax) * (point.x - ax)
                               + (point.y - ay) * (point.y - ay)
                               + (point.z - az) * (point.z - az);
                    return d;
                }
            };
    ...
    Vector2 evaluateDCubicFrame(Vector2 a, Vector2 c1, Vector2 c2, Vector2 b, float alpha, Vector2 out) {
        ...
        out.x = -3 * c1.x * alpha * (2 - 2 * alpha) + 6 * c2.x * alpha * (1 - alpha)
                - 3 * a.x * (1 - alpha) * (1
                                           - alpha)
                - 3 * c2.x * alpha * alpha + 3 * b.x * alpha * alpha + 3 * c1.x * (1 - alpha) * (1 - alpha);
        out.y = -3 * c1.y * alpha * (2 - 2 * alpha) + 6 * c2.y * alpha * (1 - alpha)
                - 3 * a.y * (1 - alpha) * (1
                                           - alpha)
                - 3 * c2.y * alpha * alpha + 3 * b.y * alpha * alpha + 3 * c1.y * (1 - alpha) * (1 - alpha);
    ...
    Vector3 evaluateDCubicFrame3(Vector3 a, Vector3 c1, Vector3 c2, Vector3 b, float alpha, Vector3 out) {
        ...
        out.y = -3 * c1.y * alpha * (2 - 2 * alpha) + 6 * c2.y * alpha * (1 - alpha)
                - 3 * a.y * (1 - alpha) * (1
                                           - alpha)
                - 3 * c2.y * alpha * alpha + 3 * b.y * alpha * alpha + 3 * c1.y * (1 - alpha) * (1 - alpha);
        out.z = -3 * c1.z * alpha * (2 - 2 * alpha) + 6 * c2.z * alpha * (1 - alpha)
                - 3 * a.z * (1 - alpha) * (1
                                           - alpha)
                - 3 * c2.z * alpha * alpha + 3 * b.z * alpha * alpha + 3 * c1.z * (1 - alpha) * (1 - alpha);
    ...
    Vector2 evaluateD2CubicFrame(Vector2 a, Vector2 c1, Vector2 c2, Vector2 b, float alpha, Vector2 out) {
        ...
        out.x = -12 * c2.x * alpha - 6 * c1.x * (2 - 2 * alpha)
                + 3 * a.x * (2 - 2 * alpha)
                + 6 * b.x * alpha
                + 6 * c1.x * alpha
                + 6 * c2.x * (1 - alpha);
        out.y = -12 * c2.y * alpha - 6 * c1.y * (2 - 2 * alpha)
                + 3 * a.y * (2 - 2 * alpha)
                + 6 * b.y * alpha
                + 6 * c1.y * alpha
                + 6 * c2.y * (1 - alpha);
    ...
    Vector3 evaluateD2CubicFrame3(Vector3 a, Vector3 c1, Vector3 c2, Vector3 b, float alpha, Vector3 out) {
        ...
        out.y = -12 * c2.y * alpha - 6 * c1.y * (2 - 2 * alpha)
                ...
                + 6 * c1.y * alpha
                + 6 * c2.y * (1 - alpha);
        out.z = -12 * c2.z * alpha - 6 * c1.z * (2 - 2 * alpha)
                + 3 * a.z * (2 - 2 * alpha)
                + 6 * b.z * alpha
                + 6 * c1.z * alpha
                + 6 * c2.z * (1 - alpha);
    ...
    public static
    Vector2[] fastBounds(CachedLine c) {
        Vector2[] minMax = {new Vector2(Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY),
                            new Vector2(Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY)};

        boolean touched = false;
        for (int i = 0; i < c.events.size(); i++) {

            if (c.events.get(i).args != null) for (int q = 0; q < c.events.get(i).args.length / 2; q++) {
                float tx = ((Number) c.events.get(i).args[q * 2 + 0]).floatValue();
                float ty = ((Number) c.events.get(i).args[q * 2 + 1]).floatValue();

                if (tx < minMax[0].x) minMax[0].x = tx;
                if (ty < minMax[0].y) minMax[0].y = ty;
                if (tx > minMax[1].x) minMax[1].x = tx;
                if (ty > minMax[1].y) minMax[1].y = ty;
                touched = true;
            }
    ...
    public static
    Vector3[] fastBounds3(CachedLine c) {
        Vector3[] minMax = {new Vector3(Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY),
        ...
        for (int i = 0; i < c.events.size(); i++) {
            ...
            if (c.events.get(i).args != null) {
                ...
                if (tz == null) {
                    ...
                    tz = new Vector3(((Number) tz).floatValue(),
                                     ((Number) tz).floatValue(),
                                     ((Number) tz).floatValue());
                }
                for (int q = 0; q < c.events.get(i).args.length / 2; q++) {
                    ...
                    if (tx < minMax[0].x) minMax[0].x = tx;
                    if (ty < minMax[0].y) minMax[0].y = ty;
                    if (ttz < minMax[0].z) minMax[0].z = ttz;
                    if (tx > minMax[1].x) minMax[1].x = tx;
                    if (ty > minMax[1].y) minMax[1].y = ty;
                    if (ttz > minMax[1].z) minMax[1].z = ttz;
                    touched = true;
                }
            }
        }
    ...
    public static
    Vector2[] fastBoundsAndMidpoints(CachedLine c) {
        Vector2[] minMax = {new Vector2(Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY),
                            new Vector2(Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY)};
    ...
    public static
    Vector2[] fastBoundsMoveTos(CachedLine c) {
        Vector2[] minMax = {new Vector2(Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY),
                            new Vector2(Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY)};
        if (c == null) {
        ...
        while (cursor.hasNextSegment()) {
            ...
            if (cursor.getCurrent().method.equals(iLine_m.moveTo_m)) {
                ...
                if (z) {
                    if (t.x < minMax[0].x) minMax[0].x = t.x;
                    if (t.y < minMax[0].y) minMax[0].y = t.y;
                    if (t.x > minMax[1].x) minMax[1].x = t.x;
                    if (t.y > minMax[1].y) minMax[1].y = t.y;
                    touched = true;
                }
            }
        }
    ...
    static public
    Vector2 getPointOnLine(CachedLine of, float dot) {

        if (of.events.size() == 0) return null;
    ...
    public static
    boolean hitTest(CachedLine l, Vector2 point, float distance) {
    ...
    static public
    CachedLineCursor hitTest2(CachedLine l, Vector2 point, float distance) {
        CachedLineCursor cursor = new CachedLineCursor(l);
        Vector2 t = new Vector2();
        boolean touched = false;
        Vector2 a = new Vector2();
    ...
    public static
    void markWithSelf(CachedLine line, Prop<Event> attribute) {
    ...
    public static
    Pair<FitBox, Axis[]> orientedBounds(List<CachedLine> cc) {
    ...
    static public
    void splitCubicFrame(Vector2 a,
                         Vector2 c1,
                         Vector2 c2,
                         Vector2 b,
                         float alpha,
    ...
    static public
    void splitCubicFrame3(Vector3 a,
                          Vector3 c1,
                          Vector3 c2,
                          Vector3 b,
                          float alpha,
    ...
    public static
    CachedLine transformLine(CachedLine input, Vector2 before, Vector2 scale, Quaternion rotate, Vector2 after) {
        CachedLine r = new CachedLine();

        for (CachedLine.Event<?> e : input.events) {
            Event ll = e.copy();
            ll.container = r;
            if (ll.args != null) for (int i = 0; i < ll.args.length / 2; i++) {
                Vector2 v2 =
                        new Vector2(((Number) e.args[2 * i]).floatValue(), ((Number) e.args[2 * i + 1]).floatValue());
                if (before != null) {
                    v2.add(before);
                }
                if (scale != null) {
    ...
    public static
    CachedLine transformLine3(CachedLine input, Vector3 before, Vector3 scale, Quaternion rotate, Vector3 after) {
        CachedLine r = new CachedLine();
        float lastZ = 0;
        ...
        for (CachedLine.Event<?> e : input.events) {
            ...
            if (e.args == null || e.args.length == 0) {
                ...
                if (z == null) z = 0.0f;
                if (z instanceof Vector3) z = ((Vector3) z).x;

                Vector3 a = new Vector3(((Number) e.args[0]).floatValue(),
                                        ((Number) e.args[1]).floatValue(),
                                        ((Number) z).floatValue());
                if (before != null) a.add(before);
                if (scale != null) a.scale(scale);
                if (rotate != null) rotate.rotateVector(a);
                if (after != null) a.add(after);
                ...
                }
                {
                    Object z = e.getAttributes().get(iLinearGraphicsContext.z_v);
                    if (z == null) z = 0.0f;
                    if (z instanceof Vector3) z = ((Vector3) z).y;

                    Vector3 a = new Vector3(((Number) e.args[2]).floatValue(),
                                            ((Number) e.args[3]).floatValue(),
                                            ((Number) z).floatValue());
                    if (before != null) a.add(before);
                    if (scale != null) a.scale(scale);
                    if (rotate != null) rotate.rotateVector(a);
                ...
                }
                {
                    Object z = e.getAttributes().get(iLinearGraphicsContext.z_v);
                    if (z == null) z = 0.0f;
                    if (z instanceof Vector3) z = ((Vector3) z).z;

                    Vector3 a = new Vector3(((Number) e.args[4]).floatValue(),
                                            ((Number) e.args[5]).floatValue(),
                                            ((Number) z).floatValue());
                    if (before != null) a.add(before);
                    if (scale != null) a.scale(scale);
                    if (rotate != null) rotate.rotateVector(a);
    ...
    public static
    CachedLine transformLine3(CachedLine input, CoordinateFrame frame) {
        CachedLine r = new CachedLine();
        float lastZ = 0;
    ...
    public
    CachedLine fixProperties(CachedLine newLine, CachedLine oldLine) {
    ...
    public
    boolean isIntersecting(CachedLine line, Rect bounds) {
    ...
    public
    float amountIntersecting(CachedLine line, Rect bounds) {
        GeneralPath g = this.lineToGeneralPath(line);
    ...
    public
    CachedLine lineAsStroked(CachedLine line, BasicStroke stroke, boolean fixProperties) {
    ...
    public
    CachedLine lineAsStroked(CachedLine line, float width) {
    ...
    public
    GeneralPath lineToGeneralPath(CachedLine line) {
        return lineToGeneralPath(line, false);
    }

    public
    GeneralPath lineToGeneralPath(CachedLine transformedLine, boolean linesOnly) {
        ...
        for (CachedLine.Event e : transformedLine.events) {
            ...
            else if (e.method.equals(iLine_m.close_m)) path.closePath();
            else if (e.method.equals(iLine_m.cubicTo_m)) {
                if (!linesOnly) path.curveTo(((Number) e.args[0]).floatValue(),
                                             ((Number) e.args[1]).floatValue(),
                                             ((Number) e.args[2]).floatValue(),
                                             ((Number) e.args[3]).floatValue(),
                                             ((Number) e.args[4]).floatValue(),
                                             ((Number) e.args[5]).floatValue());
                else path.lineTo(((Number) e.args[4]).floatValue(), ((Number) e.args[5]).floatValue());
            }
        }
    ...
    CachedLine piToCachedLine(PathIterator pi) {
        ...
        while (!pi.isDone()) {
            ...
            if (ty == PathIterator.SEG_CLOSE) {
                ...
                if (lastAt == null) lastAt = new Vector2(cc[4], cc[5]);
                else {
                    lastAt.x = cc[4];
                    lastAt.y = cc[5];
                }
                ...
                if (lastAt == null || (Math.abs(lastAt.x - cc[0]) + Math.abs(lastAt.y - cc[1]) > 1e-15))
                    in.lineTo(cc[0], cc[1]);
                if (lastAt == null) lastAt = new Vector2(cc[0], cc[1]);
                else {
                    lastAt.x = cc[0];
                    lastAt.y = cc[1];
                }
                ...
                if ((Math.abs(lastAt.x - cc[2]) + Math.abs(lastAt.y - cc[3]) > 1e-15)) {
                    in.cubicTo((cc[0] - lastAt.x) * (2 / 3f) + lastAt.x,
                               (cc[1] - lastAt.y) * (2 / 3f) + lastAt.y,
                               (cc[0] - cc[2]) * (2 / 3f) + cc[2],
                               (cc[1] - cc[3]) * (2 / 3f) + cc[3],
                               cc[2],
                               cc[3]);
        ...
        for (int i = 0; i < ret.events.size() - 1; i++) {
            if (ret.events.get(i).method.equals(iLine_m.moveTo_m) && ret.events.get(i
                                                                                    + 1).method.equals(iLine_m.close_m)) {
                ret.events.remove(i);
                ret.events.remove(i);
                i--;
            }
        ...
        if (ret.events.size() == 0) return null;
    ...
    void piToCachedLine(PathIterator pi, CachedLine contInto, boolean firstIsMoveto, boolean fixClose) {
        ...
        while (!pi.isDone()) {
            ...
            if (ty == PathIterator.SEG_CLOSE) {
                ...
                if (lastAt == null) lastAt = new Vector2(cc[2], cc[3]);
                else {
                    lastAt.x = cc[2];
                    lastAt.y = cc[3];
                }
    ...
    public
    CachedLine reverse(CachedLine c) {
    ...
    public
    CachedLine simpleSubdivideAll(CachedLine c) {
        CachedLine out = new CachedLine();
    ...
    public
    CachedLine simpleSubdivideAllAsCurves(CachedLine c) {
        CachedLine out = new CachedLine();
    ...
    public
    CachedLine simpleSubdivideAllAsCurves3(CachedLine c) {
        CachedLine out = new CachedLine();
    ...
    public
    CachedLine simpleSubdivideAllCurves(CachedLine c) {
        CachedLine out = new CachedLine();
    ...
    static public
    Vector3 getDestination3(Event event) {
        Vector2 d = event.getDestination();
        Object o = event.getAttributes().get(iLinearGraphicsContext.z_v);
        if (o == null) o = 0f;
        if (o instanceof Number) return new Vector3(d.x, d.y, ((Number) o).floatValue());
        else return new Vector3(d.x, d.y, ((Vector3) o).z);
    }

    static public
    Vector3 getControl1(Event event) {
        Vector2 d = event.getAt(0);
        Object o = event.getAttributes().get(iLinearGraphicsContext.z_v);
        if (o == null) o = 0f;
        if (o instanceof Number) return new Vector3(d.x, d.y, ((Number) o).floatValue());
        else return new Vector3(d.x, d.y, ((Vector3) o).x);
    }

    static public
    Vector3 getControl2(Event event) {
        Vector2 d = event.getAt(1);
        Object o = event.getAttributes().get(iLinearGraphicsContext.z_v);
        if (o == null) o = 0f;
        if (o instanceof Number) return new Vector3(d.x, d.y, ((Number) o).floatValue());
        else return new Vector3(d.x, d.y, ((Vector3) o).y);
    }

    static public
    Vector3 getDestination3(Event event, float def_z) {
        Vector2 d = event.getDestination();
        Object o = event.getAttributes().get(iLinearGraphicsContext.z_v);
        if (o == null) o = def_z;
        if (o instanceof Number) return new Vector3(d.x, d.y, ((Number) o).floatValue());
    ...
    static public
    Vector3 getControl1(Event event, float def_z) {
        Vector2 d = event.getAt(0);
        Object o = event.getAttributes().get(iLinearGraphicsContext.z_v);
        if (o == null) o = def_z;
        if (o instanceof Number) return new Vector3(d.x, d.y, ((Number) o).floatValue());
    ...
    static public
    Vector3 getControl2(Event event, float def_z) {
        Vector2 d = event.getAt(1);
        Object o = event.getAttributes().get(iLinearGraphicsContext.z_v);
        if (o == null) o = def_z;
        if (o instanceof Number) return new Vector3(d.x, d.y, ((Number) o).floatValue());
    ...
    static public
    boolean circleTo(CachedLine into, Vector2 c1, Vector2 dest, float limit, float over) {
        ...
        if (d > limit || d == 0 || Float.isInfinite(d) || Float.isNaN(d)) {
            ...
            LinePointIntersectionInfo in = IntersectionPrimatives.lineToLine(at.toVector3(),
                                                                             newCenter1.toVector3(),
                                                                             dest.toVector3(),
                                                                             newCenter2.toVector3());

            Vector2 newCenter = in.closestPoint.toVector2();

            arc.setArcByTangent(new Point2D.Float(at.x, at.y),
                                new Point2D.Float(newCenter.x, newCenter.y),
                                new Point2D.Float(dest.x, dest.y),
                                d);
    ...
    static public
    CachedLine noise(CachedLine line, final float level) {
    ...
    static public
    CachedLine noClose(CachedLine line) {
        Iterator<Event> ii = line.events.iterator();
        while (ii.hasNext()) if (!ii.next().hasDestination()) ii.remove();
        return line;
    }

    static public
    CachedLine fixClose(CachedLine line) {
        Iterator<Event> ii = line.events.iterator();
    ...
    public static
    CachedLine transformLineOffsetFrom(CachedLine input, Rect frame, Vector2 o) {
        CachedLine r = new CachedLine();
    ...
    public
    CachedLine transformLine(CachedLine input, AffineTransform t) {
    ...
    public
    GeneralPath lineToGeneralPathProjected(CachedLine transformedLine, Projector projector, float width, float height) {
    ...
    Pair<Vector2, Vector2> controlPointsFor(Vector2 a, Vector2 b, Vector2 c, Vector2 d, Number t1, Number t2) {
        t1 = t1 == null
             ? b.distanceFrom(a) / (b.distanceFrom(a) + c.distanceFrom(b) + d.distanceFrom(c))
             : t1.floatValue();
        t2 = t2 == null
             ? 1 - (c.distanceFrom(d) / (b.distanceFrom(a) + c.distanceFrom(b) + d.distanceFrom(c)))
             : t2.floatValue();
