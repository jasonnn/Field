...
class DirectMesh {
    ...
    public
    void wire(DynamicMesh_long inside, CachedLine target) {
        MeshCache c = lineCache.get(inside);
        if (c == null) {
            lineCache.put(inside, c = new MeshCache(inside));
            inside.getUnderlyingGeometry().addChild(c);
        ...
        if (!skip) c.adaptors.add(new LineAdaptor(target));
    }

    public
    void unwire(DynamicMesh_long inside, CachedLine target) {
        MeshCache c = lineCache.get(inside);
        if (c == null) {
            return;
        }
    ...
    class LineAdaptor {
        ...
        public
        LineAdaptor(CachedLine from) {
            this.from = from;
        }

        public
        void clearAndCopy(DynamicMesh_long inside) {
...
//				;//System.out.println(" cache miss");
    ...
    public
    SubMesh_long makeConcrete(CachedLine from, SubMesh_long prev) {
        ...
        if (defaultColor == null) defaultColor = from.getProperties().get(iLinearGraphicsContext.color);
        if (defaultColor == null) defaultColor = new Vector4(1, 1, 1, 1);
        ...
        if (from.events.size() < 6) {
            if (from.events.size() >= 4) {
                if (from.events.get(0).method.equals(iLine_m.moveTo_m)
                    && from.events.get(1).method.equals(iLine_m.lineTo_m)
                    && from.events.get(2).method.equals(iLine_m.lineTo_m)
                    && from.events.get(3).method.equals(iLine_m.lineTo_m)
                    && (from.events.size() == 4
                        || from.events.get(4).method.equals(iLine_m.close_m)
                        || (from.events.get(4).method.equals(iLine_m.lineTo_m)
                            && from.events.get(4).getDestination3().distanceFrom(from.events.get(0).getDestination3())
                               < 1e-4))) {
                    ...
                    for (int i = 0; i < propertiesInOrder.size(); i++) {
                        if (from.events.get(0).attributes != null) {
                            ...
                            if (d == null) d = defaultsInOrder.get(i);
                    ...
                    for (int i = 0; i < propertiesInOrder.size(); i++) {
                        if (from.events.get(2).attributes != null) {
                            ...
                            if (dd == null) dd = defaultsInOrder.get(i);
                ...
                if (from.events.get(0).method.equals(iLine_m.moveTo_m)
                    ...
                    && from.events.get(2).method.equals(iLine_m.lineTo_m)
                    && (from.events.size() == 3
                        || from.events.get(3).method.equals(iLine_m.close_m)
                        || (from.events.get(3).method.equals(iLine_m.lineTo_m)
                            && from.events.get(3).getDestination3().distanceFrom(from.events.get(0).getDestination3())
                               < 1e-4))) {
        ...
        else doSlowTess(from, cache, propertiesInOrder, channelsInOrder, defaultsInOrder);

        return cache;
    }

    private
    void doStarTess(CachedLine from,
                    final SubMesh_long cache,
                    List<Prop> propertiesInOrder,
                    List<Integer> channelsInOrder,
                    List<Object> defaultsInOrder) {
...
//				tess.line(at, z, lastProperties, thisProperties);
                ...
                for (int i = 0; i < fixedCurveSampling; i++) {
                    ...
                    for (int j = 0; j < propertiesInOrder.size(); j++) {
                        xProperties.put(channelsInOrder.get(j),
                                        blend(lastProperties.get(j),
                                              thisProperties.get(j),
                                              (i + 1f) / fixedCurveSampling));
                    }
...
//					tess.line(at, o, lastProperties, xProperties);
    ...
    protected
    void doSlowTess(CachedLine from,
                    final SubMesh_long cache,
                    List<Prop> propertiesInOrder,
                    final List<Integer> channelsInOrder,
                    List<Object> defaultsInOrder) {
        Vector3 lastClose = new Vector3(0, 0, 0);
        Vector3 at = new Vector3(0, 0, 0);
        ...
        SimpleTess tess = new SimpleTess() {
            ...
            @Override
            protected
            void decorateVertex(int vertex, List<Object> properties) {
        ...
        for (Event e : from.events) {
            if (e.method.equals(iLine_m.moveTo_m)) {
                ...
                for (int i = 0; i < fixedCurveSampling; i++) {
                    ...
                    for (int j = 0; j < propertiesInOrder.size(); j++) {
                        xProperties.add(blend(lastProperties.get(j),
                                              thisProperties.get(j),
                                              (i + 1f) / fixedCurveSampling));
                    }
    ...
    Object blend(Object a, Object b, float f) {
        ...
        if (a instanceof Number) return ((Number) a).floatValue() * (1 - f) + f * ((Number) b).floatValue();
        if (a instanceof Vector2) return new Vector2().lerp((Vector2) a, (Vector2) b, f);
        if (a instanceof Vector3) return new Vector3().lerp((Vector3) a, (Vector3) b, f);
        if (a instanceof Vector4) return new Vector4().lerp((Vector4) a, (Vector4) b, f);

        return null;
    }

    private
    void setAux(SubMesh_long cache, Integer value, Object a) {

        // ;//System.out.println(" set aux :" + value + " " + a);

        if (a == null) return;

        if (a instanceof Number) cache.setAux(cache.getVertexCursor() - 1, value, ((Number) a).floatValue());
            ...
            cache.setAux(cache.getVertexCursor() - 1, value, ((Vector3) a).x, ((Vector3) a).y, ((Vector3) a).z);
                                                                                 ...
                                                                                 ((Vector4) a).y,
                                                                                 ((Vector4) a).z,
                                                                                 ((Vector4) a).w * currentTotalOpacity);
        else cache.setAux(cache.getVertexCursor() - 1,
                          value,
                          ((Vector4) a).x,
                          ((Vector4) a).y,
                          ((Vector4) a).z,
                          ((Vector4) a).w);
        else System.err.println(" can't setAux for <" + a + "> <" + a.getClass() + ">");
    }
}
