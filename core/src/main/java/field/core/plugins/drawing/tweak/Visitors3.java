...
class Visitors3 {
    ...
    class BaseFilter3 {
        ...
        public
        CachedLine visitPositions(CachedLine in, PositionVisitor3 v) {
        ...
        public
        CachedLine visitNodes(CachedLine in, NodeVisitor3 v) {
            out = LineUtils.transformLine(in, null, null, null, null);
            CachedLineCursor c = new CachedLineCursor(out);
            currentOutput = c;
            currentInput = new CachedLineCursor(in);
            ...
            while (c.hasNextSegment()) {
                ...
                if (current != null) currentEventToSet.setAt3(currentIndexToSet, current);
                if (next != null && nextIsTangent) nextEventToSet.setAt3(nextIndexToSet, next);
    ...
    public
    interface PositionVisitor3 {
        public
        void visitPosition(Vector3 v, SubSelection part, BaseFilter3 inside);
    }

    public
    interface NodeVisitor3 {
        public
        void visitNode(Vector3 before, Vector3 now, Vector3 after, boolean beforeIsTangent, boolean afterIsTangent);
    }
