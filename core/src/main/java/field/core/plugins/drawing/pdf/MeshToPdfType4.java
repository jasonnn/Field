...
class MeshToPdfType4 {
    ...
    public
    MeshToPdfType4(TriangleMesh mesh) {
        this.mesh = mesh;
    ...
    public
    byte[] serializeFloatStream(List<EdgeRecord> r, float xmin, float xmax, float ymin, float ymax) {
    ...
    private
    void chase(HashSet<EdgeRecord> all,
               EdgeRecord start,
        ...
        if (code == 0) {
            ...
            if (start.nextSharedEdge != null) {
                ...
                if ((start.nextSharedEdge.start.vertexNumber == va || start.nextSharedEdge.start.vertexNumber == vb)
                    && (start.nextSharedEdge.end.vertexNumber == va || start.nextSharedEdge.end.vertexNumber == vb)) {
                    // we guessed wrong
                    int o = vc;
                    vc = vb;
                    vb = o;
                ...
                if ((start.nextSharedEdge.start.vertexNumber == va || start.nextSharedEdge.start.vertexNumber == vc)
                    && (start.nextSharedEdge.end.vertexNumber == va || start.nextSharedEdge.end.vertexNumber == vc)) {
                    chase(all, start.next, bos, xmin, xmax, ymin, ymax, 2, va, vb, vc);
                }
                else {
                    assert ((start.nextSharedEdge.start.vertexNumber == vb
                             || start.nextSharedEdge.start.vertexNumber == vc) && (start.nextSharedEdge.end.vertexNumber
                                                                                   == vb
                                                                                   || start.nextSharedEdge.end.vertexNumber
                                                                                      == vc));
...
//			}
            ...
            if (all.contains(start.next)) {
                if (start.nextSharedEdge != null) {
                    if ((start.nextSharedEdge.start.vertexNumber == vb2
                         || start.nextSharedEdge.start.vertexNumber == newVertex)
                        && (start.nextSharedEdge.end.vertexNumber == vb2
                            || start.nextSharedEdge.end.vertexNumber == newVertex)) {
                        chase(all, start.next, bos, xmin, xmax, ymin, ymax, 2, newVertex, vb, vc);
                    }
                    else {
                        assert ((start.nextSharedEdge.start.vertexNumber == vc2
                                 || start.nextSharedEdge.start.vertexNumber == newVertex)
                                && (start.nextSharedEdge.end.vertexNumber == vc2
                                    || start.nextSharedEdge.end.vertexNumber == newVertex));
            ...
            if (all.contains(start.next)) {
                if (start.nextSharedEdge != null) {
                    if ((start.nextSharedEdge.start.vertexNumber == vc2
                         || start.nextSharedEdge.start.vertexNumber == newVertex)
                        && (start.nextSharedEdge.end.vertexNumber == vc2
                            || start.nextSharedEdge.end.vertexNumber == newVertex)) {
                        ...
                        assert ((start.nextSharedEdge.start.vertexNumber == va2
                                 || start.nextSharedEdge.start.vertexNumber == newVertex)
                                && (start.nextSharedEdge.end.vertexNumber == va2
                                    || start.nextSharedEdge.end.vertexNumber == newVertex)) : start.nextSharedEdge
                                                                                              + " "
                                                                                              + va2
                                                                                              + ' '
                                                                                              + newVertex
    ...
    static public
    class PdfShading_type4 extends PdfShading {
        public
        PdfShading_type4(PdfWriter writer, TriangleMesh mesh) {
            super(writer);
