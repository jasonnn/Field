...
import field.core.plugins.drawing.tweak.NodeModifiers.*;
...
class TweakSplineUI {
    ...
    class Selectable {
        ...
        public
        Selectable(SelectedVertex v, SubSelection... s) {
            current = v;
    ...
    class SelectedVertex {
        ...
        public
        SelectedVertex(CachedLine l, CachedLineCursor c) {
        ...
        public
        SelectedVertex(CachedLine line, int nodeNumber) {
            onLine = line;
        ...
        void freeze() {
            ...
            if (vertex.getDict().isTrue(iLinearGraphicsContext.noTweak_v, false)) return;
        ...
        public
        float getAmountSelectedFor(SubSelection s) {
            if (amountSelected == null) return whatSelected.contains(s) ? 1 : 0;
            ...
            if (amount == 0) whatSelected.remove(s);
            return amount;
        }

        public
        void ongoingPositionFor(SubSelection s, Vector2 at) {
            if (absPosition.containsKey(s)) at.setValue(absPosition.get(s));
            else if (frozenPosition.containsKey(s)) at.setValue(frozenPosition.get(s));
            else positionFor(s, at);

        }

        public
        Vector2 positionFor(SubSelection s, Vector2 at) {
            ...
            if (context instanceof iTransformingContext) {
                if (at == null) at = new Vector2();
                if (s == SubSelection.postion)
                    ((iTransformingContext) context).convertIntermediateSpaceToDrawingSpace(((iTransformingContext) context)
                                                                                                    .getIntermediateSpaceForEvent(onLine,
                                                                                                                                  vertex,
                                                                                                                                  -1),
                                                                                            at);
        ...
        public
        void setAmountSelectedFor(SubSelection s, float a) {
        ...
        public
        void setPositionFor(CachedLine onLine, SubSelection s, Vector2 at) {
            if (writeToThawInstead) {
                ...
                if (context instanceof iTransformingContext) {
                    if (s == SubSelection.postion) {
                        Object inter =
                                ((iTransformingContext) context).getIntermediateSpaceForEvent(onLine, vertex, -1);
                        Vector2 current = new Vector2();
                        ((iTransformingContext) context).convertIntermediateSpaceToDrawingSpace(inter, current);
                        ((iTransformingContext) context).setIntermediateSpaceForEvent(onLine,
                                                                                      vertex,
                                                                                      -1,
                                                                                      inter,
                                                                                      current,
                        ...
                        Object inter = ((iTransformingContext) context).getIntermediateSpaceForEvent(onLine,
                                                                                                     onLine.events.get(vertexIndex
                                                                                                                       + 1),
                                                                                                     0);
                        Vector2 current = new Vector2();
                        ((iTransformingContext) context).convertIntermediateSpaceToDrawingSpace(inter, current);
                    ...
                    if (s == SubSelection.postion) {
                        onLine.events.get(vertexIndex).args[vertex.args.length - 2] = at.x;
                        onLine.events.get(vertexIndex).args[vertex.args.length - 1] = at.y;
                    }
                    else if (s == SubSelection.previousControl) {
                        onLine.events.get(vertexIndex).args[2] = at.x;
                        onLine.events.get(vertexIndex).args[3] = at.y;
                    }
                    else if (s == SubSelection.nextControl) {
                        onLine.events.get(vertexIndex + 1).args[0] = at.x;
                        onLine.events.get(vertexIndex + 1).args[1] = at.y;
                    }
                }
            }
        }
        ...
        public
        void setThawPositionFor(SubSelection s, Vector2 at) {
            absPosition.put(s, at);
        }

        public
        void thawInto(CachedLine onLine) {
            for (SubSelection s : whatSelected) {
    ...
    public
    class SelectionMousePeer implements iMousePeer {
        ...
        public
        void keyPressed(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
        ...
        public
        void keyReleased(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
        }

        public
        void keyTyped(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
            ...
            if (arg0.character == '7') {
                ...
                if ((arg0.stateMask & SWT.SHIFT) == 0) selection.clear();
                ...
                for (SelectedVertex v : selection) {
                    if (v.onLine.events.size() > (v.vertexIndex + 1) && v.onLine.events.get(v.vertexIndex
                                                                                            + 1).method.equals(iLine_m.cubicTo_m))
                        v.whatSelected.add(SubSelection.nextControl);
                    v.whatSelected.add(SubSelection.postion);
                    if (v.vertex.method.equals(iLine_m.cubicTo_m)) v.whatSelected.add(SubSelection.previousControl);
                }
            }
                ...
                for (SelectedVertex v : selection) {
                    ...
                    if (p == null) touched.put(v.onLine, new Pair<Integer, Integer>(v.vertexIndex, v.vertexIndex));
                    else {
                        if (v.vertexIndex < p.left) p.left = v.vertexIndex;
                        if (v.vertexIndex > p.right) p.right = v.vertexIndex;
                    }
                }
        ...
        public
        void mouseClicked(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
        ...
        public
        void mouseDragged(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
            ...
            if (isMarquee) {
                ...
                arg0.doit = false;
                if (!getCurrentSelectionTool().mouseDrag(arg0)) setCurrentSelectionTool(null);
            }
            else {

                if (selection.size() > 0) arg0.doit = false;
        ...
        public
        void mouseEntered(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
        }

        public
        void mouseExited(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
        }

        public
        void mouseMoved(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
        }

        public
        void mousePressed(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {
...
//			System.err.println(" hittesting <" + arg0.x + " " + arg0.y + "> against <" + selectedableVertex.size() + ">");
            ...
            if (hit != null) {
                ...
                if (!Platform.isPopupTrigger(arg0)) {
                    if ((arg0.start & SWT.SHIFT) != 0) {
                        if (hit.alreadyAdded && hit.current.whatSelected.containsAll(hit.add)) removeFromSelection(hit);
                ...
                if (Platform.isPopupTrigger(arg0) && (arg0.stateMask & SWT.SHIFT) != 0) {
                    ...
                    BetterPopup menu = new SmallMenu().createMenu(upup,
                                                                  GLComponentWindow.getCurrentWindow(inside)
                                                                                   .getCanvas()
                                                                                   .getShell(),
                                                                  null);
        ...
        public
        void mouseReleased(ComponentContainer inside, org.eclipse.swt.widgets.Event arg0) {

            if (arg0.button == SWT.BUTTON3) return;
            ...
            if (isMarquee) {
                ...
                arg0.doit = false;
                if (!getCurrentSelectionTool().mouseUp(arg0)) setCurrentSelectionTool(null);
            }
            else {
                ...
                if (selection != null && selection.size() > 0 && !firstDrag) arg0.doit = false;
    ...
    public
    ArrayList<SelectedVertex> deselectAllNow() {
    ...
    public
    ArrayList<SelectedVertex> reselectNow(ArrayList<SelectedVertex> rr) {
        ArrayList<SelectedVertex> old = new ArrayList<SelectedVertex>(selection);
        ...
        IAcceptor<String> set(String to) {
            ...
            new IVisualElementOverrides.MakeDispatchProxy().getOverrideProxyFor(inside)
                                                           .getProperty(inside,
                                                                        ...
                                                                        pss);
            if (pss.get() != null) pss.get().promiseForKey(inside).beginExecute();
            ...
            if (pss.get() != null) pss.get().promiseForKey(inside).endExecute();
            return this;
        }
    ...
    static HashMap<String, iSimpleSelectionTool> knownSelectionTools =
            new LinkedHashMap<String, iSimpleSelectionTool>();
     ...
     */
    public
    interface iSimpleSelectionTool {
        public
        ...
        public
        boolean mouseDown(org.eclipse.swt.widgets.Event at);

        public
        boolean mouseDrag(org.eclipse.swt.widgets.Event at);

        public
        boolean mouseUp(org.eclipse.swt.widgets.Event at);
    ...
    public
    TweakSplineUI(List<CachedLine> on) {
        this.on = new ArrayList<CachedLine>(on);
    }

    public
    void addToSelection(Selectable hit) {
    ...
    public
    TweakSplineUI copyFrom(TweakSplineUI tweaking) {
        if (tweaking != null) tweaking.copyTo(this);
        return this;
    }

    public
    void copyTo(TweakSplineUI into) {
        if (into.on.size() == on.size()) {
            for (SelectedVertex s : selection) {
                if (s.onLine != null) {
                    ...
                    if (i != -1)
                        if (into.on.get(i) != null) if (on.get(i).events.size() == into.on.get(i).events.size()) {
    ...
    public
    List<CachedLine> getOutput() {
        if (!isReplacing) return on;
        ...
        for (CachedLine original : on) {
            if (!touched.contains(original)) o.add(original);
            else {
    ...
    SplineComputingOverride getSplineComputingOverride() {
        ...
        if (over instanceof iMixinProxy) {
            ...
            for (Object o : list)
                if (o instanceof SplineComputingOverride) return (SplineComputingOverride) o;
        }
        return null;
    }
    ...
    public
    void nudgeSelection(int dx, int dy, boolean shift) {
    ...
    public
    void offsetSelection(float dx, float dy, float odx, float ody, float ox, float oy, int modifiers) {
        ...
        if (currentTool instanceof AbsoluteTool) {
            for (SelectedVertex v : selection) {
                ...
                if (v.vertex.getDict().isTrue(iLinearGraphicsContext.noTweak_v, false)) continue;

                for (SubSelection s : v.whatSelected) {
                    Vector2 a = v.frozenPosition.get(s);

                    v.setThawPositionFor(s,
                                         new Vector2(a.x + currentInfo.lastOdx * v.getAmountSelectedFor(s),
                                                     a.y + currentInfo.lastOdy * v.getAmountSelectedFor(s)));
                }
            }
        }
        else {
    ...
    void paint() {
                    ...
                    while (cursor.hasNextSegment()) {
                        ...
                        if (cursor.current.method.equals(iLine_m.close_m)) {
                            ...
                            if (customContext != null) {
                                Object intermediate =
                                        ((iTransformingContext) customContext).getIntermediateSpaceForEvent(l,
                                                                                                            cursor.getCurrent(),
                                                                                                            -1);
                                if (!((iTransformingContext) customContext).convertIntermediateSpaceToDrawingSpace(intermediate,
                                                                                                                   at)) {
                                    hidden = true;
                                    continue;
                                }
                            ...
                            if (known.containsKey(cursor.getCurrent())
                                && known.get(cursor.getCurrent()).whatSelected.contains(SubSelection.postion)) {
                            }
                            else {
        ...
        for (SelectedVertex v : selection) {
            ...
            if (v.whatSelected.contains(SubSelection.postion)) {
                ...
                if (firstText) {
                    if (text == null) text = " _self.lines[" + on.indexOf(v.onLine) + "].events[" + v.vertexIndex + ']';
                    pointLine.getProperties().put(iLinearGraphicsContext.containsText, true);
                    ...
                    if (numText++ < 5) {
                        ...
                        if (text == null) text = "  " + on.indexOf(v.onLine) + ", " + v.vertexIndex;
                        pointLine.getProperties().put(iLinearGraphicsContext.containsText, true);
                        points.setPointAttribute(iLinearGraphicsContext.text_v, text);
                        SnippetsPlugin.addText(this.inside, text);
                ...
                if (v.amountSelected != null && v.amountSelected.containsKey(SubSelection.postion)) {
                    ...
                    amounts.setPointAttribute(iLinearGraphicsContext.strokeColor_v,
                                              new Vector4(1, 0, 0, v.getAmountSelectedFor(SubSelection.postion)));
                ...
                if (v.vertexIndex < v.onLine.events.size() - 1 && !v.onLine.getProperties()
                                                                           .isTrue(iLinearGraphicsContext.containsDepth,
                                                                                   false)) {
...
//					}
                    ...
                    if (v.amountSelected != null && v.amountSelected.containsKey(SubSelection.previousControl)) {
                        ...
                        amounts.setPointAttribute(iLinearGraphicsContext.strokeColor_v,
                                                  new Vector4(0.5f,
                                                              0,
                                                              0,
                                                              v.getAmountSelectedFor(SubSelection.previousControl)));
            ...
            if (v.onLine.events.size() > v.vertexIndex + 1) {
                        ...
                        if (v.amountSelected != null && v.amountSelected.containsKey(SubSelection.nextControl)) {
                            ...
                            amounts.setPointAttribute(iLinearGraphicsContext.strokeColor_v,
                                                      new Vector4(0.5f,
                                                                  0,
                                                                  0,
                                                                  v.getAmountSelectedFor(SubSelection.nextControl)));
                ...
                else v.whatSelected.remove(SubSelection.nextControl);
            }
        }
        ...
        if (isMarquee) {
            ...
            marqueLine.getInput()
                      .lineTo((float) (marqueeRect.x + marqueeRect.w), (float) (marqueeRect.y + marqueeRect.h));
    ...
    public
    void removeFromSelection(Selectable hit) {
    ...
    void selectionTextBounce() {
        ...
        if (description.size() == 1) {
            ...
            new PopupTextBox.Modal().getString(PopupTextBox.Modal.elementAt(inside),
                                               "selection : ",
                                               string,
                                               new SelectionStringAcceptor());
        }
    ...
    public
    void setExtraArgumentsForTool(BaseTool tool, List<SelectedVertex> selection) {
        for (SelectedVertex v : selection) {
            iLinearGraphicsContext context = v.onLine.getProperties().get(iLinearGraphicsContext.context);
            if (context instanceof iTransformingContext) {
    ...
    protected
    void filterMouseInfoByModifiers(int modifiers) {
    ...
    protected
    Selectable hittest(float x, float y) {
        Vector2 v = new Vector2(x, y);
        float min = (float) (30 * Math.sqrt(GLComponentWindow.getCurrentWindow(null).getXScale()
                                            * GLComponentWindow.getCurrentWindow(null).getYScale()));
    ...
    void setCurrentSelectionTool(iSimpleSelectionTool currentSelectionTool) {
        ...
        if (currentSelectionTool != null) if (!currentSelectionTool.begin(selection, TweakSplineUI.this)) {
    ...
    iSimpleSelectionTool getCurrentSelectionTool() {
        ...
        else return currentSelectionTool;
    }
