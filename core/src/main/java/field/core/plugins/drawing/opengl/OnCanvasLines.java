...
class OnCanvasLines {
    ...
    OnCanvasLines(iAcceptsSceneListElement on, FullScreenCanvasSWT canvas) {
        ...
        canvas.registerMouseEventDelegate(new MouseEventDelegate() {
            ...
            void mouseMove(MouseEvent e) {
                ...
                Event ee = toEvent(e);
                if (down) interaction.mouseDragged(null, ee);
                else interaction.mouseMoved(null, ee);
            }

            protected
            Event toEvent(MouseEvent e) {
                Event ee = new Event();
                ee.x = e.x;
                ee.y = e.y;
                ee.button = e.button;
    ...
    @HiddenInAutocomplete
    public
    CoreImageCanvasUtils getCoreGraphicsContext() {
        return coreGraphicsContext;
    }

    public
    List<Object> getLayer(String name) {
    ...
    public
    CachedLineLayer getDirectLineLayer(String name) {
    ...
    public
    CachedLineLayer getDirectMeshLayer(String name) {
    ...
    public
    CachedLineLayer getDirectPointLayer(String name) {
    ...
    class DirectLayer {
        ...
        void distribute() {
            ...
            if (submit.getMod() == oldMod && !interpolator.isDoingWork()) return;
    ...
    public
    DirectLayer getDirectLayer(String name) {
    ...
    public
    void installWebbrowserSupport(boolean useRectTextures) {
