...
public
class VertMarqueeTool implements iSimpleSelectionTool {
    ...
    void paint() {
        ...
        marqueLine.getInput()
                  .moveTo((float) downAt.x,
                          GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2()).y);
        marqueLine.getInput()
                  .lineTo((float) (upAt.x),
                          GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2()).y);
        marqueLine.getInput()
                  .lineTo((float) (upAt.x),
                          GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(0, 2000)).y);
        marqueLine.getInput()
                  .lineTo((float) (downAt.x),
                          GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2(0, 2000)).y);
        marqueLine.getInput()
                  .lineTo((float) (downAt.x),
                          GLComponentWindow.getCurrentWindow(null).transformWindowToCanvas(new Vector2()).y);
