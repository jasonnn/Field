...
public
class ApplyTool implements iCoordTransformation {

    static HashMap<String, Class> foundClasses = new HashMap<String, Class>();

    public
    ApplyTool(String toolname) {
    ...
    public
    ApplyTool(String toolname, boolean maskp, boolean maskn, boolean maska) {
        ...
        if (found != null) this.found = found;
            ...
            // look in visitors
            try {
                this.found = this.getClass()
                                 .getClassLoader()
                                 .loadClass("field.core.plugins.drawing.tweak.Visitors$" + toolname);
            } catch (ClassNotFoundException e) {
            }
            if (this.found == null) try {
                this.found = this.getClass()
                                 .getClassLoader()
                                 .loadClass("field.core.plugins.drawing.tweak.NodeModifiers$" + toolname);
            } catch (ClassNotFoundException e) {
            }

            if (this.found == null) System.err.println(ANSIColorUtils.red(" can't find class <" + toolname + '>'));
            else {
                found = this.found;
        ...
        if (this.found != null) {
            ...
            if (foundConstructor == null) System.err.println(ANSIColorUtils.red(" can't find constructor from <"
                                                                                + Arrays.asList(c)
                                                                                + "> / <"
                                                                                + args.length
                                                                                + '>'));
    ...
    public
    ApplyTool(String toolname, boolean maskp, boolean maskn, boolean maska, Object... args) {
        this.args = args;
    ...
    public
    void transformNode(float selectedAmount, final SelectedVertex vertex) {
        if (foundConstructor != null) {
            try {
                ...
                else if (o instanceof WritingReturn) {
