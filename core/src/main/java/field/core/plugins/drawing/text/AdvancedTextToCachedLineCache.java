...
public
class AdvancedTextToCachedLineCache {
    ...
    LinkedHashMap<CacheRecord, CacheRecord> cache = new LinkedHashMap<CacheRecord, CacheRecord>() {
        @Override
        protected
        boolean removeEldestEntry(Map.Entry<CacheRecord, CacheRecord> eldest) {
    ...
    public
    CacheRecord find(String font, String text, int size) {
        t.font = font;
        t.text = text;
        t.size = size;
        return cache.get(t);
    ...
    public
    void record(CachedLine c, String font, String text, int size, float ox, float oy, Rect upper, Rect full) {
