...
class LateExecutingDrawing {
    ...
    class WrappedObject {
        public
        WrappedObject(Object to, CapturedEnvironment variable) {
            this.o = to;
            this.env = variable;
        }
    ...
    void scanLine(CachedLine line, Dict properties) {
        ...
        if (!properties.isTrue(iLinearGraphicsContext.containsCode, false)) return;
    ...
    public
    CachedLine prepLine(CachedLine line, Dict properties) {

        if (properties == null) return line;

        if (!properties.isTrue(iLinearGraphicsContext.containsCode, false)) return line;

        for (CachedLine.Event e : line.events) {
            if (e.attributes != null) {
                Object code = e.attributes.get(iLinearGraphicsContext.code_v);
                if (code != null) line = executeCode(line, code, e);
            }
        }
    ...
    protected
    CachedLine executeCode(CachedLine line, Object code, Event e) {
        if (code instanceof WrappedObject && ((WrappedObject) code).env != null) {
            ...
            PyObject result = f.__call__(new PyObject[]{Py.java2py(line), Py.java2py(e), Py.java2py(this)},
                                         new String[]{"line", "event", "context"});
    ...
    protected
    CachedLine interpretResult(Object result, CachedLine line, Object code, Event e) {
        if (result instanceof Vector2) e.setAt(-1, ((Vector2) result));
        if (result instanceof Vector3) e.setAt(-1, ((Vector3) result).toVector2());
    ...
    public
    interface iInterpretLine {
        public
        Object execute(CachedLine line, CachedLine.Event event, LateExecutingDrawing context);
    }

    public static
    void addCode(CachedLine.Event e, Object to) {

        to = new WrappedObject(to,
        ...
        if (x == null) e.getAttributes().put(iLinearGraphicsContext.code_v, to);
        else if (x instanceof List) ((List) x).add(to);
        else {
    ...
    class Relativize implements iInterpretLine {
        ...
        public
        Relativize(CachedLine target, CachedLine.Event source) {
        ...
        public
        Relativize(CachedLine.Event target, CachedLine.Event source) {
            atSet = new Vector2();
        ...
        public
        Object execute(CachedLine line, Event event, LateExecutingDrawing context) {
            ...
            else ;//System.out.println(" couldn't find marker");
            return null;
        }
    }
