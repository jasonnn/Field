...
class SplineComputingOverride extends DefaultOverride implements iVisualElementOverrides, iChangeParticipant {
    ...
    class PLineList extends ArrayList {
        ...
        public
        PLineList(Collection<CachedLine> c) {
            super(c);
        }

        public
        PLineList(int initialCapacity) {
            super(initialCapacity);
        }

        public
        void put(String name, Object element) {
            int x = 0;
        ...
        boolean add(Object o) {
            o = filter(o);
            if (o instanceof CachedLine) return super.add(o);
            return false;
        }
        ...
        Object set(int index, Object element) {
            element = filter(element);
            if (element != null) return super.set(index, element);
            return null;
        }
    ...
    public static final VisualElementProperty<List<CachedLine>> computed_linesToDraw =
            new VisualElementProperty<List<CachedLine>>("lines").setFreezable();

    public static final VisualElementProperty<VEList> computed_elaborates =
            new VisualElementProperty<VEList>("elaborates");
    public static final VisualElementProperty<VEList> computed_elaboratedBy =
            new VisualElementProperty<VEList>("elaboratedBy");

    public static final VisualElementProperty<Integer> computed_computeWhen =
            new VisualElementProperty<Integer>("lineWhen");

    public static final VisualElementProperty<String> onChange = new VisualElementProperty<String>("onChange_v");

    public static final VisualElementProperty<String> onFrameChange =
            new VisualElementProperty<String>("onFrameChange_v");
    ...
    public static final VisualElementProperty<Boolean> shouldAutoComputeRect =
            new VisualElementProperty<Boolean>("shouldAutoComputeRect");

    public static final VisualElementProperty<Number> global_opacity =
            new VisualElementProperty<Number>("globalOpacity");
    ...
    static {
        ...
        FieldPyObjectAdaptor2.injectedSelfMethods.put("tweaks",
                                                      PythonInterface.getPythonInterface()
                                                                     .executeStringReturnPyObject("def __tweaks(self):\n"
                                                                                                  + "\tself.overrides.executeTweaks(self)\n",
                                                                                                  "__tweaks"));

        PythonInterface.getPythonInterface()
                       .execString("from field.core.plugins.drawing.opengl import OptimizeCachedLineSet");

        FieldPyObjectAdaptor2.injectedSelfMethods.put("optimizeLines",
                                                      PythonInterface.getPythonInterface()
                                                                     .executeStringReturnPyObject("def __optimize(self, exclude=None):\n"
    ...
    void executePropertyOfElement(VisualElementProperty<String> property, iVisualElement source) {
        ...
        try {
            ...
            if (eei != null) eei.executeFragment(text);
            else PythonInterface.getPythonInterface().execString(text);
    ...
    void executeStringInElement(VisualElementProperty<String> property, iVisualElement source, String s) {
        try {
            ...
            promise.beginExecute();
            if (eei != null) eei.executeFragment(e);
            else PythonInterface.getPythonInterface().execString(e);
            promise.endExecute();
    ...
    VisualElement createSubElaboration(String name, CachedLine line) {
        ...
        Triple<VisualElement, PlainDraggableComponent, SplineComputingOverride> created =
                VisualElement.createWithName(bounds,
                                             c.get(0),
                                             VisualElement.class,
        ...
        try {
            new LoadInternalWorkspaceFile().copyPlainTextToProperty("SubElaboration_python_source.py",
                                                                    created.left,
                                                                    PythonPlugin.python_source);
            new LoadInternalWorkspaceFile().copyPlainTextToProperty("SubElaboration_tweak.py", created.left, tweak);
            new LoadInternalWorkspaceFile().copyPlainTextToProperty("SubElaboration_onChange.py",
                                                                    created.left,
                                                                    onChange);
    ...
    VisitCode deleted(iVisualElement source) {
        ...
        if (list != null) if (list.remove(source)) {
        }
        return super.deleted(source);
    }

    public
    void fireAndFireChange() {
        long was = this.getHash();
        executePropertyOfElement(onChange, forElement);
        long now = this.getHash();
        delayedRepaint();
        fireChange();
    }
            ...
            try {
                ...
                if (isSelected && !previousWasSelected) {
                    ...
                    if (ref.get() != null) {
                        ...
                        if (tweaking != null) tweaking.deinstallMousePeer(forElement);
                    ...
                    if (GLComponentWindow.currentContext != null) {
                        ...
                        for (CachedLine c : instructions) {
                            ...
                            if (c != null) {
                                injectOpacity(c);
                                n += c.events.size();
                                if (n < 100) injectFocused(c, focused);
                                else injectFocused(c, false);
                                ...
                                if (offset != null) {
                                    ...
                                    if ((previous == null) || fn) {
                                        CachedLine c2 =
                                                LineUtils.transformLineOffsetFrom(c, source.getFrame(null), offset);
                                ...
                                else context.submitLine(c, c.getProperties());

                                if (fn) c.getProperties().remove(iLinearGraphicsContext.forceNew);

                            }
                            n++;
                        }
                    }
                    else System.err.println(" No drawing context for window ? ");
                }
                ...
                if (isSelected) {
                    ...
                    selectedLine.getProperties()
                                .put(iLinearGraphicsContext.color,
                                     new Vector4(0.0f, 0.0f, 0.0f, 0.6f * (focused ? 0.5f : 1)));
                    ...
                    if (tweaking != null) tweaking.paint();
                ...
                if (isMarked) {
                    ...
                    in.lineTo((float) (currentFrame.x + currentFrame.w + 5),
                    ...
                    selectedLine.getProperties()
                                .put(iLinearGraphicsContext.color,
                                     new Vector4(1.0f, 0.0f, 0.0f, 0.6f * (focused ? 0.5f : 1)));
                    selectedLine.getProperties().put(iLinearGraphicsContext.notForExport, true);
                    if (GLComponentWindow.currentContext != null)
                        GLComponentWindow.currentContext.submitLine(selectedLine, selectedLine.getProperties());
                    ...
                    if (el != null) {
                        for (iVisualElement ve : el) {
                            ...
                            elByLine.moveTo((float) (currentFrame.x + (currentFrame.w / 2)),
                                            (float) (currentFrame.y + (currentFrame.h / 2)));
                            elByLine.lineTo((float) (toFrame.x + (toFrame.w / 2)),
                                            (float) (toFrame.y + (toFrame.h / 2)));
                            elByLine.close();
                            injectOpacity(elBy);
...

                    {
                        ...
                        selectedLine.getProperties()
                                    .put(iLinearGraphicsContext.color,
                                         new Vector4(0.0f, 0.0f, 0.0f, 0.3f * (focused ? 0.5f : 1)));
                        ...
                        selectedLine.getProperties()
                                    .put(iLinearGraphicsContext.fillColor, new Vector4(0.67, 0.67, 0.67, 0.15f));
                        selectedLine.getProperties().put(iLinearGraphicsContext.notForExport, true);
    ...
    protected
    void getLinesToDraw(Ref<List<CachedLine>> ref) {
    ...
    public
    Rect recomputeBounds(List<CachedLine> instructions, Rect currentFrame) {
        ...
        for (CachedLine c : instructions) {
            ...
            if (c.getProperties().get(iLinearGraphicsContext.offsetFromSource) != null) return currentFrame;

            if (bounds != null) {
                touched = true;
                if (bounds[0].x < minx) minx = bounds[0].x;
                if (bounds[0].y < miny) miny = bounds[0].y;
                if (bounds[1].x > maxx) maxx = bounds[1].x;
                if (bounds[1].y > maxy) maxy = bounds[1].y;
                break;
            }
        }
    ...
    private
    long longSplineHash(List<CachedLine> instructions) {
        int hashCode = 1;
        // if (true)
        // return 0;
    ...
    @SuppressWarnings("unchecked")
    protected
    void initializeDrawngInstructions(Ref<List<CachedLine>> ref) {
    ...
    boolean injectFocused(CachedLine c, boolean f) {
        ...
        else c.getProperties().put(iLinearGraphicsContext.shouldHighlight, false);
        return true;
    }

    protected
    boolean injectOpacity(CachedLine c) {
        if (c != null) c.getProperties().put(iLinearGraphicsContext.source, forElement);
        Number g = forElement.getProperty(global_opacity);
        if (g != null) {
            if (c != null) c.getProperties().put(iLinearGraphicsContext.totalOpacity, g);
