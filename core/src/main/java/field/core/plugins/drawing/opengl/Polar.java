...
class Polar {
    ...
    class PolarOutput {
        ...
        public
        PolarOutput(CachedLine on, Vector2 at, Vector2 heading) {
            this.on = on;
        ...
        public
        PolarOutput(Vector2 at, Vector2 heading) {
            this(new CachedLine(), at, heading);
        }

        public
        Vector2 move(PolarMove move) {
        ...
        public
        Vector2 advance(PolarMove move) {
            Vector2 currentlyAt = new Vector2(at);
            Vector2 previouslyAt = null;
            Vector2 currentlyHeading = new Vector2(heading);
        ...
        public
        Vector2 speculative(PolarMove m) {
            Vector2 currentlyAt = new Vector2(at);
            Vector2 previouslyAt = null;
            Vector2 currentlyHeading = new Vector2(heading);
        ...
        public
        CachedLine getOutput() {
            return on;
        }
    }

    public
    interface iPolarVisitor {
        public
        void beginSubpath(Vector2 at, Vector2 heading);

        public
        void visitPolarMove(PolarMove m, PolarFilter in);
    }
    ...
    class PolarFilter {
        ...
        public
        PolarFilter(CachedLine input) {
            this.input = input;
        }

        public
        CachedLine visitPolar(iPolarVisitor visit) {
            ...
            while (c.hasNextSegment()) {
                ...
                if (!c.getCurrent().hasDestination()) continue;
                ...
                if (c.getBefore() == null) {
                    ...
                    previousPrevious = (c.getCurrentIndex() < 2)
                                       ? new Vector2(previous).sub(new Vector2(pos).sub(previous))
                                       : input.events.get(c.getCurrentIndex() - 2).getDestination();
                }
                ...
                Vector2 c1 = c.getCurrent().method.equals(iLine_m.cubicTo_m)
                             ? c.getCurrent().getAt(0).sub(previous)
                             : new Vector2(pos).sub(previous).scale(1 / 3f);
                Vector2 c2 = c.getCurrent().method.equals(iLine_m.cubicTo_m)
                             ? c.getCurrent().getAt(1).sub(pos)
                             : new Vector2(previous).sub(pos).scale(1 / 3f);

                Vector2 nextHeading = (c.getAfter() != null && c.getAfter().hasDestination())
                                      ? c.getAfter().getDestination().sub(pos)
                                      : new Vector2(nowHeading);
    ...
    static public
    class AllMoves implements iPolarVisitor {
    ...
    static public
    Vector2 rotate(Vector2 currentlyHeading, float alpha, float main) {
