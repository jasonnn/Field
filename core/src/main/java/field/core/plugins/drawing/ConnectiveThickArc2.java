...
class ConnectiveThickArc2 implements iUpdateable {
    ...
    Shape reconstructGlyph() {
        ...
        path.curveTo(startTopIntermediate.x,
                     startTopIntermediate.y,
                     endTopIntermediate.x,
                     endTopIntermediate.y,
                     endTopIntermediate1.x,
                     endTopIntermediate1.y);
    ...
    protected
    void drawNow(List<CachedLine> lines) {
        if (gate.evaluate() == 0) {
            lines.remove(destination);
        }


        if (fillColor.hasChanged(true)
            || outlineColor.hasChanged(true)
            || startRect.hasChanged(true)
            || endRect.hasChanged(true)
            || startThickness.hasChanged(true)
            || endThickness.hasChanged(true)) {
