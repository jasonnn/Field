...
abstract
class AbstractGraphics2D extends Graphics2D implements Cloneable {
    /**
    ...
    /**
     * @param textAsShapes if true, all text is turned into shapes in the convertion. No text
     *                     is output.
     */
    public
...
class GraphicContext implements Cloneable {
    ...
    /**
     * @param defaultDeviceTransform Default affine transform applied to map the user space to the user
     *                               space.
     */
    public
    ...
    public
    void setClip(Shape clip) {
        if (clip != null) this.clip = transform.createTransformedShape(clip);
        else this.clip = null;
    }
    ...
    void setTransform(AffineTransform Tx) {
        ...
        if (!Tx.isIdentity()) transformStack.add(TransformStackElement.createGeneralTransformElement(Tx));
    }
        ...
        if ((antialiasingHint != RenderingHints.VALUE_TEXT_ANTIALIAS_ON) && (antialiasingHint
            ...
            if (antialiasingHint != RenderingHints.VALUE_TEXT_ANTIALIAS_OFF) {
                ...
                if ((antialiasingHint != RenderingHints.VALUE_ANTIALIAS_ON) && (antialiasingHint
                    ...
                    if (antialiasingHint == RenderingHints.VALUE_ANTIALIAS_OFF) isAntialiased = false;
                }
            }
            else isAntialiased = false;
 ...
 */
abstract
class TransformStackElement implements Cloneable {
   ...
   */
    ...
    public static
    TransformStackElement createGeneralTransformElement(AffineTransform txf) {
...
class TransformType {
     ...
     */
    private
    TransformType(int val, String desc) {
        this.desc = desc;
        this.val = val;
    }
