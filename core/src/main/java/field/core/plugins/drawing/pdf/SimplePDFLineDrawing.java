...
class SimplePDFLineDrawing {
        ...
        SmallLineEmitter lineEmitter = new SmallLineEmitter() {
            ...
            void emitCubicFrame(Vector2 a,
                ...
                if (firstOut || lastOut.distanceFrom(a) > 1e-5) {
                    ...
                    path.moveTo(a.x * outputTransform.x + outputTransform.z,
                                a.y * outputTransform.y + outputTransform.w);
                    setStrokeProperties(name, properties);
                    firstOut = false;
                    lastOut = new Vector2(b);
                }
                path.curveTo(c1.x * outputTransform.x + outputTransform.z,
                             c1.y * outputTransform.y + outputTransform.w,
                             c2.x * outputTransform.x + outputTransform.z,
                             c2.y * outputTransform.y + outputTransform.w,
                             b.x * outputTransform.x + outputTransform.z,
                             b.y * outputTransform.y + outputTransform.w);
                lastOut.x = b.x;
                lastOut.y = b.y;
            }
    ...
    class SimplePDFFill implements iDrawingAcceptor {
        ...
        SmallLineEmitter lineEmitter = new SmallLineEmitter() {
            ...
            void emitCubicFrame(Vector2 a,
                                ...
                                Dict properties,
                                iLinearGraphicsContext context) {
                Vector4 outputTransform = SimplePDFLineDrawing.outputTransform;
                ...
                void update() {
                    ...
                    for (CachedLine.Event e : line.events) {
                        ...
                        if (!e.method.equals(iLine_m.close_m)) {
                            ...
                            context.getOutput()
                                   .addTemplate(unitSquare,
                                                -size / 2
                                                + (e.getDestination().x) * outputTransform.x
                                                + outputTransform.z,
                                                -size / 2
                                                + (e.getDestination().y) * outputTransform.y
                                                + outputTransform.w);
    ...
    public
    class SynthGradientPDFFill implements iDrawingAcceptor {
        private final BasePDFGraphicsContext context;
        ...
        public
        SynthGradientPDFFill(BasePDFGraphicsContext context) {
            this.context = context;
            lineEmitter.addTrackedProperty(iLinearGraphicsContext.strokeColor_v, new iMetric<Vector4, Vector4>() {
                public
                float distance(Vector4 from, Vector4 to) {
        ...
        DrawingResult accept(List<iUpdateable> soFar, final CachedLine line, final Dict properties) {
            ...
            final TessLineEmitter fillEmitter = new TessLineEmitter() {
                ...
                void emitCubicFrame(Vector2 a,
                                    ...
                                    Dict properties,
                                    iLinearGraphicsContext context) {
                    super.emitCubicFrame(transform(a),
                                         transform(c1),
                                         transform(c2),
                                         transform(b),
                                         name,
                ...
                Vector2 transform(Vector2 v) {
                    return new Vector2(v.x * outputTransform.x + outputTransform.z,
                                       v.y * outputTransform.y + outputTransform.w);
                }
            ...
            return new DrawingResult(DrawingResultCode.cont, new iUpdateable() {
                ...
                void update() {
                    ...
                    PdfShading_type4 shading = new MeshToPdfType4.PdfShading_type4(context.getWriter(),
                                                                                   (TriangleMesh) fillEmitter.mesh.getUnderlyingGeometry());
                    context.getOutput().saveState();
                    context.getOutput().paintShading(shading);
                    context.getOutput().restoreState();
