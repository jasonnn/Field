...
public
class ThreedComputingOverride extends SplineComputingOverride {

    public static final VisualElementProperty<ThreedContext> context =
            new VisualElementProperty<ThreedContext>("context_");
        ...
        if (c == null) {
            ...
            if (element != null) {
                ...
                final LineInteraction3d interaction = new LineInteraction3d(defaultContext.getCamera()) {
                    @Override
                    protected
                    GeneralPath filter(GeneralPath gp) {
                        return gp;
                    }
                };
                ...
                if (cc != null) c.setTransformState(cc);
                else {
                ...
                ball = new ArcBall(c.getCamera(), forElement.getFrame(null), element) {
                    ...
                    public
                    void keyPressed(field.core.windowing.GLComponentWindow.ComponentContainer inside, Event arg0) {
                        if (isSelected() || spaceDown) super.keyPressed(inside, arg0);
                    }
                };
    ...
    public
    ThreedContext getDefaultContext() {
        if (defaultContext == null) makeDefaultContext();
        ...
        if (source == forElement) {
            ...
            try {
                if (s != null) {
                    ...
                    defaultContext.getCamera()
                                  .setViewport((int) topLeft.x,
                                               h - (int) bottomRight.y,
                                               (int) (bottomRight.x - topLeft.x),
                                               (int) (bottomRight.y - topLeft.y));
                    defaultContext.getCamera()
                                  .setAspect(Math.abs(bottomRight.x - topLeft.x) / Math.abs(bottomRight.y - topLeft.y));
                    defaultContext.getCamera().performPass(null);
                    glScissor((int) topLeft.x,
                              h - (int) bottomRight.y,
                              (int) (bottomRight.x - topLeft.x),
                              (int) (bottomRight.y - topLeft.y));
                ...
                if (!stereo) list.update();

                if (!(anaglyph || stereo)) CoreHelpers.glPopAttrib();

                if (anaglyph || stereo) glEnable(GL_STEREO);
                ...
                else if (stereo) glDrawBuffer(GL_BACK_LEFT);
                ...
                else if (stereo) glDrawBuffer(GL_BACK_RIGHT);
