...
class DirectLine {
    ...
    class LineCache extends OnePassElement {
        ...
        public
        LineCache(DynamicLine_long inside) {
...
//	}
    ...
    class LineAdaptor {
        ...
        public
        void clearAndCopy(DynamicLine_long inside) {
...
//			;//System.out.println(" finished <"+inside.getUnderlyingGeometry()+">");
    ...
    //TODO explode
    public
    SubLine_long makeConcrete(CachedLine from, SubLine_long prev) {
            ...
            if (e.method.equals(iLine_m.moveTo_m)) {
                ...
                for (int i = 0; i < fixedCurveSampling; i++) {
                    ...
                    for (Map.Entry<Prop, Integer> p : trackedProperties.entrySet()) {
                        ...
                        if (lastE != null) if (lastE.attributes != null) {
                            Object d = e.getAttributes().get(p.getKey());
                            if (d != null) b = d;
                        }
...
//				cache.lineTo(lastClose);
        ...
        if (ongoing) cache.getLine().endLine();
        cache.close();
    ...
    private
    void setAux(SubLine_long cache, Integer value, Object a) {
    ...
    public
    void wire(DynamicLine_long inside, CachedLine target) {
    ...
    public
    void unwire(DynamicLine_long inside, CachedLine target) {
        LineCache c = lineCache.get(inside);
        if (c == null) {
            return;
        }
