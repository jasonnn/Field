...
class PathFlattener3 {
    ...
    public
    PathFlattener3(CachedLine c, float tol) {
        this.c = c;
        this.tol = tol;
    ...
    public
    List<Mapping> getMappingSublist(float length) {
        int found = Collections.binarySearch((List) mappings, length, searchDistance);
        if (found >= 0) return mappings.subList(0, found + 1);
...
//		;//System.out.println(" left <"+l1+"> <"+l2+">");
    ...
    private
    void emitLinearFrame(float dotStart, float dotEnd, Vector3 a, Vector3 b) {
    ...
    private
    void emitCubicFrame(float dotStart, float dotEnd, Vector3 a, Vector3 c1, Vector3 c2, Vector3 b) {
