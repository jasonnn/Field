...
public
class ArcBall implements iMousePeer, iPaintPeer {
    ...
    private
    void pushBookmark() {
        if (back.isEmpty() || !back.get(back.size() - 1).equals(on.getState())) back.add(on.getState());
        forward.clear();
    }
    ...
    void backward() {
        ...
        while (on.getState().equals(in) && !back.isEmpty()) {
            ...
            on.setState(n);
            updateCamera();
            if (repaintt != null) repaintt.requestRepaint();
        }
    ...
    public
    ArcBall(BasicCamera on, Rect rect, GLComponentWindow repaintt) {
    ...
    private
    void rebuildGrid(Vector3 up, Vector3 origin) {
    ...
    public
    void setThreedContext(BaseGLGraphicsContext context) {
    ...
    public
    void ball(Vector2 ndcStart, Vector2 ndcEnd) {
        state.reorthogonalizeUp();
        on.setState(state);
    ...
    public
    void pan(Vector2 ndcStart, Vector2 ndcEnd) {
        on.setState(state);
    ...
    public
    void scale(Vector2 ndcStart, Vector2 ndcEnd) {
        on.setState(state);
    ...
    Vector3 sphereMap(Vector2 a) {
        ...
        if (length > 1) return new Vector3(a.x, -a.y, 0).normalize();
    ...
    HashSet<Character> interesting =
            new HashSet<Character>(Arrays.asList(new Character[]{'a', 's', 'd', 'w', 'q', 'z'}));
    ...
    void keyTyped(ComponentContainer inside, Event arg0) {
        ...
        if (interesting.contains(arg0.character)) {
            ...
            if (!down.isEmpty() && (repaintt != null)) repaintt.requestRepaint();
    ...
    void orthogonalize2() {
...

        {
            ...
            m = new Vector3().set(ax, sg)
                             .scale(-1)
                             .setMagnitude(on.getState().getPosition().sub(on.getState().getLookAt()).mag())
                             .add(on.getState().getLookAt());
            out.position = m;
            out.reorthogonalizeUp();
        }
    ...
    void paint(RootComponent inside) {
        ...
        if (!down.isEmpty() && (repaintt != null) && spaceDown) repaintt.requestRepaint();
        //
        // if (down.size() > 0) {
     ...
     */
    public
    void paintNow(iLinearGraphicsContext context) {
        float width = 20;
    ...
    private
    Vector2 rectMap(float x, float y) {
