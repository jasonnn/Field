...
public
class SimpleLineDrawing {

    public boolean doFakeAa = false;

    public
    class ObjectFastHighlight implements iDrawingAcceptor<CachedLine> {

        private final BaseGLGraphicsContext context;

        public
        ObjectFastHighlight(BaseGLGraphicsContext context) {
            this.context = context;
        }

        public
        DrawingResult accept(List<iDynamicMesh> soFar, final CachedLine line, Dict properties) {
            if (!properties.isTrue(iLinearGraphicsContext.shouldHighlight, false)) return null;
            ...
            DrawingResult result = new DrawingResult(DrawingResultCode.cont, new IUpdateable() {
                ...
                void update() {
                    ...
                    int ii = 0;
                    if (line.events.size() < 200) for (Event e : line.events) {
                        if (e.hasDestination()) {
                            e.getDestination(v2);
    ...
    public
    class PlainGLFillDefault implements iDrawingAcceptor<CachedLine> {
        ...
        public
        PlainGLFillDefault(BaseGLGraphicsContext context) {
            this.context = context;
        }
        ...
        DrawingResult accept(List<iDynamicMesh> soFar, final CachedLine line, final Dict properties) {
            ...
            if (fopacityMul == 0) return null;

            final DynamicMesh_long outputLine =
                    getCachedMesh(context, properties.isTrue(iLinearGraphicsContext.soloCache, false));
            final DynamicMesh_long outputLineOld =
                    getOldCachedMesh(context, properties.isTrue(iLinearGraphicsContext.soloCache, false));
            ...
            DrawingResult result = new DrawingResult(DrawingResultCode.cont, new IUpdateable() {
                ...
                void update() {
                    ...
                    if (fillEmitter.mesh.getCopy() == null) fillEmitter.mesh.makeCopy();
    ...
    public
    class PlainGL3dFillDefault implements iDrawingAcceptor<CachedLine> {
        ...
        public
        PlainGL3dFillDefault(BaseGLGraphicsContext context) {
            this.context = context;
        }
        ...
        DrawingResult accept(List<iDynamicMesh> soFar, final CachedLine line, final Dict properties) {
            ...
            final DynamicMesh_long outputLine = late
                                                ? getCachedMeshLate(context)
                                                : getCachedMesh(context,
                                                                properties.isTrue(iLinearGraphicsContext.soloCache,
                                                                                  false));
            final DynamicMesh_long outputLineOld = properties.isTrue(iLinearGraphicsContext.soloCache, false)
                                                   ? outputLine
                                                   : (late
                                                      ? outputLine
                                                      : getOldCachedMesh(context,
                                                                         properties.isTrue(iLinearGraphicsContext.soloCache,
                                                                                           false)));
    ...
    public
    class PlainGLLineDefault implements iDrawingAcceptor<CachedLine> {
        ...
        SmallLineEmitter_long lineEmitter = new SmallLineEmitter_long() {
            ...
            protected
            void decorateVertex(int v1, List<Object> name, SubLine_long outputLine, Dict properties) {
        ...
        DrawingResult accept(List<iDynamicMesh> soFar, final CachedLine line, final Dict properties) {
            ...
            if (properties.isTrue(iLinearGraphicsContext.strokeType, false)) return null;
            ...
            DrawingResult result = new DrawingResult(DrawingResultCode.cont, new IUpdateable() {
                ...
                void update() {
                    ...
                    x.close();
                    if (subLine.getCopy() == null) subLine.makeCopy();
    ...
    public
    class PlainGLPointDefault implements iDrawingAcceptor<CachedLine> {
        private final BaseGLGraphicsContext context;

        private Dict properties;

        public
        PlainGLPointDefault(BaseGLGraphicsContext context) {
            this.context = context;
        }
            ...
            DrawingResult result = new DrawingResult(DrawingResultCode.cont, new IUpdateable() {
                ...
                Vector4 colorFor(Event current) {
                    ...
                    if (color == null) color = current.attributes != null
                                               ? current.attributes.get(iLinearGraphicsContext.pointColor_v)
                                               : null;
                    if (color == null) color = current.attributes != null
                ...
                private
                Vector4 pointSizeFor(Event current) {
                    Number size = null;
                    if (size == null) size = properties.get(iLinearGraphicsContext.pointSize);
                    if (size == null) size = current.getAttributes().getFloat(iLinearGraphicsContext.pointSize_v, 5f);
                    if (size == null) size = 5f;
    ...
    public
    class PlainGL3dPointDefault implements iDrawingAcceptor<CachedLine> {
        private final BaseGLGraphicsContext context;

        private Dict properties;

        public
        PlainGL3dPointDefault(BaseGLGraphicsContext context) {
            this.context = context;
        }
    ...
    public
    class StrokedGLLinelDefault implements iDrawingAcceptor<CachedLine> {
        ...
        public
        StrokedGLLinelDefault(BaseGLGraphicsContext context) {
            this.context = context;
        }

        public
        DrawingResult accept(List<iDynamicMesh> soFar, final CachedLine xline, final Dict properties) {
    ...
    void drawInto(CachedLine line,
                  ...
                  SmallLineEmitter_long lineEmitter) {

        CachedLineCursor cursor = new CachedLineCursor(line);

        lineEmitter.globalGeometricScale = context == null
                                           ? 0.4f
                                           : context.getGlobalProperties()
                                                    .getFloat(iLinearGraphicsContext.geometricScale, 1f);
        lineEmitter.globalFlatnessScale =
                context == null ? 1f : context.getGlobalProperties().getFloat(iLinearGraphicsContext.flatnessScale, 1);
        ...
        while (cursor.hasNextSegment()) {
            ...
            if (outputLine instanceof DynamicLine_long) ((DynamicLine_long) outputLine).startLine();
            if (outputLine instanceof SubLine) ((SubLine) outputLine).getLine().startLine();
            if (outputLine instanceof SubLine_long) ((SubLine_long) outputLine).getLine().startLine();
            ...
            if (outputLine instanceof DynamicLine_long) ((DynamicLine_long) outputLine).endLine();
            if (outputLine instanceof SubLine) ((SubLine) outputLine).getLine().endLine();
            if (outputLine instanceof SubLine_long) ((SubLine_long) outputLine).getLine().endLine();
        ...
        DrawingResult accept(List<iDynamicMesh> soFar, final CachedLine line, final Dict properties) {
            ...
            DrawingResult result = new DrawingResult(DrawingResultCode.cont, new IUpdateable() {
                ...
                void update() {
                    ...
                    boolean usesAdjacencyForLines =
                            context.getGlobalProperties().isTrue(iLinearGraphicsContext.usesAdjacency, false);
                    if (usesAdjacencyForLines) outputLine.getUnderlyingGeometry().sendAdjacency();
    ...
    public
    DynamicLine_long getCachedLineForWidth(BaseGLGraphicsContext context, float f) {
        if (f < 0) f = 1;
        if (f > 100) f = 100;
    ...
    public
    DynamicLine_long getCachedLineForWidth(BaseGLGraphicsContext context, float f, boolean mustbeuniq) {
        if (mustbeuniq) {
            DynamicLine_long nl = DynamicLine_long.unshadedLine(context.getVertexProgram(), f);
    ...
    public
    DynamicLine_long getOldCachedLineForWidth(BaseGLGraphicsContext context, float f) {
        if (f < 0) f = 1;
        if (f > 100) f = 100;
    ...
    public
    SimpleLineDrawing installInto(BaseGLGraphicsContext context) {
    ...
    void drawInto(CachedLine line,
        ...
        lineEmitter.globalGeometricScale =
                context.getGlobalProperties().getFloat(iLinearGraphicsContext.geometricScale, 1f);
        lineEmitter.globalFlatnessScale =
        ...
        while (cursor.hasNextSegment()) {
            ...
            if (outputLine instanceof DynamicLine)
                if (usesAdjacencyForLines) ((DynamicLine) outputLine).endLineWithAdjacency();
                else ((DynamicLine) outputLine).endLine();

            if (outputLine instanceof DynamicLine_long)
                if (usesAdjacencyForLines) ((DynamicLine_long) outputLine).endLineWithAdjacency();
                else ((DynamicLine_long) outputLine).endLine();
            if (outputLine instanceof SubLine)
                if (usesAdjacencyForLines) ((SubLine) outputLine).getLine().endLineWithAdjacency();
                else ((SubLine) outputLine).getLine().endLine();
            if (outputLine instanceof SubLine_long)
                if (usesAdjacencyForLines) ((SubLine_long) outputLine).getLine().endLineWithAdjacency();
                else ((SubLine_long) outputLine).getLine().endLine();
            lineEmitter.endContour();
        ...
        if (usesAdjacencyForLines) {
            if (outputLine instanceof DynamicLine) ((DynamicLine) outputLine).getUnderlyingGeometry().sendAdjacency();
            if (outputLine instanceof DynamicLine_long)
                ((DynamicLine_long) outputLine).getUnderlyingGeometry().sendAdjacency();
            if (outputLine instanceof SubLine) ((SubLine) outputLine).getLine().getUnderlyingGeometry().sendAdjacency();
            if (outputLine instanceof SubLine_long)
