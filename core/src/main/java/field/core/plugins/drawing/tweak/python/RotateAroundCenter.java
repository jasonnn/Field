...
public
class RotateAroundCenter implements iWholeCoordTransform {
    ...
    public
    RotateAroundCenter(float ox, float oy, float dx, float dy) {
        this.dx = dx;
        this.dy = dy;
        this.ox = ox;
        this.oy = oy;
    ...
    void setNodes(List<Pair<SelectedVertex, Float>> all) {
        ...
        for (int i = 0; i < all.size(); i++) {
            if (all.get(i).left.whatSelected.contains(SubSelection.previousControl)) {
                Vector2.add(all.get(i).left.positionFor(SubSelection.previousControl, null),
                            all.get(i).right,
                            center,
                            center);
            ...
            if (all.get(i).left.whatSelected.contains(SubSelection.nextControl)) {
                Vector2.add(all.get(i).left.positionFor(SubSelection.nextControl, null),
                            all.get(i).right,
                            center,
                            center);
                total += all.get(i).right;
        ...
        r = new Quaternion(new Vector3((dx + ox) - center.x, (dy + oy) - center.y, 0),
                           new Vector3(ox - center.x, oy - center.y, 0));
    ...
    void transformNode(float selectedAmount, SelectedVertex vertex) {
        if (vertex.whatSelected.contains(SubSelection.previousControl)) {
            Vector2 a = transform(vertex.positionFor(SubSelection.previousControl, null),
                                  vertex.getAmountSelectedFor(SubSelection.previousControl));
    ...
    private
    Vector2 transform(Vector2 a, float am) {
