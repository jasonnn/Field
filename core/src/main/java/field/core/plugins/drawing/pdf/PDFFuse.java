...
class PDFFuse {
    ...
    public
    PDFFuse(String fileIn, String fileOut) {
        try {
    ...
    public
    void addImage(String filename, Rect intoRect) throws DocumentException, IOException {
    ...
    public
    void addImageMultiply(String filename, Rect intoRect, float opacity) throws DocumentException, IOException {
        Image pdfimage = Image.getInstance(filename);
        over.saveState();
    ...
    public
    void addImageScreen(String filename, Rect intoRect, float opacity) throws DocumentException, IOException {
        Image pdfimage = Image.getInstance(filename);
        over.saveState();
        PdfGState gs1 = new PdfGState();
        gs1.setFillOpacity(opacity);
    ...
    public
    void addImageScreen(String filename, Rect intoRect, float opacity, float rotation)
        ...
        over.addImage(pdfimage,
                      (float) (intoRect.w * Math.cos(rotation)),
                      (float) (Math.sin(rotation) * intoRect.w),
                      (float) (-Math.sin(rotation) * intoRect.h),
                      (float) (Math.cos(rotation) * intoRect.h),
                      (int) intoRect.x,
                      (int) intoRect.y);
        over.restoreState();
    }
    ...
    public
    void addPdf(String filename) throws IOException {
        PdfImportedPage imp = stamp.getImportedPage(new PdfReader(filename), 1);
        over.addTemplate(imp, 0, 0);
    }

    public
    void addPdf(String filename, Vector2 offset) throws IOException {
        PdfImportedPage imp = stamp.getImportedPage(new PdfReader(filename), 1);
        over.addTemplate(imp, offset.x, offset.y);
    }

    public
    void addPdf(String filename, Vector2 offset, float scale) throws IOException {
        PdfImportedPage imp = stamp.getImportedPage(new PdfReader(filename), 1);
        over.addTemplate(imp, scale, 0, 0, scale, offset.x, offset.y);
    }

    public
    void addPdf(String filename, float[] opacity, String[] mode, Vector2[] ox) throws IOException {
        // try {
        PdfImportedPage imp = stamp.getImportedPage(new PdfReader(filename), 1);
