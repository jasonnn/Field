...
public
class AdvancedTextToCachedLine {
    ...
    public
    AdvancedTextToCachedLine(String fontName, int fontSize) {
    ...
    public
    AdvancedTextToCachedLine(Font font) {
    ...
    public
    CachedLine getLine(String text, final float ox, final float oy) {
        return getLine(text, ox, oy, 0);
    }

    public
    CachedLine getLine(String text, final float ox, final float oy, float wrap) {
        if (wrap == 0) {
        ...
        aNativeTextLayoutVisitor vv = new aNativeTextLayoutVisitor() {
            ...
            @Override
            public
            void visitBegin(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float oxy) {
            ...
            @Override
            public
            void visitPathCubicCurveTo(float x, float y, float x1, float y1, float x2, float y2) {
                x /= fontScale;
                y /= fontScale;
                x1 /= fontScale;
                y1 /= fontScale;
            ...
            @Override
            public
            void visitPathLineTo(float x, float y) {
                x /= fontScale;
                y /= fontScale;
            ...
            @Override
            public
            void visitPathMoveTo(float x, float y) {
                x /= fontScale;
                y /= fontScale;
                line.moveTo(ox + x, oy + y);
            }
        };
        if (wrap == 0) at.acceptString(text, vv);
        ...
        if (wrap == 0) cache.record(cl, fontName, text, fontSize, ox, oy, upperRect, fullRect);
    ...
    public
    Rect getMiddleBounds() {
        GeneralPath p = new LineUtils().lineToGeneralPath(lastCachedLine, true);
        float f1 = 0.45f;
        float f2 = 0.1f;
        Area a = new Area(p);
        Rectangle2D.Double def =
                new Rectangle2D.Double(upperRect.x, upperRect.y + (upperRect.h * f1), upperRect.w, upperRect.h * f2);
        a.intersect(new Area(def));
        Rectangle2D bounds = a.getBounds2D();
        if (a.isEmpty()) {
            bounds = def;
    ...
    private
    void markWithMoveLock(CachedLine cl) {
