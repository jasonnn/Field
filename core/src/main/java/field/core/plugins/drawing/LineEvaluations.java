...
public
class LineEvaluations {

    private CachedLine on;
    private final Vector2[] bounds;

    public
    LineEvaluations(CachedLine on) {
        this.on = on;
        ...
        if (on.getProperties().get(iLinearGraphicsContext.offsetFromSource) != null) {
            ...
            if (mm == null) {
                ...
                CachedLine c2 = LineUtils.transformLineOffsetFrom(on,
                                                                  source.getFrame(null),
                                                                  on.getProperties()
                                                                    .get(iLinearGraphicsContext.offsetFromSource));
    ...
    public
    List<Vector2> eval(float x) {
        List<Vector2> r =
                Intersections.intersect(on, new Vector2(x, bounds[0].y - 10), new Vector2(x, bounds[1].y + 10));
        return r;
    }

    static public
    CachedLine intersect(CachedLine on, float x) {
        ...
        List<Pair<Event, Event>> o =
                Intersections.intersectAndSubdivide(Collections.singletonList(e.on), Collections.singletonList(v), 4);
        return e.on;
    }
