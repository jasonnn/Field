...
class VersioningSystem {
    ...
    VersioningSystem newDefault() {
        ...
        if (useGit) return new GitVersioningSystem(SystemProperties.getDirProperty("versioning.dir"),
                                                   SystemProperties.getProperty("fluid.scratch",
                                                                                SystemProperties.getProperty("main.class")
                                                                                + ".xml"),
                                                   "sheet.xml");
        VersioningSystem r = new HGVersioningSystem(SystemProperties.getDirProperty("versioning.dir"),
                                                    SystemProperties.getProperty("fluid.scratch",
                                                                                 SystemProperties.getProperty("main.class")
                                                                                 + ".xml"),
                                                    "sheet.xml");
    ...
    public static
    VersioningSystem newDefault(String filename) {
        // VersioningSystem r = new
        // SVNVersioningSystem(SystemProperties.getDirProperty("versioning.dir"),
        // SystemProperties.getProperty("fluid.scratch",
        // SystemProperties.getProperty("main.class") + ".xml"),
        ...
        VersioningSystem r =
                new HGVersioningSystem(SystemProperties.getDirProperty("versioning.dir"), filename, "sheet.xml");
        // VersioningSystem.versioningSystem.set(r);
        return r;
    }
    ...
    Object objectRepresentationFor(String read) {
        ...
        if (read.startsWith("string>")) return read.substring("string>".length());
    ...
    public
    VersioningSystem(String fullPathToRepositoryDirectory, String sheetSubdirectory, String xmlFilename) {
    ...
    void notifyCopyFileToProperty(String filename,
        ...
        if (!f.exists()) f.mkdir();
        String dest = fullPathToSheetDirectory
                      + '/'
                      + targetElement.getUniqueID()
                      + '/'
    ...
    public
    void setCurrentLogMessage(String currentLogMessage) {
        this.currentLogMessage = currentLogMessage;
        if (currentLogMessage.trim() != null && currentLogMessage.trim().isEmpty()) currentLogMessage = "(no message)";
    }

    public
    void suspendJournaling() {
        journalingDisabled++;
    }
    ...
    void synchronizeElementWithFileStructure(IVisualElement to) {
        ...
        if (f.exists()) {
            ...
            if (properties != null) for (String p : properties) {
        ...
        while (i.hasNext()) {
            ...
            if (op instanceof CopyElementOp) {
                if ((((CopyElementOp) op).newElement).equals(path)) {
                    scmCopyDirectory(((CopyElementOp) op).oldElement.getFile(),
                                     ((CopyElementOp) op).newElement.getFile());
                    i.remove();
                }
            }
        ...
        while (i.hasNext()) {
            ...
            if (op instanceof CopyPropertyOp) {
                if ((((CopyPropertyOp) op).newProperty).isSameElementAs(path)) {
                    scmCopyFile(((CopyPropertyOp) op).oldProperty.getFile(),
                                ((CopyPropertyOp) op).newProperty.getFile());
                    i.remove();
                }
            }
        }
        ...
        for (Map.Entry<Object, Object> e : properties.entrySet()) {
            ...
            if (key instanceof VisualElementProperty) {
                if (((VisualElementProperty) key).containsSuffix("v")) {
                    ...
                    justCreated |= setFileFromString(pathProperty,
                                                     stringRepresentationFor(to.getProperty((VisualElementProperty) key)),
                                                     false);
    ...
    protected
    boolean setFileFromString(Path path, String string, boolean shouldAdd) {
        if (string != null) {
            try {
                ...
                if (shouldAdd && !new File(path.getPath()).exists()) scmAddFile(path.getFile());
                return newFile;
            } catch (IOException e) {
                e.printStackTrace();
            }
