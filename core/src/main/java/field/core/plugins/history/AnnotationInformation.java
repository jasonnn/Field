...
class AnnotationInformation {
    ...
    public
    AnnotationInformation(HistoryExplorerHG hgSupport, String file) {
        this.hgSupport = hgSupport;
        this.file = file;
    }

    public
    int compressedHistoryLengthForLine(int line) {
        return compressedTrace(line).size();
    }

    public
    ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> compressedTrace(int line) {
        ...
        ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> a =
                new ArrayList<Triple<VersionNode, Integer, Pair<String, String>>>();
    ...
    public
    void construct(String currentContents) {
    ...
    public
    Pair<Integer, Integer> domainForLine(int line, int maxLine) {
...
//			;//System.out.println(" comparing <" + c + ">\n and <" + x + ">");
    ...
    public
    Pair<Integer, Integer> domainForLine(int line, int maxLine, Date before) {
        ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> c = compressedTrace(line);
        Pair<Integer, Integer> startstop = new Pair<Integer, Integer>(line, line);
        while (startstop.left > 0) {
            ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> x = compressedTrace(startstop.left - 1);
            if (equiv(c, x, before)) startstop.left--;
            else break;
        }
        while ((startstop.right + 1) < maxLine) {
            ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> x = compressedTrace(startstop.right + 1);
            if (equiv(c, x, before)) startstop.right++;
            else break;
        }
    ...
    public
    int historyLengthForLine(int line) {
        return traceLine(line).size();
    }

    public
    List<Triple<VersionNode, Integer, Pair<String, String>>> traceLine(int line) {
        ...
        for (VersionNode v : versions) {
            ...
            if (diffset != null) {
                ...
                List<iMarker<ChannelDifferences<String>.EditRelationship>> m =
                        diffset.relationships.getSlice(line, line + 1).getIterator().remaining();
                ...
                if (!m.isEmpty() && (m.get(0).getPayload().right != null) && (m.get(0).getPayload().left != null)) {
                    ...
                    a.add(new Triple<VersionNode, Integer, Pair<String, String>>(v,
                                                                                 line,
                                                                                 new Pair<String, String>(was,
                                                                                                          becomes)));
                    string = was;
    ...
    boolean equiv(ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> c,
        ...
        for (int i = 0; i < c.size(); i++) {
            ...
            if (a.left.revision != b.left.revision) return false;
        }
        return true;
    }
    ...
    boolean equiv(ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> c,
                  ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> x,
                  Date before) {
        ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> c2 =
                new ArrayList<Triple<VersionNode, Integer, Pair<String, String>>>();
        ArrayList<Triple<VersionNode, Integer, Pair<String, String>>> x2 =
                new ArrayList<Triple<VersionNode, Integer, Pair<String, String>>>();

        for (int i = 0; i < c.size(); i++) {
            if (c.get(i).left.date.before(before) || c.get(i).left.date.equals(before)) c2.add(c.get(i));
        }

        for (int i = 0; i < x.size(); i++) {
            if (x.get(i).left.date.before(before) || x.get(i).left.date.equals(before)) x2.add(x.get(i));
        }
