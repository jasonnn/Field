...
class GitVersioningQueries {
    ...
    class VersionsOfFile {
        ...
        String getContents() {
            if (contents == null) {
                ...
                while (p2.contains("//")) p2 = p2.replaceAll("//", "/");
                ExecuteCommand e = new ExecuteCommand(workspace,
                                                      new String[]{GitVersioningSystem.gitCommand,
                                                                   "show",
                                                                   commit + ':' + p2},
                                                      true);
                e.waitFor(true);
    ...
    class DiffSet {
        ...
        public
        Snippet snippetsForVersions(int lineStart, int lineEnd) {
        ...
        public
        ChannelDifferences<String>.EditRelationship mapLine(int a) {
                        ...
                        for (iMarker<String> m : e.getPayload().left) {
                            ...
                            if ((m.getTime() == a) && !e.getPayload().right.isEmpty()) {
                                ...
                                if (p.type == EditType.equivalence) {
                                    return (int) e.getPayload().right.get(Math.min(e.getPayload().right.size() - 1,
                                                                                   index)).getTime();
                                }
    ...
    public
    GitVersioningQueries(String workspace) {
        this.workspace = workspace;
    }

    public
    List<VersionsOfFile> versionsForFile(String now, String path) {
        ExecuteCommand e = new ExecuteCommand(workspace,
                                              new String[]{GitVersioningSystem.gitCommand,
                                                           "log",
                                                           "--date",
        ...
        while (i < lines.length) {
            ...
            if (lines[i].startsWith("A ")) v.op = Op.added;
            if (lines[i].startsWith("M ")) v.op = Op.modified;
            if (lines[i].startsWith("D ")) v.op = Op.deleted;
