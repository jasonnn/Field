...
class VersionMenu {
    ...
    public
    VersionMenu(Composite inside, PythonTextEditor editor) {
        this.editor = editor;
        ...
        box = new BetterComboBox(inside, new String[]{"(no versions)"}) {
            ...
            void updateSelection(int index, String text) {
                if (index < actions.size()) actions.get(index).update();
            }
    ...
    void doMakeMenu(final IVisualElement element, VisualElementProperty<String> prop) {
        ...
        if (range.y == 0) {
            ...
            for (int ii = 0; ii < v.size() - 1; ii++) {
                DiffSet ds =
                        q.new DiffSet(v.get(ii).getContents().split("\n"), v.get(ii + 1).getContents().split("\n"));
            ...
            box.setHoverUpdate(new iHoverUpdate() {
                ...
                public
                void update(int index) {
                    if (index > 0) actions.get(index - 1).update();
                }
    ...
    void streamline(String[] s) {
        ...
        for (int i = 1; i < m.size(); i++) {
            ...
            for (int dd = 0; dd < a.length; dd++) {
                ...
                if (a[dd].equals(b[dd])) {
                    if (dd < a.length - 1) s[i] += "<g>" + b[dd] + end + "</g>";
                }
                else s[i] += b[dd] + end;
            }
        }
    ...
    @NextUpdate
    protected
    void swapInRange_next(final int[] cs, final int[] ce, final Snippet s) {
        swapInRange(cs, ce, s.contents);
    }

    protected
    void swapInRange(int[] cs, int[] ce, String contents) {
