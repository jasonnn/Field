...
public
class GitVersioningSystem extends VersioningSystem {

    public static String gitCommand = "/usr/bin/git";

    static {
        if (!new File(gitCommand).exists()) gitCommand = "/usr/bin/git";
        if (!new File(gitCommand).exists()) gitCommand = "/opt/local/bin/git";
        if (!new File(gitCommand).exists()) gitCommand = "/usr/local/bin/git";
        if (!new File(gitCommand).exists()) gitCommand = "/bin/git";
        if (!new File(gitCommand).exists()) gitCommand = System.getProperty("user.home") + "/bin/git";
        if (!new File(gitCommand).exists()) gitCommand = "/Applications/Xcode.app/Contents/Developer/usr/bin/git";

        if (!new File(gitCommand).exists()) System.err.println("ERROR: cant find a git installation anywhere");
    }

    public
    GitVersioningSystem(String fullPathToRepositoryDirector, String sheetsub, String xmlFilename) {
    ...
    void scmCopyDirectory(File from, File to) {
        if (to.exists()) {
            ...
            executeCommand(fullPathToRepositoryDirectory,
                           "/bin/cp",
                           "-r",
