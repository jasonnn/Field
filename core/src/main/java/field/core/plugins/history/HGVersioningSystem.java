...
public
class HGVersioningSystem extends VersioningSystem {
    ...
    void computeHGCommand() {
        ...
        if (!new File(hgCommand).exists()) hgCommand = "/usr/bin/hg";
        if (!new File(hgCommand).exists()) hgCommand = "/opt/local/bin/hg";
        if (!new File(hgCommand).exists()) hgCommand = "/usr/local/bin/hg";
        if (!new File(hgCommand).exists()) hgCommand = "/bin/hg";
        if (!new File(hgCommand).exists()) hgCommand = "/Library/Frameworks/Python.framework/Versions/Current/bin/hg";
        if (!new File(hgCommand).exists())
            hgCommand = "/System/Library/Frameworks/Python.framework/Versions/Current/bin/hg";
        if (!new File(hgCommand).exists()) hgCommand = System.getProperty("user.home") + "/bin/hg";
        if (!new File(hgCommand).exists()) hgCommand = "C:/Program Files/Mercurial/hg.exe";
        if (!new File(hgCommand).exists()) System.err.println("ERROR: cant find a mercurial installation anywhere");
    }

    public
    HGVersioningSystem(String fullPathToRepositoryDirectory, String sheetsub, String xmlFilename) {
    ...
    public
    void rollbackDirectory(String string, long revision) {
    ...
    void scmCommitDirectory(File path) {
        scmAddFile(path);
        executeCommand(fullPathToSheetDirectory,
                       new String[]{hgCommand, "commit", "-m", '"' + currentLogMessage + '"', path.getAbsolutePath()});
    }
    ...
    void scmCopyDirectory(File from, File to) {
        if (to.exists()) executeCommand(fullPathToSheetDirectory,
                                        new String[]{hgCommand,
                                                     "copy",
                                                     "--after",
                                                     from.getAbsolutePath(),
                                                     to.getAbsolutePath()});
        else executeCommand(fullPathToSheetDirectory,
                            new String[]{hgCommand, "copy", from.getAbsolutePath(), to.getAbsolutePath()});
    }
    ...
    void scmDeleteDirectory(File path) {
        ...
        if (!path.exists()) executeCommand(fullPathToSheetDirectory,
                                           new String[]{hgCommand, "remove", "--after", path.getAbsolutePath()});
        else executeCommand(fullPathToSheetDirectory, new String[]{hgCommand, "remove", path.getAbsolutePath()});
    ...
    public
    void scmMoveFile(File from, File to) {
        executeCommand(fullPathToSheetDirectory,
                       new String[]{hgCommand, "rename", from.getAbsolutePath(), to.getAbsolutePath()});
    }
