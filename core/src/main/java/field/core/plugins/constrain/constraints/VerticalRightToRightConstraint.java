...
public
class VerticalRightToRightConstraint extends VerticalLeftToLeftConstraint {

    @Override
    protected
    ClLinearEquation createConstraint(VariablesForRect vLeft, VariablesForRect vRight) {
        return new ClLinearEquation(ClLinearExpression.Plus(vLeft.variableX, new ClLinearExpression(vLeft.variableW)),
                                    ClLinearExpression.Plus(vRight.variableX,
                                                            new ClLinearExpression(vRight.variableW)));
    }
    ...
    Vector3 point1(VariablesForRect vLeft) {
        return new Vector3(vLeft.variableX.value() + vLeft.variableW.value(),
                           vLeft.variableY.value() + (vLeft.variableH.value() / 2),
                           0);
    }
    ...
    Vector3 point2(VariablesForRect vRight) {
        return new Vector3(vRight.variableX.value() + vRight.variableW.value(),
                           vRight.variableY.value() + (vRight.variableH.value() / 2),
                           0);
    }
}
