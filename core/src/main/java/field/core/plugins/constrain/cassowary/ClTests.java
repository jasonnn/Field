...
class ClTests extends CL {
    ...
    public static final
    boolean addDelete2()
            throws ExCLInternalError, ExCLRequiredFailure, ExCLConstraintNotFound, ExCLNonlinearExpression {
        boolean fOkResult = true;
        ClVariable x = new ClVariable("x");
    ...
    boolean casso1() throws ExCLInternalError, ExCLRequiredFailure {
        ...
        fOkResult =
                fOkResult && ((CL.approx(x, 10.0) && CL.approx(y, 13.0)) || (CL.approx(x, 7.0) && CL.approx(y, 10.0)));
        //System.out.println("x == " + x.value() + ", y == " + y.value());
        return (fOkResult);
    }
    ...
    boolean inconsistent2() throws ExCLInternalError, ExCLRequiredFailure {
        try {
            ClVariable x = new ClVariable("x");
            ClSimplexSolver solver = new ClSimplexSolver();
            solver.addConstraint(new ClLinearInequality(x, CL.GEQ, 10.0))
    ...
    boolean addDel(int nCns, int nVars, int nResolves)
            throws ExCLInternalError, ExCLRequiredFailure, ExCLNonlinearExpression, ExCLConstraintNotFound {
    ...
    void main(String[] args) throws ExCLError {
        //    try
        {
            ...
            if (args.length > 0) cns = Integer.parseInt(args[0]);
            if (args.length > 1) vars = Integer.parseInt(args[1]);
            if (args.length > 2) resolves = Integer.parseInt(args[2]);
