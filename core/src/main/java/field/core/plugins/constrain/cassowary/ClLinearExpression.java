...
class ClLinearExpression extends CL {
    public
    ClLinearExpression(ClAbstractVariable clv, double value, double constant) {
        if (CL.fGC) System.err.println("new ClLinearExpression");
        _constant = new ClDouble(constant);
        _terms = new HashMap<ClAbstractVariable, ClDouble>(1);
        if (clv != null) _terms.put(clv, new ClDouble(value));
    }
    ...
    public
    ClLinearExpression(ClAbstractVariable clv, double value) {
        this(clv, value, 0.0);
    }

    public
    ClLinearExpression(ClAbstractVariable clv) {
        this(clv, 1, 0);
    }

    // for use by the clone method
    protected
    ClLinearExpression(ClDouble constant, Map<ClAbstractVariable, ClDouble> terms) {
        if (CL.fGC) System.err.println("clone ClLinearExpression");
        ...
        for (Map.Entry<ClAbstractVariable, ClDouble> clAbstractVariableClDoubleEntry : terms.entrySet()) {

            _terms.put(clAbstractVariableClDoubleEntry.getKey(),
                       (ClDouble) clAbstractVariableClDoubleEntry.getValue().clone());
        }
    }
    ...
    ClLinearExpression multiplyMe(double x) {
        ...
        for (Map.Entry<ClAbstractVariable, ClDouble> clAbstractVariableClDoubleEntry : _terms.entrySet()) {

            ClDouble cld = clAbstractVariableClDoubleEntry.getValue();
    ...
    public final
    ClLinearExpression times(ClLinearExpression expr) throws ExCLNonlinearExpression {
        if (isConstant()) {
    ...
    public final
    ClLinearExpression plus(ClVariable var) throws ExCLNonlinearExpression {
    ...
    public final
    ClLinearExpression minus(ClVariable var) throws ExCLNonlinearExpression {
        return ((ClLinearExpression) clone()).addVariable(var, -1.0);
    }

    public final
    ClLinearExpression divide(double x) throws ExCLNonlinearExpression {
        if (CL.approx(x, 0.0)) {
            throw new ExCLNonlinearExpression();
        }
        return times(1.0 / x);
    ...
    public final
    ClLinearExpression divide(ClLinearExpression expr) throws ExCLNonlinearExpression {
        if (!expr.isConstant()) {
            throw new ExCLNonlinearExpression();
        }
    ...
    public final
    ClLinearExpression divFrom(ClLinearExpression expr) throws ExCLNonlinearExpression {
    ...
    public final
    ClLinearExpression addExpression(ClLinearExpression expr) {
    ...
    public final
    ClLinearExpression addVariable(ClAbstractVariable v, double c) { // body largely duplicated below
    ...
    ClLinearExpression setVariable(ClAbstractVariable v, double c) {
        //assert(c != 0.0);
        ClDouble coeff = _terms.get(v);
        if (coeff != null) coeff.setValue(c);
        else _terms.put(v, new ClDouble(c));
        return this;
    }
    ...
    public final
    ClAbstractVariable anyPivotableVariable() throws ExCLInternalError {
        if (isConstant()) {
    ...
    void changeSubject(ClAbstractVariable old_subject, ClAbstractVariable new_subject) {
        ...
        if (cld != null) cld.setValue(newSubject(new_subject));
        else _terms.put(old_subject, new ClDouble(newSubject(new_subject)));
    }
    ...
    double coefficientFor(ClAbstractVariable var) {
        ...
        if (coeff != null) return coeff.doubleValue();
        else return 0.0;
    }
    ...
    public final
    Map<ClAbstractVariable, ClDouble> terms() {
        return _terms;
    }
    ...
    String toString() {
        ...
        if (!CL.approx(_constant.doubleValue(), 0.0) || _terms.isEmpty()) {
    ...
    public static
    ClLinearExpression Times(ClLinearExpression e1, ClLinearExpression e2) throws ExCLNonlinearExpression {
        return e1.times(e2);
    }

    public static
    ClLinearExpression Divide(ClLinearExpression e1, ClLinearExpression e2) throws ExCLNonlinearExpression {
        return e1.divide(e2);
    }
    ...
    private final ClDouble _constant;

    private final Map<ClAbstractVariable, ClDouble> _terms; // from ClVariable to ClDouble
}
