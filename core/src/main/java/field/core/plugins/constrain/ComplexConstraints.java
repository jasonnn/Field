...
class ComplexConstraints implements iPlugin {
        ...
        <T> TraversalHint setProperty(IVisualElement source, VisualElementProperty<T> prop, Ref<T> to) {
            if (prop == BasicDrawingPlugin.frameManipulationBegin) {
                ...
                if (participatesInConstraints.get(source) != null) {
                    ...
                    Rect rect = new Rect(r.variableX.value(),
                                         r.variableY.value(),
                                         r.variableW.value(),
                                         r.variableH.value());
        ...
        TraversalHint shouldChangeFrame(IVisualElement source, Rect newFrame, Rect oldFrame, boolean now) {
            ...
            if (participatesInConstraints.get(source) != null) {
                ...
                if (currentlyMoving.contains(source)) {
                    ...
                    else ;//System.out.println(" error or nothing to do ?");
                }
                else {
    ...
    public static final VisualElementProperty<ComplexConstraints> complexConstraints_plugin =
            new VisualElementProperty<ComplexConstraints>("//plugin_complexConstraints");
    ...
    @Mirror
    protected
    void addEditFor(VariablesForRect r, Rect f) {
        try {
    ...
    @Mirror
    protected
    void addSuggestionFor(VariablesForRect r, Rect rr) {
    ...
    @Mirror
    protected
    void updateFrameFromVariables(VariablesForRect r) {
        Rect rect = new Rect(roundy(r.variableX.value()),
                             roundy(r.variableY.value()),
                             roundy(r.variableW.value()),
    ...
    @Mirror
    protected
    void updateVariablesFromFrame(VariablesForRect r, Rect q) {
        Rect oldRect = new Rect(roundy(r.variableX.value()),
                                roundy(r.variableY.value()),
                                roundy(r.variableW.value()),
                                roundy(r.variableH.value()));
