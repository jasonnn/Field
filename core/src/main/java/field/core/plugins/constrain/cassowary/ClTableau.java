...
class ClTableau extends CL {
    ...
    private final
    void insertColVar(ClAbstractVariable param_var, ClAbstractVariable rowvar) {
        Set rowset = _columns.get(param_var);
        if (rowset == null) _columns.put(param_var, rowset = new Set());
    ...
    protected final
    ClLinearExpression removeRow(ClAbstractVariable var) throws ExCLInternalError {
    ...
    protected final
    Hashtable<ClAbstractVariable, Set> columns() {
        return _columns;
    }

    protected final
    Hashtable<ClAbstractVariable, ClLinearExpression> rows() {
        return _rows;
    }
