...
class OSCInput implements iUpdateable {
    ...
    public
    interface DispatchableHandler extends Handler {
        public
        void handle(String s, Object[] args);
    }
     ...
     */
    public
    Handler setDefaultHandler(Handler h) {
    ...
    public
    Handler registerHandler(String dest, Handler h) {
    ...
    void dispatch(String dest, List args) {
        ...
        else if (defaultHandler != null) dispatchTo(defaultHandler, dest, args);
    }
    ...
    String readString(ByteBuffer b) {
        ...
        b.position(position);
        b.get(bytes);
        if (b.remaining() > 0) b.get(); // the null term
    ...
    public
    Double readDouble(ByteBuffer b) {
