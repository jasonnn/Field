...
class UDPNIOListener implements IUpdateable, Runnable {
     ...
     */
    public
    UDPNIOListener(int port, int maxPacketSize, int numBuffers) {
        this.channel = makeChannel(port);
        setupBuffersAndGo(maxPacketSize, numBuffers, true, false);
    }

    public
    UDPNIOListener(DatagramChannel channel, int maxPacketSize, int numBuffers) {
    ...
    public
    UDPNIOListener(int port, int maxPacketSize, int numBuffers, boolean useNativeBuffers, boolean flushData) {
        this.channel = makeChannel(port);
        setupBuffersAndGo(maxPacketSize, numBuffers, useNativeBuffers, flushData);
    }

    public
    UDPNIOListener(DatagramChannel channel,
                   int maxPacketSize,
                   int numBuffers,
    ...
    protected
    DatagramChannel makeChannel(int port) {
        this.port = port;
...
//			channel.connect(address);
    ...
    protected
    void setupBuffersAndGo(int maxPacketSize, int numBuffers, boolean useNativeBuffers, boolean flushData) {
    ...
    public
    SocketAddress whereDidThisBufferComeFrom() {
     ...
     */
    protected
    void processBuffer(ByteBuffer bb) {
    }
