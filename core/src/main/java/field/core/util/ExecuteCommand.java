...
class ExecuteCommand {
    ...
    public
    ExecuteCommand(String directory, String command) {
        this(directory, command, true);
    }

    public
    ExecuteCommand(String directory, String command, boolean redirectError) {
        String[] s = command.split(" ");
        go(directory, redirectError, s);
    }

    public
    ExecuteCommand(String directory, String[] s, boolean redirectError) {
        go(directory, redirectError, s);
    }
    ...
    public
    int waitFor(final boolean waitForOutput, long millis) {
        try {
    ...
    public
    int waitFor(boolean waitForOuput) {
        try {
            int rr = executing.waitFor();
            if (waitForOuput) t.join();
    ...
    private
    void go(String directory, boolean redirectError, String[] s) {
        ...
        if (redirectError) pb.redirectErrorStream();
        try {
            executing = pb.start();
