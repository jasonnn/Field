...
class Hungarian {
    ...
    private final IntBuffer tempDistance =
            ByteBuffer.allocateDirect(maxHungarianSize * maxHungarianSize * 4).asIntBuffer();
     ...
     */
    public
    IntBuffer maximumAssigment(FloatBuffer benifits, int m, int n, IntBuffer answer) {
    ...
    public
    IntBuffer minimumAssignment(FloatBuffer distances, int m, int n, IntBuffer answer) {
        if (true) throw new IllegalArgumentException(" this native code has changed");
...
//			tempDistance.put(i,(int)(0.05f*Integer.MAX_VALUE*(temp[0]-temp[1])/(distances.get(i)-temp[1]+1)));
            tempDistance.put(i,
                             (int) ((1.0f / distances.capacity()) * Integer.MAX_VALUE * (temp[0] - temp[1]) / (distances
                                                                                                                       .get(i)
                                                                                                               - temp[1]
                                                                                                               + 1)));
        }
