...
public
class Histogram<T> implements iHistogram<T>, Serializable {
    Map<T, MutableFloat> counts = new HashMap<T, MutableFloat>();

    float normalization = 0;

    public
    <X> X average(iAverage<? super T, X> average) {
        double total = 0;
    ...
    public
    float get(T t, float def) {
        MutableFloat f = counts.get(t);
        return (float) ((f == null) ? def : f.d);
    }

    public
    Set<Entry<T, MutableFloat>> getEntries() {
    ...
    public
    float getNormalized(T t, float def) {
    ...
    public
    Set<Entry<T, MutableFloat>> getSortedEntries() {
    ...
    public
    void incorporate(Histogram<T> h, float rawWeight, float normalizedWeight, float partialWeight) {
    ...
    public
    Histogram<T> mul(Histogram<T> h, float empty) {
...
//		counts.keySet().retainAll(h.counts.keySet());
    ...
    public
    void multiplyWeightBy(T t, float by) {
        MutableFloat f = counts.get(t);
        if (f == null) f = new MutableFloat(0);
        normalization -= f.d;
        f.d *= by;
        normalization += f.d;
    }
    ...
    T select() {
        ...
        while (i.hasNext()) {
            ...
            if (v <= (cum / normalization)) return e.getKey();
        }
