...
class PlaneCarver {
    ...
    public
    PlaneCarver(List<Vector3> points, float inside, int numInterations, int maxMembership, int minMembership) {
        ...
        while (true) {
            ...
            maxMembership = (int) Math.min(pop.size() * 0.9f,
                                           Math.min(ran.maxMembership * 1.1f, (int) (maxMembership * 0.95f) - 1));

            if (maxMembership < minMembership) break;
        }
    }
