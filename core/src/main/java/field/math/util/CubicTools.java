...
class CubicTools {
     ...
     * samples.
     *
     * @param alpha  The blending parameter. <code>alpha = 0</code> yields
    ...
    float cubicTCB(float before, float now, float next, float after, float alpha, Vector4 tcbm1, Vector4 tcbm2) {
        float t1 = (float) (0.5
                            * ((1 - tcbm2.x) * (1 - tcbm2.y) * (1 + tcbm2.z) * (next - now) + (1 - tcbm2.x)
                                                                                              * (1
                                                                                                 + tcbm2.y)
                                                                                              * (1 - tcbm2.z)
                                                                                              * (after - next))
                            * tcbm2.w);
        float t2 = (float) (0.5
                            * ((1 - tcbm1.x) * (1 + tcbm1.y) * (1 + tcbm1.z) * (now - before) + (1 - tcbm1.x)
                                                                                                * (1
                                                                                                   - tcbm1.y)
                                                                                                * (1 - tcbm1.z)
                                                                                                * (next - now))
    ...
    public static
    float hermiteExtrapolate(float t, float m0, float m1) {
        if (t >= 0 && t <= 1) {
            float t2 = t * t;
            float t3 = t2 * t;
    ...
    private static
    float checkNAN(float f, float x) {
        return Float.isNaN(f) ? x : f;
    }

    private static
    void cubic(float alpha, Vector4 before, Vector4 now, Vector4 next, Vector4 after, Vector4 out) {
        out.x = (cubic(alpha, before.x, now.x, next.x, after.x));
        out.y = (cubic(alpha, before.y, now.y, next.y, after.y));
        out.z = (cubic(alpha, before.z, now.z, next.z, after.z));
