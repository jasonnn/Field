...
class PlaneRansac {
    ...
    class Model {
        ...
        public
        Model(Triple<Vector3, Vector3, Vector3> t) {
        ...
        float distanceTo(Vector3 v) {
            ...
            if (offset.distanceFrom(v) < 1f) return Math.abs(d);
            else return Math.abs(d) + 1;
        }

        public
        Model(List<Vector3> v, Model m) {
            f = new FitBox();
            ...
            area = (axes[2].positiveLength - axes[2].negativeLength) * (axes[1].positiveLength
                                                                        - axes[2].negativeLength);
    ...
    public
    PlaneRansac(List<Vector3> points, float inside, int numIterations, int minMembership) {
        ...
        for (int i = 0; i < numIterations; i++) {
            ...
            if (num < minMembership) {
                ...
                if (num > maxMembership) maxMembership = num;
                continue;
            }
