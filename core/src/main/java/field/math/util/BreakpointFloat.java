...
class BreakpointFloat implements iBlendable<BreakpointFloat>, iHasScalar {
    ...
    public
    BreakpointFloat blendRepresentation_newZero() {
        BreakpointFloat f = new BreakpointFloat();
        return f;
    }

    public
    BreakpointFloat cerp(BreakpointFloat before,
                         float beforeTime,
                         BreakpointFloat now,
                         float nowTime,
                         BreakpointFloat next,
                         float nextTime,
                         BreakpointFloat after,
                         float afterTime,
                         float a) {
    ...
    public
    BreakpointFloat lerp(BreakpointFloat before, BreakpointFloat now, float a) {

        switch (before.next) {
            case cubic: {
                float x = LineUtils.evaluateCubicFrame(before.value,
                                                       before.controlNext,
                                                       now.controlPrevious,
                                                       now.value,
                                                       a);
    ...
    public
    BreakpointFloat setValue(BreakpointFloat to) {
        this.value = to.value;
