...
class FitBox {
    ...
    public
    Vector3 getCloseFit(Vector3 xyz, Axis[] a) {
        Vector3 at = new Vector3(center);

        for (int z = 0; z < a.length; z++) {
            Vector3.add(a[z].direction,
                        (xyz.get(z) * (a[z].positiveLength - a[z].negativeLength)) + a[z].negativeLength,
                        at,
                        at);
        }
        return at;
    ...
    public
    Vector3 getCloseFitNormalized(Vector3 xyz, Axis[] a) {
        Vector3 at = new Vector3(center);
    ...
    public
    void closeFit(Axis[] axis, List<Vector3> points) {
    ...
    public
    void medianFit(Axis[] axis, List<Vector3> points) {
        for (int z = 0; z < axis.length; z++) {
...
//			}
    ...
    public
    Collection<Vector3> extremalPoints(Axis[] along, List<Vector3> p) {
        ...
        for (int i = 0; i < maxis.length; i++) {
            if (maxis[i] != null) s.add(maxis[i]);
            if (minis[i] != null) s.add(minis[i]);
        }
    ...
    public
    void axesRescale(Vector3 from, Vector3 to, float scale1, float scale2, float scale3) {
        tmp.sub(from, center);
