...
class CoordinateFrame implements iCoordinateFrame.iMutable,
    ...
    public
    CoordinateFrame(Quaternion rotation, Vector3 translation, Vector3 scale) {
    ...
    public
    CoordinateFrame(Quaternion rotation, Vector3 translation) {
        this.setRotation(rotation);
        this.setTranslation(translation);
        isUnityScale = true;
    }
    ...
    public
    CoordinateFrame blendRepresentation_newZero() {
        return new CoordinateFrame();

    }

    public
    CoordinateFrame cerp(CoordinateFrame before,
                         CoordinateFrame now,
                         CoordinateFrame next,
                         CoordinateFrame after,
                         float a) {
        before.cleanTRS();
        now.cleanTRS();
        next.cleanTRS();
    ...
    public
    iCoordinateFrame.iMutable get(iCoordinateFrame.iMutable inplace) {
        if (inplace == null) return new CoordinateFrame(this.rotation, this.translation, this.scale);
        return inplace.setRotation(rotation).setTranslation(translation).setScale(scale);
    }

    public
    Matrix4 getMatrix(Matrix4 matrix) {
        cleanMatrix();
    ...
    Quaternion getRotation(Quaternion rotation) {
        cleanTRS();
        if (rotation == null) rotation = new Quaternion();
    ...
    public
    Vector3 getTranslation(Vector3 translation) {
        cleanTRS();
    ...
    public
    CoordinateFrame lerp(CoordinateFrame before, CoordinateFrame now, float a) {
        before.cleanTRS();
        now.cleanTRS();
    ...
    public
    CoordinateFrame multiply(iCoordinateFrame left, iCoordinateFrame right) {
    ...
    public
    CoordinateFrame setRotation(Quaternion rotation) {
        cleanTRS();
    ...
    public
    CoordinateFrame setScale(Vector3 scale) {
        cleanTRS();
    ...
    public
    CoordinateFrame setTranslation(Vector3 translation) {
        cleanTRS();
    ...
    public
    CoordinateFrame setValue(CoordinateFrame to) {
    ...
    public
    Vector3 transformDirection(Vector3 position) {
        cleanMatrix();
