...
public
class MatrixN implements Cloneable, Serializable {
    public static
    class DimensionMismatchException extends RuntimeException {
        public
        DimensionMismatchException(String s) {
            super(s);
        }
    ...
    class SingularMatrixNException extends RuntimeException {
        public
        SingularMatrixNException(String s) {
            super(s);
        }

        public
        SingularMatrixNException() {
        }
    }
    ...
    class NonSquareMatrixNException extends RuntimeException {
        public
        NonSquareMatrixNException(String s) {
            super(s);
        }

        public
        NonSquareMatrixNException() {
        }
    }
    ...
    class InPlaceMatrixNMultException extends RuntimeException {
        public
        InPlaceMatrixNMultException(String s) {
            super(s);
        }

        public
        InPlaceMatrixNMultException() {
        }
    }
     ...
     * Same as get(i,j)
     *
     * @throws java.lang.ArrayIndexOutOfBoundsException if index out of range
     */
    public final
     ...
     */
    public
    void writeToMatlabFile(String filename) {
        try {
     ...
     */
    public final
    void set(MatrixN m) throws DimensionMismatchException {
        if ((numColumns() != m.numColumns()) || (numRows() != m.numRows()) || (numColumns() == 0) || (numRows() == 0)) {
            throw new DimensionMismatchException("Matrices' dimensions must match for in-place set. This matrix "
                                                 + "was "
                                                 + numRows()
                                                 + 'x'
                                                 + numColumns()
     ...
     *
     * @throws java.lang.ArrayIndexOutOfBoundsException if m is wrong size
     */
    public final
    void makeTranspose(MatrixN m) {
        for (int i = 0; i < numRows(); i++)
    ...
    void resize(int rows, int cols) {
        if (numRows() == rows && numColumns() == cols) return;
    ...
    void roundZeros() {
        ...
        for (i = 0; i < numRows(); i++)
            for (j = 0; j < numColumns(); j++)
                if (Math.abs(rep[i][j]) < CLOSE_ENOUGH_TO_ZERO) rep[i][j] = 0.0;
    }
     ...
     */
    public
    void getColumn(int c, VectorN x) {
        for (int i = 0; i < numRows(); i++)
            x.set(i, get(i, c));
    }
     ...
     */
    public
    void setColumn(int c, VectorN x) {
     ...
     */
    public
    void setDiagonal(VectorN d) {
        int n = d.size();
        resize(n, n);
        int i, j;
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
                if (i == j) rep[i][j] = d.get(i);
                else rep[i][j] = 0.0;
            }
        }
    }
     ...
     * @throws DimensionMismatchException if <code>this</code> and
     *                                    <code>b</code> do not match in
     *                                    dimension.
     * @see MatrixN#add(MatrixN, MatrixN, MatrixN).
     */
    public
    MatrixN add(MatrixN b) throws DimensionMismatchException {
     ...
     */
    public
    MatrixN sub(MatrixN b) throws DimensionMismatchException {
        MatrixN out = new MatrixN(b.numRows(), b.numColumns());
        sub(this, b, out);
        return out;
    }
     ...
     */
    public
    VectorN preMult(VectorN a) throws DimensionMismatchException {
     ...
     */
    public
    VectorN postMult(VectorN a) throws DimensionMismatchException {
     ...
     * @throws DimensionMismatchException if this and A do not match in
     *                                    dimension.
     * @see MatrixN#mult(MatrixN, MatrixN, MatrixN)
     */
    public
    MatrixN preMult(MatrixN A) throws DimensionMismatchException {
     ...
     */
    public
    MatrixN postMult(MatrixN A) throws DimensionMismatchException {
    ...
    public
    LUDState decomposeLU() throws SingularMatrixNException {
        // do all work on copySource
        if (m_temp != null) m_temp.copyRepFrom(this);
        else m_temp = new MatrixN(this);

        if (vv_temp == null) vv_temp = new VectorN(numRows());
        ...
        for (i = 0; i < n; i++) {
            big = 0.0;
            for (j = 0; j < n; j++)
                if ((temp = Math.abs(m.rep[i][j])) > big) big = temp;
            if (big == 0.0) {
                // error("Singular
                // matrix in
                // decomposeLU.
        ...
        for (j = 0; j < n; j++) {
            ...
            indx[j] = imax;
            if (m.rep[j][j] == 0.0) m.rep[j][j] = TINY;
     ...
     *
     * @throws SingularMatrixNException  if singular matrix.
     * @throws NonSquareMatrixNException if non square
     */
    public
    MatrixN inverse() throws SingularMatrixNException, NonSquareMatrixNException {
     ...
     * @throws SingularMatrixNException  if singular matrix.
     * @throws NonSquareMatrixNException if not square.
     */
    public
    void inverse(MatrixN inv) throws SingularMatrixNException, NonSquareMatrixNException {
        int rows = numRows();
        int cols = numColumns();
        if (rows != cols) {
            // error("Can't invert
     ...
     * @throws DimensionMismatchException if args do not match in dimension.
     */
    public static
    void add(MatrixN a, MatrixN b, MatrixN out) throws DimensionMismatchException {
     ...
     */
    public static
    void sub(MatrixN a, MatrixN b, MatrixN out) throws DimensionMismatchException {
        if (!checkDims(a, b, out)) {
            // error("sub: can't add
            // dissimilar matrices");
            throw new DimensionMismatchException();
     ...
     */
    public static
    void elemMult(MatrixN a, MatrixN b, MatrixN out) throws DimensionMismatchException {
        if (!checkDims(a, b, out)) {
     ...
     * @param out vector where result is stored. out = v
     *            m
     * @throws DimensionMismatchException  if args do not match in dimension.
     * @throws InPlaceMatrixNMultException if out is one of other args.
     */
    public static
    void mult(VectorN a, MatrixN b, VectorN out) throws DimensionMismatchException, InPlaceMatrixNMultException {
        if (a == out) {
        ...
        if (a.dim() != b.numRows() || out.dim() != b.numColumns()) {
            throw new DimensionMismatchException("mult(Vec a, MatrixN b, Vec out): bad dimensions: "
                                                 + "1x"
                                                 + a.dim()
                                                 + " * "
                                                 + b.numRows()
     ...
     */
    public static
    void mult(MatrixN a, VectorN b, VectorN out) throws DimensionMismatchException, InPlaceMatrixNMultException {
        if (b == out) {
        ...
        if (a.numColumns() != b.dim() || out.dim() != a.numRows()) {
            throw new DimensionMismatchException("mult(MatrixN a, Vec b, Vec out): bad dimensions: "
                                                 + a.numRows()
                                                 + 'x'
                                                 + a.numColumns()
                                                 + " * "
     ...
     * @param a   matrix
     * @param b   matrix
     * @param out matrix where result is stored. out = a
     *            b
     * @throws DimensionMismatchException  if args do not match in dimension.
     * @throws InPlaceMatrixNMultException if out == a || out == b
     */
    public static
    void mult(MatrixN a, MatrixN b, MatrixN out) throws DimensionMismatchException, InPlaceMatrixNMultException {
        if (a == out || b == out) {
        ...
        if (a.numColumns() != b.numRows() || out.numRows() != a.numRows() || out.numColumns() != b.numColumns()) {
            throw new DimensionMismatchException("mult(MatrixN A, MatrixN B, "
                                                 + "MatrixN out): bad dimensions on "
     ...
     */
    public static
    double multTriple(VectorN a, MatrixN b, VectorN c) {
        if (a.dim() != b.numRows() || b.numColumns() != c.dim()) {
            throw new DimensionMismatchException("multTriple dimension mismatch "
                                                 + a.dim()
                                                 + "!="
                                                 + b.numRows()
                                                 + " || "
     ...
     */
    public
    double determinant() throws NonSquareMatrixNException, SingularMatrixNException {
        int rows = numRows();
        int cols = numColumns();
        if (rows != cols) {
     ...
     */
    protected static
    boolean checkDims(MatrixN a, MatrixN b, MatrixN out) {
        return (a.numRows() == b.numRows()
                && a.numRows() == out.numRows()
                && a.numColumns() == b.numColumns()
                && a.numColumns() == out.numColumns());
    }
     ...
     */
    public
    void increment(int r, int c, float f) {
        rep[r][c] += f;
    }

    public
    void mult(int r, int c, float f) {
        rep[r][c] += f;
    }
    ...
    class LUDState {
         ...
         */
        protected
        LUDState(MatrixN m, int[] permutation, int din) {
            lud = m;
         ...
         *
         * @param b input. The product of
         *          <code>A</code> and
         *          <code>x</code>.
         * @param x output. Solution
         *          stored here. Must
         *          match in dimension.
         * @throws DimensionMismatchException when x.dim() !=
         *                                    b.dim()
         */
        public
        void backSub(VectorN b, VectorN x) {
            if (b.dim() != x.dim()) {
