...
class IntersectionPrimatives {
    ...
    class LinePointIntersectionInfo {
        ...
        String toString() {
            return "intersection at <"
                   ...
                   + "\n  normalizedID ="
                   + normalizedIntersectionDistance
                   + "\n  distandalong line ="
                   + distanceAlongLine;
        }
    }
    ...
    LinePointIntersectionInfo lineSegmentToPoint(Vector3 lineStart, Vector3 lineEnd, Vector3 point) {
        ...
        if (info.distanceAlongLine < 0) info.distanceAlongLine = 0;
        if (info.distanceAlongLine > mag) info.distanceAlongLine = mag;
    ...
    public static
    LinePointIntersectionInfo lineToLine(Vector3 lineOrigin,
                                         Vector3 lineTangent,
                                         Vector3 lineFrom,
                                         Vector3 lineFromTangent) {
        LinePointIntersectionInfo info = new LinePointIntersectionInfo();
    ...
    public static
    LinePointIntersectionInfo lineToLineSegment(Vector3 lineOrigin,
                                                Vector3 lineTangent,
                                                Vector3 lineSegmentFrom,
                                                Vector3 lineSegmentFromTangent) {
        LinePointIntersectionInfo info = new LinePointIntersectionInfo();
    ...
    public static
    LinePointIntersectionInfo lineToLineSegment(Vector2 lineOrigin,
                                                Vector2 lineTangent,
                                                Vector2 lineSegmentFrom,
                                                Vector2 lineSegmentFromTangent) {
        LinePointIntersectionInfo info = new LinePointIntersectionInfo();
    ...
    public static
    LinePointIntersectionInfo lineToPlane(Vector3 lineOrigin,
                                          Vector3 lineTangent,
                                          Vector3 planeOrigin,
                                          Vector3 planeLeft,
                                          Vector3 planeUp) {
        ...
        if (Float.isNaN(lambda) || Float.isInfinite(lambda)) return null;
    ...
    public static
    Pair<Vector3, Vector3> triangleToTriangleToLineSegment(Vector3 v0,
                                                           Vector3 v1,
                                                           Vector3 v2,
                                                           Vector3 u0,
                                                           Vector3 u1,
        ...
        if (Math.abs(du0) < 1e-10) du0 = 0.0f;
        if (Math.abs(du1) < 1e-10) du1 = 0.0f;
        if (Math.abs(du2) < 1e-10) du2 = 0.0f;
        ...
        if (Math.abs(dv0) < 1e-10) dv0 = 0.0f;
        if (Math.abs(dv1) < 1e-10) dv1 = 0.0f;
        if (Math.abs(dv2) < 1e-10) dv2 = 0.0f;
        ...
        boolean coplanar = compute_intervals_isectline(v0,
                                                       v1,
                                                       v2,
                                                       vp0,
                                                       vp1,
    ...
    private static
    boolean compute_intervals_isectline(Vector3 VERT0,
                                        Vector3 VERT1,
                                        Vector3 VERT2,
                                        float VV0,
                                        float VV1,
    ...
    public static
    LinePointIntersectionInfo lineSegmentToLineSegment(Vector3 pkSegment0_Origin,
                                                       Vector3 pkSegment0_Direction,
                                                       float pkSegment0_Extent,
                                                       Vector3 pkSegment1_Origin,
                                                       Vector3 pkSegment1_Direction,
                                                       float pkSegment1_Extent) {
                    ...
                    if (fS1 >= -fExtDet1) {
                        ...
                        else // region 3 (side)
                        {
                            ...
                            if (fTmpS0 < -pkSegment0_Extent) {
                                ...
                                fSqrDist =
                                        fS0 * (fS0 - (((float) 2.0) * fTmpS0)) + fS1 * (fS1 + ((float) 2.0) * fB1) + fC;
                            }
                    ...
                    if (fS1 >= -fExtDet1) {
                        ...
                        // (side)
                        {
                            ...
                            if (fTmpS1 < -pkSegment1_Extent) {
                                fS1 = -pkSegment1_Extent;
                                fSqrDist =
                                        fS1 * (fS1 - (((float) 2.0) * fTmpS1)) + fS0 * (fS0 + ((float) 2.0) * fB0) + fC;
                            }
    ...
    public static
    LinePointIntersectionInfo lineSegmentToLineSegment(Vector2 pkSegment0_Origin,
                                                       Vector2 pkSegment0_Direction,
                                                       float pkSegment0_Extent,
                                                       Vector2 pkSegment1_Origin,
                                                       Vector2 pkSegment1_Direction,
                                                       float pkSegment1_Extent) {
    ...
    public static
    LinePointIntersectionInfo lineSegmentToPoint(Vector2 lineStart, Vector2 lineEnd, Vector2 point) {
        LinePointIntersectionInfo info = new LinePointIntersectionInfo();
    ...
    public static
    LinePointIntersectionInfo lineToTriangle(Vector2 start, Vector2 dir, Vector2 a, Vector2 b, Vector2 c) {
    ...
    public static
    LinePointIntersectionInfo lineToTriangle(Vector3 start, Vector3 dir, Vector3 a, Vector3 b, Vector3 c) {
    ...
    private static
    void projectOntoPlane(Vector3 o, Vector3 normal, Vector3 z) {
    ...
    Vector4 toQuadBarycentric(Vector3 v0, Vector3 v1, Vector3 v2, Vector3 v3, Vector3 v, Vector3 normal) {
        ...
        if (s0.mag() < 1e-6) return new Vector4(1, 0, 0, 0);
        if (s1.mag() < 1e-6) return new Vector4(0, 1, 0, 0);
        if (s2.mag() < 1e-6) return new Vector4(0, 0, 1, 0);
        if (s3.mag() < 1e-6) return new Vector4(0, 0, 0, 1);
