...
public final
class VectorN implements Cloneable, Serializable {
     ...
     *
     * @throws DimensionMismatchException if dimensions are wrong It IS safe for a == out or b == out.
     */
    public static
    void add(VectorN a, VectorN b, float w, VectorN out) {
     ...
     */
    public static
    void add(VectorN a, VectorN b, VectorN out) {
        if ((a.dim() != b.dim()) || (a.dim() != out.dim())) {
            throw new DimensionMismatchException("add: can't add Vec with different dimension");
        }
        for (int i = 0; i < a.dim(); i++) {
     ...
     */
    public static
    void hadd(VectorN in, Vector3 to, float weight, VectorN out) {
     ...
     */
    public static
    void lerp(VectorN from, float alpha, VectorN to, VectorN out) {
     ...
     */
    public static
    void sub(VectorN a, VectorN b, VectorN out) {
        if ((a.dim() != b.dim()) || (a.dim() != out.dim())) {
     ...
     */
    public
    VectorN(double x0, double x1, double x2) {
        this(3);
        set(0, (float) x0);
        set(1, (float) x1);
        set(2, (float) x2);
     ...
     */
    public
    VectorN(double x0, double x1, double x2, double x3) {
        this(4);
        set(0, (float) x0);
        set(1, (float) x1);
        set(2, (float) x2);
     ...
     *
     * @throws DimensionMismatchException for mismatch dimensions
     */
    public
    VectorN add(VectorN b) {
     ...
     *
     * @throws DimensionMismatchException if different dimension vectors
     */
    public
    double angleBetween(VectorN b) {
        double amag = this.mag();
        double bmag = b.mag();
        if ((amag == 0.0) || (bmag == 0.0)) return 0.0;
     ...
     *
     * @throws DimensionMismatchException if this.dim() != from.dim()
     */
    public
    void copyFrom(VectorN from) {
        if (dim() != from.dim()) {
     ...
     *
     * @throws IllegalArgumentException if this or b is not of dimension 3.
     */
    public
    VectorN cross(VectorN b) throws IllegalArgumentException {
        if ((dim() != 3) || (b.dim() != 3)) {
     ...
     */
    public
    void cross(VectorN b, VectorN out) throws IllegalArgumentException {
        if ((dim() != 3) || (b.dim() != 3)) {
     ...
     *
     * @throws DimensionMismatchException if different dimensions
     */
    public
    double distanceFrom(VectorN v) {
        if (dim() != v.dim()) {
    ...
    double dot(VectorN v) {
        if (dim() != v.dim()) throw new DimensionMismatchException("can't dot differing dimension vectors");
    ...
    @Override
    public
    boolean equals(Object otherVec) {
    ...
    public
    int maxIndex() {
        if (rep.length == 0) return -1;

        if (rep.length == 1) return 0;
     ...
     */
    public
    void outer(VectorN b, MatrixN out) {
        int xn = dimension();
        int yn = b.dimension();
        for (int i = 0; i < xn; i++)
            for (int j = 0; j < yn; j++)
    ...
    public
    void resize(int n) {
        if (rep.length == n) return;

        float[] new_rep = new float[n];
        int i;
        for (i = 0; i < n; i++) {
            if (i < rep.length) new_rep[i] = rep[i];
            else new_rep[i] = 0.0f;
        }
        rep = new_rep;
    }
     ...
     *
     * @throws DimensionMismatchException if <code>a</code> and <code>b</code> are of different dimensions
     */
    public
    void scale(VectorN a, VectorN b) {
     ...
     * Returns <code>this - b</code>
     *
     * @throws DimensionMismatchException for mismatched dimension
     */
    public
    VectorN sub(VectorN b) {
        VectorN out = new VectorN(this.dim());
        sub(this, b, out);
        return out;
    }
    ...
    Vector3 toVec3(Vector3 v) {
        ...
        if (dim() != 3) throw new DimensionMismatchException("Converting Vec to Vec3.");
