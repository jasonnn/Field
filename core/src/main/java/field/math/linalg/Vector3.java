...
class Vector3 implements java.io.Serializable,
    ...
    public
    Vector3(FloatBuffer positions, int vertexNumber) {
     ...
     */
    public final
    Vector3 add(Vector3 t1, Vector3 t2) {
        this.x = t1.x + t2.x;
        this.y = t1.y + t2.y;
        this.z = t1.z + t2.z;
        return this;
     ...
     */
    public final
    Vector3 sub(Vector3 t1, Vector3 t2) {
        this.x = t1.x - t2.x;
        this.y = t1.y - t2.y;
        this.z = t1.z - t2.z;
        return this;
     ...
     */
    public final
    Vector3 scale(float s, Vector3 t1) {
        this.x = s * t1.x;
        this.y = s * t1.y;
        this.z = s * t1.z;
        return this;
     ...
     */
    public final
    Vector3 scaleAdd(float s, Vector3 t1, Vector3 t2) {
        this.x = s * t1.x + t2.x;
        this.y = s * t1.y + t2.y;
        this.z = s * t1.z + t2.z;
        return this;
     ...
     */
    public final
    Vector3 scaleAdd(float s, Vector3 t1) {
     ...
     */
    public
    boolean epsilonEquals(Vector3 t1, float epsilon) {
        float diff;
     ...
     */
    public final
    void clamp(float min, float max, Vector3 t) {
        if (t.x > max) {
            x = max;
        }
        else if (t.x < min) {
     ...
     */
    public final
    void clampMin(float min, Vector3 t) {
        if (t.x < min) {
            x = min;
        }
        else {
     ...
     */
    public final
    void clampMax(float max, Vector3 t) {
        if (t.x > max) {
            x = max;
        }
        else {
     ...
     */
    public final
    Vector3 interpolate(Vector3 t1, Vector3 t2, float alpha) {
     ...
     */
    public final
    Vector3 interpolate(Vector3 t1, float alpha) {
    ...
    public
    Vector3 blendRepresentation_add(Vector3 a, Vector3 b, Vector3 o) {
        if (o == null) o = new Vector3();
    ...
    public
    Vector3 lerp(Vector3 from, Vector3 to, float a) {
        x = from.x * (1 - a) + to.x * (a);
        y = from.y * (1 - a) + to.y * (a);
        z = from.z * (1 - a) + to.z * (a);
        return this;
    ...
    // for representation
    public
    Vector3 blendRepresentation_multiply(float by, Vector3 in) {
        in.x = in.x * by;
        in.y = in.y * by;
        in.z = in.z * by;
        return in;
    ...
    public
    Vector3 blendRepresentation_newZero() {
        return new Vector3();
    }

    public
    Vector3 blendRepresentation_duplicate(Vector3 t) {
        return new Vector3(t);
    }

    public
    float distanceFrom(Vector3 from1, Vector3 from2) {
    ...
    public
    Vector3 setMagnitude(float f) {
        this.scale(f / this.mag());
        return this;
    }

    public
    Vector3 projectOut(Vector3 direction) {
    ...
    public
    Vector3 projectOut(Vector3 direction, float scale) {
        float magDir = direction.mag();
    ...
    PyObject __finditem__(PyObject key) {
        ...
        int start = ((PySlice) key).start instanceof PyNone ? 0 : ((PyInteger) ((PySlice) key).start).getValue();
        int stop = ((PySlice) key).stop instanceof PyNone ? 3 : ((PyInteger) ((PySlice) key).stop).getValue();
    ...
    public
    double distanceFromWeighted(Vector3 o, Vector3 w) {
        return (float) Math.sqrt((x - o.x) * (x - o.x) * w.x + (y - o.y) * (y - o.y) * w.y + (z - o.z)
                                                                                             * (z - o.z)
                                                                                             * w.z);
    ...
    public
    void setValue(FloatBuffer v, int vertex) {
        x = v.get(vertex * 3 + 0);
        y = v.get(vertex * 3 + 1);
        z = v.get(vertex * 3 + 2);
    }
    ...
    public
    Vector3 multThree(Vector3 abc, Vector3 a, Vector3 b, Vector3 c) {
    ...
    public
    Vector3 add(Vector3 value, float a) {
        this.x += value.x * a;
        this.y += value.y * a;
        this.z += value.z * a;
        return this;
    ...
    public
    Vector3 rotateBy(Vector3 axis, float angle) {
        return new Quaternion().set(axis, angle).rotateVector(this);
    }

    public
    Vector3 rotateBy(float angle) {
...
//		;//System.out.println(" returning <" + a + ">");
    ...
    public
    void generateComplementBasis(Vector3 rkU, Vector3 rkV) {
        float fInvLength;
    ...
    public
    Vector3 computeAnOrthogonalVector3() {
        int x = maxIndex();
        int m = minIndex();
    ...
    public
    Vector3 noise(Random r, float level) {
