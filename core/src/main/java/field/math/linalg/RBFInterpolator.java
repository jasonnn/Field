...
public
class RBFInterpolator implements Serializable {
    ...
    class Example implements Serializable {
        ...
        public
        double distanceFromExample(VectorN new_x) {
     ...
     * @param inputs           the domain points to interpolate between, given as a List of Vecs.
     * @param outputs          the function values corresponding to the points in <code>inputs</code>. Given as a List of Vecs. Must contain the same number of Vecs as <code>inputs</code>.
     */
    public
    RBFInterpolator(int input_dimension,
                    int output_dimension,
                    List<VectorN> inputs,
                    List<VectorN> outputs,
     ...
     */
    public
    void interpolate(VectorN x, VectorN y) {
     ...
     */
    public
    double interpolateComponent(VectorN x, int component) {
        return interpolateLinear(x, component) + interpolateRadial(x, component);
    }

    public
    double interpolateLinear(VectorN x, int j) {
    ...
    public
    double interpolateRadial(VectorN x, int j) {
    ...
    private
    VectorN getInputs(int i) {
        return getExample(i).getInput();
    }

    private
    VectorN getLinearWeights(int output) {
        return linear_weights.get(output);
    }

    private
    VectorN getOutputs(int i) {
        return getExample(i).getOutput();
    }

    private
    VectorN getRadialWeights(int output) {
    ...
    double linearBasis(VectorN x, int i) {
        if (i == 0) return 1.0;
        else return x.get(i - 1);
    }

    private
    MatrixN makeLinearDesignMatrixN() {
        int n = numExamples();
    ...
    private
    MatrixN makeRadialDesignMatrixN() {
        int i, j;
        int n = numExamples();
     ...
     */
    private
    List<VectorN> solveLinearPortion(List<VectorN> weights) {
     ...
     */
    private
    void solveRadialPortion(List<VectorN> weights, List<VectorN> residuals) {
        SVD svd = null;
        try {
