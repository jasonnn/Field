...
class SVD {
     ...
     */
    public
    SVD(MatrixN a) throws SVDException {
     ...
     *
     * @param range the matrix to store the basis matrix
     *              in.
     * @throws DimensionMismatchException if the argument is not mxn.
     */
    public
    void getRangeBasisMatrixN(MatrixN range) throws DimensionMismatchException {
        try {
            range.set(rangeBasis);
        } catch (DimensionMismatchException e) {
            throw new DimensionMismatchException("SVD.getRangeBasisMatrixN: required "
                                                 + numRows()
                                                 + 'x'
                                                 + numColumns()
                                                 + " matrix and got "
     ...
     * @throws DimensionMismatchException if w.size() < numColumns().
     */
    public
    void getSingularValues(VectorN w) throws DimensionMismatchException {
        if (w.size() < numColumns()) {
            throw new DimensionMismatchException("SVD.getSingularValues: expected Vec("
                                                 + numColumns()
                                                 + ") and got Vec("
                                                 + w.size()
                                                 + ')');
     ...
     * @throws DimensionMismatchException if nullspace is not nxn.
     */
    public
    void getNullspaceBasisMatrixN(MatrixN nullspace) {
        try {
            nullspace.set(nullspaceBasis);
        } catch (DimensionMismatchException e) {
            int n = rangeBasis.numColumns();
            throw new DimensionMismatchException("SVD.getNullspaceValues(): expected "
                                                 + n
                                                 + 'x'
                                                 + n
                                                 + " and got "
     ...
     */
    public
    MatrixN getNullspaceBasisMatrixN() {
     ...
     */
    public
    void setMaxIters(int max_iters) {
     ...
     */
    public
    void getNullspaceVector(int j, VectorN nullspace) {
     ...
     *
     * @param a the mxn matrix to decompose. m >= n.
     *          If not, use MatrixN.resize(m,m) to
     *          make a zero-padded square.
     * @throws SVDException if there is an error in the SVD
     *                      process.
     */
    public
    void performSVD(MatrixN a) throws SVDException {
        int m = a.numRows();
        int n = a.numColumns();
        /*
		 * if (m < n) throw new
     ...
     */
    public
    void setSingularThreshold(double thresh) {
     ...
     */
    public
    void setSingularThresholdAsTolerance(double tolerance) {
     ...
     * solve only.
     *
     * @param b the right hand side. Must be size() ==
     *          m.
     * @param x the x is placed in this object. It
     *          will be resize to n if it is not.
     * @throws DimensionMismatchException if b.size() != numRows().
     * @see #setSingularThreshold(double)
     * @see #setSingularThresholdAsTolerance(double)
     */
    public
    void solve(VectorN b, VectorN x) throws DimensionMismatchException {
        if (b.size() != numRows()) throw new DimensionMismatchException("SVD.solve: expected b vector of dimension "
                                                                        + numRows()
                                                                        + "and got one of dimension "
                                                                        + b.size());
     ...
     * @throws DimensionMismatchException if the dimensions of themultiply
     *                                    are wrong.
     */
    public static
    void calculateResidual(MatrixN a, VectorN x, VectorN b, VectorN r) throws DimensionMismatchException {
    ...
    private
    void thresholdSingularValues() {
        int i;
        int n = singularValues.size();
        for (i = 0; i < n; i++) {
            if (singularValues.get(i) < singular_threshold) threshSingularValues.set(i, 0.0);
            else threshSingularValues.set(i, singularValues.get(i));
        }
    }
     ...
     *
     * @param a the matrix to be SVD'd, m>=n (zero pad
     ...
     */
    private
    void svdcmp(double[][] a, int m, int n, float[] w, double[][] v) throws SVDException {
        boolean flag = false;
        int i, its, j, jj, k;
        int l = 0;
        int nm = 0;
        ...
        for (k = n - 1; k >= 0; k--) {
            for (its = 1; its <= max_iters; its++) {
                ...
                for (l = k; l >= 0; l--) {
					 ...
					 * safe.
					 */
                    if (Math.abs(rv1[l]) + anorm == anorm || nm == -1) {
                        flag = false;
                        break;
                    }
                ...
                if (its == max_iters) throw new SVDException("SVD.svdcmp(): no convergence after "
                                                             + max_iters
                                                             + " iterations.  Whassup wi' dat?");
                x = w[l];
                nm = k - 1;
                y = w[nm];
                g = rv1[nm];
    ...
    double pythag(double a, double b) {
        ...
        absa = Math.abs(a);
        absb = Math.abs(b);
        if (absa > absb) return absa * Math.sqrt(1.0 + square(absb / absa));
        else return (absb == 0.0 ? 0.0 : absb * Math.sqrt(1.0 + square(absa / absb)));
    }
