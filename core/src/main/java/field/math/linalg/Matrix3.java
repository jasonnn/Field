 ...
 */
public
class Matrix3 implements java.io.Serializable, Cloneable {
    ...
    static
    int compute_qr(double[] s, double[] e, double[] u, double[] v) {
    ...
    static
    double compute_rot(double f, double g, double[] sin, double[] cos, int index, int first) {
        int i__1;
        double d__1, d__2;
        double cs, sn;
        int i;
    ...
    static
    double compute_shift(double f, double g, double h) {
        double d__1, d__2;
    ...
    static
    void compute_svd(double[] m, double[] outScale, double[] outRot) {
        int i, j;
        double g, scale;
    ...
    //
    static
    void luBacksubstitution(float[] matrix1, int[] row_perm, float[] matrix2) {
        ...
        for (k = 0; k < 3; k++) {
            ...
            rv -= 3;
            matrix2[(cv + (3 * 1))] =
                    (matrix2[(cv + (3 * 1))] - (matrix1[rv + 2] * matrix2[cv + 3 * 2])) / matrix1[rv + 1];

            rv -= 3;
            matrix2[(cv + (4 * 0))] =
                    (matrix2[(cv + (3 * 0))] - (matrix1[rv + 1] * matrix2[cv + 3 * 1]) - (matrix1[rv + 2] * matrix2[cv
                                                                                                                    + 3
                                                                                                                      * 2]))
                    / matrix1[rv + 0];
    ...
    //
    static
    boolean luDecomposition(float[] matrix0, int[] row_perm) {
    ...
    static
    void mat_mul(float[] m1, float[] m2, float[] m3) {
        int i;
    ...
    static
    void svdReorder(double[] m, double[] t1, double[] t2, double[] scales, double[] outRot, double[] outScale) {
        ...
        if (almostEqual(Math.abs(scales[0]), Math.abs(scales[1])) && almostEqual(Math.abs(scales[1]),
                                                                                 Math.abs(scales[2]))) {
            for (i = 0; i < 9; i++) {
                outRot[i] = rot[i];
            }
    ...
    static
    void transpose_mat(double[] in, double[] out) {
        out[0] = in[0];
        out[1] = in[3];
        out[2] = in[6];
    ...
    static
    void transpose_mat(float[] in, float[] out) {
        out[0] = in[0];
        out[1] = in[3];
        out[2] = in[6];
     ...
     */
    public final
    void add(float scalar, Matrix3 m1) {
     ...
     */
    public
    boolean epsilonEquals(Matrix3 m1, float epsilon) {
        float diff;
     ...
     */
    public final
    void getColumn(int column, Vector3 v) {
        if (column == 0) {
            v.x = m00;
            v.y = m10;
            v.z = m20;
    ...
    public
    Vector3 getScale(Vector3 localScale) {
     ...
     */
    public final
    void mul(float scalar, Matrix3 m1) {
     ...
     */
    public final
    void mul(Matrix3 m1, Matrix3 m2) {
        if ((this != m1) && (this != m2)) {
     ...
     */
    public final
    void mulNormalize(Matrix3 m1, Matrix3 m2) {
     ...
     */
    public final
    void mulTransposeBoth(Matrix3 m1, Matrix3 m2) {
        if ((this != m1) && (this != m2)) {
     ...
     */
    public final
    void mulTransposeLeft(Matrix3 m1, Matrix3 m2) {
        if ((this != m1) && (this != m2)) {
     ...
     */
    public final
    void mulTransposeRight(Matrix3 m1, Matrix3 m2) {
        if ((this != m1) && (this != m2)) {
     ...
     *
     * @param m1 the matrix3d to be converted to float
     */
    public final
    void set(Matrix3 m1) {
        this.m00 = m1.m00;
     ...
     */
    public final
    void sub(Matrix3 m1, Matrix3 m2) {
        this.m00 = m1.m00 - m2.m00;
        this.m01 = m1.m01 - m2.m01;
        this.m02 = m1.m02 - m2.m02;
     ...
     */
    public final
    void transform(Vector3 t, Vector3 result) {
        float x, y, z;
        x = m00 * t.x + m01 * t.y + m02 * t.z;
        y = m10 * t.x + m11 * t.y + m12 * t.z;
     ...
     */
    final
    void getScaleRotate(float[] scales, float[] rots) {
