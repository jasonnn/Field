...
class TopologyMerging {
    ...
    class CompressRun<T extends NodeImpl<T>> implements iProvidesQueue {
                ...
                q = new GlossGraph<T>(q) {
                    ...
                    T createNewNodeFor(Collection<T> domain, Collection<T> fringe) {
                        ...
                        if (fringe.size() == 2) {
                            ...
                            T first, last;
                            first = last = di.next();
                            while (di.hasNext()) last = di.next();

                            if (first.getChildren().contains(b)) {
                                assert last.getChildren().contains(a) : last
                                                                        + " "
                                                                        + last.getChildren()
                                                                        + ' '
                                                                        + first
        ...
        public
        CompressRun<T> setBiconnect(boolean biconnect) {
    ...
    class EliminateShortBranches<T extends NodeImpl<T>> implements iProvidesQueue {
        ...
        public
        EliminateShortBranches(List<T> all) {
        ...
        List<T> perform() {
            ...
            for (T a : all) {
                if (!app.hasSeen(a)) app.apply(a);
                else ;//System.out.println("skipped <" + a + ">");
            }
            queue.update();
            return getAll();
        ...
        private
        boolean nothingConnectedTo(HashSet<T> all, NodeImpl n) {
            for (T t : all)
        ...
        @InQueue
        protected
        void eliminate(List<T> stack) {
            for (T t : stack)
                if (!all.contains(t)) return;
    ...
    class ExtractBorderOf<T> {
        ...
        public
        List<T> extractBorder(List<T> all) {
    ...
    class GlossGraph<T extends NodeImpl<T>> implements iProvidesQueue {
        ...
        public
        GlossGraph<T> setBiconnect(boolean biconnect) {
            this.biconnect = biconnect;
            return this;
        }
        ...
        protected
        T mergeNodes(Collection<T> domain, Collection<T> fringe) {
    ...
    class LineChaser<T> {
        ...
        public
        ArrayList<List<T>> chase(List<T> all) {
    ...
    class MakeSingleLoop<T extends NodeImpl<T>> {
        ...
        protected
        List<T> shortestPath(T at, final T to) {
            return new TopologyAStarSearch<T>(new TopologyViewOfGraphNodes<T>(),
                                              new TopologySearching.AStarMetric<T>() {
                                                  public
    ...
    class MakeSingleLoop2<T extends NodeImpl<T>> {
        ...
        protected
        List<T> shortestPath(T at, final T to, List<T> soFar2) {
            return new TopologySearching.TopologyAStarSearch<T>(new TopologyViewOfGraphNodes<T>(),
                                                                new TopologySearching.AStarMetric<T>() {
                                                                    public
                                                                    double distance(T from, T to) {
                                                                        return Math.sqrt(MakeSingleLoop2.this.distance(from,
                                                                                                                       to));
                                                                    }
    ...
    class MakeSinglyConnected_biconnected<T extends NodeImpl<T>> {
        ...
        void perform(List<T> all) {
            ...
            while (changed) {
                ...
                Collections.sort(domains, new Comparator<List<T>>() {
                    ...
                    public
                    int compare(List<T> o1, List<T> o2) {
        ...
        protected
        float distance(List<T> from, List<T> name) {
        ...
        protected
        NodeImpl<T>[] returnConnected() {
            return new NodeImpl[]{r1, r2};
        }

        protected
        T[] returnConnected(List<T> from, List<T> name) {
    ...
    class MakeSinglyConnected_biconnectedTopology<T> {
        ...
        protected
        Object[] returnConnected(List<T> from, List<T> name) {
    ...
    <T> List<List<T>> extractDomainsBiConnected(List<T> all, ITopology<T> top) {
        ...
        TopologySearching.TopologyVisitory_depthFirst<T> gather =
                new TopologySearching.TopologyVisitory_depthFirst<T>(true, top) {
                    @Override
                    protected
                    TraversalHint visit(T n) {
    ...
    boolean isEnd(NodeImpl n, List<? extends NodeImpl> stack) {
        return n.getChildren().size() == 0 || (n.getChildren().size() == 1
                                               && stack.size() > 2
                                               && n.getChildren().get(0) == stack.get(stack.size() - 2));
    }
    ...
    boolean isLineConnected(NodeImpl t) {
        ...
        // connected to
        // one child,
        if (t.getChildren().size() == 1) return true;
        // or it's
        // connected to
        // two, both of
        // which are
        ...
        // back to this
        // one
        if (t.getChildren().size() != 2) return false;
        return ((NodeImpl) t.getChildren().get(0)).getChildren().contains(t) || ((NodeImpl) t.getChildren()
                                                                                             .get(1)).getChildren()
                                                                                                     .contains(t);
    }
    ...
    boolean isLineConnectedAndNotEnd(NodeImpl t) {
        ...
        if (t.getChildren().size() == 1) {
            if (((NodeImpl) t.getChildren().get(0)).getChildren().contains(t)) return false;
            else return true;
        }
        // or it's
        // connected to
