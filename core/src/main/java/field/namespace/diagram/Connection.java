 ...
 */
public
class Connection<t_up, t_down> implements DiagramZero.iConnection {
    ...
    public
    Connection(iMarkerRef<t_up> from,
               final Object relationshipFrom,
               iMarkerRef<t_down> to,
               final Object relationshipTo) {
        ...
        up.addNotify(no_up = new DiagramZero.iMarkerRefNotify<t_up>() {
            ...
            public
            void markerRefNowInvalid(iMarkerRef ref) {
                if (wasValid) {
            ...
            public
            void markerRefNowDifferent(iMarkerRef<t_up> ref, iMarker<t_up> was) {
                if (wasValid) {
                    ...
                    if ((marker != null) && (marker instanceof Marker))
                        ((Marker) marker).addConnection(relationshipFrom, Connection.this);
                }
                else {
                }
            }
        ...
        down.addNotify(no_down = new DiagramZero.iMarkerRefNotify<t_down>() {
            ...
            public
            void markerRefNowDifferent(iMarkerRef<t_down> ref, iMarker<t_down> was) {
                if (wasValid) {
                    if ((was != null) && (was instanceof Marker)) ((Marker) was).removeConnection(relationshipTo);
                    iMarker<t_down> marker = ref.getMarker();
                    if ((marker != null) && (marker instanceof Marker))
                        ((Marker) marker).addConnection(relationshipTo, Connection.this);
                }
                else {
                }
            }
    ...
    public
    Connection(Channel.Marker from, final Object relationshipFrom, Channel.Marker to, final Object relationshipTo) {
        this(new UpdateableMarkerRef<t_up>(from),
             relationshipFrom,
             new UpdateableMarkerRef<t_down>(to),
             relationshipTo);
    }
    ...
    public
    void addNotify(iConnectionNotify notify) {
        this.notify.add(notify);
    }

    public
    void removeNotify(iConnectionNotify notify) {
