...
public
class Channel<T> implements DiagramZero.iChannel<T>, DiagramZero.iMarkerFactory<T>, Iterable<iMarker<T>> {
    ...
    public
    iChannel<T> getSlice(final double from, final double to) {
        if (!sliceIsGreedy) return new ChannelFilter(new iMarkerPredicate<T>() {
            public
            boolean is(iMarker<? extends T> marker) {
                return (marker.getTime() >= from) && (marker.getTime() < to);
            }
        }, this) {
            @Override
            protected
            List sublistRootStorage() {
    ...
    List propagateTo(String tag, Class clazz, Method method, Object... args) {
        if (tag.startsWith("channel")) return getChildrenChannels();
        else {
            return root;
        }
    }
    ...
    public
    Channel<T> setSliceIsGreedy(boolean sliceIsGreedy) {
    ...
    void add(Marker marker) {
        ...
        if (indiciesAreCorrect) for (int i = index; i < root.size(); i++)
            root.get(i).setIndex(i);
    ...
    protected
    void becomeUnsorted(Marker marker, double old, double at) {
        ...
        if (old < at) {
            ...
            // special case
            if (root.get(marker.index + 1).getTime() >= at) return;
    ...
    public
    class ChannelFilter implements DiagramZero.iChannel<T> {
        ...
        public
        ChannelFilter(final iMarkerPredicate<T> predicate) {
            ...
            channel.addNotify(notify = new aChannelNotify<T>() {
                ...
                void markerRemoved(iMarker<T> removed) {
                    if (predicate.is(removed)) dirty = true;
                }
            });
        }
        ...
        public
        ChannelFilter(final iMarkerPredicate<T> predicate, Channel<T> channel) {
            this.predicate = predicate;
        ...
        public
        ChannelFilter(final iMarkerPredicate<T> predicate, final ChannelFilter channel) {
            ...
            channel.addNotify(notify = new aChannelNotify<T>() {
                ...
                public
                void markerAdded(iMarker<T> added) {
                    if (ChannelFilter.this.predicate.is(added)) dirty = true;
                }

                @Override
                public
                void markerChanged(iMarker<T> changed) {
                    if (cachedStorage == null) return;
                ...
                public
                void markerRemoved(iMarker<T> removed) {
                    if (ChannelFilter.this.predicate.is(removed)) dirty = true;
                }
            });
        }

        public
        void addNotify(final iChannelNotify notify) {
        ...
        public
        void catchUpNotify(iChannelNotify notify) {
        ...
        iChannel<T> getSlice(final double from, final double to) {
            ...
            return new ChannelFilter(new iMarkerPredicate<T>() {
                public
                boolean is(iMarker<? extends T> marker) {
                    return predicate.is(marker)
                           && marker.getTime() + marker.getDuration() > from
                           && marker.getTime() < to;
                }
            }, this);
        }
        ...
        protected
        iChannelNotify unwrap(iChannelNotify notify) {
            return getWrappedNotifies().get(notify);
        }

        protected
        iChannelNotify wrap(final iChannelNotify notify) {
            return new iChannelNotify() {
                ...
                void endMarkerNotify() {
                    if (!first) notify.endMarkerNotify();
                    first = true;
                }

                public
                void markerAdded(iMarker added) {
                    if (predicate.is(added)) {
                        if (first) {
                            first = false;
                            notify.beginMarkerNotify();
                ...
                public
                void markerChanged(iMarker changed) {
                ...
                public
                void markerRemoved(iMarker removed) {
        ...
        BetterWeakHashMap<iChannelNotify, iChannelNotify> getWrappedNotifies() {
            return wrappedNotifies == null
                   ? wrappedNotifies = new BetterWeakHashMap<iChannelNotify, iChannelNotify>()
                   : wrappedNotifies;
        }

        private final
        class LocalMarkerIterator implements iMarkerIterator<T> {
    ...
    public
    interface iMarkerPredicate<T> {
        public
        boolean is(iMarker<? extends T> marker);
    }

    static public
    class Marker<T> implements DiagramZero.iMarker<T>, iContainer {
        ...
        public
        Marker(double at, double duration, T payload, Channel<T> channel) {
            this.channel = channel;
            this.t = at;
        ...
        public
        Marker(float at, float duration, T payload, Channel<T>.ChannelFilter channel) {
            this.channel = channel.channel;
            this.t = at;
            this.duration = duration;
        ...
        public
        void addConnection(Object label, iConnection c) {
        ...
        public
        void addNotify(iMarkerNotify notify) {
        ...
        public
        void removeConnection(Object relationshipFrom) {
        ...
        public
        void removeNotify(iMarkerNotify notify) {
            this.notify.remove(notify);
        }

        public
        void setDuration(double at) {
