...
public
class PredicateReference<T> extends UpdateableMarkerRef<T> {
    ...
    public
    PredicateReference(iChannel<T> from, iExtendedMarkerPredicate<T> predicate) {
    ...
    protected static
    <T> iMarker<T> scanAll(iChannel<T> from, iExtendedMarkerPredicate<T> predicate) {
        List<iMarker<T>> m = from.getIterator().remaining();

        iMarker<T> c = null;
        for (iMarker<T> marker : m) {
            PredicateReturn r = predicate.is(marker, c);
            if ((c == null) && (r == PredicateReturn.is)) c = marker;
