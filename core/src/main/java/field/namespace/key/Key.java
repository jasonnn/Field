...
class Key implements Serializable {
    ...
    public
    Key(String s, ContextTopology lt) {
        this.rep = s;
        ...
        if (o == null) internedKeys_byTree.put(lt, o = new HashMap());
    ...
    protected
    boolean generateAllocationStackTrace() {
    ...
    public
    StackTraceElement whereAllocated() {
    ...
    Class loadClass(Class c) {
        ...
        for (int i = 0; i < f.length; i++) {
            if (Modifier.isStatic(f[i].getModifiers())) try {
                f[i].get(null);
