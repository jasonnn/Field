...
class CKey extends Key {
    ...
    public
    CKey(String s, SimpleContextTopology ct) {
        super(s, ct);
    ...
    public
    CKey lookup(final String lookup) {
    ...
    public
    CKey with(Key k, Object e) {
        executionStack.add(new With(k, e));
        return this;
    }

    public
    CKey with(Key k, float e) {
        executionStack.add(new With(k, e));
        return this;
    }
    ...
    protected
    class With implements StackElement {
        Key k;
        Object value;

        public
        With(Key k, Object value) {
            this.k = k;
            this.value = value;
        }
        ...
        public
        Object exit(Object o) {
            if (oldValue != failure) localContextTree.set(k.getName(), oldValue);
            return o;
        }
    }
        ...
        Object base(Object o) {
            ...
            if (!cached) {
                ...
                if ((objectCall == null) && (floatCall == null) && (intCall == null) && (voidCall == null)) {
                    throw new PhiException("lookup inside BasePhi failed, couldn't find any declared method with parameter type Object, float, int or void",
            ...
            try {
                ...
                if (objectCall != null) o = objectCall.invoke(this, o);
                else if (floatCall != null) o = floatCall.invoke(this, new Float(toFloat(o)));
                else if (intCall != null) o = floatCall.invoke(this, new Integer((int) toFloat(o)));
                else if (voidCall != null) o = voidCall.invoke(this);
    ...
    protected
    Object run(Object o) {
        if (debug) ;//System.out.println(" run: stack is <" + executionStack + ">");
        ...
        for (int i = 0; i < executionStack.size(); i++) {
            ...
            o = e.enter(o);
            if (debug) ;//System.out.println("##### becomes o <" + o + "> after <" + e + ">");
        }
        //for (int i = 0; i < executionStack.size(); i++)
        for (int i = executionStack.size() - 1; i >= 0; i--) {
            StackElement e = (StackElement) executionStack.get(i);
    ...
    class PhiException extends RuntimeException {
        public
        PhiException(String message) {
            super(message);
        }

        public
        PhiException(String message, StackTraceElement[] e) {
            super(message + (e == null
                             ? "\n    turn on asserts for better stacktrace"
                             : "allocation stacktrace is <" + Arrays.asList(e) + '>'));
        }
    }
