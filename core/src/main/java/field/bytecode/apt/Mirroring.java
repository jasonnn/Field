...
class Mirroring {
    ...
    public
    interface iBoundFloatMember extends iFloatProvider, iBoundMember<Float> {
    }

    public
    interface iBoundDoubleMember extends iDoubleProvider, iBoundMember<Double> {
    }

    public
    interface iBoundMember<t_is> extends iProvider<t_is>, iAcceptor<t_is> {
    }

    public
    interface iBoundNoArgsMethod<t_returns> extends iProvider<t_returns>, iUpdateable {
    }

    public
    interface iMethodFunction<t_class, t_returns, t_accepts> {
        public
        <A extends t_class> iFunction<t_returns, t_accepts> function(A to);

        public
        <A extends t_class> iFunction<Collection<? extends t_returns>, t_accepts> function(final Collection<A> to);
    }
    ...
    class MirrorDoubleMember<t_class> extends MirrorMember<t_class, Double> {
        ...
        @Override
        public
        <A extends t_class> iBoundDoubleMember boundMember(final A to) {
    ...
    class MirrorMethod<t_class, t_returns, t_accepts> implements iMethodFunction<t_class, t_returns, t_accepts> {
        ...
        public
        <A extends t_class> iFunction<t_returns, t_accepts> function(final A to) {
            return new iFunction<t_returns, t_accepts>() {
                @SuppressWarnings("unchecked")
                public
                t_returns f(t_accepts in) {
        ...
        <A extends t_class> iFunction<Collection<? extends t_returns>, t_accepts> function(final Collection<A> to) {
            return new iFunction<Collection<? extends t_returns>, t_accepts>() {
                @SuppressWarnings("unchecked")
                public
                Collection<? extends t_returns> f(t_accepts in) {
                    // fixme,
                    // this
                    // could
                    // be
    ...
    public static
    class MirrorNoArgsMethod<t_class, t_returns> implements iMethodFunction<t_class, t_returns, Object>,
                                                            iFunction<t_returns, t_class> {
        protected Method method;
        ...
        public
        <A extends t_class> iFunction<t_returns, Object> function(final A to) {
            return new iFunction<t_returns, Object>() {
                @SuppressWarnings("unchecked")
                public
                t_returns f(Object in) {
                    return (t_returns) invoke(to, in);
                }
            };
        }
        ...
        public
        <A extends t_class> iFunction<Collection<? extends t_returns>, Object> function(final Collection<A> to) {
            return new iFunction<Collection<? extends t_returns>, Object>() {
                @SuppressWarnings("unchecked")
                public
                Collection<? extends t_returns> f(Object in) {
                    // fixme,
                    // this
                    // could
                    // be
    ...
    class MirrorNoReturnMethod<t_class, t_accepts> implements iMethodFunction<t_class, Object, t_accepts> {
        ...
        protected
        MirrorNoReturnMethod(Method method) {
            this.method = method;
            this.method.setAccessible(true);
        }
        ...
        public
        <A extends t_class> iFunction<Object, t_accepts> function(final A to) {
            return new iFunction<Object, t_accepts>() {
                @SuppressWarnings("unchecked")
                public
                Object f(t_accepts in) {
                    return invoke(to, in);
                }
            };
        }
        ...
        <A extends t_class> iFunction<Collection<?>, t_accepts> function(final Collection<A> to) {
            return new iFunction<Collection<?>, t_accepts>() {
                ...
                Collection<?> f(t_accepts in) {
                    if (to.isEmpty()) return Collections.EMPTY_LIST;
                    if (to.size() == 1) return Collections.singletonList(invoke(to.iterator().next(), in));
        ...
        <A> Object invoke(final A to, final Object... with) {
            try {
                ...
                System.err.println(" while invoking <"
                                   + method
                                   + "> on <"
                                   + to
                                   + "> with <"
    ...
    class MirrorNoReturnNoArgsMethod<t_class> implements iMethodFunction<t_class, Object, Object> {
        ...
        public
        <A extends t_class> iFunction<Object, Object> function(final A to) {
