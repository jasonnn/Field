...
class mRun<T> extends aRun {
    ...
    public
    mRun(Method m, boolean before) {
        this.m = m;
        this.before = before;
        // find a method with the same arguments
        this.localM = ReflectionTools.findMethodWithParameters(m.getName(),
                                                               m.getParameterTypes(),
                                                               ReflectionTools.getAllMethods(this.getClass()));
        if (this.localM == null) this.localM = ReflectionTools.findMethodWithParameters(m.getParameterTypes(),
                                                                                        this.getClass()
                                                                                            .getDeclaredMethods());
