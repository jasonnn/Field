...
public
class DeferedInQueue extends DeferCallingFast {

    private final boolean through;

    public
    DeferedInQueue(String name,
                   int access,
                   ASMMethod onMethod,
    ...
    class AtomicIntegerMap<T> {
        ...
        void inc(T t) {
            synchronized (m) {
                AtomicInteger o = m.get(t);
                if (o == null) m.put(t, new AtomicInteger(1));
                else {
                    o.incrementAndGet();
                }
            }
...
//		;//System.out.println(" exec :" + originalMethod + " " + inside.test(queue) + " " + through+" ");
    ...
    void go(final Object fromThis,
        ...
        try {
            ...
            System.err.println(" inside DeferedInQueue +"
                               + fromThis
                               + ' '
                               + originalMethod
                               + ' '
            ...
            if (out != null) queue.deregisterUpdateable(out);
            inside.dec(queue);
