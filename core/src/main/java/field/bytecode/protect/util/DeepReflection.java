...
class DeepReflection {
    ...
    private static final ThreadLocal<MyDeque> dq = new ThreadLocal<MyDeque>() {
        @Override
        protected
        MyDeque initialValue() {
            return new MyDeque();
        }
    };

    static
    class MyDeque extends ArrayDeque<Class> {

        public
        void pushAll(Class[] classes) {
    ...
    static
    void addDirectParents(Class c, MyDeque deq, boolean depthFirst) {
    ...
    static
    Collection<Class> getDirectParents(Class c) {
        Class sup = c.getSuperclass();
        Class[] inter = c.getInterfaces();
        if ((sup != null) && (sup != Object.class)) {
            if (inter.length > 0) {
    ...
    public static
    Set<Class> getInterfaces(Class base, Set<Class> accum, boolean maybeIncludeSelf, boolean depthFirst) {
    ...
    public static
    Set<Class> getSuperClasses(Class base) {
        return getSuperClasses(base, new LinkedHashSet<Class>(), true);
    }

    public static
    Set<Class> getSuperClasses(Class base, Set<Class> accum, boolean includeSelf) {
