...
class DynamicMesh_long implements iDynamicMesh, iInside, iRemoveable, iAcceptsSceneListElement {
    ...
    void close() {
        ...
        if (openCount == 0) {
            ...
            if ((shrink_policy_tries != 0) && ((closeCount % shrink_policy_tries) == 0)) {
                if (((vertexCursor / shrink_policy_min_fraction) < (cachedVertexBuffer.capacity() / 3))
                    || ((triangleCursor / shrink_policy_min_fraction) < (cachedTriangleBuffer.capacity()
                                                                         / elementSize))) {
                    shrink = true;
        ...
        if (openCount < 0) assert false : "to many closes";
    }
    ...
    @HiddenInAutocomplete
    public
    void copyFrom(SubMesh_long subMesh) {
    ...
    @HiddenInAutocomplete
    public
    void copyFrom(SubMesh_long subMesh, boolean canSkip) {

        if (subMesh.delegate.cachedVertexBuffer == null) return;
        if (subMesh.delegate.cachedTriangleBuffer == null) return;
        ...
        if (canSkip && !noCache) if (subMesh.copy != null) {
        ...
        for (int i = 0; i < subMesh.delegate.cachedAuxBuffer.length; i++) {
            if (subMesh.delegate.cachedAuxBuffer[i] != null) {
                ...
                subMesh.delegate.cachedAuxBuffer[i].limit(subMesh.delegate.vertexCursor
                                                          * subMesh.delegate.cachedAuxBufferWidth[i]);
            }
        }
        ...
        this.inject(subMesh.delegate.cachedVertexBuffer,
                    subMesh.delegate.cachedTriangleBuffer,
                    zeroTo15,
                    subMesh.delegate.cachedAuxBuffer,
                    0);
    ...
    private
    void debugMeshCaching(SubMesh_long subMesh, int vertexCursor) {
        ...
        try {
            ...
            if (cachedVertexBuffer == null) cachedVertexBuffer = basis.vertex();

            cachedVertexBuffer.position(vertexCursor * 3);

            if (cachedTriangleBuffer == null) cachedTriangleBuffer = basis.longTriangle();
    ...
    @HiddenInAutocomplete
    public
    Vector3 getPositionOfVertex(int v1, Vector3 v) {
        if (v == null) v = new Vector3();
        return v.set(cachedVertexBuffer.get(3 * v1),
                     cachedVertexBuffer.get(3 * v1 + 1),
                     cachedVertexBuffer.get(3 * v1 + 2));
    }
    ...
    @HiddenInAutocomplete
    public
    void hintPrimitiveType(int primType) {
    }

    @HiddenInAutocomplete
    public
    void inject(FloatBuffer vertex, IntBuffer triangle, int[] auxs, FloatBuffer[] auxBuffers, int oldVertexCursor) {
    ...
    private
    void safePut(IntBuffer to, IntBuffer from) {
    ...
    public
    int nextFace(int v1, int v2, int v3) {
        dirty = true;
        ...
        int[] rec;
        if (faceToVertex.size() <= triangleCursor) faceToVertex.add(rec = new int[3]);
        else rec = (int[]) faceToVertex.get(triangleCursor);
        rec[0] = v1;
        rec[1] = v2;
        rec[2] = v3;
    ...
    public
    void nextContour(List<Vector3> m) {
        ...
        if (t == null) t = new DynamicMesh_tessSupport(this);
     ...
     */
    public
    int nextFace(Vector3 v1, Vector3 v2, Vector3 v3) {
        dirty = true;
        checkTriangleStorage(triangleCursor, 1);
    ...
    public
    int nextVertex(float x, float y, float z) {
        dirty = true;
        checkVertexStorage(vertexCursor, 3);
        int vertexCursor3 = 3 * vertexCursor;
    ...
    void open() {
        if (openCount == 0) {
            ...
            if (copy == null) copy = new HashedCopy();
            else copy.reset();
        }
        openCount++;
    }
    ...
    public
    void setAux(int vertex, int id, float a) {
        dirty = true;
    ...
    public
    void setAux(int vertex, int id, float a, float b) {
        dirty = true;
    ...
    public
    void setAux(int vertex, int id, float a, float b, float c) {
        dirty = true;
    ...
    public
    void setAux(int vertex, int id, float a, float b, float c, float d) {
        dirty = true;
    ...
    @HiddenInAutocomplete
    public
    void setPositionOfVertex(int n, Vector3 v1) {
        dirty = true;
        checkVertexStorage(n, 3);
        int vertexCursor3 = 3 * n;

        assert vertexCursor3 < cachedVertexBuffer.capacity() : n
                                                               + " "
                                                               + vertexCursor3
                                                               + ' '
        ...
        cachedVertexBuffer.put(vertexCursor3 + 1, v1.get(1));
        cachedVertexBuffer.put(vertexCursor3 + 2, v1.get(2));
        if (vertexCursor < n + 1) vertexCursor = n + 1;
        clearCache();
    }

    @HiddenInAutocomplete
    public
    iDynamicMesh setShrinkPolicy(int shrink_policy_tries, float shrink_policy_min_fraction) {
    ...
    public
    int[] vertexForFace(int face) {
        return (int[]) faceToVertex.get(face);
    }

    protected
    void checkAuxStorage(int id, int width) {
    ...
    protected
    void checkTriangleStorage(int at, int plus) {
        if (cachedTriangleBuffer == null) cachedTriangleBuffer = basis.longTriangle();
        cachedTriangleBuffer.clear();
        if (cachedTriangleBuffer.capacity() < ((at + plus) * elementSize)) {
            basis.rebuildTriangle((int) (plus * elementSize
                                         + growthFactor * cachedTriangleBuffer.capacity() / elementSize));
            dirty = true;
    ...
    protected
    void checkVertexStorage(int at, int plus) {
        ...
        try {
            ...
            if (cachedVertexBuffer.capacity() < ((at * 3 + plus))) {
                ...
                basis.rebuildVertex((int) (plus / 3 + growthFactor * Math.max((at + plus / 3),
                                                                              cachedVertexBuffer.capacity() / 3)));
                ...
                for (int i = 0; i < cachedAuxBuffer.length; i++)
                    if (cachedAuxBuffer[i] != null) {
                        cachedAuxBuffer[i] =
                                basis.aux(i, cachedAuxBufferWidth[i]).put((FloatBuffer) cachedAuxBuffer[i].rewind());
                    }
            }
    ...
    @HiddenInAutocomplete
    public
    void nextFace(int v1) {
        dirty = true;
        if (elementAt == 2) {
            nextFace(elementFace[0], elementFace[1], v1);
            elementAt = 0;
        }
        else elementFace[elementAt++] = v1;
    }
    ...
    public
    void rectTextured(float x, float y, float w, float h, float tw, float th) {
        int a = nextVertex(x, y, 0);
        int b = nextVertex(x + w, y, 0);
        int c = nextVertex(x + w, y + h, 0);
        int d = nextVertex(x, y + h, 0);
