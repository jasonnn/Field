 ...
 */
public
class DynamicLine_long extends DynamicMesh_long implements iLineOutput {
    ...
    public
    DynamicLine_long(LineList_long from) {
        super(from);
        this.elementSize = 2;
    }
    ...
    @HiddenInAutocomplete
    public
    void beginSpline(iLineIdentifier identifier) {
    ...
    public
    void curveTo(Vector3 to, Vector3 cp1, Vector3 cp2, int samples) {
    ...
    public
    void setAuxOnLine(int id, float a1) {
        for (int m = lineStartAt; m < vertexCursor; m++)
            setAux(m, id, a1);
    }

    public
    void setAuxOnLine(int id, float a1, float a2) {
        for (int m = lineStartAt; m < vertexCursor; m++)
            setAux(m, id, a1, a2);
    }

    public
    void setAuxOnLine(int id, float a1, float a2, float a3) {
        for (int m = lineStartAt; m < vertexCursor; m++)
            setAux(m, id, a1, a2, a3);
    }

    public
    void setAuxOnLine(int id, float a1, float a2, float a3, float a4) {
        for (int m = lineStartAt; m < vertexCursor; m++)
    ...
    @Override
    public
    LineList_long getUnderlyingGeometry() {
    ...
    public
    int nextEdge(Vector3 v1, Vector3 v2) {
        ...
        int[] rec;
        if (faceToVertex.size() <= triangleCursor) faceToVertex.add(rec = new int[2]);
    ...
    public
    int nextFace_adjacency(int v1, int v2, int v3, int v4) {
        nextFace(v1, v2);
        nextFace(v3, v4);
        return triangleCursor;
    }
    ...
    void setAuxOnSpline(int id, float a1) {
        ...
        if (from == null) auxSplineCursorData[id] = from = new float[1];
    ...
    void setAuxOnSpline(int id, float a1, float a2) {
        float[] from = auxSplineCursorData[id];
        if (from == null) auxSplineCursorData[id] = from = new float[2];
    ...
    void setAuxOnSpline(int id, float a1, float a2, float a3) {
        float[] from = auxSplineCursorData[id];
        if (from == null) auxSplineCursorData[id] = from = new float[3];
    ...
    void setAuxOnSpline(int id, float a1, float a2, float a3, float a4) {
        ...
        if (auxSplineCursors[id] > vertexCursor) auxSplineCursors[id] = splineStartAt;
