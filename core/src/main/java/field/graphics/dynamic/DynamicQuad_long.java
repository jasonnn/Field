...
class DynamicQuad_long extends DynamicMesh_long {
    ...
    public
    DynamicQuad_long(QuadMesh_long from) {
        super(from);
        this.elementSize = 4;
    }
    ...
    @Override
    public
    QuadMesh_long getUnderlyingGeometry() {
    ...
    public
    int nextFace(int v1, int v2, int v3, int v4) {
        ...
        int[] rec;
        if (faceToVertex.size() <= triangleCursor) faceToVertex.add(rec = new int[4]);
