 ...
 */
public
class CompleteEdgeRep2 implements Serializable {
    ...
    class Edge implements Serializable {
        ...
        boolean equals(Object obj) {
            ...
            return (start.vertexNumber == o.start.vertexNumber && end.vertexNumber == o.end.vertexNumber)
                   || (start.vertexNumber == o.end.vertexNumber && end.vertexNumber == o.start.vertexNumber);
    ...
    public
    interface iNotification extends Serializable {
        ...
        public
        void faceAdded(Face f);

        public
        void faceRemoved(Face f);
        ...
        public
        void biFaceAdded(BiFace v);

        public
        void biFaceRemoved(BiFace v);
    }
    ...
    protected abstract
    class Opcode implements Serializable {
    ...
    CompleteEdgeRep2() {
        opcodeComparator = new Comparator() {
            ...
            int compare(Object o1, Object o2) {
                ...
                return ((Opcode) o1).priority < ((Opcode) o2).priority
                       ? 1
                       : (((Opcode) o1).priority == ((Opcode) o2).priority ? subpriority : -1);
            }
        };
    }
    ...
    public
    CompleteEdgeRep2 add(List<?> vertex, List<Integer> triangles) {
    ...
    public
    CompleteEdgeRep2 add(TriangleMesh_long from) {
    ...
    public
    BiFace addCompletelyNewBiFace(int innerFaceNumber) {
        beginOperation();
    ...
    public
    BiFace extendBiFaceEdge(BiFace biface, int edgeNumber) {
        ...
        while (found != edgeNumber) {
            assert x < f.edges.length : "couldn't find edge number <"
                                        + edgeNumber
                                        + "> that isn't <"
                                        + biface.hiddenEdge
                                        + "> in <"
                                        + Arrays.asList(f.edges)
                                        + '>';
     ...
     */
    public
    BiFace[] quadSectBiFace(BiFace biface) {
        ...
        while (i.hasNext()) {
            ...
            if (faceList.size() == 1) {
                ...
                assert nbf.hiddenEdge != null : face1
                                                + " "
                                                + face2
                                                + ' '
                                                + Arrays.asList(face1.edges)
                                                + ' '
                                                + Arrays.asList(face2.edges);
    ...
    public
    Vertex addVertexBisectingEdgeAndFaces(Edge bisect) {
        beginOperation();
        Vertex v = new Vertex(allVertex.size());
        vertexAdded(v);
        allVertex.add(v);
    ...
    Vertex[] sortNewVerticesFromExtendedBiFaceEdge(BiFace older, BiFace newer) {
        ...
        assert vertexOlder.size() == 4 : vertexOlder
                                         + " "
                                         + Arrays.asList(older.face1.vertex)
                                         + " "
                                         + Arrays.asList(older.face2.vertex);

        vertexNewer.addAll(Arrays.asList(newer.face1.vertex));
        vertexNewer.addAll(Arrays.asList(newer.face2.vertex));

        assert vertexNewer.size() == 4 : vertexNewer
                                         + " "
                                         + Arrays.asList(newer.face1.vertex)
                                         + " "
                                         + Arrays.asList(newer.face2.vertex);
    ...
    public
    Face hasFace(Vertex v1, Vertex v2, Vertex v3) {
    ...
    public
    Face addFaceOverEdges(Edge e1, Edge e2, Edge e3) {
        beginOperation();
    ...
    public
    Face addFaceOverVertices(Vertex v1, Vertex v2, Vertex v3) {
        beginOperation();
    ...
    public
    Face addNewFaceBetweenEdges(Edge e1, Edge e2) {
        Face newFace = new Face();
        allFace.add(newFace);
        faceAdded(newFace);
    ...
    public
    void catchupNotification(iNotification not) {
        for (int i = 0; i < allVertex.size(); i++)
            not.vertexAdded(allVertex.get(i));
        for (int i = 0; i < allEdge.size(); i++)
            not.edgeAdded(allEdge.get(i));
        for (int i = 0; i < allFace.size(); i++)
            not.faceAdded(allFace.get(i));
        for (int i = 0; i < allBiFace.size(); i++)
            not.biFaceAdded(allBiFace.get(i));
    }
...
//	}
    ...
    private
    Vertex vertex(Vertex[] alreadyHere, int voffset, int i) {
...
//	}
    ...
    public
    Face addVertexToMakeNewTriangle(Edge farEdge) {
        beginOperation();
    ...
    public
    boolean checkInvarients() {
        for (int i = 0; i < allFace.size(); i++) {
            Face f = allFace.get(i);
            for (int n = 0; n < f.edges.length; n++) {
                ...
                assert intersection.size() == 1 : intersection
                                                  + " "
                                                  + e.nonEdgeVertex
                                                  + " "
                                                  + Arrays.asList(f.vertex)
                                                  + " "
                                                  + Arrays.asList(f.edges)
                                                  + " "
                                                  + n;
            ...
            for (int n = 0; n < f.bifaces.size(); n++) {
                ...
                assert indexOf(b.hiddenEdge, b.face1.edges) != -1
                       && indexOf(b.hiddenEdge, b.face2.edges) != -1 : b.hiddenEdge
                                                                       + " "
                                                                       + Arrays.asList(b.face1.edges)
                                                                       + " "
                                                                       + Arrays.asList(b.face2.edges);
            }
            ...
            for (int n = 0; n < f.edges.length; n++) {
                Edge e = f.edges[n];
                if (!firstMention.contains(e.start)) firstMention.add(e.start);
                else assert secondMention.add(e.start) : "malformed face <"
                                                         + Arrays.asList(f.edges)
                                                         + "> <"
                                                         + Arrays.asList(f.vertex)
                                                         + ">";
                if (!firstMention.contains(e.end)) firstMention.add(e.end);
                else assert secondMention.add(e.end) : "malformed face <"
                                                       + Arrays.asList(f.edges)
                                                       + "> <"
                                                       + Arrays.asList(f.vertex)
                                                       + ">";
                assert e.start != e.end : "malformed edge <"
                                          + Arrays.asList(f.edges)
                                          + "> <"
                                          + Arrays.asList(f.vertex)
                                          + ">";

                assert indexOf(e.start, f.vertex) != -1 : "malformed face, edge referes to non-face vertex <"
                                                          + e
                                                          + "> <"
                                                          + Arrays.asList(f.vertex)
                                                          + ">";
                assert indexOf(e.end, f.vertex) != -1 : "malformed face, edge referes to non-face vertex <"
                                                        + e
                                                        + "> <"
                                                        + Arrays.asList(f.vertex)
                                                        + ">";
        ...
        for (int i = 0; i < allEdge.size(); i++) {
            ...
            for (int j = 0; j < e.faces.size(); j++) {
                Face f = e.faces.get(j);
                assert allFace.contains(f);
                assert (indexOf(e, f.edges) != -1) : Arrays.asList(f.edges)
                                                     + " <-"
                                                     + e
                                                     + " "
                                                     + f
    ...
    public
    Edge edge(int i) {
        return allEdge.get(i);
    }

    protected
    void edgeAdded(Edge e1) {
    ...
    private
    float faceArea(int i, int j, int k, FloatBuffer vertex3) {
    ...
    Face[] quadSectFace(Face f) {
        ...
        while (num < 3) {
            ...
            while (otherTwo.size() < 2) {
                assert checkInvarients() && at < allocated.length : " failed to find two unallocated edges "
                                                                    + Arrays.asList(edges)
                                                                    + " "
                                                                    + firstUnallocated;
                if (allocated[at] == false) {
                    if ((edges[at].start == firstUnallocated.start) || (edges[at].start == firstUnallocated.end)) {
                        for (int i = 0; i < edges[at].end.edges.size(); i++) {
                            Edge e = edges[at].end.edges.get(i);
                            if (e != edges[at]) {
                                if ((e.start == firstUnallocated.start)
                                    || (e.end == firstUnallocated.end)
                                    || (e.start
                                        == firstUnallocated.end)
                                    || (e.end == firstUnallocated.start)) if (indexOf(e, edges) != -1) {
                    ...
                    if ((edges[at].end == firstUnallocated.start) || (edges[at].end == firstUnallocated.end)) {
                        for (int i = 0; i < edges[at].start.edges.size(); i++) {
                            Edge e = edges[at].start.edges.get(i);
                            if (e != edges[at]) {
                                if ((e.start == firstUnallocated.start)
                                    || (e.end == firstUnallocated.end)
                                    || (e.start
    ...
    protected
    void removeDegenerateFaces(List possiblyDegenerate) {
        beginOperation();
        ...
        for (int i = 0; i < possiblyDegenerate.size(); i++) {
            ...
            for (int n = 0; n < 3; n++) {
                ...
                for (int e = 0; e < f.vertex[n].edges.size(); e++) {
                    Edge ed = f.vertex[n].edges.get(e);
                    if (s.containsKey(ed)) {
                        // we
                        // need
                        // to
    ...
    private
    void replaceEdge(Edge going, Edge with) {
    ...
    public
    void removeNotification(iNotification not) {
        notifications.remove(not);
    }

    public
    void removeOnlyFace(Face face) {
        ...
        for (int i = 0; i < face.edges.length; i++) {
            if (face.edges[i].faces.size() == 1) {
                ...
                else removeCount.put(face.edges[i].start.edges, new Integer(rc++));
                ...
                else removeCount.put(face.edges[i].end.edges, new Integer(rc++));
            }
        }
            ...
            if (face.edges[i].faces.size() == 1) {
                ...
                if (face.edges[i].start.edges.size() == 1) {
                    ...
                    for (int n = 0; i < v.edges.size(); i++) {
                        Edge ee = v.edges.get(n);
                        for (int m = 0; m < ee.faces.size(); m++) {
                            Face ff = ee.faces.get(m);
                ...
                else face.edges[i].start.edges.remove(face.edges[i]);
                ...
                else face.edges[i].end.edges.remove(face.edges[i]);
            }
            else {
    ...
    public
    void replaceVerticesWithVertex(List vertices, Vertex replacement) {
        beginOperation();
    ...
    public
    void resetVertexNumber() {
        // if (true) return;
        for (int i = 0; i < allVertex.size(); i++) {
            allVertex.get(i).vertexNumber = i;
        }
        for (int i = 0; i < allFace.size(); i++) {
            allFace.get(i).faceNumber = i;
        }
        for (int i = 0; i < allEdge.size(); i++) {
            allEdge.get(i).edgeNumber = i;
        }
    }

    public
    FloatBuffer returnVertex2FromDelaunayTriangulation() {
        return vertex2;
    }
    ...
    public
    BiFace newBiFace(Face f1, Face f2) {
        // find common edge
    ...
    Vertex vertex(int v1) {
        return allVertex.get(v1);
    }

    protected
    void vertexAdded(Vertex v1) {
        if (notifications.size() == 0) return;
    ...
    public
    CompleteEdgeRep2 duplicate(CompleteEdgeRep2 ret) {
        ret.beginOperation();
    ...
    public
    void addBiFaceOverFaces(Face f, Face f2, Edge longest2) {
        beginOperation();
    ...
    public static
    CompleteEdgeRep2 load(String filename) {
        try {
    ...
    void readObject(java.io.ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        opcodeComparator = new Comparator() {
            ...
            int compare(Object o1, Object o2) {
                return ((Opcode) o1).priority < ((Opcode) o2).priority
                       ? 1
                       : (((Opcode) o1).priority == ((Opcode) o2).priority ? 0 : -1);
            }
        };
    }
