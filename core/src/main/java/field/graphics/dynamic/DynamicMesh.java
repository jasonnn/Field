...
class DynamicMesh implements iDynamicMesh, iInside, iRemoveable, field.graphics.core.Base.iAcceptsSceneListElement {
    ...
    DynamicMesh coloredMesh(iAcceptsSceneListElement acceptsSceneListElement) {
        ...
        if (acceptsSceneListElement != null) acceptsSceneListElement.addChild(lines);
        DynamicMesh ret = new DynamicMesh(lines);
        return ret;
    }
    ...
    DynamicMesh coloredTexturedMesh(iAcceptsSceneListElement basicSceneList) {
        ...
        lines.rebuildVertex(0);
        lines.addChild(program);
        if (basicSceneList != null) basicSceneList.addChild(lines);
        DynamicMesh ret = new DynamicMesh(lines);
        return ret;
    }
...
//			}
            ...
            if ((shrink_policy_tries != 0) && (closeCount % shrink_policy_tries == 0)) {
                if ((vertexCursor / shrink_policy_min_fraction < cachedVertexBuffer.capacity() / 3) || (triangleCursor
...
//		;//System.out.println(" copy from <" + subMesh.delegate.cachedVertexBuffer + " " + subMesh.delegate.cachedTriangleBuffer);
    ...
    public
    void inject(FloatBuffer vertex, ShortBuffer triangle, int[] auxs, FloatBuffer[] auxBuffers, int oldVertexCursor) {
...
//		;//System.out.println(" next face <" + v1 + "> : " + elementAt + " " + elementFace[0] + " " + elementFace[1]);
    ...
    void checkTriangleStorage(int at, int plus) {
        ...
        if (cachedTriangleBuffer == null) cachedTriangleBuffer = basis.triangle();
