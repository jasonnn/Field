...
public
class StereoCamera extends BasicCamera {
    ...
    public
    StereoCamera setIOFrustra(float i) {
        this.io_frustra = i;
        return this;
    }

    public
    StereoCamera setIOPosition(Vector3 v) {
        this.io_position = new Vector3(v);
        return this;
    }

    public
    StereoCamera setIOPosition(float m) {
        this.io_position = new Vector3(m, m, m);
        return this;
    }

    public
    StereoCamera setIOLookAt(float x) {
        this.io_lookat = x;
        return this;
    }
    ...
    void performPass() {
...

        {
            ...
            CoreHelpers.gluLookAt(position.x + (flipped * (io_position.x) * FullScreenCanvasSWT.getSide().x * left.x),
                                  ...
                                  lookAt.z + (flipped * io_lookat * FullScreenCanvasSWT.getSide().x * left.z),
                                  up.x,
                                  up.y,
                                  up.z);
            ...
            if (!texture0IsRight)
                CoreHelpers.gluLookAt(position.x + (flipped * io_position.x * FullScreenCanvasSWT.getSide().x * left.x),
                                      ...
                                      up.y,
                                      up.z);
            else CoreHelpers.gluLookAt(position.x + (flipped * io_position.x * 1 * left.x),
                                       ...
                                       lookAt.z + (flipped * io_lookat * 1 * left.z),
                                       up.x,
                                       up.y,
                                       up.z);
        ...
        if (FullScreenCanvasSWT.getSide() == FullScreenCanvasSWT.StereoSide.left) randomSource.left();
        else if (FullScreenCanvasSWT.getSide() == FullScreenCanvasSWT.StereoSide.right) randomSource.right();
