...
public
class ConcatMeshBlast {

    private final ArrayList<Header> headers;
    private final FileChannel channelOut;

    public
    ConcatMeshBlast(String[] names, String out) throws IOException {
        ...
        for (String n : names) {
            ...
            for (Map.Entry<Integer, Integer> ii : h.maximumDimensions.entrySet()) {
                ...
                if (q == null) newMaxes.put(ii.getKey(), ii.getValue());
                else newMaxes.put(ii.getKey(), Math.max(ii.getValue(), q));
            }
            for (Map.Entry<Integer, Integer> ii : h.strides.entrySet()) {
                Integer q = newStrides.get(ii.getKey());
                if (q == null) newStrides.put(ii.getKey(), ii.getValue());
                else if (!q.equals(ii.getValue())) throw new IllegalArgumentException(" strides for <"
                                                                                      + ii.getKey()
                                                                                      + "> not compatible <"
                                                                                      + newStrides
                                                                                      + "> <"
    ...
    static public
    void concatSavedMeshSetIndex(String[] names, String out) {
