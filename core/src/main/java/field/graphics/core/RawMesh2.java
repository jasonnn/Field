...
class RawMesh2 extends OnePassListElement {
    ...
    public
    void setVertexPatch(int aux, int frame) {
    ...
    void doPerformPass() {
        ...
        glDrawRangeElements(GL_TRIANGLES,
                            0,
                            (int) frame.get(0).length / 3,
                            (int) frame.get(-1).length,
                            GL_UNSIGNED_INT,
                            0);
        glBindVertexArrayAPPLE(0);
    ...
    void doSetup() {
        ...
        for (int i = 1; i < 16; i++) {
            ...
            if (header.maximumDimensions.get(i) == null) continue;
    ...
    public
    iCoordinateFrame getCoordinateFrame() {
        if (inFrame == null) return new CoordinateFrame();
        else return inFrame.get();
    }
