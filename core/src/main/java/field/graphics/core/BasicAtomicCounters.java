 ...
 */
public
class BasicAtomicCounters extends TwoPassElement {
    ...
    public
    BasicAtomicCounters(int id, StandardPass prePass, StandardPass postPass, int num) {
    ...
    void post() {
        ...
        mapped = glMapBufferRange(ARBShaderAtomicCounters.GL_ATOMIC_COUNTER_BUFFER,
                                  0,
                                  num * 4,
    ...
    protected
    void pre() {
        glBindBufferBase(ARBShaderAtomicCounters.GL_ATOMIC_COUNTER_BUFFER, id, buffer);
    }
    ...
    void setup() {
        buffer = glGenBuffers();
        glBindBuffer(ARBShaderAtomicCounters.GL_ATOMIC_COUNTER_BUFFER, buffer);
        glBufferData(ARBShaderAtomicCounters.GL_ATOMIC_COUNTER_BUFFER, num * 4, GL15.GL_STREAM_DRAW);
        glBindBuffer(ARBShaderAtomicCounters.GL_ATOMIC_COUNTER_BUFFER, 0);
