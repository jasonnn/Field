...
public
class BasicCamera extends OnePassListElement implements IBasicCamera {

    @HiddenInAutocomplete
    public
    class FakeCamera extends OnePassElement {
    ...
    class Projector implements Serializable {
        ...
        boolean equals(Object obj) {
            ...
            if (m1 == null) {
                if (other.m1 != null) return false;
            }
            else if (!m1.equals(other.m1)) return false;
            if (p1 == null) {
         ...
         */
        public
        Vector3 toPixel(Vector3 m, float width, float height) {
            tmp.x = m.x;
            tmp.y = m.y;
            tmp.z = m.z;
            tmp.w = 1.0f;
            ...
            return new Vector3(width * (tmp.x / tmp.w / 2 + 0.5f),
                               height * (tmp.y / tmp.w / 2 + 0.5f),
                               tmp.z / tmp.w / 2 + 0.5f);
        }
         ...
         */
        public
        Vector3[] createIntersectionRay(float x, float y) {
         ...
         */
        public
        Vector3 fromPixel(float x, float y, Vector3 near) {
            Vector3[] o = createIntersectionRay(x, y);
            Vector3 centerNext = IntersectionPrimatives.lineToPoint(o[0],
                                                                    new Vector3(o[1]).sub(o[0]).normalize(),
                                                                    near).closestPoint;
            return centerNext;
        }
        ...
        @HiddenInAutocomplete
        public
        void createIntersectionRay(float x,
                                   float y,
                                   Vector3 rayIntersection1,
                                   Vector3 rayIntersection2,
                                   float width,
                                   float height) {
            if (model == null) {
    ...
    class State implements Serializable, IBlendable<State>, iExtensible {
         ...
         */
        public static
        State add(State a, double w, State to) {
            State o = new State();
         ...
         */
        public
        State setFancyBlend(boolean fancyBlend) {
         ...
         */
        static public
        State blend(State a, State b, double alpha) {
            State o = new State();
            ...
            if (a.fancyBlend || b.fancyBlend) {
                ...
                double distance =
                        a.position.distanceFrom(a.target) * (1 - alpha) + alpha * b.position.distanceFrom(b.target);
                q3 = q3.inverse();
        ...
        static public
        State blend2(State a, State b, double alpha) {
            State o = new State();
            o.position = new Vector3().lerp(a.position, b.position, (float) alpha);
            o.target = new Vector3().lerp(a.target, b.target, (float) alpha);
            o.up = new Vector3().lerp(a.up, b.up, (float) alpha);
            ...
            if (a.fancyBlend || b.fancyBlend) {
                ...
                {
                    Quaternion q3 =
                            new Quaternion().interpolate(new Quaternion(), new Quaternion(d2, d1), (float) alpha);
                    float distance = (float) (a.position.distanceFrom(a.target) * (1 - alpha)
                                              + alpha * b.position.distanceFrom(b.target));
                ...
                {
                    Quaternion q3 =
                            new Quaternion().interpolate(new Quaternion(), new Quaternion(b.up, a.up), (float) alpha);
        ...
        @HiddenInAutocomplete
        public
        Vector3 getPosition(Vector3 object) {
            if (object == null) object = new Vector3();
            return object.setValue(position);
        }

        @HiddenInAutocomplete
        public
        Vector3 getUp(Vector3 object) {
            if (object == null) object = new Vector3();
        ...
        @HiddenInAutocomplete
        public
        Vector3 getLeft(Vector3 object) {
            if (object == null) object = new Vector3();
        ...
        Matrix4 modelViewMatrix() {
            Matrix4 m = new Matrix4(BasicCamera.computeModelViewNow(position.x,
                                                                    position.y,
                                                                    position.z,
                                                                    target.x,
                                                                    target.y,
         ...
         */
        public
        State blendRepresentation_newZero() {
            return new State();
        }
         ...
         */
        public
        State lerp(State before, State now, float a) {
        ...
        State cerp(State before,
            ...
            r.fov = (float) CubicTools.cubic(a,
                                             before.fov,
                                             beforeTime,
                                             now.fov,
                                             nowTime,
                                             ...
                                             after.fov,
                                             afterTime);
            r.near = (float) CubicTools.cubic(a,
                                              before.near,
                                              beforeTime,
                                              now.near,
                                              nowTime,
                                              ...
                                              after.near,
                                              afterTime);
            r.far = (float) CubicTools.cubic(a,
                                             before.far,
                                             beforeTime,
                                             now.far,
                                             nowTime,
            ...
            r.rx = (float) CubicTools.cubic(a,
                                            ...
                                            after.rx,
                                            afterTime);
            r.aspect = (float) CubicTools.cubic(a,
                                                before.aspect,
                                                beforeTime,
                                                now.aspect,
                                                nowTime,
        ...
        public
        Vector3 getPointAlongLookAt(float towards) {
            return getPointAlongLookAt(null, towards);
        }

        @HiddenInAutocomplete
        public
        Vector3 getPointAlongLookAt(Vector3 output, float towards) {
        ...
        boolean equals(Object obj) {
            ...
            if (cmp(aspect, other.aspect)) return false;
            if (cmp(far, other.far)) return false;
            if (cmp(fov, other.fov)) return false;
            if (cmp(near, other.near)) return false;
            if (cmp(position, other.position)) return false;
            if (cmp(rx, other.rx)) return false;
            if (cmp(sx, other.sx)) return false;
            if (cmp(sy, other.sy)) return false;
            if (cmp(target, other.target)) return false;
            if (cmp(up, other.up)) return false;
            return true;
        }
     ...
     */
    public
    void copyTo(BasicCamera target) {
    ...
    @HiddenInAutocomplete
    public
    void createIntersectionRay(double x,
                               double y,
                               Vector3 rayIntersection1,
                               Vector3 rayIntersection2,
                               int width,
    ...
    @HiddenInAutocomplete
    public
    BasicCamera getCameraAlignedAxes(Vector3 upVec, Vector3 viewVec, Vector3 rightVec) {
        getUp(upVec);
        getViewRay(viewVec);
        viewVec.normalize();
    ...
    @HiddenInAutocomplete
    public
    double[] getCurrentModelViewMatrix(double[] mm) {

        if (modelView == null) return null;

        if (mm == null) mm = new double[16];
        for (int i = 0; i < 16; i++)
            mm[i] = modelView[i];
        return mm;
    }
    ...
    @HiddenInAutocomplete
    public
    float[] getCurrentModelViewMatrix(float[] p) {
        if (p == null) p = new float[16];
        if (modelView == null) {
    ...
    @HiddenInAutocomplete
    public
    float[] getCurrentModelViewMatrixNow(float[] p) {
        if (p == null) p = new float[16];
        model.rewind();
    ...
    @HiddenInAutocomplete
    public
    double[] getCurrentProjectionMatrix(double[] mm) {
    ...
    @HiddenInAutocomplete
    public
    float[] getCurrentProjectionMatrix(float[] p) {
        if (p == null) p = new float[16];
    ...
    @HiddenInAutocomplete
    public
    float[] getCurrentProjectionMatrixNow(float[] p) {
        if (p == null) p = new float[16];
     ...
     */
    public
    float getFrustrumShiftY() {
        return tshift;
    }

    @HiddenInAutocomplete
    public
    Vector3 getLookAt(Vector3 inplace) {
        if (inplace == null) inplace = new Vector3();
        inplace.set(lookAt);
        return inplace;
    }
    ...
    @HiddenInAutocomplete
    public
    void getModelMatrix(FloatBuffer model) {
        model.put(modelView);
    }

    @HiddenInAutocomplete
    public
    CoordinateFrame getModelViewMatrix() {
    ...
    @HiddenInAutocomplete
    public
    Vector3 getPosition(Vector3 inplace) {
        if (inplace == null) inplace = new Vector3();
    ...
    @HiddenInAutocomplete
    public
    void getProjectionMatrix(float[] o) {
        System.arraycopy(projection, 0, o, 0, o.length);
    }

    @HiddenInAutocomplete
    public
    void getProjectionMatrix(FloatBuffer model) {
     ...
     */
    public
    int[] getViewport(int[] viewport) {
        if (viewport == null) viewport = new int[4];
        viewport[0] = oX;
        viewport[1] = oY;
        viewport[2] = width;
        viewport[3] = height;
     ...
     */
    public
    Vector3 getViewRay(Vector3 inplace) {
    ...
    void performPass() {
        ...
        synchronized (BasicContextManager.gluLock) {
            ...
            boolean wasDirty = projectionDirty || modelViewDirty;
            {
                ...
                CoreHelpers.glFrustum(-right + (right * (rshift)),
                                      right + right * (rshift),
                                      -top + top * tshift,
                                      top + top * tshift,
                                      near,
...

            {
                ...
                CoreHelpers.gluLookAt(position.x,
                                      position.y,
                                      position.z,
                                      lookAt.x,
                                      lookAt.y,
            ...
            if (Base.trace) ;// System.out.println(" --------- applied camera <"
            // + this + "> (" + name + ")");
    ...
    @HiddenInAutocomplete
    public
    Vector2 project(Vector3 worldPt) {
    ...
    @HiddenInAutocomplete
    public
    Vector3 project3(Vector3 worldPt) {
     ...
     */
    public
    void rotateAroundCenter(float rad) {
    ...
    @HiddenInAutocomplete
    public
    void setFrustrumMul(float frustrumMul) {

        if (frustrumMul < 0.02) frustrumMul = 0.02f;
        if (frustrumMul > 30) frustrumMul = 30;
     ...
     */
    public
    void setFrustrumShift(float rightShift, float topShift) {

        if (rightShift > 30) rightShift = 30;
        if (rightShift < -30) rightShift = -30;
        if (topShift > 30) topShift = 30;
        if (topShift < -30) topShift = -30;

        if (!Float.isNaN(rightShift)
            && !Float.isInfinite(rightShift)
            && !Float.isNaN(topShift)
            && !Float.isInfinite(topShift)) {
     ...
     */
    public
    void setPerspective(float fov, float aspect, double near, float far) {
        projectionDirty = true;
        this.fov = fov;
        this.aspect = aspect;
    ...
    @HiddenInAutocomplete
    public
    Vector3 unProject(double x, double y, double distance) {
        projectionn.rewind();
        for (int i = 0; i < 16; i++)
            model.put(modelView[i]);
        double[] pm = new double[16];
    ...
    @HiddenInAutocomplete
    public
    Vector3 unProject(double x, double y, double[] plane) {
     ...
     */
    public
    BasicCamera setNearFar(double near, double far) {
        this.near = (float) near;
        this.far = (float) far;
        projectionDirty = true;
        modelViewDirty = true;
    ...
    @HiddenInAutocomplete
    public
    float[] computeModelViewNow() {

        if (modelView == null) return new Matrix4().get((float[]) null);
        // ;//System.out.println(" model view is <"+new
        // Matrix4(modelView)+">");

        float[] c =
                computeModelViewNow(position.x, position.y, position.z, lookAt.x, lookAt.y, lookAt.z, up.x, up.y, up.z);
        modelView = c;
    ...
    @HiddenInAutocomplete
    static public
    float[] computeModelViewNow(float eyex,
                                float eyey,
                                float eyez,
                                float centerx,
                                float centery,
