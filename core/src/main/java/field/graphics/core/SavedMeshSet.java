...
class SavedMeshSet {
    ...
    public
    SavedMeshSet(String rootFilename) {
    ...
    public
    Frame addFrameWithAux(List<TriangleMesh_long> from) {
        Frame f = new Frame();
        f.numMesh = from.size();
        ...
        for (TriangleMesh_long g : from) {
            ...
            if (g instanceof LineList_long) ((LineList_long) g).checkLine();
    ...
    public
    Frame addFrameWithAux(PointList g) {
        Frame f = new Frame();
        f.numMesh = 1;
    ...
    public
    Frame addFrameWithAuxNoTopology(LineList_long g) {
        Frame f = new Frame();
        f.numMesh = 1;
    ...
    public
    Frame addFrame(List<TriangleMesh_long> from) {
        Frame f = new Frame();
        f.numMesh = from.size();
    ...
    void save() throws IOException {
        ...
        for (int i = 0; i < frame.get(0).numMesh; i++) {
            ...
            for (int q = 0; q < frame.size(); q++) {
                ...
                if (m.numTriangle > numTriangle) numTriangle = m.numTriangle;
                if (m.numVertex > numVertex) numVertex = m.numVertex;
            }
            ...
            blast.configureChannel(-1,
                                   frame.get(0).meshes.get(i).primativeSize,
                                   numTriangle * frame.get(0).meshes.get(i).primativeSize);
    ...
    public
    void saveWithAux() throws IOException {
        new PythonUtils().persistAsXML(frame, rootFilename + ".index");
        for (int i = 0; i < frame.get(0).numMesh; i++) {
            MeshBlast blast = new MeshBlast(rootFilename + '.' + i + ".mesh");
    ...
    public
    void saveWithAuxNoTopology() throws IOException {
        new PythonUtils().persistAsXML(frame, rootFilename + ".index");
        for (int i = 0; i < frame.get(0).numMesh; i++) {
            MeshBlast blast = new MeshBlast(rootFilename + '.' + i + ".mesh");
