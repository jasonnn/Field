...
public
class LayeredFrameBuffer extends BasicTextures.BaseTexture implements iDisplayable, iHasFBO, iHasTexture {
    ...
    public
    LayeredFrameBuffer(int width, int height, int depth, boolean useFloat) {
        this.width = width;
        this.height = height;
...
//		System.out.println(" -- layeredframe buffer <" + this + "> display -- " + status + " " + GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT);
    ...
    void setup() {
        ...
        glTexImage3D(textureTarget,
                     0,
                     useFloat ? (BasicFrameBuffers.use32 ? GL_RGBA32F : GL_RGBA16F) : GL_RGBA,
                     width,
                     height,
                     depth,
                     0,
                     GL12.GL_BGRA,
                     useFloat ? (BasicFrameBuffers.use32 ? GL_FLOAT : GL_HALF_FLOAT) : GL_UNSIGNED_BYTE,
                     (ByteBuffer) null);
    ...
    iSceneListElement getOnscreenList(final int output,
        ...
        BasicGLSLangProgram onscreenProgram = (new BasicGLSLangProgram("content/shaders/NDCvertex.glslang",
                                                                       "content/shaders/PutImageProcessingOnscreenFragmentSquareArray.glslang"));
