...
class ShaderPreprocessor {
    ...
    public
    class AddExpression implements iExpressionHandler {
        public
        String handle(ShaderPreprocessor here, StringStream r, String expression, String[] parameters)
                throws PreprocessorException {
            if (parameters.length != 1)
                throw new PreprocessorException(" bad parameters for add <" + Arrays.asList(parameters) + '>');
    ...
    public
    class ConditionalAddExpression implements iExpressionHandler {
        public
        String handle(ShaderPreprocessor here, StringStream r, String expression, String[] parameters)
                throws PreprocessorException {
            if (parameters.length != 2) throw new PreprocessorException(" bad parameters for conditional add <"
    ...
    public
    class ConditionalNAddExpression implements iExpressionHandler {
        public
        String handle(ShaderPreprocessor here, StringStream r, String expression, String[] parameters)
                throws PreprocessorException {
            if (parameters.length != 2) throw new PreprocessorException(" bad parameters for conditional Nadd <"
                                                                        + Arrays.asList(parameters)
                                                                        + ">");
            iVariable v = here.getVariable(parameters[0]);
            String t = ShaderPreprocessor.readBlock(r);
    ...
    class DefaultVariable implements iVariable {
        ...
        public
        void add(String add) throws PreprocessorException {
            text += read(add) + "\n";
        }

        public
        void conditionalAdd(String condition, String add) throws PreprocessorException {
            if (conditions.contains(condition)) add(read(add));
        }

        public
        void conditionalNAdd(String condition, String add) throws PreprocessorException {
            if (!conditions.contains(condition)) add(read(add));
        }
    ...
    public
    class HandleExpression implements iExpressionHandler {
        public
        String handle(ShaderPreprocessor here, StringStream r, String expression, String[] parameters)
                throws PreprocessorException {
            if (parameters.length != 2)
                throw new PreprocessorException(" bad parameters for handle <" + Arrays.asList(parameters) + ">");

            Exception e2 = null;
            Exception e3 = null;

            try {
                iExpressionHandler e =
                        (iExpressionHandler) this.getClass().getClassLoader().loadClass(parameters[1]).newInstance();
            ...
            try {
                iExpressionHandler e = (iExpressionHandler) this.getClass()
                                                                .getClassLoader()
                                                                .loadClass("innards.graphics.basic." + parameters[1])
                                                                .newInstance();
                handlers.put(parameters[0], e);
                return "";
            } catch (ClassNotFoundException e) {
            ...
            throw new PreprocessorException(" couldn't instantiate <"
                                            + parameters[1]
                                            + "> exceptions are <"
                                            + e2
                                            + "> <"
    ...
    public
    class IncludeExpression implements iExpressionHandler {
        public
        String handle(ShaderPreprocessor here, StringStream r, String expression, String[] parameters)
                throws PreprocessorException {
            if (parameters.length != 1)
                throw new PreprocessorException(" bad parameters for include<" + Arrays.asList(parameters) + ">");
    ...
    interface iVariable {
        ...
        public
        void setCondition(String condition) throws PreprocessorException;
    }
    ...
    class PreprocessorException extends Exception {
        ...
        public
        PreprocessorException(IOException e) {
            initCause(e);
        }
    ...
    public
    class SetExpression implements iExpressionHandler {
        public
        String handle(ShaderPreprocessor here, StringStream r, String expression, String[] parameters)
                throws PreprocessorException {
            if (parameters.length != 2)
                throw new PreprocessorException(" bad parameters for set <" + Arrays.asList(parameters) + ">");
            iVariable v = here.getVariable(parameters[0]);
    ...
    public
    class UseExpression implements iExpressionHandler {
        public
        String handle(ShaderPreprocessor here, StringStream r, String expression, String[] parameters)
                throws PreprocessorException {
            if (parameters.length != 1 && parameters.length != 2)
                throw new PreprocessorException(" bad parameters for use <" + Arrays.asList(parameters) + ">");
    ...
    public
    ShaderPreprocessor(String textFileRoot) {
    ...
    public
    String filenameToString(String filename) throws PreprocessorException {
        try {
            BufferedReader reader = new BufferedReader(new FileReader(new File(textFileRoot + "/" + filename)));
            StringBuilder b = new StringBuilder();
            while (reader.ready()) b.append(reader.readLine() + "\n");
            return b.toString();
    ...
    public
    iVariable newDefaultVariable(String string) {
        return new DefaultVariable();
    }

    public
    String read(String text) throws PreprocessorException {
    ...
    private
    String handleExpression(StringStream reader) throws IOException, PreprocessorException {
        String opcode = "";
        while (reader.ready()) {
            char c = reader.read();
            if (c != '{') {
        ...
        if (handler == null) throw new PreprocessorException("unknown handler <"
                                                             + opcode
                                                             + "> <"
                                                             + expression
                                                             + "> <"
    ...
    String readBlock(StringStream reader) throws PreprocessorException {
        ...
        if (!"{".equals(skipped.trim()))
            throw new PreprocessorException(" parse error, junk characters <" + skipped + ">");
