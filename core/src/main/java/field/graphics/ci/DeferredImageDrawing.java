...
class DeferredImageDrawing {
    ...
    class ImageReference {
        ...
        public
        ImageReference(String filename) {
            this.filename = filename;
        }

        public
        ImageReference(String filename, Image master) {
            this.filename = filename;
            this.master = master;
        ...
        public
        Image getMasterImage(final CoreImageCanvasUtils utils) {
        ...
        public
        Destination getDestination(CoreImageCanvasUtils u) {
            if (destination == null) {
                if (master == null) getMasterImage(u);

                if (master == null) return null;

                destination = u.new Destination((int) extents.w, (int) extents.h);
                destination.moveCIImageToHere(master,
                                              (int) extents.x,
                                              (int) extents.y,
                                              (int) extents.w,
                                              (int) extents.h);
        ...
        public
        Thumbnail getLastDrawn() {
            return lastDrawn;
        }

        public
        Image getHalfImage(final CoreImageCanvasUtils utils) {
            ...
            if (master != null) {
                ...
                workerThread.new Task() {
                    ...
                    void run() {
                        ...
                        if (master == null) master = utils.new Image(filename);
        ...
        public
        Image getQuarterImage(final CoreImageCanvasUtils utils) {
        ...
        public
        Image getEighthImage(final CoreImageCanvasUtils utils) {
            lastDrawn = Thumbnail.eighth;

            if (eighth != null) return eighth;
        ...
        public
        Pair<Image, Float> getScaleOptimalImage(CoreImageCanvasUtils u) {
            ...
            if (previousScale > x * 0.75 && previousScale < x * 1.5) {
                ...
                if (lastDrawn == Thumbnail.full) return new Pair<Image, Float>(getMasterImage(u), 1f);
                else if (lastDrawn == Thumbnail.half) return new Pair<Image, Float>(getHalfImage(u), 0.5f);
                else if (lastDrawn == Thumbnail.quarter) return new Pair<Image, Float>(getQuarterImage(u), 0.25f);
                else if (lastDrawn == Thumbnail.eighth) return new Pair<Image, Float>(getEighthImage(u), 0.125f);
                return null;
            }
            ...
            if (lower == null) lower = Thumbnail.eighth;

            if (lower == Thumbnail.full) {
                if (master != null) return new Pair<Image, Float>(getMasterImage(u), 1f);
                else {
                    new Pair<Image, Float>(getMasterImage(u), 1f);
                    if (half != null) return new Pair<Image, Float>(getHalfImage(u), 0.5f);
                    else if (quarter != null) return new Pair<Image, Float>(getQuarterImage(u), 0.25f);
                    else if (eighth != null) return new Pair<Image, Float>(getEighthImage(u), 0.125f);
                    return new Pair<Image, Float>(getMasterImage(u), 1f);
                }
            }
                ...
                if (half != null) return new Pair<Image, Float>(getHalfImage(u), 0.5f);
                    ...
                    if (quarter != null) return new Pair<Image, Float>(getQuarterImage(u), 0.25f);
                    else if (eighth != null) return new Pair<Image, Float>(getEighthImage(u), 0.125f);
                ...
                if (quarter != null) return new Pair<Image, Float>(getQuarterImage(u), 0.25f);
                    ...
                    if (eighth != null) return new Pair<Image, Float>(getEighthImage(u), 0.125f);
                    if (half != null) return new Pair<Image, Float>(getHalfImage(u), 0.5f);
            ...
            void run() {
                while (true) {
                    try {
                        ...
                        System.err.println("exceptioin <"
                                           + t
                                           + "> inside deferredimagedrawing worker thread ------------------------------------");
                        t.printStackTrace();
                    }
                }
            }
    ...
    public
    class DrawsImage implements iDrawingAcceptor<CachedLine> {
        private final BaseGLGraphicsContext context;

        public
        DrawsImage(BaseGLGraphicsContext context) {
            ...
            DrawingResult result = new DrawingResult(DrawingResultCode.cont, new IUpdateable() {
                        ...
                        if (e.attributes != null && e.attributes.get(iLinearGraphicsContext.image_v) != null) {
                            ...
                            m.register("o" + System.identityHashCode(e), new IUpdateable() {
                                ...
                                void update() {
                                    ...
                                    if (zr instanceof Number) z = ((Number) zr).floatValue();
                                    else if (zr instanceof Vector3) z = ((Vector3) zr).x;

                                    if (zr != null) render(a.x,
                                                           a.y,
                                                           z,
                                                           ...
                                                           e.attributes.get(image3d_right),
                                                           e.attributes.get(image3d_up));
                                    else render(a,
                                                e.attributes.get(iLinearGraphicsContext.image_v),
                                                colorFor(e),
                                                opacityMul,
                                    ...
                                    if (side[0] == GL11.GL_BACK || side[0] == GL11.GL_BACK_RIGHT) m.remove();
                                }
                ...
                void render(float x,
                    ...
                    if (image instanceof ImageReference) {
                        ...
                        if (right == null) right = new Vector3(1, 0, 0);
                        ...
                        if (dest != null) {
                            ...
                            dest.drawIntoCanvasNow(new Vector3(x, y, z).add(right, (float) -e.w * 0.5f)
                                                                       .add(up, (float) -e.h * 0.5f),
                                                   new Vector3(x, y, z).add(right, (float) e.w * 0.5f)
                                                                       .add(up, (float) -e.h * 0.5f),
                                                   new Vector3(x, y, z).add(right, (float) e.w * 0.5f)
                                                                       .add(up, (float) e.h * 0.5f),
                                                   new Vector3(x, y, z).add(right, (float) -e.w * 0.5f)
                                                                       .add(up, (float) e.h * 0.5f),
                                                   new Vector4(1, 1, 1, opacityMul));
                ...
                private
                void render(Vector2 destination, Object image, Vector4 colorFor, float opacityMul, Number v) {
                    ...
                    if (image instanceof ImageReference) {
                        ...
                        Rect e = ir.extents;
                        if (e != null) {
                            if (shouldCull(destination, e)) return;
                        }

                        if (shouldCull(destination)) return;

                        Pair<Image, Float> image2 = ir.getScaleOptimalImage(utils);

                        if (image2.left != null) {
                            if (shouldCull(destination, ir.extents)) return;
                            //System.out.println(" rendering image with scale <" + 1 / image2.right + ">");
                            image2.left.drawNow(destination.x, destination.y, 1 / image2.right);
                            ir.uploaded = true;
                        }
                        else {
                            //System.out.println(" deferred rendering system \u2014 missing image <" + ir + ">");
                        ...
                        if (v == null) ((Image) image).drawNow(destination.x, destination.y);
                        else ((Image) image).drawNow(destination.x, destination.y, v.floatValue());
                    }
                    else if (image instanceof Accumulator) {
                        if (v == null) ((Accumulator) image).getOutputImage().drawNow(destination.x, destination.y);
                        else ((Accumulator) image).getOutputImage()
                                                  .drawNow(destination.x, destination.y, v.floatValue());
                    }
                }
        ...
        protected
        boolean shouldCull(Vector2 destination) {
        ...
        protected
        boolean shouldCull(Vector2 destination, Rect e) {
    ...
    static public
    Vector4 colorPickFromImage(BaseGLGraphicsContext installedContext, Vector2 position) {
        Set<CachedLine> lines = installedContext.getAllLines();
        for (CachedLine c : lines) {
            if (!c.getProperties().isTrue(iLinearGraphicsContext.containsImages, false)) continue;
            ...
            for (Event e : c.events) {
                if (e.attributes != null && e.attributes.get(iLinearGraphicsContext.image_v) != null) {
                    ...
                    Vector4 v = pick(position,
                                     e.getDestination(new Vector2()),
                                     e.attributes.get(iLinearGraphicsContext.image_v));
                    if (v != null) return v;
                }
            }
    ...
    private static
    Vector4 pick(Vector2 target, Vector2 point, Image image) {
        ...
        if (!x.isInside(target)) return null;
    ...
    public
    void installInto(BaseGLGraphicsContext installedContext, CoreImageCanvasUtils utils) {
        this.utils = utils;
