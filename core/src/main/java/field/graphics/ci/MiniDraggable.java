...
//			Double p = new Rectangle2D.Double(hitBox.x, hitBox.y, hitBox.w, hitBox.h);
    ...
    private
    SubControl hit(MouseEvent e) {
        for (SubControl c : controls) {
            if (derelativize(c.hitBox).insetAbsolute(-4).isInside(new Vector2(e.getX(), e.getY()))) return c;
        }
        return null;
    }

    protected
    Vector2 relativize(Vector2 v) {
        return v;
    }

    protected
    Vector2 derelativize(Vector2 v) {
        return v;
    }
