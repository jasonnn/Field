...
class SimpleImageDrawing {
            ...
            DrawingResult result = new DrawingResult(DrawingResultCode.cont, new iUpdateable() {
                ...
                void render(Vector2 destination, Object image, Vector4 colorFor, float opacityMul, Number v) {
                    ...
                    if (image instanceof Image) {
                        ...
                        if (useRawDrawing) {
                            if (v == null) ((Image) image).drawNowRaw(destination.x, destination.y, 1);
                            else ((Image) image).drawNowRaw(destination.x, destination.y, v.floatValue());
                        ...
                        if (useRawDrawing) {
                            if (v == null)
                                ((Accumulator) image).getOutputImage().drawNowRaw(destination.x, destination.y, 1);
                                                      ...
                                                      .drawNowRaw(destination.x, destination.y, v.floatValue());
                        }
                        else {
    ...
    public
    SimpleImageDrawing installInto(BaseGLGraphicsContext installedContext) {
        installedContext.addAcceptor(new DrawsImage(installedContext));
        return this;
    }

    public
    SimpleImageDrawing setUseRawDrawing(boolean useRawDrawing) {
