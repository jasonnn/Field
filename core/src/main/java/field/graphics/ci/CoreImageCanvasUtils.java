...
class CoreImageCanvasUtils {
    ...
    class Destination {
        ...
        public
        void drawIntoCanvasNow(Vector3 a, Vector3 b, Vector3 c, Vector3 d, Vector4 tint) {
        ...
        public
        void moveCIImageToHere(Image c, int width, int height) {
            if ((drawn != null) && (drawn.coreimage == c.coreimage)) return;
        ...
        public
        void moveCIImageToHere(Image c, int x, int y, int width, int height) {
            if ((drawn != null) && (drawn.coreimage == c.coreimage)) return;
        ...
        void finalize() throws Throwable {
            ...
            if (deletionQueue != null) fbo.delete(deletionQueue);
            super.finalize();
        }
    }
    ...
    public
    class Filter implements iHandlesAttributes {
        ...
        void set(String key, float value) {
            if (checkInput(key)) new CoreImage().filter_setValueFloat(filter, key, value);
        }

        public
        void set(String key, Image value) {
            if (value.coreimage == 0) {
                throw new IllegalArgumentException("No image for '" + key + '\'');
            }
            if (checkInput(key)) new CoreImage().filter_setValueImage(filter, key, value.coreimage);
        }

        public
        void set(String key, Vector2 value) {
            if (checkInput(key)) new CoreImage().filter_setValueVector2(filter, key, value.x, value.y);
        }
        ...
        void set(String key, Rect value) {
            if (checkInput(key)) new CoreImage().filter_setValueVector4(filter,
                                                                        key,
                                                                        (float) value.x,
                                                                        (float) value.y,
                                                                        (float) value.w,
        ...
        void setAttribute(String name, Object value) {
            if (value instanceof Number) set(name, ((Number) value).floatValue());
            else if (value instanceof Vector2) set(name, ((Vector2) value));
            else if (value instanceof Image) set(name, ((Image) value));
            else if (value instanceof Vector4) set(name, ((Vector4) value));
            else if (value instanceof Rect) set(name, ((Rect) value));
            else throw new ClassCastException(" can't handle type <" + value.getClass() + '>');
        }

        private
        boolean checkInput(String key) {
            return inputs().contains(key);
        }

        private
        boolean checkOutput(String key) {
    ...
    class Accumulator {
        ...
        public
        Accumulator(double x, double y, double w, double h, boolean isFloat) {
    ...
    class Image {
        ...
        public
        Image(int[] raw, int width, int height) {
        ...
        public
        Image(byte[] raw, int width, int height) {
        ...
        public
        Image(FloatBuffer rgbaf, int width, int height) {
        ...
        public
        Image(FloatBuffer greyf, int width, int height, boolean grey) {
            if (!grey) {
                coreimage = new CoreImage().image_createWithARGBFData(greyf, width, height);
                //System.out.println("B image <" + this + "> is pointing to <" + coreimage + ">");
            }
            else {
                FloatBuffer a = ByteBuffer.allocateDirect(greyf.capacity() * 4 * 4)
        ...
        public
        Image(ByteBuffer rgbaf, int width, int height) {
        ...
        public
        void drawNow(float x, float y) {
            if (coreimage == 0) ;//System.out.println(" drawing released image");

            //System.out.println(" drawing an image");

            drawNow(x,
                    y,
                    new CoreImage().image_getExtentWidth(coreimage),
                    new CoreImage().image_getExtentHeight(coreimage),
                    new CoreImage().image_getExtentX(coreimage),
                    new CoreImage().image_getExtentY(coreimage),
                    new CoreImage().image_getExtentWidth(coreimage),
                    new CoreImage().image_getExtentHeight(coreimage));
        }

        public
        void drawNow(float x, float y, float scale) {
            if (coreimage == 0) ;//System.out.println(" drawing released image");
            ...
            drawNow(x,
                    y,
                    new CoreImage().image_getExtentWidth(coreimage) * scale,
                    new CoreImage().image_getExtentHeight(coreimage) * scale,
                    new CoreImage().image_getExtentX(coreimage),
                    new CoreImage().image_getExtentY(coreimage),
                    new CoreImage().image_getExtentWidth(coreimage),
                    new CoreImage().image_getExtentHeight(coreimage));
        ...
        public
        void drawNowRaw(float x, float y, float scale) {
            if (coreimage == 0) ;//System.out.println(" drawing released image");
        ...
        public
        void drawNow(float x, float y, float dw, float dh, float sx, float sy, float sw, float sh) {
            if (coreimage == 0) {
            ...
            if (false) {
                ...
                new CoreImage().context_drawImageNow(getContext(),
                                                     coreimage,
                                                     topLeft.x,
                                                     topLeft.y,
                ...
                if (!CoreHelpers.isCore) glEnable(GL_TEXTURE_2D);
        ...
        public
        void drawNowRaw(float x, float y, float dw, float dh, float sx, float sy, float sw, float sh) {
            if (coreimage == 0) {
                //System.out.println(" drawing released image");
                return;
            }
        ...
        public
        Rect getExtents() {
            return new Rect(new CoreImage().image_getExtentX(coreimage),
                            new CoreImage().image_getExtentY(coreimage),
                            new CoreImage().image_getExtentWidth(coreimage),
                            new CoreImage().image_getExtentHeight(coreimage));
        }
        ...
        FloatBuffer toFloatBuffer() {
            ...
            Rect e = getExtents();
            FloatBuffer f = ByteBuffer.allocateDirect(4 * 4 * ((int) (e.w) * (int) (e.h)))
                                      .order(ByteOrder.nativeOrder())
                                      .asFloatBuffer();
            gotten = f;
        ...
        public
        void drawNow3d(float x, float y, float z) {
    ...
    public
    Filter customFilter(String text, List<String> args) {
        String a = "";
        for (String s : args) {
            if (!a.isEmpty()) a = a + '#';
            a += s;
