...
class ImageProcessing implements iImageProcessor, iAcceptsSceneListElement {
    ...
    class TextureWrapper extends BasicUtilities.TwoPassElement {
        ...
        public
        TextureWrapper setTarget(int target) {
            this.target = target;
            return this;
        }
    ...
    class StereoTextureWrapper extends BasicUtilities.TwoPassElement {
        ...
        protected
        void post() {
            if (left.get() == -1) return;
            if (right.get() == -1) return;
            glActiveTexture(GL_TEXTURE0 + unit);
        ...
        void pre() {
            ...
            glBindTexture(rect ? GL_TEXTURE_RECTANGLE : GL_TEXTURE_2D,
                          (FullScreenCanvasSWT.getSide() == StereoSide.left ? left : right).get());
    ...
    public static
    TriangleMesh placeOnscreen(BasicSceneList into,
                               iImageProcessor from,
                               int output,
                               Rect r,
                               float width,
                               float height,
    ...
    iReposition placeOnscreenWithMask(final BasicSceneList into,
        ...
        final BasicGLSLangProgram onscreenProgram =
                (width == 1 && height == 1
                 ? new BasicGLSLangProgram("content/shaders/NDCvertex.glslang",
                                           "content/shaders/PutImageProcessingOnscreenFragmentSquareMask.glslang")
                 : new BasicGLSLangProgram("content/shaders/NDCvertex.glslang",
                                           "content/shaders/PutImageProcessingOnscreenFragmentRectMask.glslang"));
        onscreenProgram.new SetIntegerUniform("depthTexture", 0);
    ...
    public
    ImageProcessing(TwoPassImageProcessing i,
                    int width,
                    int height,
                    boolean useRect,
                    boolean genMipmap,
    ...
    public
    ImageProcessing(TwoPassImageProcessingTwoOutput i,
                    int width,
                    int height,
                    boolean useRect,
                    boolean genMipmap,
...
//		glPixelStorei(GL_UNPACK_CLIENT_STORAGE_APPLE, 0);
        ...
        glTexImage2D(useRect ? GL_TEXTURE_RECTANGLE : GL_TEXTURE_2D,
                     0,
                     useFloat ? GL_RGBA16F : GL_RGBA8,
                     width,
                     height,
                     0,
                     GL_RGBA,
                     useFloat ? GL11.GL_FLOAT : GL_UNSIGNED_BYTE,
                     (ByteBuffer) null);
    ...
    public
    void join(FullScreenCanvasSWT c, Method m) {
        aRun arun = new aRun() {
            @Override
            public
    ...
    public
    ImageProcessing setBlendMode(int colorSrc, int colorDest, int alphaSrc, int alphaDest) {
        this.blendMode = new int[]{colorSrc, colorDest, alphaSrc, alphaDest};
        return this;
    }

    public
    ImageProcessing setBlendMode(KnownBlendMode known) {
        this.blendMode = new int[]{known.sc, known.dc, known.sa, known.da};
        return this;
    }

    public
    ImageProcessing setClearColor(Vector4 clearColor) {
    ...
    public
    ByteBuffer getByteImage(ByteBuffer storage) {
        if (storage == null) {
    ...
    // renderbuffer as well
    public
    FloatBuffer getFloatImage(FloatBuffer storage) {
        if (storage == null && this.storage == null) {
            this.storage =
            storage = ByteBuffer.allocateDirect(width * height * 4 * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
        }
        else if (storage == null) storage = this.storage;
    ...
    void useHighResolutionMesh(int devision) {
        ...
        for (int x = 0; x < devision + 1; x++) {
            for (int y = 0; y < devision + 1; y++) {
                ...
                tex.put((useRect ? width : 1) * x / (float) devision)
                   .put((useRect ? height : 1) * y / (float) devision);
            }
        }
        ...
        for (int x = 0; x < devision; x++) {
            for (int y = 0; y < devision; y++) {
                s.put((short) (y * (devision + 1) + x))
                 .put((short) (y * (devision + 1) + x + 1))
