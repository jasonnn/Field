...
class Processors {
    ...
    class AddGrain extends BasicGLSLangProgram implements iProcessesMesh {
        ...
        public
        void setNoiseParameters(Vector4 noiseParameters) {
    ...
    class Grainer {
        ...
        public
        Grainer(BasicGLSLangProgram prog) {
    ...
    class Simple7Kernel extends SimpleKernel {
        ...
        public static final Vector3[] vert7 = {new Vector3(0, -3.75, 0.05f),
                                               new Vector3(0, -2.5, 0.1f),
                                               new Vector3(0, -1.25, 0.2f),
                                               new Vector3(0, 1.25, 0.2f),
                                               new Vector3(0, 2.5, 0.1f),
                                               new Vector3(0, 3.75, 0.05f)};
        public static final float vert7_center = 1f;
        public static final Vector3[] horiz7 = {new Vector3(-3.75, 0, 0.05f),
                                                new Vector3(-2.5, 0, 0.1f),
                                                new Vector3(-1.25, 0, 0.2f),
                                                new Vector3(1.25, 0, 0.2f),
                                                new Vector3(2.5, 0, 0.1f),
                                                new Vector3(3.75, 0, 0.05f)};
        ...
        Simple7Kernel(IFloatProvider scale, Vector3[] offsets, final float centerWeight) {
            super(scale,
                  ...
                  "content/shaders/VertBlur7Fragment.glslang");
    ...
    class SimpleFeedbackDOFKernel extends SimpleKernel {
        ...
        SimpleFeedbackDOFKernel(IFloatProvider scale, Vector3[] offsets) {
            super(scale,
                  ...
                  "content/shaders/processors/VertBlurDOFFeedbackFragment.glslang");
        ...
        void process(iGeometry process) {
            ...
            for (int j = 0; j < o.length; j++) {
                for (int i = 0; i < oTexture.capacity() / 2; i++) {
                    ...
                    o[j].put((float) (x * (relativeOffsets ? 0 : 1) + offsets[j].x * s * (randomOffsets
                                                                                          ? (StereoCamera.getRandomNumber()
                                                                                             + 3)
                                                                                          : 3)));
                    o[j].put((float) (y * (relativeOffsets ? 0 : 1) + offsets[j].y * s * (randomOffsets
                                                                                          ? (StereoCamera.getRandomNumber()
                                                                                             + 3)
                                                                                          : 3)));
                    o[j].put(offsets[j].z);
        ...
        public
        void setFeedbackParameters(Vector4 feedbackParameters) {
    ...
    class SimpleFeedbackKernel extends SimpleKernel {
        ...
        SimpleFeedbackKernel(IFloatProvider scale, Vector3[] offsets) {
            super(scale,
                  offsets,
                  "content/shaders/VertBlurNDCvertex.glslang",
                  "content/shaders/processors/VertBlurFeedbackFragment.glslang");
            new SetUniform("feedbackParameters", feedbackParameters);
    ...
    class SimpleKernel extends BasicGLSLangProgram implements iProcessesMesh {
        public static final Vector3[] vertical = {new Vector3(0, -1.5, 0.1f),
                                                  new Vector3(0, -0.75, 0.2f),
                                                  new Vector3(0, -0, 0.4f),
                                                  new Vector3(0, 0.75, 0.2f),
                                                  new Vector3(0, 1.5, 0.1f)};

        public static final Vector3[] horizontal = {new Vector3(-1.5, 0, 0.1f),
                                                    new Vector3(-0.75, 0, 0.2f),
                                                    new Vector3(0, 0, 0.4f),
                                                    new Vector3(0.75, 0, 0.2f),
                                                    new Vector3(1.5, 0, 0.1f)};

        public static final Vector3[] vertical2 = {new Vector3(0, -2, 0.2f),
                                                   new Vector3(0, -1, 0.2f),
                                                   new Vector3(0, 0, 0.2f),
                                                   new Vector3(0, 1, 0.2f),
                                                   new Vector3(0, 2, 0.2f)};

        public static final Vector3[] horizontal2 = {new Vector3(-2, 0, 0.2f),
                                                     new Vector3(-1, 0, 0.2f),
                                                     new Vector3(0, 0, 0.2f),
                                                     new Vector3(1, 0, 0.2f),
                                                     new Vector3(2, 0, 0.2f)};

        public static final Vector3[] laplacian = {new Vector3(-1, -1, -1),
                                                   new Vector3(1, -1, -1),
                                                   new Vector3(0, 0, 4),
                                                   new Vector3(1, 1, -1),
                                                   new Vector3(-1, 1, -1)};

        public static final Vector3[] omni = {new Vector3(-1, -1, 0.2f),
                                              new Vector3(1, -1, 0.2f),
                                              new Vector3(0, 0, 0.2f),
                                              new Vector3(1, 1, 0.2f),
                                              new Vector3(-1, 1, 0.2f)};

        public static final Vector3[] omni2 = {new Vector3(-1, -1, 0.1f),
        ...
        void process(iGeometry process) {
            ...
            for (int j = 0; j < o.length; j++) {
                ...
                for (int i = 0; i < oTexture.capacity() / 2; i++) {
                    float x = oTexture.get(i * 2 + 0);
                    float y = oTexture.get(i * 2 + 1);
                    float a = (float) (x * (relativeOffsets ? 0 : 1) + offsets[j].x * s * (randomOffsets
                                                                                           ? (StereoCamera.getRandomNumber()
                                                                                              - 0.5)
                                                                                           : 1));
                    o[j].put(a);
                    float b = (float) (y * (relativeOffsets ? 0 : 1) + offsets[j].y * s * (randomOffsets
                                                                                           ? (StereoCamera.getRandomNumber()
                                                                                              - 0.5)
                                                                                           : 1));
                    o[j].put(b);
        ...
        public
        void setAlphaParameters(Vector4 alphaParameters) {
