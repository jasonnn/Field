...
class FullScreenCanvasSWT implements IUpdateable, iThreedDrawingSurface, iAcceptsSceneListElement {
    ...
    @HiddenInAutocomplete
    public static final Method method_beforeLeftFlush =
            ReflectionTools.methodOf("beforeLeftFlush", FullScreenCanvasSWT.class);
    @HiddenInAutocomplete
    public static final Method method_beforeRightFlush =
            ReflectionTools.methodOf("beforeRightFlush", FullScreenCanvasSWT.class);
    ...
    public
    FullScreenCanvasSWT(boolean inAWindow) {
        ...
        frame.setBackground(Launcher.display.getSystemColor(SWT.COLOR_BLACK));
        ...
        if (Platform.isMac()) {
            ...
            try {
                canvas = (Canvas) this.getClass()
                                      ...
                                      .newInstance(comp, SWT.NONE, data);
        ...
        if (SystemProperties.getIntProperty("	", 0) == 1) new MiscNative().enterKiosk_safe();
...
//			getCanvas().setCursor(c);
            ...
            Color white = Launcher.display.getSystemColor(SWT.COLOR_WHITE);
            Color black = Launcher.display.getSystemColor(SWT.COLOR_BLACK);
            PaletteData palette = new PaletteData(new RGB[]{white.getRGB(), black.getRGB()});
            ImageData sourceData = new ImageData(16, 16, 1, palette);
            sourceData.transparentPixel = 0;
            Cursor cursor = new Cursor(Launcher.display, sourceData, 0, 0);
    ...
    @DispatchOverTopology(topology = Cont.class)
    public
    void beforeLeftFlush() {
    }

    @HiddenInAutocomplete
    @DispatchOverTopology(topology = Cont.class)
    public
    void beforeRightFlush() {
    }
...
//			glDebugMessageControl(GL11.GL_DONT_CARE, GL11.GL_DONT_CARE, GL11.GL_DONT_CARE, null, true);
            ...
            try {
                ...
                if (stereo) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        try {
                            if (!leftOnly) glEnable(GL11.GL_STEREO);
                            else glDisable(GL11.GL_STEREO);
                        } catch (OpenGLException e) {
                        }

                        if (side % 2 == 0)
                            glDrawBuffer((totalFlip | totalFlip_instance) ? GL_BACK_RIGHT : GL_BACK_LEFT);
                        else glDrawBuffer((totalFlip | totalFlip_instance) ? GL_BACK_LEFT : GL_BACK_RIGHT);

                        if (leftOnly) glDrawBuffer(GL_BACK);
                        ...
                        if (side % 2 == 0) {
                            if (anaglyph) glColorMask(true, false, false, true);
                            if (doLeft) {
                            ...
                            if (anaglyph) glColorMask(false, true, true, true);
                            if (doRight) {
                        ...
                        if (side % 2 == 0 || leftOnly) if (doFlush()) {
    ...
    StereoSide getSide() {
        ...
        return (side % 2 == 0 ? StereoSide.left : StereoSide.right);
    }
    ...
    @HiddenInAutocomplete
    public
    void overwriteCamera(BasicCamera c) {
        this.camera = c;
    }

    @HiddenInAutocomplete
    public
    void registerKeyboadEventDelegate(KeyListener listener) {
        canvas.addKeyListener(listener);
    }

    @HiddenInAutocomplete
    public
    void registerMouseEventDelegate(MouseEventDelegate delegate) {
     ...
     */
    public
    FullScreenCanvasSWT setVisible(boolean b) {
    ...
    @HiddenInAutocomplete
    public
    FullScreenCanvasSWT withKeyboardControlledCamera_smooth() {
    ...
    void createInitialLists() {
        ...
        camera = (stereo || sequentialStereo || passiveStereo)
                 ? new StereoCamera()
                 : ((layeredStereo || activeLayered) ? new LayeredStereoCamera() : new BasicCamera());
     ...
     */
    public
    void setSceneListSide(boolean left) {
    ...
    void reshape(int x, int y, int width, int height) {
        ...
        else camera.setPerspective(camera.getFov(), width / (float) height, camera.getNear(), camera.getFar());
    ...
    @HiddenInAutocomplete
    public
    OnCanvasLines getOnCanvasPLine() {
     ...
     */
    public
    OnCanvasLines getOnCanvasLines() {
        if (onCanvasPLine == null) {
            onCanvasPLine = new OnCanvasLines(this.getBothEyes(), this);
        }
        return onCanvasPLine;
    ...
    void addDefaultHandlers() {
        this.registerMouseEventDelegate(new MouseEventDelegate() {
            ...
            @Override
            public
            void mouseMove(MouseEvent arg0) {
                if (down) onMouseDrag.invoke(arg0);
                else onMouseMove.invoke(arg0);
            }

            @Override
            public
            void mouseScrolled(MouseEvent arg0) {
    ...
    public
    When getWhen() {
        if (when == null) when = new When(this.getSceneList());
        return when;
    }
     ...
     */
    public
    StereoSide getSceneListSide() {
        if (!(this.stereo || this.sequentialStereo || this.passiveStereo)) return StereoSide.middle;
        return sceneList == leftSceneList ? StereoSide.left : StereoSide.right;
    }

    @HiddenInAutocomplete
    public
    Method getCurrentFlushMethod() {
        if (getSceneListSide() == StereoSide.middle) return method_beforeFlush;
        return getSceneListSide() == StereoSide.left ? method_beforeLeftFlush : method_beforeRightFlush;
    }

    @HiddenInAutocomplete
    public
    FullScreenCanvasSWT pressSpaceToSave(final String directory) {
     ...
     */
    public
    void saveAsPNG(final String filename) {
        aRun r = new aRun() {
            @Override
            public
        ...
        if (!(stereo || sequentialStereo || passiveStereo)) Cont.linkWith(this, method_beforeLeftFlush, r);
        else Cont.linkWith(this, method_beforeFlush, r);
    }
