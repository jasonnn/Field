...
class CoordinateFrameCamera implements IUpdateable {
    ...
    class SavedCamera implements Serializable {
        ...
        public
        SavedCamera(CoordinateFrameCamera camera) {
        ...
        public
        void load(CoordinateFrameCamera camera) {
        ...
        private
        void readObject(ObjectInputStream i) throws IOException, ClassNotFoundException {
            i.defaultReadObject();
//			try {
...
//			}
    ...
    public
    CoordinateFrameCamera(BasicCamera camera) {
        this.camera = camera;
    ...
    public
    Iterator deserializeSaved(String string) {
        try {
    ...
    public
    SavedCamera getCamera(String k) {
     ...
     */
    public
    Quaternion getQuat(Vector3 up, Vector3 forward) {
    ...
    IInplaceProvider<iCoordinateFrame.iMutable>[] loadCamera(String called) {
        ...
        if (camera != null) camera.load(this);
        if (camera == null) {
            return null;
        }
    ...
    public
    void moveAndRotate(double Langle,
                       double Uangle,
                       float Funits,
                       float Lunits,
                       float Uunits,
        ...
        if ((ray.z < 0.99 && ray.z > -0.99) || !worldUp) look.blendRepresentation_add(ray, at, look);

        Vector3 temp = new Vector3();

        //moveForward
        Vector3.add(ray, Funits, at, temp);
        if (Math.abs(temp.x) <= maxX && Math.abs(temp.y) <= maxY && Math.abs(temp.z) <= maxZ && (trans == null || (trans
                                                                                                                   != null
                                                                                                                   &&
                                                                                                                   Math.abs(temp.distanceFrom(trans))
                                                                                                                   > minDist))) {
    ...
    public
    void moveAndRotateAround(double leftAngle, float upUnits, float forwardUnits, Vector3 origin) {
    ...
    public
    void moveForwardAlong(float units) {
        resync();
        units *= speed;
        // move position towards lookAt <b>in world coordinates</b>
        Vector3 look = new Vector3();
        ...
        if (at.distanceFrom(currentLookAt) > 1e-3) setPositionInWorld(at);
    ...
    public
    void panRight(double amount) {
        panLeft(-amount);

    }

    public
    void panUp(double angle) {
        resync();
        angle *= speed;
    ...
    public
    void rotateLeftAround(double angle) {
        resync();
    ...
    public
    void rotateUpAround(double angle) {
        resync();
        angle *= speed;
        //		work out what the real up vector is by lookAtVec x (lookAtVec x suposed up)
        // in world coordinates;
     ...
     */
    public
    void serializeSaved(String string) {
        try {
    ...
    public
    void setLookInWorld(Vector3 to) {
        if (lookAtFrame != null) {
            lookAtFrame.get(frame);
            frame.invert();
            frame.transformPosition(lookAt.set(to));
        }
        else lookAt.setValue(to);
    }

    public
    void setPositionInWorld(Vector3 to) {
        if (positionFrame != null) {
            positionFrame.get(frame);
            frame.invert();
            frame.transformPosition(position.set(to));
        }
        else position.setValue(to);
    }
    ...
    protected
    void lookInWorld(Vector3 into) {
        if (lookAtFrame != null) {
            lookAtFrame.get(frame);
            frame.transformPosition(into.set(lookAt));
        }
        else into.setValue(lookAt);
    }
    ...
    protected
    void upInWorld(Vector3 into) {
        if (positionFrame != null) {
            positionFrame.get(frame);
            frame.transformDirection(into.set(up));
        }
        else into.setValue(up);
    }
