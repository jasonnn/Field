 ...
 */
public
class KeyboardControlledCamera_smooth extends CoordinateFrameCamera implements KeyListener,
    ...
    class CharacterIsDown {
        ...
        public
        CharacterIsDown(boolean b, int code) {
            isShift = b;
            this.code = code;
        }

        public
        CharacterIsDown(char c, boolean b, int code) {
            character = c;
            isShift = b;
            this.code = code;
        }
        ...
        public
        CharacterIsDown setDoFade(boolean doFade) {
            this.doFade = doFade;
            return this;
        }
    }
    ...
    LinkedHashMap<CharacterIsDown, BaseMath.MutableFloat> charactersDownBuffer =
            new LinkedHashMap<CharacterIsDown, BaseMath.MutableFloat>();

    /**
     * @param camera
     */
    public
    KeyboardControlledCamera_smooth(BasicCamera camera) {
        super(camera);
    ...
    void keyPressed(KeyEvent e) {
        ...
        if (!charactersDown.containsKey(d)) charactersDown.put(d, 1f);
    }
    ...
    void update() {
        ...
        while (ii.hasNext()) {
            ...
            if (c.doFade && gestureIsDown) {
                ...
                float alpha = 0.5f;
                if (x == null) charactersDownBuffer.put(n.getKey(), new MutableFloat(f * (1 - alpha)));
                else charactersDownBuffer.put(n.getKey(),
                                              new MutableFloat((x.floatValue() * alpha) + ((1 - alpha) * f)));
            }
            else if (c.doFade) {
                f = f * 0.4f;
                if (f < 1e-3) {
    ...
    private
    void moveTowards(CharacterIsDown key, float f) {
        MutableFloat m = charactersDownBuffer.get(key);
        if (m == null) charactersDownBuffer.put(key, m = new MutableFloat(0));
        if (f > m.d) m.d = m.d * smoothing_up + (1 - smoothing_up) * f;
        else m.d = m.d * smoothing + (1 - smoothing) * f;
        if ((m.d < 1e-4) && (m.d > f)) charactersDownBuffer.remove(key);
    }

    public
    void setCanvas(FullScreenCanvasSWT c) {
        this.canvas = c;
    }
    ...
    void keyPressed(org.eclipse.swt.events.KeyEvent e) {
        ...
        else d = new CharacterIsDown((char) e.keyCode, false, e.keyCode);
    ...
    void gesture(GestureEvent e) {
        ...
        if (e.detail == SWT.GESTURE_ROTATE) {
            ...
            if (e.rotation > 0) {
                CharacterIsDown d = new CharacterIsDown((char) SWT.KEYPAD_7, true, SWT.KEYPAD_7).setDoFade(true);
                charactersDown.put(d, (float) e.rotation);
                // charactersDownBuffer.put(d, new
                // MutableFloat(e.xDirection));
                CharacterIsDown d2 = new CharacterIsDown((char) SWT.KEYPAD_9, true, SWT.KEYPAD_9).setDoFade(true);
                charactersDown.remove(d2);
                charactersDownBuffer.remove(d2);
            }
            if (e.rotation < 0) {
                CharacterIsDown d = new CharacterIsDown((char) SWT.KEYPAD_9, true, SWT.KEYPAD_9).setDoFade(true);
                charactersDown.put(d, (float) Math.abs(e.rotation));
                // charactersDownBuffer.put(d, new
                // MutableFloat(Math.abs(e.xDirection)));
                CharacterIsDown d2 = new CharacterIsDown((char) SWT.KEYPAD_7, true, SWT.KEYPAD_7).setDoFade(true);
                charactersDown.remove(d2);
                charactersDownBuffer.remove(d2);
            }
        }
        ...
        if (e.detail == SWT.GESTURE_PAN) {
            ...
            if (e.xDirection > 0) {
                CharacterIsDown d = new CharacterIsDown((char) SWT.ARROW_RIGHT, true, SWT.ARROW_RIGHT).setDoFade(true);
                charactersDown.put(d, (float) e.xDirection);
                // charactersDownBuffer.put(d, new
                // MutableFloat(e.xDirection));
                CharacterIsDown d2 = new CharacterIsDown((char) SWT.ARROW_LEFT, true, SWT.ARROW_LEFT).setDoFade(true);
                charactersDown.remove(d2);
                charactersDownBuffer.remove(d2);
            }
            if (e.xDirection < 0) {
                CharacterIsDown d = new CharacterIsDown((char) SWT.ARROW_LEFT, true, SWT.ARROW_LEFT).setDoFade(true);
                charactersDown.put(d, (float) Math.abs(e.xDirection));
                // charactersDownBuffer.put(d, new
                // MutableFloat(Math.abs(e.xDirection)));
                CharacterIsDown d2 = new CharacterIsDown((char) SWT.ARROW_RIGHT, true, SWT.ARROW_RIGHT).setDoFade(true);
                charactersDown.remove(d2);
                charactersDownBuffer.remove(d2);
            }
            if (e.yDirection > 0) {
                CharacterIsDown d = new CharacterIsDown((char) SWT.PAGE_UP, true, SWT.PAGE_UP).setDoFade(true);
                charactersDown.put(d, (float) e.yDirection);
                // charactersDownBuffer.put(d, new
                // MutableFloat(Math.abs(e.yDirection)));
                CharacterIsDown d2 = new CharacterIsDown((char) SWT.PAGE_DOWN, true, SWT.PAGE_DOWN).setDoFade(true);
                charactersDown.remove(d2);
                charactersDownBuffer.remove(d2);
            }
            if (e.yDirection < 0) {
                CharacterIsDown d = new CharacterIsDown((char) SWT.PAGE_DOWN, true, SWT.PAGE_DOWN).setDoFade(true);
                charactersDown.put(d, (float) Math.abs(e.yDirection));
                // charactersDownBuffer.put(d, new
                // MutableFloat(Math.abs(e.yDirection)));
                CharacterIsDown d2 = new CharacterIsDown((char) SWT.PAGE_UP, true, SWT.PAGE_UP).setDoFade(true);
                charactersDown.remove(d2);
                charactersDownBuffer.remove(d2);
            }
        }
            ...
            if (e.magnification > lastMag) {
                CharacterIsDown d = new CharacterIsDown((char) SWT.ARROW_UP, true, SWT.ARROW_UP).setDoFade(true);

                float q = (float) (e.magnification - lastMag) / lastMag;

                charactersDown.put(d, Math.min(1, q * 20));

                CharacterIsDown d2 = new CharacterIsDown((char) SWT.ARROW_DOWN, true, SWT.ARROW_DOWN).setDoFade(true);
                charactersDown.remove(d2);
                charactersDownBuffer.remove(d2);
                ...
                CharacterIsDown d2 = new CharacterIsDown((char) SWT.ARROW_UP, true, SWT.ARROW_UP).setDoFade(true);
                charactersDown.remove(d2);
                charactersDownBuffer.remove(d2);

            }
            if ((e.magnification > 0) && (e.magnification < 3)) lastMag = (float) e.magnification;
        }
