 ...
 */
public
class NetworkInput implements Runnable {
    ...
    public
    interface Handler extends AnyHandler {
        public
        void handle(String name, float f);
    }

    public
    interface VecHandler extends AnyHandler {
        public
        void handle(String name, VectorN v);
    }

    public
    interface ObjectHandler<T> extends AnyHandler {
        public
        void handle(String name, T o);
    }
    ...
    public
    NetworkInput(String group, int port) {
        try {
    ...
    public
    Handler register(Handler h, String name) {
        Handler ha = (Handler) dispatchTable.put(name, h);
        return ha;
    }

    public
    VecHandler register(VecHandler h, String name) {
        VecHandler ha = (VecHandler) dispatchTable.put(name, h);
        return ha;
    }

    public
    ObjectHandler register(ObjectHandler h, String name) {
        ObjectHandler ha = (ObjectHandler) dispatchTable.put(name, h);
        return ha;
    }

    public
    AnyHandler registerDefaultHandler(AnyHandler h) {
        ...
        try {
            ...
            while (true) {
                ...
                if (dim == 1) {
                    ...
                    if (debugArrival) System.err.println(" net <" + name + "> : = <" + value + '>');
                    set(name, value);
                }
                else if (dim == -1) {
                    ...
                    if (debugArrival) System.err.println(" net <" + name + "> := <" + v + '>');
                    set(name, v);
                }
                try {
...
//			System.exit(1);
    ...
    public
    void set(String name, VectorN value) {
        // find it
        Object o = dispatchTable.get(name);
        if (o == null) o = defaultHandler;
        if (o instanceof Handler) {
            Handler handler = (Handler) o;
            handler.handle(name, value.get(0));
        }
        else if (o instanceof VecHandler) {
            VecHandler handler = (VecHandler) o;
            handler.handle(name, value);
    ...
    public static
    float decode(byte[] from, int at) {
        return Float.intBitsToFloat(decodeInt(from, at));
    }

    public static
    int decodeInt(byte[] from, int at) {
    ...
    public static
    String decodeString(byte[] from, int at) {
        char[] result;
    ...
    class RecieveFloat implements IFloatProvider {
        ...
        public
        RecieveFloat(String name, float def) {
            this.value = def;
