 ...
 */
public
class HashMapOfLists<K, V> extends HashMap<K, Collection<V>> {
    ...
    public
    HashMapOfLists(HashMapOfLists<K, V> map) {
        super(map);
    }

    public
    void addAllToList(K key, Collection<V> value) {
    ...
    public
    void addToListHead(K key, V value) {
        Collection<V> l = this.get(key);
        if (l == null) {
            this.put(key, l = newList());
        }
    ...
    public
    Collection<V> getAndMakeCollection(K key) {
        Collection l = this.get(key);
        if (l == null) this.put(key, l = newList());
        return l;
    }
    ...
    public
    void putAll(Map<? extends K, ? extends Collection<V>> a) {
    ...
    void remove(K key, V value) {
        ...
        if (c != null) {
            c.remove(value);
            if (c.isEmpty()) this.remove(key);
        }
    }

    public
    void removeFromAllLists(V with) {
