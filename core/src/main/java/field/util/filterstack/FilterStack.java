...
class FilterStack<T> implements IUpdateable, IProvider<T>, iHandlesAttributes {
    ...
    interface iSelectPosition<T> {
        public
        Position select(Unit<T> at, Position desc);
    }
    ...
    public
    void add(Unit<T> unit, iSelectPosition<T> s) {
    ...
    public
    void addLast(Unit<T> unit) {
        stack.add(unit);
        lookupStack.put(unit.name, unit);
    }

    public
    T blend(T a, T b, double ea) {
        if (ea == 0) return a;
        if (ea == 1) return b;
    ...
    void update() {
        ...
        for (Unit<T> t : savedStack) {
            ...
            if (t.inputName == null) t.input = previousUnit == null ? this.input.get() : previousUnit.lastOutput;
            else {
                Unit<T> a = lookupStack.get(t.inputName);
                t.input =
                        a == null ? (previousUnit == null ? this.input.get() : previousUnit.lastOutput) : a.lastOutput;
            }
        ...
        out = this.input.get();
        for (Unit<T> t : savedStack) {
            if (t.outputName == null) out = t.lastTotalOutput = previousUnit.lastOutput;
            else {
        ...
        for (Unit<T> t : savedStack) {
            if (t instanceof iProvidesWindowStatus)
                if (((iProvidesWindowStatus) t).getWindowState() == WindowStatus.finished) stack.remove(t);
        }
