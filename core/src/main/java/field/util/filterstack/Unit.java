...
class Unit<T> {
    ...
    enum WindowStatus {
        ...
        public
        WindowStatus combine(WindowStatus w) {
            if (w == null) return this;
            if (this.ordinal() < w.ordinal()) return w;
            return this;
        }
    }

    public
    interface iProvidesWindowStatus {
        public
        WindowStatus getWindowState();
    }
    ...
    void update(double time) {
        ...
        if (lastOutput == null) eb = 0;
        if (lastTotalOutput == null) eg = 0;
        ...
        if (o == null) {
            ...
            eo = filterStack.blend(filterStack.blend(filterStack.blend(o, blendInput, ea), lastOutput, eb),
                                   lastTotalOutput,
                                   eg);
        }
    ...
    protected
    WindowStatus getAccumulativeWindowStatus() {
        WindowStatus w = WindowStatus.unknown;

        if (this instanceof iProvidesWindowStatus) w = w.combine(((iProvidesWindowStatus) this).getWindowState());
