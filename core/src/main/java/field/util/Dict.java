...
class Dict implements Serializable {
    ...
    public
    float getFloat(Prop<? extends Number> n, float def) {
    ...
    int safeHash(Map<Prop, Object> d) {
        ...
        for (Entry<Prop, Object> e : ee) {
            ...
            Object v = e.getValue();
            c += k.hashCode();
            if (v instanceof PyObject) c += System.identityHashCode(v);
            else c += ((v == null) ? 0 : v.hashCode());
        }
        return c;
    }

    public
    boolean isTrue(Prop<?> prop, boolean def) {
        ...
        if (p instanceof String) return !((String) p).isEmpty();
        return true;
    }
    ...
    public
    <T> Dict putToList(Prop<? extends Collection<T>> key, T value) {
        Collection<T> c = (Collection<T>) dictionary.get(key);
        if (c == null) dictionary.put(key, c = new HashSet<T>());
        c.add(value);
        return this;
    }

    public
    <T> Dict putToMap(Prop<? extends Map<String, T>> key, String tok, T value) {
        Map<String, T> c = (Map<String, T>) dictionary.get(key);
        if (c == null) dictionary.put(key, c = new HashMap<String, T>());
        c.put(tok, value);
        return this;
    }

    public
    <T> Dict putToMapOfLists(Prop<? extends HashMapOfLists<String, T>> key, String tok, T value) {
        HashMapOfLists<String, T> c = (HashMapOfLists<String, T>) dictionary.get(key);
        if (c == null) dictionary.put(key, c = new HashMapOfLists<String, T>());
        c.addToList(tok, value);
        return this;
    }

    public
    <T> void removeFromList(Prop<? extends Collection<T>> p, T q) {
    ...
    public
    long longHash(Set<String> ex) {
        long start = 1;
        Set<Entry<Prop, Object>> e = dictionary.entrySet();
        for (Entry<Prop, Object> ee : e) {
            if (!ex.contains(ee.getKey().name)) start = start * 31L + (long) ((ee.getValue() instanceof PyObject)
                                                                              ? System.identityHashCode(ee.getValue())
                                                                              : ee.hashCode());
        }
        return start;
