...
class NetworkOutput {
    public static String DEFAULT_GROUP =
            SystemProperties.getProperty("NetworkDoubleProvider.group.output", "239.45.89.15");
    public static int DEFAULT_PORT = SystemProperties.getIntProperty("NetworkDoubleProvider.port.output", 1230);
    public static String DEFAULT_INTERFACE =
            SystemProperties.getProperty("NetworkDoubleProvider.interface.output", "en0");
    ...
    public
    NetworkOutput(String group, int port, String interfac) {
        this.port = port;
        try {
        ...
        dp = new DatagramPacket(new byte[NetworkInput.MAX_PACKET_SIZE],
                                NetworkInput.MAX_PACKET_SIZE,
                                multicastGroup,
                                port);
    }
    ...
    public
    void send(String name, VectorN vec) {
        byte[] buffer = dp.getData();
    ...
    public static
    int encode(byte[] to, int at, float me) {
        return encode(to, at, Float.floatToRawIntBits(me));
    }

    public static
    int encode(byte[] to, int at, int me) {
        to[at] = (byte) (me >> 24);
    ...
    public static
    int encodeString(byte[] to, int at, String me) {
    ...
    private static
    int encode(byte[] buffer, int i, Object objects) {
