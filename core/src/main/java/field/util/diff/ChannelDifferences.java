...
public
class ChannelDifferences<A> {

    public static
    class DefaultMarkerEquality<A> implements iEquality<List<iMarker<A>>> {

        public
        boolean areEqual(List<iMarker<A>> o1, List<iMarker<A>> o2) {
            if (o1 == null || o2 == null) return false;
            if (o1.size() != o2.size()) return false;
            for (int i = 0; i < o1.size(); i++)
                if (!o1.get(i).getPayload().equals(o2.get(i).getPayload())) return false;
            return true;
        }

        public
        int hashCodeFor(List<iMarker<A>> o1) {
            if (o1 == null) return 0;
            int i = 0;
    ...
    public
    ChannelDifferences(Channel<A> left, Channel<A> right) {
        this.left = left;
        this.right = right;
    ...
    public
    ChannelDifferences(Channel<A> left, Channel<A> right, iEquality<A> e) {
        this.left = left;
        this.right = right;
        aLeft = unwrap(left, new ArrayList()).toArray();
        aRight = unwrap(right, new ArrayList()).toArray();
    ...
    public
    Channel<EditRelationship> generateTranspositions(Channel<EditRelationship> in,
                                                     int lookb,
                                                     iEquality<List<iMarker<A>>> equality) {
        ...
        for (iMarker<EditRelationship> e : new ArrayList<iMarker<EditRelationship>>(in.getIterator().remaining())) {
            ...
            if (removed != null) {
                ...
                if (e.getPayload().type == EditType.deletion) {
                    ...
                    in.makeMarker(in,
                                  er2.left.get(0).getTime(),
                                  er2.left.get(er2.left.size() - 1).getTime() + er2.left.get(er2.left.size() - 1)
                                                                                        .getDuration() - er2.left.get(0)
                                                                                                                 .getTime(),
                                  er2);
                }
                ...
                if (stack.size() > lookb) stack.removeLast();
            }
        }
        return in;
    }
    ...
    public
    Channel<EditRelationship> generateUnequivalences(Channel<EditRelationship> in, int min) {
        iMarker<EditRelationship> last = null;
        for (iMarker<EditRelationship> e : new ArrayList<iMarker<EditRelationship>>(in.getIterator().remaining())) {
            if (last != null
                && last.getPayload().type == EditType.deletion
                && e.getPayload().type == EditType.insertion
                && last.getPayload().left != null
                && e.getPayload().right != null
                && Math.abs(last.getPayload().left.size() - e.getPayload().right.size()) < min) {
                ...
                in.makeMarker(in,
                              last.getPayload().left.get(0).getTime(),
                              last.getPayload().left.get(last.getPayload().left.size() - 1).getTime()
                              + last.getPayload().left.get(last.getPayload().left.size() - 1).getDuration()
                              - last.getPayload().left.get(0).getTime(),
                              er2);
            }
            last = e;
    ...
    public
    List<Pair<iMarker<A>, iMarker<A>>> getNonChanges() {
    ...
    public
    Channel<EditRelationship> makeRelationships() {
        ...
        for (int i = 0; i < changes.size(); i++) {
            ...
            for (int n = previous; n < c.line0; n++) {
                ...
                editRelationships.makeMarker(editRelationships,
                                             leftMarkers.get(n).getTime(),
                                             leftMarkers.get(n).getDuration(),
                                             er);
                previous++;
            }
            if (c.inserted != 0) {
                ...
                editRelationships.makeMarker(editRelationships,
                                             er.right.get(0).getTime(),
                                             er.right.get(er.right.size() - 1).getTime() + er.right.get(er.right.size()
                                                                                                        - 1)
                                                                                                   .getDuration()
                                             - er.right.get(0).getTime(),
                                             er);
            ...
            if (c.deleted != 0) {
                ...
                er.right = null;
                editRelationships.makeMarker(editRelationships,
                                             er.left.get(0).getTime(),
                                             er.left.get(er.left.size() - 1).getTime() + er.left.get(er.left.size() - 1)
                                                                                                .getDuration()
                                             - er.left.get(0).getTime(),
                                             er);
            }
    ...
    protected
    Marker<EditRelationship> checkBehindAndRemove(iMarker<EditRelationship> e,
                                                  LinkedList<Marker<EditRelationship>> stack,
                                                  iEquality<List<iMarker<A>>> equality) {
        if (e.getPayload().type == EditType.insertion) {
            for (Marker<EditRelationship> marker : stack) {
                if (marker.getPayload().type == EditType.deletion && equality.areEqual(marker.getPayload().left,
                                                                                       e.getPayload().right)) {
                    stack.remove(marker);
                    ((Marker<EditRelationship>) e).remove();
                    return marker;
        ...
        if (e.getPayload().type == EditType.deletion) {
            for (Marker<EditRelationship> marker : stack) {
                if (marker.getPayload().type == EditType.insertion && equality.areEqual(marker.getPayload().right,
                                                                                        e.getPayload().left)) {
                    stack.remove(marker);
                    ((Marker<EditRelationship>) e).remove();
                    return marker;
    ...
    public
    Channel<EditRelationship> clumpEquivalences(Channel<EditRelationship> relationships) {
