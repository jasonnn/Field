...
class Diff {
    ...
    public
    Diff(Object[] a, Object[] b, iEquality e) {
    ...
    private
    int diag(int xoff, int xlim, int yoff, int ylim) {
    ...
    void compareseq(int xoff, int xlim, int yoff, int ylim) {
		...
		/* Handle simple cases. */
        if (xoff == xlim) while (yoff < ylim) filevec[1].changed_flag[1 + filevec[1].realindexes[yoff++]] = true;
        else if (yoff == ylim) while (xoff < xlim) filevec[0].changed_flag[1 + filevec[0].realindexes[xoff++]] = true;
        else {
    ...
    interface ScriptBuilder {
         ...
         */
        public
        Change build_script(boolean[] changed0, int len0, boolean[] changed1, int len1);
    }
    ...
    static
    class ReverseScript implements ScriptBuilder {

        public
        Change build_script(final boolean[] changed0, int len0, final boolean[] changed1, int len1) {
            Change script = null;
            int i0 = 0, i1 = 0;
    ...
    static
    class ForwardScript implements ScriptBuilder {
     ...
     */
    public
    Change diff(final ScriptBuilder bld) {
    ...
    class file_data {
        ...
        private
        byte[] discardable(final int[] counts) {
        ...
        private
        void filterDiscards(final byte[] discards) {
            final int end = buffered_lines;
                    ...
                    if (provisional * 4 > length) {
                        ...
                        for (j = 0, consec = 0; j < length; j++)
                            if (discards[i + j] != 2) consec = 0;
								...
								/* Back up to start of subrun, to cancel it all. */ j -= consec;
         ...
         */
        private
        void discard(final byte[] discards) {
            final int end = buffered_lines;
            int j = 0;
            for (int i = 0; i < end; ++i)
        ...
        file_data(Object[] data, Map h) {
            ...
            for (int i = 0; i < data.length; ++i) {
                ...
                if (ir == null) h.put(data[i], new Integer(equivs[i] = equiv_max++));
            ...
            for (; ; ) {
                ...
                for (; ; ) {
                    ...
                    if (end != i_end
                        ...
                        && !other_changed[1 + j]
                        && end != i_end
                        && !((preceding >= 0 && start == preceding) || (other_preceding >= 0
