 ...
 */
public
class HungarianConvenience3<t_element> {
...
//		targetBuffer = ByteBuffer.allocateDirect(max * 4 * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    public
    float getDistance(int source, int target) {
        return distanceMatrix.get((target * (sourceLength + sources)) + source);
    }

    public
    int mapTargetToSource(int targetElement) {
    ...
    float modeAssignmentDistance() {
        ...
        if ((distance.length % 2) == 1) return distance[distance.length / 2];
        else return (distance[distance.length / 2] + distance[((distance.length / 2) + 1)]) / 2;
    }

    public
    String printDetailedMatchResults() {
        ...
        for (int i = 0; i < targetLength; i++) {
            ...
            r += ("  map <" + i + ((i >= targetLength) ? "(sink)" : "") + "> -> <" + to + ((to >= sourceLength)
                                                                                           ? "(source)"
                                                                                           : "") + '>');
            if (to != -1) r += (" d = <" + distanceMatrix.get((i * (sourceLength + sources)) + to) + ">\n");
            else r += "\n";
            if (to != -1) {
        ...
        r += ("  distance <"
              + totalAssignmentDistanceSinkless()
              + " sinkless or "
              + totalAssignmentDistance()
              + " including sinks>");
        return r;
    }

    public
    void setDefaultDistances(float d) {
        for (int x = 0; x < (sourceLength + sources); x++) {
            for (int y = 0; y < (targetLength + sinks); y++) {
                if ((x < sourceLength) && (y < targetLength)) {
                    distanceMatrix.put((y * (sourceLength + sources)) + x, d);
    ...
    public
    void setDistance(int source, int target, float d) {
        distanceMatrix.put((target * (sourceLength + sources)) + source, d);
    }

    public
    void setSource(int num, t_element to) {
...
//		sourceBuffer.put(num * 4 + 3, 0);
    ...
    public
    void setSourcesAndSinks(int sources,
                            int sinks,
                            float distanceToSource,
                            float distanceToSink,
                            float sourceDistanceToSink) {
    ...
    public
    void setTarget(int num, t_element to) {
...
//		targetBuffer.put(num * 4 + 3, 0);
    ...
    public
    float totalAssignmentDistanceSinkless() {
        float d = 0;
        for (int i = 0; i < targetLength; i++) {
            int to = mapTargetToSource(i);
            if (to < sourceLength) {
